...
function ManageBombers(iTeam, iAirSubteam)
    ...
    if M28Utilities.IsTableEmpty(tAvailableBombers) == false then
        ...
        if M28Map.iMapSize > 512 then
            if M28Utilities.bLoudModActive and M28Utilities.bQuietModActive then
            ...
            if not(M28Map.bIsCampaignMap) or ((bIgnoreMidpointPlayableCheck or (M28Conditions.IsLocationInPlayableArea(tLZOrWZData[M28Map.subrefMidpoint]))) and (bDontCheckForPacifism or not(M28Conditions.AdjacentToPacifistZone(iPlateauOrZero, iLandOrWaterZone)))) then
                ...
                if (not(bOnlyIncludeIfMexToProtect) or IsHighValueZoneToProtect()) and (M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefTEnemyUnits]) == false or (bConsiderAttackingEnemyGunships and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftLZEnemyAirUnits]) == false)) then
                    ...
                    if iOurGunshipThreat >= 10000 then
                        if M28Utilities.bLoudModActive and (not(M28Utilities.bQuietModActive) or M28Team.tTeamData[iTeam][M28Team.refiGunshipLosses] >= math.max(10000, M28Team.tTeamData[iTeam][M28Team.refiGunshipKills])) then

                        ...
                        if oUnit[M28UnitInfo.refbSpecialMicroActive] then
                            ...
                            elseif (tLastOrder[M28Orders.subrefiOrderType] == M28Orders.refiOrderIssueGroundAttack and tLastOrder[M28Orders.subreftOrderPosition] and M28Utilities.GetDistanceBetweenPositions(tLastOrder[M28Orders.subreftOrderPosition], oUnit:GetPosition()) <= 90)
                                ...
                                if oExistingValidAttackTarget and EntityCategoryContains(M28UnitInfo.refCategoryAirAA, oUnit.UnitId) and IsAirUnitInCombat(oUnit, iTeam) then
                                    ...
                                    if oUnit[M28Orders.reftiLastOrders][1][M28Orders.subrefiOrderType] == M28Orders.refiOrderUnloadTransport then
                                        if oUnit[refbEmergencyDropActive] and M28Utilities.IsTableEmpty(oUnit:GetCargo()) == false then
                                            bProceed = false
                                            table.insert(tInUseUnits, oUnit)
                                            if bDebugMessages == true then LOG(sFunctionRef..': Transport doign emergency drop so including in table of in use units') end
                                        elseif (oUnit[refiTargetZoneForDrop] or oUnit[refiTargetIslandForDrop]) and M28Utilities.IsTableEmpty(oUnit:GetCargo()) == false then
...
function GetRallyPointValueOfLandZone(iTeam, tLZData, tLZTeamData, iPlateau)
    ...
    iCurAAValue = (tLZTeamData[M28Map.subrefLZThreatAllyGroundAA] or 0) * iCurFactor - (tLZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) * 4 - (tLZTeamData[M28Map.refiEnemyAirAAThreat] or 0)
    ...
    if M28Utilities.IsTableEmpty(M28Team.tAirSubteamData[iAirSubteam][M28Team.subreftoFriendlyM28Brains]) == false then
        ...
        if M28Utilities.IsTableEmpty(tUnitsToProtect) == false then
            ...
            if oPriorityUnitBeingSupported then
                if EntityCategoryContains(M28UnitInfo.refCategoryAirNonScout, oPriorityUnitBeingSupported.UnitId) then
                    iFriendlyAAFromUnitToProtect = M28UnitInfo.GetAirThreatLevel({ oPriorityUnitBeingSupported }, false, true, false)
                    if EntityCategoryContains(M28UnitInfo.refCategoryGunship, oPriorityUnitBeingSupported.UnitId) then
                        iFriendlyAAFromUnitToProtect = math.max(iFriendlyAAFromUnitToProtect, (M28Team.tAirSubteamData[iAirSubteam][M28Team.refiOurGunshipAAThreat] or 0))
                    end
                end
            end
            if bDebugMessages == true then LOG(sFunctionRef..': Finished going through all units to protect, tClosestMidpoint, being the zone midpoint for the priority unit to protect='..repru(tClosestMidpoint)) end
            ...
            if tClosestMidpoint then
                ...
                if iDistToTarget <= 50 then
                    ...
                    if bFarBehindOnAir and iFriendlyAAFromUnitToProtect > 0 then
                        if M28Team.tTeamData[iTeam][M28Team.subrefiOurAirAAThreat] + iFriendlyAAFromUnitToProtect > (M28Team.tAirSubteamData[iAirSubteam][M28Team.refiFarBehindFactor] or 0.8) * M28Team.tTeamData[iTeam][M28Team.refiEnemyAirAAThreat] then
                    ...
                    if not(bFarBehindOnAir) then
                        ...
                        if oPriorityUnitBeingSupported then
                            ...
                            if EntityCategoryContains(M28UnitInfo.refCategoryFatboy, oPriorityUnitBeingSupported.UnitId) and M28Utilities.IsTableEmpty(oPriorityUnitBeingSupported[M28Land.reftoAssignedMAAGuards]) == false then
                                iAirAAThreshold = math.max(3000, M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat] + iFriendlyAAFromUnitToProtect)
                            ...
                            elseif EntityCategoryContains(M28UnitInfo.refCategoryAirToGround, oPriorityUnitBeingSupported.UnitId) then
                                ...
                                iAirAAThreshold = math.max(iAirToGroundThreat * 0.1, M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat] * 1.2, M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat] + iFriendlyAAFromUnitToProtect)
                            else
                                ...
                                iAirAAThreshold = M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat] + iFriendlyAAFromUnitToProtect
                            end
                    ...
                    local iZoneGroundAAThreshold = (iGroundAAThreshold or 0) * 0.1 --threshold for doing a more detailed check
                    local iZoneAirAAThreshold --threshold for doing a more detailed check
                    iZoneAirAAThreshold = iAirAAThreshold
                    iZoneGroundAAThreshold = (iGroundAAThreshold or 0) * 0.75
                    ...
                    for iDist = iSearchInterval, iRoundedDist, iSearchInterval do
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering moving closer to the unit to protects zone, iDist='..iDist..'; tCurTarget='..repru(tCurTarget)..'; iCurPlateau='..(iCurPlateau or 'nil')..'; iCurZone='..(iCurZone or 'nil')) end
                            ...
                            if iCurPlateau == 0 then
                                ...
                                if not(tbWaterZonesConsidered[iCurZone]) then
                                    ...
                                    if bCurTargetTooDangerous and bDoDetailedCheck then
                                        bCurTargetTooDangerous = DoesEnemyHaveAAThreatAlongPath(iTeam, iStartPlateauOrZero, iStartLandOrWaterZone, iCurPlateau, iCurZone, not(M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir]), iGroundAAThreshold, iZoneAirAAThreshold, false, iAirSubteam, true)
                                        if bCurTargetTooDangerous and oPriorityUnitBeingSupported and EntityCategoryContains(M28UnitInfo.refCategoryAirNonScout, oPriorityUnitBeingSupported.UnitId) then
                                            local tCurWZTeamData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iCurZone]][M28Map.subrefPondWaterZones][iCurZone][M28Map.subrefWZTeamData][iTeam]
                                            if tCurWZTeamData[M28Map.subrefiThreatEnemyGroundAA] < 0.5 * iGroundAAThreshold then
                                                bCurTargetTooDangerous = DoesEnemyHaveAAThreatAlongPath(iTeam, iStartPlateauOrZero, iStartLandOrWaterZone, iCurPlateau, iCurZone, not(M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir]), iGroundAAThreshold, iZoneAirAAThreshold, false, iAirSubteam, true, nil, nil, nil, tCurTarget)
                                                if bDebugMessages == true then LOG(sFunctionRef..': bCurTargetTooDangerous after second detailed check='..tostring(bCurTargetTooDangerous)) end
                                            end
                                            if bDebugMessages == true then LOG(sFunctionRef..': bCurTargetTooDangerous is too dangerous after detailed WZ check, is it too dangerous after adjusting midpoint to be tcurposition='..tostring(bCurTargetTooDangerous)..';  tCurWZTeamData[M28Map.subrefiThreatEnemyGroundAA]='..( tCurWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 'nil')) end
                                        end
                                    end
                                    if bCurTargetTooDangerous then
                                        if bDebugMessages == true then
                                            local tWZData, tWZTeamData = M28Map.GetLandOrWaterZoneData(tCurTarget, true, iTeam)
                                            LOG(sFunctionRef..': Water Zone is too dangerous so will stop looking further, refiEnemyAirAAThreat='..(tWZTeamData[M28Map.refiEnemyAirAAThreat] or 'nil')..'; subrefiThreatEnemyGroundAA='..(tWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 'nil'))
                                        end
                                        break
                                    else
                                        ...
                                        iPrevLandZone = nil
                                        if bDebugMessages == true then LOG(sFunctionRef..': This water zone seems ok to treat as the support point, P='..iCurPlateau..'; Z='..iCurZone) end
                                    end
                                end
                            else
                                --Have a valid land zone
                                ...
                                if not(tbPlateauAndLandZonesConsidered[iCurPlateau][iCurZone]) then
                                    ...
                                    if bCurTargetTooDangerous and bDoDetailedCheck then
                                        ...
                                        bCurTargetTooDangerous = DoesEnemyHaveAAThreatAlongPath(iTeam, iStartPlateauOrZero, iStartLandOrWaterZone, iCurPlateau, iCurZone, not(M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir]), iGroundAAThreshold, iZoneAirAAThreshold, false, iAirSubteam, true)
                                        if bCurTargetTooDangerous and oPriorityUnitBeingSupported and EntityCategoryContains(M28UnitInfo.refCategoryAirNonScout, oPriorityUnitBeingSupported.UnitId) then
                                            local tCurLZTeamData = M28Map.tAllPlateaus[iCurPlateau][M28Map.subrefPlateauLandZones][iCurZone][M28Map.subrefLZTeamData][iTeam]
                                            if tCurLZTeamData[M28Map.subrefiThreatEnemyGroundAA] < 0.5 * iGroundAAThreshold then
                                                bCurTargetTooDangerous = DoesEnemyHaveAAThreatAlongPath(iTeam, iStartPlateauOrZero, iStartLandOrWaterZone, iCurPlateau, iCurZone, not(M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir]), iGroundAAThreshold, iZoneAirAAThreshold, false, iAirSubteam, true, nil, nil, nil, tCurTarget)
                                                if bDebugMessages == true then LOG(sFunctionRef..': bCurTargetTooDangerous after second detailed check for LZ='..tostring(bCurTargetTooDangerous)) end
                                            end
                                            if bDebugMessages == true then LOG(sFunctionRef..': bCurTargetTooDangerous is too dangerous after detailed LZ check, is it too dangerous after adjusting midpoint to be tcurposition='..tostring(bCurTargetTooDangerous)..';  tCurWZTeamData[M28Map.subrefiThreatEnemyGroundAA]='..( tCurLZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 'nil')) end
                                        end
                                    ...
                                    if bCurTargetTooDangerous then
                                        if bDebugMessages == true then LOG(sFunctionRef..': Land Zone is too dangerous so will stop looking further') end
                                        break
                                    else
                                        --Record this as the 'previous' entry so if we run into a dnagerous location later on we will use this
                                        iPrevPlateau = iCurPlateau
                                        ...
                                        if bDebugMessages == true then LOG(sFunctionRef..': This land zone seems ok to treat as the support point, P='..iCurPlateau..'; Z='..iCurZone) end
                                    end
                                end
                            end
                        else
        ...
        if not(tSupportRallyPoint) then
            ...
            local iSupportPlateau, iSupportLZOrWZ = M28Map.GetClosestPlateauOrZeroAndZoneToPosition(tSupportRallyPoint)
...
function DoesEnemyHaveAAThreatAlongPath(iTeam, iStartPlateauOrZero, iStartLandOrWaterZone, iEndPlateauOrZero, iEndLandOrWaterZone, bIgnoreAirAAThreat, iGroundAAThreatThreshold, iAirAAThreatThreshold, bUsingTorpBombers, iAirSubteam, bDoDetailedCheckForAA, bReturnGroundAAThreatInstead, tOptionalStartMidpointAdjustForDetailedCheck, bReturnGroundAAUnitsAlongsideAAThreat, tOptionalEndMidpointAdjustForDetailedCheck)
        ...
        for iEntry, tPlateauAndLandZone in tBasePathingTable[subreftPlateauAndLandZonesInPath] do
            ...
            if bReturnGroundAAThreatInstead then
                if bDoDetailedCheckForAA then
                    ...
                    if (tLZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) > 0 then
                        iGroundAAThreat = iGroundAAThreat + tLZTeamData[M28Map.subrefiThreatEnemyGroundAA]
    ...
    if M28Utilities.IsTableEmpty(tBasePathingTable[subreftWaterZonesInPath]) == false then
        for iEntry, iWaterZone in tBasePathingTable[subreftWaterZonesInPath] do
            ...
            if bDebugMessages == true then
                LOG(sFunctionRef..': Checking along water zone path, iEntry='..iEntry..'; iWaterZone='..iWaterZone..'; IsThereAAInWaterZone='..tostring(IsThereAAInWaterZone(tWZTeamData, bIgnoreAirAAThreat, iGroundAAThreatThreshold, iAirAAThreatThreshold))..'; tWZTeamData groundAA='..tWZTeamData[M28Map.subrefiThreatEnemyGroundAA]..'; AirAA threat in WZ='..tWZTeamData[M28Map.refiEnemyAirAAThreat])
            ...
            if bReturnGroundAAThreatInstead then
                if bDoDetailedCheckForAA then
                    ...
                    if (tWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) > 0 then
                        iGroundAAThreat = iGroundAAThreat + tWZTeamData[M28Map.subrefiThreatEnemyGroundAA]
                        if bReturnGroundAAUnitsAlongsideAAThreat then --redundancy - not currently planning on using for this, only for detailed check
...
function AddEnemyAirUnitsAlongPath(iTeam, iStartPlateauOrZero, iStartLandOrWaterZone, iEndPlateauOrZero, iEndLandOrWaterZone, tbAlongPathPlateauAndZonesAlreadyAdded, tEnemyAirTargets, iAirAAThreshold, iGroundAAThreshold, iMaxModDistPercent, iMinDistToEnemyBase)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': Near start at time '..GetGameTimeSeconds()..', iStartPlateauOrZero='..iStartPlateauOrZero..'; iStartLandOrWaterZone='..iStartLandOrWaterZone..'; iEndPlateauOrZero='..iEndPlateauOrZero..'; iEndLandOrWaterZone='..iEndLandOrWaterZone..'; Is table of land azones in path empty='..tostring(M28Utilities.IsTableEmpty(tBasePathingTable[subreftPlateauAndLandZonesInPath]) or false)..'; Is table of water zones in path empty='..tostring(M28Utilities.IsTableEmpty(tBasePathingTable[subreftWaterZonesInPath]) or false)) end
    local iAngleToDestination, iDistanceToDestination, iAngleToInterim, iDistanceToInterim, iInterimBestRange, tStartZoneMidpoint
    ...
    if M28Utilities.IsTableEmpty(tBasePathingTable[subreftPlateauAndLandZonesInPath]) == false then
        for iEntry, tPlateauAndLandZone in tBasePathingTable[subreftPlateauAndLandZonesInPath] do
            if not(tbAlongPathPlateauAndZonesAlreadyAdded[tPlateauAndLandZone[1]][tPlateauAndLandZone[2]]) then
                ...
                if bDebugMessages == true then LOG(sFunctionRef..': Considering entry '..iEntry..'; iCurPlateau='..iCurPlateau..'; iCurZone='..iCurZone..'; tLZTeamData mod dist='..tLZTeamData[M28Map.refiModDistancePercent]..'; enemy groundAA='..(tLZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 'nil')..'; AirAA threat='..(tLZTeamData[M28Map.refiEnemyAirAAThreat] or 'nil')..'; iGroundAAThreshold='..iGroundAAThreshold..'; iAirAAThreshold='..iAirAAThreshold..'; LZData midpoint='..repru(tLZData[M28Map.subrefMidpoint])..'; iMaxModDistPercent='..iMaxModDistPercent) end
                if M28Utilities.IsTableEmpty(tLZTeamData[M28Map.reftLZEnemyAirUnits]) == false and (tLZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) <= iGroundAAThreshold and (tLZTeamData[M28Map.refiEnemyAirAAThreat] or 0) <= iAirAAThreshold and tLZTeamData[M28Map.refiModDistancePercent] <= iMaxModDistPercent then
...
function ManageAirAAUnits(iTeam, iAirSubteam)
    ...
    if M28Utilities.IsTableEmpty(tAvailableAirAA) == false then
        ...
        if M28Team.tAirSubteamData[iAirSubteam][M28Team.refbHaveAirControl] then
            iLowerPriorityEnemyAirAADefaultThreshold = math.max(40000, M28Team.tTeamData[iTeam][M28Team.subrefiOurAirAAThreat])
            ...
            iLowerPriorityEnemyAirAADefaultThreshold = M28Team.tTeamData[iTeam][M28Team.subrefiOurAirAAThreat] * 0.25
                ...
                if bDontCheckPacifistArea or not(M28Conditions.AdjacentToPacifistZone(iPlateau, iLandZone)) then
                    ...
                    if not(iAlongPathAAThreshold) or refiAASearchType == refiAvoidOnlyGroundAA then
                        if tLZTeamData[M28Map.refiModDistancePercent] <= 0.25 or tLZTeamData[M28Map.subrefLZSValue] >= 2000 or ( tLZTeamData[M28Map.subrefLZSValue] >= 10 and tLZTeamData[M28Map.refiModDistancePercent] <= 0.4) then
                            ...
                            iAlongPathAAThreshold = iLowerPriorityEnemyAirAADefaultThreshold
                        end
                    end
                    ...
                    local iGroundAAThreshold = iOptionalGroundThreatThresholdOverride
                    if (tLZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) == 0 and (tLZTeamData[M28Map.refiEnemyAirAAThreat] or 0) == 0 and M28Utilities.IsTableEmpty(tLZTeamData[M28Map.reftLZEnemyAirUnits]) == false and M28Utilities.IsTableEmpty(EntityCategoryFilterDown(M28UnitInfo.refCategoryTransport, tLZTeamData[M28Map.reftLZEnemyAirUnits])) then
                        --Are presumably only targeting scouts so avoid AA
        ...
        function AddEnemyAirInWaterZoneIfNoAA(iWaterZone, bAddAdjacentZones, refiAASearchType, iOptionalGroundThreatThresholdOverride, iOptionalAirThreatThresholdOverride, iOptionalMaxDistToEdgeOfAdjacentZone, tOptionalStartPointForEdgeOfAdacentZone, toOptionalUnitOverride, iOptionalAdjacentZoneSearchType, iOptionalMinDistToClosestEnemyBaseIfGroundThreat)
            --See above for refiAASearchTypes, i.e. refiAvoidAllAA, refiAvoidOnlyGroundAA, refiIgnoreAllAA
            ...
            if not(tbAdjacentWaterZonesConsidered[refiAASearchType][iWaterZone]) and (bAddAdjacentZones or not(tbWaterZonesConsidered[refiAASearchType][iWaterZone])) then
                ...
                if bDontCheckPacifistArea or not(M28Conditions.AdjacentToPacifistZone(0, iWaterZone)) then
                    ...
                    local iGroundAAThreshold = iOptionalGroundThreatThresholdOverride
                    if (tWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) == 0 and (tWZTeamData[M28Map.refiEnemyAirAAThreat] or 0) == 0 and M28Utilities.IsTableEmpty(tWZTeamData[M28Map.reftLZEnemyAirUnits]) == false and M28Utilities.IsTableEmpty(EntityCategoryFilterDown(M28UnitInfo.refCategoryTransport, tWZTeamData[M28Map.reftLZEnemyAirUnits])) then
                        --Are presumably only targeting scouts so avoid AA
                        iGroundAAThreshold = 0
                    end
                    ...
                    if M28Utilities.IsTableEmpty(toOptionalUnitOverride or tWZTeamData[M28Map.reftWZEnemyAirUnits]) == false then
                        ...
                        local iMinDistToEnemyBase
                        if refiAASearchType == refiIgnoreAllAA and iOptionalMinDistToClosestEnemyBaseIfGroundThreat and DoesEnemyHaveAAThreatAlongPath(iTeam, iStartPlateauOrZero, iStartLandOrWaterZone, 0, iWaterZone, refiAASearchType == refiAvoidOnlyGroundAA, iGroundAAThreshold, iAlongPathAAThreshold, nil, nil, bUseDetailedCheck) then iMinDistToEnemyBase = iOptionalMinDistToClosestEnemyBaseIfGroundThreat end
                        if refiAASearchType == refiIgnoreAllAA or not(DoesEnemyHaveAAThreatAlongPath(iTeam, iStartPlateauOrZero, iStartLandOrWaterZone, 0, iWaterZone, refiAASearchType == refiAvoidOnlyGroundAA, iGroundAAThreshold, iAlongPathAAThreshold, nil, nil, bUseDetailedCheck)) then
        ...
        if M28Utilities.IsTableEmpty(tAvailableAirAA) == false then
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Will consider priority units for defence if have available AirAA, is table of available AirAA empty='..tostring(M28Utilities.IsTableEmpty(tAvailableAirAA))..'; Is table of priority defence units empty='..tostring(M28Utilities.IsTableEmpty(M28Team.tAirSubteamData[iAirSubteam][M28Team.reftACUExpAndPriorityDefenceOnSubteam]))) end
            if M28Utilities.IsTableEmpty(tAvailableAirAA) == false then
                if M28Utilities.IsTableEmpty(M28Team.tAirSubteamData[iAirSubteam][M28Team.reftACUExpAndPriorityDefenceOnSubteam]) == false then
                    ...
                    local toPriorityUnitsByPlateauAndZone = {}
                    for iUnit, oUnit in M28Team.tAirSubteamData[iAirSubteam][M28Team.reftACUExpAndPriorityDefenceOnSubteam] do
                        iPlateauOrZero, iLandOrWaterZone = M28Map.GetClosestPlateauOrZeroAndZoneToPosition(oUnit:GetPosition())
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering priority unit '..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; P='..(iPlateauOrZero or 'nil')..'Z'..(iLandOrWaterZone or 'nil')) end
                    ...
                    if M28Utilities.IsTableEmpty(toPriorityUnitsByPlateauAndZone) == false then
                        ...
                        local iGroundAAModifier = 0
                        local iAirAAModifier = 0
                        for iPlateauOrZero, tPriorityUnitsInZones in toPriorityUnitsByPlateauAndZone do
                            if bDebugMessages == true then LOG(sFunctionRef..': Considering iPlateauOrZero='..(iPlateauOrZero or 'nil')..' will cycle through every zone and its priority units') end
                            for iLandOrWaterZone, tUnits in tPriorityUnitsInZones do
                                local oClosestUnitToEnemyBase

                                local tUnitLZOrWZData
                                local tUnitLZOrWZTeamData
                                if bDebugMessages == true then LOG(sFunctionRef..': About to consider adding enemy air in land or water zone near priority units, first unit in table='..(tUnits[1].UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(tUnits[1]) or 'nil')..' to protect, iPlateauOrZero='..(iPlateauOrZero or 'nil')..'; iLandOrWaterZone='..(iLandOrWaterZone or 'nil')) end
                                if iPlateauOrZero == 0 then
                                    if (iLandOrWaterZone or 0) > 0 then
                                        tUnitLZOrWZData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iLandOrWaterZone]][M28Map.subrefPondWaterZones][iLandOrWaterZone]
                                        ...
                                        if not(tbWaterZonesConsidered[iLandOrWaterZone]) then
                                            ...
                                            if EntityCategoryContains(M28UnitInfo.refCategoryGunship + M28UnitInfo.refCategoryCzar, oClosestUnitToEnemyBase.UnitId) and (M28Team.tAirSubteamData[iAirSubteam][M28Team.refiOurGunshipAAThreat] or 0) >= 1000 then
                                                ...
                                                iAirAAModifier = math.max(0, iAirAAAvoidThreshold - M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat]) + (M28Team.tAirSubteamData[iAirSubteam][M28Team.refiOurGunshipAAThreat] or 0)
                                            else
                                                iGroundAAModifier = 0
                                                iAirAAModifier = 0
                                            end
                                            if bDebugMessages == true then LOG(sFunctionRef..': oClosestUnitToEnemyBase='..oClosestUnitToEnemyBase.UnitId..M28UnitInfo.GetUnitLifetimeCount(oClosestUnitToEnemyBase)..'; iGroundAAModifier='..iGroundAAModifier..'; iAirAAModifier='..iAirAAModifier..'; iGroundAAAdjacentThreshold='..iGroundAAAdjacentThreshold..'; iAirAAAvoidThreshold='..iAirAAAvoidThreshold..'; Gunship air to ground='..(M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurGunshipThreat] or 'nil')) end

                                            iCurUnitAASearchType = GetAASearchTypeForPriorityUnit(oClosestUnitToEnemyBase, iPlateauOrZero, tUnitLZOrWZData, tUnitLZOrWZTeamData)
                                    ...
                                    if (iLandOrWaterZone or 0) > 0 then
                                        ...
                                        if EntityCategoryContains(M28UnitInfo.refCategoryGunship + M28UnitInfo.refCategoryCzar, oClosestUnitToEnemyBase.UnitId) and (M28Team.tAirSubteamData[iAirSubteam][M28Team.refiOurGunshipAAThreat] or 0) >= 1000 then
                                            if M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir] then
                                                iGroundAAModifier = M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurGunshipThreat] * 0.05
                                            else
                                                iGroundAAModifier = M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurGunshipThreat] * 0.1
                                            end
                                            iAirAAModifier = math.max(0, iAirAAAvoidThreshold - M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat]) + (M28Team.tAirSubteamData[iAirSubteam][M28Team.refiOurGunshipAAThreat] or 0)
                                        else
                                            iGroundAAModifier = 0
                                        ...
                                        AddEnemyAirInLandZoneIfNoAA(iPlateauOrZero, iLandOrWaterZone, false, iCurUnitAASearchType, iGroundAAAdjacentThreshold + iGroundAAModifier, iAirAAAvoidThreshold + iAirAAModifier, nil,           nil,                                        nil,                    nil,            iClosestDistToEnemyBase - iPrioirtyDistToEnemyBaseAdj)
                                        if bDebugMessages == true then LOG(sFunctionRef..': Just finished considering adding enemy air in land zone near priority unit oClosestUnitToEnemyBase '..(oClosestUnitToEnemyBase.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oClosestUnitToEnemyBase) or 'nil')..' to protect, iAASearchType='..iAASearchType..'; iPlateauOrZero='..iPlateauOrZero..'; iLandOrWaterZone='..iLandOrWaterZone..'; tUnitLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat]='..(tUnitLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 'nil')..'; iCurUnitAASearchType='..iCurUnitAASearchType..'; AirAA threat of enemy units='..M28UnitInfo.GetAirThreatLevel(tEnemyAirTargets, true, true, false, false, false, false)) end
                ...
                if M28UnitInfo.IsUnitValid(M28Team.tAirSubteamData[iAirSubteam][M28Team.refoFrontGunship]) then
                    ...
                    if not(M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir]) or (M28Team.tAirSubteamData[iAirSubteam][M28Team.refiOurGunshipAAThreat] >= 1000 and M28Team.tAirSubteamData[iAirSubteam][M28Team.refiOurGunshipAAThreat] + M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat] > M28Team.tTeamData[iTeam][M28Team.refiEnemyAirAAThreat] * 0.9) then
...
function EnemyBaseEarlyBomber(oBomber)
    local bDebugMessages = false if M28Profiler.bGlobalDebugOverride == true then   bDebugMessages = true end
    local sFunctionRef = 'EnemyBaseEarlyBomber'
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerStart)
    ...
    if M28UnitInfo.IsUnitValid(oBomber) then
        ...
        --Get closest enemy base
        local iStartPlateauOrZero, iStartLandOrWaterZone = M28Map.GetClosestPlateauOrZeroAndZoneToPosition(oBomber:GetPosition())
        if not(iStartLandOrWaterZone) or (iStartPlateauOrZero or 0) == 0 then
            iStartPlateauOrZero, iStartLandOrWaterZone = M28Map.GetClosestPlateauOrZeroAndZoneToPosition(M28Map.GetPlayerStartPosition(oBomber:GetAIBrain()))
        end
        if iStartPlateauOrZero and iStartLandOrWaterZone and iStartPlateauOrZero > 0 then
            local tStartLZTeamData = M28Map.tAllPlateaus[iStartPlateauOrZero][M28Map.subrefPlateauLandZones][iStartLandOrWaterZone][M28Map.subrefLZTeamData][iTeam]
            tEnemyBase = tStartLZTeamData[M28Map.reftClosestEnemyBase]
        end
        if bDebugMessages == true then LOG(sFunctionRef..': tEnemyBase='..repru(tEnemyBase)..'; Dist to enemy base='..M28Utilities.GetDistanceBetweenPositions(oBomber:GetPosition(), tEnemyBase)) end
        if M28Utilities.IsTableEmpty(tEnemyBase) == false then
            ...
            if (iEnemyBasePlateau or 0) > 0 and iEnemyBaseZone then
                ...
                if M28Utilities.IsTableEmpty(tEnemyBaseLZData[M28Map.subrefHydroLocations]) == false then
                    local iCurDist
                    local iClosestHydro = 10000
                    for iHydro, tHydro in tEnemyBaseLZData[M28Map.subrefHydroLocations] do
                        iCurDist = M28Utilities.GetDistanceBetweenPositions(tEnemyBase, tHydro)
                        if iCurDist < iClosestHydro then
                            tEnemyInitialDestination = {tHydro[1], tHydro[2], tHydro[3]}
                        end
                    end
                end
                ...
                while M28UnitInfo.IsUnitValid(oBomber) and oBomber[rebEarlyBomberTargetBase] do
                    ...
                    elseif not((iStartLandOrWaterZone == iEnemyBaseZone and iStartPlateauOrZero == iEnemyBasePlateau) or M28Utilities.GetDistanceBetweenPositions(oBomber:GetPosition(), tEnemyInitialDestination) <= 90) then
                        ...
                        if not(bReachedInitialLocation) and M28Utilities.GetDistanceBetweenPositions(oBomber:GetPosition(), tEnemyInitialDestination) <= 20 then
                            bReachedInitialLocation = true
                        elseif not(bReachedEnemyStart) and M28Utilities.GetDistanceBetweenPositions(oBomber:GetPosition(), tEnemyBase) <= 20 then
                            bReachedEnemyStart = true
                        end
                        ...
                        if M28Utilities.IsTableEmpty(tEnemyBaseLZTeamData[M28Map.subrefTEnemyUnits]) == false then
                            for iUnit, oUnit in tEnemyBaseLZTeamData[M28Map.subrefTEnemyUnits] do
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': Finished searching for enemy targets, is table empty='..tostring(M28Utilities.IsTableEmpty(tEnemyTargets))..'; Time='..GetGameTimeSeconds()) end
                        if M28Utilities.IsTableEmpty(tEnemyTargets) then
                            --If reached enemy base and hydro then abort this logic and switch to normal engi hunter logic
                            if not(bReachedInitialLocation) then
                                if bDebugMessages == true then LOG(sFunctionRef..'; No enemy targets, will go to the initial destination') end
                                M28Orders.IssueTrackedMove(oBomber, tEnemyInitialDestination, 5, false, '1stBombCM', true)
                            elseif not(bReachedEnemyStart) then
                                if bDebugMessages == true then LOG(sFunctionRef..'; No enemy targets, will go to the enemy base') end
                                M28Orders.IssueTrackedMove(oBomber, tEnemyBase, 5, false, '1stBombDM', true)
                            else
                                if bDebugMessages == true then LOG(sFunctionRef..': No enemy engis or t1 pgens so will switch to default engi hunter logic') end
                                oBomber[rebEarlyBomberTargetBase] = false
                                --Return to base as temporary order (normal engi hunter logic should take over shortly)
                                M28Orders.IssueTrackedMove(oBomber, tEnemyBaseLZTeamData[M28Map.reftClosestFriendlyBase], 5, false, '1stBombXb', true)
                                break
                            end
                        else
    ...
    if oBomber[M28UnitInfo.refbSpecialMicroActive] then
        ...
        ForkThread(M28Micro.ForkedResetMicroFlag, oBomber, 0.01) --redundancy
    end
    oBomber[rebEarlyBomberTargetBase] = false
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
end
...
function ApplyEngiHuntingBomberLogic(oBomber, iAirSubteam, iTeam)
    ...
    if (iStartLandOrWaterZone or 0) > 0 then
        ...
        if tStartLZOrWZData then
            ...
            local bAbort = false
            --Run to base if zone has AA and we arent targeting an engineer already
            local tLastOrder = oBomber[M28Orders.reftiLastOrders][oBomber[M28Orders.refiOrderCount]]
            ...
            elseif (tStartLZOrWZTeamData[M28Map.refiEnemyAirAAThreat] or 0) > 0 then bDontCheckForEnemyThreats = true
            end
            if bDebugMessages == true then LOG(sFunctionRef..': bAbort after checking for groundAA in cur zone (if bomber not targeting engineer)='..tostring(bAbort)) end
            if not(bAbort) then
                    ...
                    if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subreftLandAndWaterZoneScoutingShortlist]) == false then
                        ...
                        for iEntry, tPlateauAndZoneRef in M28Team.tTeamData[iTeam][M28Team.subreftLandAndWaterZoneScoutingShortlist] do
                            ...
                            if tPlateauAndZoneRef[1] > 0 then
                                ...
                                if (tScoutingLZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) == 0 and (tScoutingLZTeamData[M28Map.refiEnemyAirAAThreat] or 0) == 0 then
                                    if bDebugMessages == true then LOG(sFunctionRef..': Considering if we want to scout the zone P'..tPlateauAndZoneRef[1]..'Z'..tPlateauAndZoneRef[2]..'; Dist to us='..M28Utilities.GetDistanceBetweenPositions(oBomber:GetPosition(), tScoutingLZData[M28Map.subrefMidpoint])..'; Is table of unbuilt mexes empty='..tostring(M28Utilities.IsTableEmpty(tScoutingLZData[M28Map.subrefMexUnbuiltLocations]))..'; Mod dist%='..(tScoutingLZTeamData[M28Map.refiModDistancePercent] or 'nil')) end
                                    if M28Utilities.IsTableEmpty(tScoutingLZData[M28Map.subrefMexUnbuiltLocations]) == false then
                                        iCurDist = M28Utilities.GetDistanceBetweenPositions(oBomber:GetPosition(), tScoutingLZData[M28Map.subrefMidpoint])
                                        --Modify the distance if enemy has groundAA threat here, or lots of mexes
                                        if iCurDist <= iClosestOnEnemySide then
                                            iModDist = iCurDist
                                            --DoesEnemyHaveAAThreatAlongPath(iTeam, iStartPlateauOrZero, iStartLandOrWaterZone, iEndPlateauOrZero, iEndLandOrWaterZone, bIgnoreAirAAThreat, iGroundAAThreatThreshold, iAirAAThreatThreshold, bUsingTorpBombers, iAirSubteam, bDoDetailedCheckForAA, bReturnGroundAAThreatInstead, tOptionalStartMidpointAdjustForDetailedCheck)
                                            iCurGroundAAThreat = DoesEnemyHaveAAThreatAlongPath(iTeam, iStartPlateauOrZero, iStartLandOrWaterZone, tPlateauAndZoneRef[1], tPlateauAndZoneRef[2], true,              1,                      400,                false,              iAirSubteam, true,                  true,                           oBomber:GetPosition())
                        ...
                        if tClosestEnemySideMidpoint then
                            M28Orders.IssueTrackedMove(oBomber, tClosestEnemySideMidpoint, 10, false, 'BScEP'..iClosestEnemyPlateauOrZero..'Z'..iClosestEnemyLZOrWZRef, false)
                            tBomberTable = nil
                        elseif tClosestMidpoint then
                            M28Orders.IssueTrackedMove(oBomber, tClosestMidpoint, 10, false, 'BScAP'..iClosestPlateauOrZero..'Z'..iClosestLZOrWZRef, false)
                            tBomberTable = nil
                        else
                            ...
                            M28Orders.IssueTrackedAggressiveMove(oBomber, tStartLZOrWZTeamData[M28Map.reftClosestEnemyBase], 10, false, 'BAtBs', false)
                            tBomberTable = nil
                        end
                    end
                end
...
function ManageBombers(iTeam, iAirSubteam)
    ...
    if M28Utilities.IsTableEmpty(tAvailableBombers) == false then
        ...
        if M28Team.tTeamData[iTeam][M28Team.refiBomberLosses] > 0 then
            iSearchSize = math.min(2, math.max(0.5, M28Team.tTeamData[iTeam][M28Team.refiBomberKills] / M28Team.tTeamData[iTeam][M28Team.refiBomberLosses]))
        end
        ...
        if M28Utilities.IsTableEmpty(tAvailableBombers) == false then
            ...
            elseif tRallyLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.2 then
                if tRallyLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.35 then
                    iAAPriorityThresholdFactor = 1
                else
                    iAAPriorityThresholdFactor = 1.25
                end
            end
            if bDebugMessages == true then LOG(sFunctionRef..': iAAPriorityThresholdFactor for zones adjacent to rally or high priority enemy targets='..iAAPriorityThresholdFactor..'; Is table of enemies in rally zone empty='..tostring(M28Utilities.IsTableEmpty(tRallyLZOrWZTeamData[M28Map.subrefTEnemyUnits]))) end
            FilterToAvailableTargets(tRallyLZOrWZTeamData[M28Map.subrefTEnemyUnits])
            if M28Utilities.IsTableEmpty( tEnemyTargets) == false then
                if bDebugMessages == true then LOG(sFunctionRef..': Attacking enemy units in the rally zone') end
            ...
            if M28Utilities.IsTableEmpty(tAvailableBombers) == false then
                ...
                if bConsiderHigherTechUnitsFirst then
                    ...
                    if M28Utilities.IsTableEmpty(tEnemyTargets) == false then
                        ...
                        if M28Utilities.IsTableEmpty(toPriorityEnemiesToTarget) == false then
                            AssignTorpOrBomberTargets(tAvailableBombers, toPriorityEnemiesToTarget, iAirSubteam, false, true)
                        end
                    end
                                    ...
                                    if not(tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero][iAdjLZ]) then
                                        ...
                                        if M28Utilities.IsTableEmpty( tEnemyTargets) == false then
                                            if tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero][iAdjLZ] == nil then
                                                if tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero] == nil then tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero] = {} end
                                                tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero][iAdjLZ] = DoesEnemyHaveAAThreatAlongPath(iTeam, iRallyPlateauOrZero, iRallyLZOrWZ, iRallyPlateauOrZero, iAdjLZ, M28Team.tAirSubteamData[iAirSubteam][M28Team.refbHaveAirControl], iMaxEnemyGroundAAThreat * iAAPriorityThresholdFactor, nil, false, iAirSubteam, true, false, nil, false)
                                            end
                                            if not(tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero][iAdjLZ]) then
                                                AssignTorpOrBomberTargets(tAvailableBombers, tEnemyTargets, iAirSubteam, false, true)
                                            end
                            ...
                            if M28Utilities.IsTableEmpty(tAvailableBombers) == false then
                                if M28Utilities.IsTableEmpty(tRallyLZOrWZData[M28Map.subrefAdjacentWaterZones]) == false then
                                    ...
                                    for iEntry, tSubtable in tRallyLZOrWZData[M28Map.subrefAdjacentWaterZones] do
                                        local iAdjWZ = tSubtable[M28Map.subrefAWZRef]
                                        if not(tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero][iAdjWZ]) then
                                            local tAdjWZTeamData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iAdjWZ]][M28Map.subrefPondWaterZones][iAdjWZ][M28Map.subrefWZTeamData][iTeam]
                                            ...
                                            if M28Utilities.IsTableEmpty( tEnemyTargets) == false then
                                                if tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero][iRallyLZOrWZ] == nil then
                                                    if tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero] == nil then tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero] = {} end
                                                    tbZoneByPlateauHasTooMuchAA[iRallyPlateauOrZero][iRallyLZOrWZ] = DoesEnemyHaveAAThreatAlongPath(iTeam, iRallyPlateauOrZero, iRallyLZOrWZ, 0, iAdjWZ, M28Team.tAirSubteamData[iAirSubteam][M28Team.refbHaveAirControl], iMaxEnemyGroundAAThreat * iAAPriorityThresholdFactor, nil, false, iAirSubteam, true, false, nil, false)
                                                end
                            ...
                            if M28Utilities.IsTableEmpty(tAvailableBombers) == false and (M28Team.tAirSubteamData[iAirSubteam][M28Team.refbOrigRallyOutsidePlayableArea] or M28Team.tTeamData[iTeam][M28Team.refbDontHaveBuildingsOrACUInPlayableArea] or (M28Team.tAirSubteamData[iAirSubteam][M28Team.refbHaveAirControl] and iAvailableBombers >= 8 and (iAvailableBombers >= 60 or M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurBomberThreat] >= 25000 or M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyAirFactoryTech] < 3 or (M28Map.bIsCampaignMap and iAvailableBombers >= 12 and (iAvailableBombers >= 12 * M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyAirFactoryTech] or M28Team.tTeamData[iTeam][M28Team.refiTimeLastNearUnitCap]))))) then
                                ...
                                if M28Utilities.IsTableEmpty(tRallyLZOrWZData[M28Map.subrefOtherLandAndWaterZonesByDistance]) == false then
                                    ...
                                    for iEntry, tPathingDetails in tRallyLZOrWZData[M28Map.subrefOtherLandAndWaterZonesByDistance] do
                                        ...
                                        if not(tbZonesConsideredByPlateau[iRallyPlateauOrZero][iOtherLZOrWZ]) and iOtherLZOrWZ and not(tbZoneByPlateauHasTooMuchAA[iOtherPlateauOrZero][iOtherLZOrWZ]) then
                                            local tOtherLZOrWZData
                                            local tOtherLZOrWZTeamData
                                            if iOtherPlateauOrZero == 0 then
                                                tOtherLZOrWZData = M28Map.tPondDetails[tPathingDetails[M28Map.subrefiPlateauOrPond]][M28Map.subrefPondWaterZones][iOtherLZOrWZ]
                                                tOtherLZOrWZTeamData = tOtherLZOrWZData[M28Map.subrefWZTeamData][iTeam]
                                                iCurGroundAAThreat = (tOtherLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0)
                                            else
                                                tOtherLZOrWZData = M28Map.tAllPlateaus[iOtherPlateauOrZero][M28Map.subrefPlateauLandZones][iOtherLZOrWZ]
                                                tOtherLZOrWZTeamData = tOtherLZOrWZData[M28Map.subrefLZTeamData][iTeam]
                                                iCurGroundAAThreat = (tOtherLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0)
        ...
        if M28Utilities.IsTableEmpty(tiWaterZoneByDistance) == false then
            ...
            for iWaterZone, iDistance in M28Utilities.SortTableByValue(tiWaterZoneByDistance, false) do
                ...
                if iMassValueOfEnemyUnits > 0 then
                    ...
                    if iAAThreatThreshold >= 500 and tWZTeamData[M28Map.refiModDistancePercent] >= 0.3 then
                        if tWZTeamData[M28Map.refiModDistancePercent] >= 0.75 then
                            iAAThreatThreshold = math.min(iAAThreatThreshold, iMassValueOfEnemyUnits)
                        elseif tWZTeamData[M28Map.refiModDistancePercent] >= 0.5 then
                            iAAThreatThreshold = math.min(iAAThreatThreshold, iMassValueOfEnemyUnits * 1.3)
                        else
...
function AssignTorpOrBomberTargets(tAvailableBombers, tEnemyTargets, iAirSubteam, bForceGroundFire, bTargetAAAndShieldsFirst, bIgnoreMicro)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': Start of code at time='..GetGameTimeSeconds()..'; bIgnoreMicro='..tostring(bIgnoreMicro or false)) end
...
function ManageGunships(iTeam, iAirSubteam)
    ...
    local tGunshipLandOrWaterZoneData, tGunshipLandOrWaterZoneTeamData, iGunshipPlateauOrZero, iGunshipLandOrWaterZone
    if M28UnitInfo.IsUnitValid(M28Team.tAirSubteamData[iAirSubteam][M28Team.refoFrontGunship]) then
        ...
        iGunshipPlateauOrZero, iGunshipLandOrWaterZone = M28Map.GetClosestPlateauOrZeroAndZoneToPosition(M28Team.tAirSubteamData[iAirSubteam][M28Team.refoFrontGunship]:GetPosition())
        if iGunshipPlateauOrZero and (iGunshipLandOrWaterZone or 0) > 0 then
            ...
            if iGunshipPlateauOrZero == 0 then
                ...
                iEnemyGroundAAThreatByGunship = (tGunshipLandOrWaterZoneTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0)
            ...
            --NOTE: This is used for deciding whether we do the calculations of via points, and for some of the higher priority gunship logic before the main code (where a more precise check is done) - for the more detailed check, look for the log with the text "About to check if shoudl run due to high AA near where gunships are"
            --Also include adjacent zones
            ...
            if M28Utilities.IsTableEmpty(tGunshipLandOrWaterZoneData[M28Map.subrefOtherLandAndWaterZonesByDistance]) == false then
                ...
                if bDebugMessages == true then LOG(sFunctionRef..': iEnemyAirAAThreatNearGunship before considering nearby zones and getunitsaroundpoint='..iEnemyAirAAThreatNearGunship) end
                for iEntry, tSubtable in tGunshipLandOrWaterZoneData[M28Map.subrefOtherLandAndWaterZonesByDistance] do
                    if tSubtable[M28Map.subrefiDistance] <= iSearchDistance then
                        local tOtherZoneData, tOtherTeamData
                        if tSubtable[M28Map.subrefbIsWaterZone] then
                            tOtherZoneData = M28Map.tPondDetails[tSubtable[M28Map.subrefiPlateauOrPond]][M28Map.subrefPondWaterZones][tSubtable[M28Map.subrefiLandOrWaterZoneRef]]
                            tOtherTeamData = tOtherZoneData[M28Map.subrefWZTeamData][iTeam]
                        else
                            tOtherZoneData = M28Map.tAllPlateaus[tSubtable[M28Map.subrefiPlateauOrPond]][M28Map.subrefPlateauLandZones][tSubtable[M28Map.subrefiLandOrWaterZoneRef]]
                            tOtherTeamData = tOtherZoneData[M28Map.subrefLZTeamData][iTeam]
                        end
                        if (tOtherTeamData[M28Map.refiEnemyAirAAThreat] or 0) > 0 then
                            iEnemyAirAAThreatNearGunship = iEnemyAirAAThreatNearGunship + 0.5 * tOtherTeamData[M28Map.refiEnemyAirAAThreat]
                        end
                    else
                        break
                    end
    ...
    if M28Utilities.IsTableEmpty(tAvailableGunships) == false then
        ...
        if not(tGunshipLandOrWaterZoneData) then
            iGunshipPlateauOrZero, iGunshipLandOrWaterZone = M28Map.GetClosestPlateauOrZeroAndZoneToPosition(oFrontGunship:GetPosition())
            if iGunshipPlateauOrZero == 0 then
                --iGunshipLandOrWaterZone = M28Map.GetWaterZoneFromPosition(oFrontGunship:GetPosition())
                tGunshipLandOrWaterZoneData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iGunshipLandOrWaterZone]][M28Map.subrefPondWaterZones][iGunshipLandOrWaterZone]
                tGunshipLandOrWaterZoneTeamData = tGunshipLandOrWaterZoneData[M28Map.subrefWZTeamData][iTeam]
        ...
        if iOurGunshipAA > M28Team.tTeamData[iTeam][M28Team.refiEnemyAirAAThreat] and iOurGunshipAA > 200 then
            if M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir] then
                ...
                iMaxEnemyAirAA = math.max(iOurGunshipAA, iOurGunshipThreat * 0.05)
                if bDebugMessages == true then LOG(sFunctionRef..': Relying on gunship AA as it is greater than enemy AA, iMaxEnemyAirAA before adjustment='..iMaxEnemyAirAA) end
            end
        elseif M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir] then
            iMaxEnemyAirAA = math.min(2000, math.max(iOurGunshipAA * 0.2, iOurGunshipThreat * 0.03))
            if bDebugMessages == true then LOG(sFunctionRef..': Far behind on air, iMaxEnemyAirAA='..iMaxEnemyAirAA) end
        elseif iOurGunshipAA > M28Team.tTeamData[iTeam][M28Team.refiEnemyAirAAThreat] * 0.75 and iOurGunshipAA + M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat] > M28Team.tTeamData[iTeam][M28Team.refiEnemyAirAAThreat] * 0.9 then
            iMaxEnemyAirAA = math.max(iOurGunshipAA * 0.75, iOurGunshipThreat * 0.05)
            if bDebugMessages == true then LOG(sFunctionRef..': Not far behind on air and our gunship AA isnt that different from enemy AirAA threat, iMaxEnemyAirAA='..iMaxEnemyAirAA) end
        else
            ...
            if iDistToSupport <= 160 then
                ...
                if M28Team.tAirSubteamData[iAirSubteam][M28Team.refbHaveAirControl] then
                    ...
                    if bDebugMessages == true then LOG(sFunctionRef..': Far from air support and dont have air control, iMaxEnemyAirAA='..iMaxEnemyAirAA) end
                end
            end
        end
        if M28Map.bIsCampaignMap then
            ...
            if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftoEnemyAirToGround]) == false then
                ...
                if M28Utilities.IsTableEmpty(tEnemySoulrippers) == false then
                    for iUnit, oUnit in tEnemySoulrippers do
                        local iCurExpPlat, iCurExpZone = M28Map.GetClosestPlateauOrZeroAndZoneToPosition(oUnit:GetPosition())
        ...
        function AddEnemyGroundUnitsToTargetsSubjectToAA(iPlateauOrZero, iLandOrWaterZone, iGunshipThreatFactorWanted, bCheckForAirAA, bOnlyIncludeIfMexToProtect, iGroundAAThresholdAdjust, bIgnoreMidpointPlayableCheck, bDetailedAACheckOverride)
            ...
            if not(M28Map.bIsCampaignMap) or ((bIgnoreMidpointPlayableCheck or (M28Conditions.IsLocationInPlayableArea(tLZOrWZData[M28Map.subrefMidpoint]))) and (bDontCheckForPacifism or not(M28Conditions.AdjacentToPacifistZone(iPlateauOrZero, iLandOrWaterZone)))) then
                ...
                if bDebugMessages == true then LOG('AddEnemyGroundUnitsToTargetsSubjectToAA: Deciding whether to add units for land zone '..iLandOrWaterZone..' plateau '..iPlateauOrZero..'; bOnlyIncludeIfMexToProtect='..tostring(bOnlyIncludeIfMexToProtect)..'; IsHighValueZoneToProtect='..tostring(IsHighValueZoneToProtect())..'; Enemy combat threat in zone if LZ='..(tLZOrWZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 0)..'; Mobile DF total if LZ='..(tLZOrWZTeamData[M28Map.subrefLZThreatEnemyMobileDFTotal] or 0)..'; Enemy AA total if LZ='..(tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0)..'; tLZOrWZTeamData[M28Map.refiModDistancePercent]='..tLZOrWZTeamData[M28Map.refiModDistancePercent]..'; bConsiderAttackingEnemyGunships='..tostring(bConsiderAttackingEnemyGunships)..'; tLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat]='..(tLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0)..'; Is table of enemy units in the zone empty='..tostring(M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefTEnemyUnits]))) end
                if (not(bOnlyIncludeIfMexToProtect) or IsHighValueZoneToProtect()) and (M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefTEnemyUnits]) == false or (bConsiderAttackingEnemyGunships and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftLZEnemyAirUnits]) == false)) then
                    local bDoDetailedGroundAACheck = bDetailedAACheckOverride
                    if IsHighValueZoneToProtect() then
                        if (tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) <= 200 * M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyAirFactoryTech] then iGunshipThreatFactorWanted = math.min(iGunshipThreatFactorWanted, math.max(iGunshipThreatFactorWanted * 0.5, 1.5))
        ...
        if not(iClosestSnipeTarget) or iClosestSnipeTarget >= 200 then
            ...
            if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftiCoreZonesByPlateau]) == false then
                ...
                for iBrain, oBrain in M28Team.tAirSubteamData[iAirSubteam][M28Team.subreftoFriendlyM28Brains] do
                    local tStartPoint = M28Map.GetPlayerStartPosition(oBrain)
                    iCurPlateauOrZero, iCurLZOrWZ = M28Map.GetClosestPlateauOrZeroAndZoneToPosition(tStartPoint)
                    if (iCurPlateauOrZero or 0) > 0 then
                        if (iCurLZOrWZ or 0) == 0 then
                ...
                if not(tTeleportTargetToMoveTo) then
                    ...
                    if not(IsThereAANearLandOrWaterZone(iTeam, iGunshipPlateauOrZero, iGunshipLandOrWaterZone, (iGunshipPlateauOrZero == 0), iOurGunshipThreat / iGunshipThreatFactorForSameZone, iMaxEnemyAirAA                  , iSearchDistance,                               oFrontGunship:GetPosition())) then
                        ...
                        if M28Utilities.IsTableEmpty(tEnemyGroundOrGunshipTargets) then
                            ...
                            if (tGunshipLandOrWaterZoneData[M28Map.subrefiThreatEnemyGroundAA] or 0) > 0 or (tGunshipLandOrWaterZoneData[M28Map.subrefiThreatEnemyGroundAA] or 0) > 0 then
            ...
            function ConsiderAdditionalDropZoneForIsland(iPlateau, iIsland, bCheckDistFromExistingDropLocations)
                ...
                if M28Utilities.IsTableEmpty(M28Map.tAllPlateaus[iPlateau][M28Map.subrefPlateauIslandLandZones][iIsland]) == false then
                    for iEntry, iLandZone in M28Map.tAllPlateaus[iPlateau][M28Map.subrefPlateauIslandLandZones][iIsland] do
                        ConsiderAddingZoneInIsland(iPlateau, iIsland, iLandZone, bCheckDistFromExistingDropLocations, nil, false)
                    end
                    --20km+ maps - go again, this time without the mex threshold
                    for iEntry, iLandZone in M28Map.tAllPlateaus[iPlateau][M28Map.subrefPlateauIslandLandZones][iIsland] do
                        ConsiderAddingZoneInIsland(iPlateau, iIsland, iLandZone, true, 3, true)
                    end
                end
            end
                            ...
                            if bBPWanted then
                                ...
                                if not(bHaveUnattachedEngineersOrFactories) then
                                    ...
                                    if not(bHaveNearbyFactoriesOrLargeThreat) then
                                        ...
                                        if bDebugMessages == true then LOG(sFunctionRef..': No major adjacent enemy threats, considering zone itself, enemy combat='..(tLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 0)..'; Enemy ground AA='..tLZTeamData[M28Map.subrefiThreatEnemyGroundAA]..'; Is table of enemy units empty='..tostring(M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subrefTEnemyUnits]))) end
                                        if not(bCampaignMap) and ((tLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 0) >= 100 or tLZTeamData[M28Map.subrefiThreatEnemyGroundAA] >= 175) then
...
function RecordPotentialCombatDropShortlist(iTeam)
    --Should only be called once per game - determines plateaus and zones where we should consider dropping with t1 arti if enemy has vulnerable mexes
    local bDebugMessages = false if M28Profiler.bGlobalDebugOverride == true then   bDebugMessages = true end
    local sFunctionRef = 'RecordPotentialCombatDropShortlist'
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerStart)

    --First record start positions for all players as dont want to drop these regardless
    ...
    local tbPlayerStartByPlateauAndZone = {}
    local tbFriendlyPlateaus = {}
    for iBrain, oBrain in toBrainsToConsider do
        ...
        if iCurPlateau and iCurLZ then
            if not(tbPlayerStartByPlateauAndZone[iCurPlateau]) then tbPlayerStartByPlateauAndZone[iCurPlateau] = {} end
            if bDebugMessages == true then LOG(sFunctionRef..': tEnemyStart='..repru(tEnemyStart)..' for brain '..oBrain.Nickname..'; iCurPlateau='..(iCurPlateau or 'nil')..'; iCurLZ='..(iCurLZ or 'nil')) end
            tbPlayerStartByPlateauAndZone[iCurPlateau][iCurLZ] = true
            if oBrain.M28Team == iTeam then tbFriendlyPlateaus[iCurPlateau] = true end
        end
    end
...
function UpdateTransportLocationShortlist(iTeam, bUpdateCombatDropShortlist)
    ...
    if bFirstTimeConsidering then
        ...
        RecordPotentialCombatDropShortlist(iTeam)
    end
...
function GetCombatDropPlateauAndLandZoneEntryRefForTransport(iTeam, oUnit)
    ...
    if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftTransportCombatPlateauLandZoneDropShortlist]) == false then
        ...
        if M28Team.tTeamData[iTeam][M28Team.refbEnemyBaseInCombatDropShortlist] then
            ...
            for iBrain, oBrain in M28Team.tTeamData[iTeam][M28Team.subreftoEnemyBrains] do
                local tEnemyStart = M28Map.GetPlayerStartPosition(oBrain)
                local iCurPlateau, iCurLZ = M28Map.GetPlateauAndLandZoneReferenceFromPosition(tEnemyStart)
                if iCurPlateau and iCurLZ and iCurPlateau > 0 then
                    if not(tEnemyStartZonesByPlateau[iCurPlateau]) then tEnemyStartZonesByPlateau[iCurPlateau] = {} end
                    tEnemyStartZonesByPlateau[iCurPlateau][iCurLZ] = true
                end
            end
        end
        ...
        M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
        return iClosestEntryRef
    end
    --local tLZData = M28Map.tAllPlateaus[137][M28Map.subrefPlateauLandZones][3]
                ...
                if iIslandToTravelTo or iWaterZoneToTravelTo then
                    ...
                    elseif iEngisHave >= 1 and oUnit[M28Orders.reftiLastOrders][1][M28Orders.subrefiOrderType] == M28Orders.refiOrderUnloadTransport then
                        ...
                        if oUnit[refbCombatDrop] then
                            iExtraEngisWanted = math.min(iEngiRemainingCapacity, 8 - iEngisHave)
                    ...
                    elseif iExtraEngisWanted > 0 then
                        ...
                        if tCurLZOrWZTeamData[M28Map.subrefLZbCoreBase] or iEngisHave == 0 then
                            ...
                            bGetMoreUnits = true
                            if oUnit[refbCombatDrop] then
                                oUnit[refiCombatUnitsWanted] = iExtraEngisWanted
                                oUnit[refiEngisWanted] = 0
                            else
                                oUnit[refiEngisWanted] = iExtraEngisWanted
                                oUnit[refiCombatUnitsWanted] = nil
                            ...
                            if tCurLZOrWZTeamData[M28Map.subrefLZbCoreBase] then
                                ...
                                if M28Utilities.IsTableEmpty(tCurLZOrWZTeamData[M28Map.reftoTransportsWaitingForUnits]) == false then
                                    for iRecordedTransport, oRecordedTransport in tCurLZOrWZTeamData[M28Map.reftoTransportsWaitingForUnits] do
                    ...
                    if M28Utilities.IsTableEmpty(tCargo) == false then
                        ...
                        if bDropNow then
                            ...
                            M28Utilities.ErrorHandler('Unexpected code event, likely overlooked something when changing code, will proceed with dropping early', true)
...
function ShouldTransportDropEarlyOrAlwaysDropAtTarget(oUnit, iTeam, bJustConsiderAlwaysDropping)
    ...
    if tLastOrder[M28Orders.subrefiOrderType] == M28Orders.refiOrderUnloadTransport then
        ...
        if not(bJustConsiderAlwaysDropping) then
            local tCurZoneData, tCurZoneTeamData = M28Map.GetLandOrWaterZoneData(oUnit:GetPosition(), true, iTeam)
            if bDebugMessages == true then LOG(sFunctionRef..': Transport oUnit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; Transport health%='..M28UnitInfo.GetUnitHealthPercent(oUnit)..'; enemy AirAA threat in this zone='..(tCurZoneTeamData[M28Map.refiEnemyAirAAThreat] or 0)) end
                ...
                function AddEnemyGroundUnitsToTargetsSubjectToAA(iPlateauOrZero, iLandOrWaterZone, bCheckForAirAA)
                    if not(tiZonesByPlateauConsidered[iPlateauOrZero][iLandOrWaterZone]) then
                        ...
                        if M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefTEnemyUnits]) == false and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefAlliedACU]) and (tLZOrWZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 0) + (tLZOrWZTeamData[M28Map.subrefThreatEnemyStructureTotalMass] or 0) + ((tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) + (tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0)) * 2 >= 1500 then
                            ...
                            if bDebugMessages == true then LOG(sFunctionRef..': Are potential enemies to target, checking overall threat vs friendly threat, ally combat total='..(tLZOrWZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] or 0)..'; Enemy combat total='..(tLZOrWZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 0)..'; tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA]='..(tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 'nil')..'; tLZOrWZTeamData[M28Map.subrefThreatEnemyStructureTotalMass]='..(tLZOrWZTeamData[M28Map.subrefThreatEnemyStructureTotalMass] or 'nil')) end
                            if (tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) >= 2000 or ((tLZOrWZTeamData[M28Map.subrefThreatEnemyStructureTotalMass] or 0) >= 100000 and ((tLZOrWZTeamData[M28Map.subrefThreatEnemyStructureTotalMass] or 0) >= (tLZOrWZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] or 0) * 0.25)) or
                                    ((tLZOrWZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] or 0) * 1.75 <= (tLZOrWZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 0) + ((tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) + (tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0)) * 0.6 + (tLZOrWZTeamData[M28Map.subrefThreatEnemyStructureTotalMass] or 0) * 0.05 and
                                            ((tLZOrWZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] or 0) <= 200 or (tLZOrWZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 0) + (tLZOrWZTeamData[M28Map.subrefThreatEnemyStructureTotalMass] or 0) + (tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) + (tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) >= 2500 + (tLZOrWZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] or 0) * 3)) then
                ...
                if M28Utilities.IsTableEmpty(tEnemyGroundTargets) or not(bHaveTargetWhereShotIsntBlocked) then
                    ...
                    if not(IsThereAANearLandOrWaterZone(iTeam, iBomberPlateauOrZero, iBomberLandOrWaterZone, (iBomberPlateauOrZero == 0), -1, iMaxEnemyAirAA)) then
                        ...
                        if (tBomberLandOrWaterZoneTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) + (tBomberLandOrWaterZoneTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0) < 1000 or M28Utilities.GetAngleDifference(M28UnitInfo.GetUnitFacingAngle(oBomber), M28Utilities.GetAngleFromAToB(oBomber:GetPosition(),M28Team.tAirSubteamData[iAirSubteam][M28Team.reftAirSubRallyPoint])) >= 25 then
                            ...
                            if M28Utilities.IsTableEmpty(tEnemyGroundTargets) == false and bHaveTargetWhereShotIsntBlocked then
                                ...
                                if M28Utilities.IsTableEmpty(tBomberLandOrWaterZoneData[M28Map.subrefOtherLandAndWaterZonesByDistance]) == false then
                                    ...
                                    for iEntry, tSubtable in tBomberLandOrWaterZoneData[M28Map.subrefOtherLandAndWaterZonesByDistance] do
                                        ...
                                        if bZoneIsCloseToFirstTargetZone then
                                            ...
                                            if bDebugMessages == true then LOG(sFunctionRef..': Have just finished calling AddEnemyGroundUnitsToTargetsSubjectToAA for bombers for zone '..tSubtable[M28Map.subrefiLandOrWaterZoneRef]..', Is table of enemy targets empty='..tostring(M28Utilities.IsTableEmpty(tEnemyGroundTargets))..'; tSubtable='..repru(tSubtable)..'; bHaveTargetWhereShotIsntBlocked='..tostring(bHaveTargetWhereShotIsntBlocked)) end
                                            if not(tFirstZoneWithTargetsMidpoint) and M28Utilities.IsTableEmpty(tEnemyGroundTargets) == false and bHaveTargetWhereShotIsntBlocked then
                ...
                if M28Utilities.IsTableEmpty(tEnemyGroundTargets) then
                                    ...
                                    if iModDist <= iDistOfBestTarget + iDistThresholdForFurtherAwayAA then
                                        ...
                                        if iModDist < iDistOfBestTarget - iDistThresholdForFurtherAwayAA or (iModDist < iDistOfBestTarget and iCurDamage >= iHighestDamage) or ((iModDist < iDistOfBestTarget + iReducedDistanceThreshold or iHighestDamage < iDamageThresholdForReducedDistance) and iCurDamage > iHighestDamage * iDamageFactorForFurtherAwayTarget) then
                                            ...
                                            if M28Logic.IsLineBlocked(aiBrain, tEstFiringPosition, oUnit:GetPosition(), iAOE) then
                                                ...
                                                if bDebugMessages == true then LOG(sFunctionRef..': Shot blocked so reducing damage to 1%, Bomber position='..repru(oBomber:GetPosition())..'; Unit position='..repru(oUnit:GetPosition())..'; iCurDist='..iCurDist..'; Angle to unit='..M28Utilities.GetAngleFromAToB(oBomber:GetPosition(), oUnit:GetPosition())..'; Bomber range='..(oBomber[M28UnitInfo.refiBomberRange] or 'nil')) end
                                            else
                                            ...
                                            if iCurDamage > iHighestDamage or (iModDist < iDistOfBestTarget and iCurDamage > iHighestDamage / iDamageFactorForFurtherAwayTarget) then
                                                ...
                                                if bDebugMessages == true then LOG(sFunctionRef..': Recording unit as the newest best target, oUnit[M28UnitInfo.refbExpBomberShotBlocked]='..tostring(oUnit[M28UnitInfo.refbExpBomberShotBlocked])) end
                                            end
                                        end
                                    end
                                end
                    ...
                    if M28UnitInfo.IsUnitValid(oBestEnemyTarget) then
                        ...
                        if not(bGivenOrderAlready) then
                            ...
                            if not(bHaveTargetWhereShotIsBlocked) and EntityCategoryContains(M28UnitInfo.refCategoryStructure, oBestEnemyTarget.UnitId) then
...
function LoadCombatUnitOntoTransport(oJustBuilt)
    local bDebugMessages = false if M28Profiler.bGlobalDebugOverride == true then   bDebugMessages = true end
    local sFunctionRef = 'LoadCombatUnitOntoTransport'
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerStart)
    ...
    if M28UnitInfo.IsUnitValid(oJustBuilt) then
        ...
        if iPlateau and iLandZone and iPlateau > 0 then
            ...
            if not(tLZTeamData[M28Map.refbActiveCombatUnitLoader]) then
                if bDebugMessages == true then LOG(sFunctionRef..': Starting loop to load combat units onto transport, unti htat prompted this='..oJustBuilt.UnitId..M28UnitInfo.GetUnitLifetimeCount(oJustBuilt)..'; time='..GetGameTimeSeconds()) end
                tLZTeamData[M28Map.refbActiveCombatUnitLoader] = true
                local bHaveTransportWantingCombatUnits = false
                local bHaveAttachedUnits
                while M28Conditions.IsTableOfUnitsStillValid(tLZTeamData[M28Map.reftoCombatUnitsLoadingOntoTransport]) and M28Conditions.IsTableOfUnitsStillValid(tLZTeamData[M28Map.reftoTransportsWaitingForUnits]) do
                    oNearestTransportToLoadInto = nil
                    bHaveTransportWantingCombatUnits = false
                    if M28Utilities.IsTableEmpty(tLZTeamData[M28Map.reftoTransportsWaitingForUnits]) == false then
                        for iTransport, oTransport in tLZTeamData[M28Map.reftoTransportsWaitingForUnits] do
                            if (oTransport[refiCombatUnitsWanted] or 0) > 0 and M28UnitInfo.IsUnitValid(oTransport) then
                                bHaveTransportWantingCombatUnits = true
                                if bDebugMessages == true then LOG(sFunctionRef..': Considering oTransport='..oTransport.UnitId..M28UnitInfo.GetUnitLifetimeCount(oTransport)..'; oTransport[refiCombatUnitsWanted]='..(oTransport[refiCombatUnitsWanted] or 'nil')..'; refoTransportUnitTryingToLoad='..(oTransport[refoTransportUnitTryingToLoad].UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oTransport[refoTransportUnitTryingToLoad]) or 'nil')..' at time='..(oTransport[refoTransportUnitTryingToLoad][refiTimeLastGivenOrderToLoadOntoTransport] or 'nil')) end
                                if not(M28UnitInfo.IsUnitValid(oTransport[refoTransportUnitTryingToLoad])) or oTransport[refoTransportUnitTryingToLoad]:IsUnitState('Attached') or GetGameTimeSeconds() - (oTransport[refoTransportUnitTryingToLoad][refiTimeLastGivenOrderToLoadOntoTransport] or 0) >= 15 then
                                    --Load nearest unit onto this
                                    iNearestTransportDist = 10000
                                    oNearestTransportToLoadInto = oTransport
                                    oCombatUnitToLoad = nil
                                    bHaveAttachedUnits = false
                                    for iCombatUnit, oCombatUnit in tLZTeamData[M28Map.reftoCombatUnitsLoadingOntoTransport] do
                                    ...
                                    if bHaveAttachedUnits then
                                        for iCurEntry = table.getn(tLZTeamData[M28Map.reftoCombatUnitsLoadingOntoTransport]), 1, -1 do
                                            if tLZTeamData[M28Map.reftoCombatUnitsLoadingOntoTransport][iCurEntry]:IsUnitState('Attached') then
                                                tLZTeamData[M28Map.reftoCombatUnitsLoadingOntoTransport][iCurEntry][M28Engineer.refiAssignedAction] = nil
                                                table.remove(tLZTeamData[M28Map.reftoCombatUnitsLoadingOntoTransport], iCurEntry)
                                            end
                                        end
                                    end

                                    if oCombatUnitToLoad then
                                        if bDebugMessages == true then LOG(sFunctionRef..': Will tell unit '..oCombatUnitToLoad.UnitId..M28UnitInfo.GetUnitLifetimeCount(oCombatUnitToLoad)..' to load onto transport '..oNearestTransportToLoadInto.UnitId..M28UnitInfo.GetUnitLifetimeCount(oNearestTransportToLoadInto)) end
                                        M28Orders.IssueTrackedTransportLoad(oCombatUnitToLoad, oNearestTransportToLoadInto, false, 'TrCombLd', false)
                                    end
                                end
                            end
                        end
                    end
                    if bDebugMessages == true then LOG(sFunctionRef..': Finished loop at time='..GetGameTimeSeconds()..'; bHaveTransportWantingCombatUnits='..tostring(bHaveTransportWantingCombatUnits or false)..'; Is table of combat units loading onto transport empty='..tostring(M28Utilities.IsTableEmpty(tLZTeamData[M28Map.reftoCombatUnitsLoadingOntoTransport]))) end
                    if not(bHaveTransportWantingCombatUnits) then
                        --Clear all units
                        if M28Utilities.IsTableEmpty(tLZTeamData[M28Map.reftoCombatUnitsLoadingOntoTransport]) == false then
                            for iCombatUnit, oCombatUnit in tLZTeamData[M28Map.reftoCombatUnitsLoadingOntoTransport] do
                                oCombatUnit[M28Engineer.refiAssignedAction] = nil
                                M28Orders.IssueTrackedClearCommands(oCombatUnit)
                            end
                            tLZTeamData[M28Map.reftoCombatUnitsLoadingOntoTransport] = {}
                        end
                        break
                ...
                if bDebugMessages == true then LOG(sFunctionRef..': Finishing the unit loader logic, time='..GetGameTimeSeconds()) end
            end
        end
    end
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
...
function ConsiderIfBomberTargetingACUShouldReassign(oUnit, oCurTarget)
    ...
    if not(oUnit[refbReassessingTarget]) then
        ...
        if M28UnitInfo.IsUnitValid(oUnit) and M28UnitInfo.IsUnitValid(oCurTarget) then
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Deciding if we want to reassign bomber '..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..' at time='..GetGameTimeSeconds()..'; Enemy groundAA in target zone='..(tTargetLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 'nil')..'; Target unit state='..M28UnitInfo.GetUnitState(oCurTarget)) end
            if tTargetLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] > 0 then
                bReassess = true
            else

                ...
                while M28UnitInfo.IsUnitValid(oBomber) and oBomber[rebEarlyBomberTargetBase] do
                    ...
                    elseif not((iStartLandOrWaterZone == iEnemyBaseZone and iStartPlateauOrZero == iEnemyBasePlateau) or M28Utilities.GetDistanceBetweenPositions(oBomber:GetPosition(), tEnemyInitialDestination) <= 90) then
                        ...
                        if M28Utilities.IsTableEmpty(tEnemyTargets) then
                            ...
                            if (not(M28Utilities.bLoudModActive or M28Utilities.bQuietModActive)) and oNearestEnemy and iClosestDist <= 70 and oBomber[M28UnitInfo.refiLastBombFired] and GetGameTimeSeconds() - oBomber[M28UnitInfo.refiLastBombFired] <= 10 and
        ...
        if M28Utilities.IsTableEmpty(tiWaterZoneByDistance) == false then
            ...
            for iWaterZone, iDistance in M28Utilities.SortTableByValue(tiWaterZoneByDistance, false) do
                ...
                if iMassValueOfEnemyUnits > 0 then
                    ...
                    -- Azraeel: Leaving this as only LoudModActive as Torp Bombers have had significant buffs (and I intend to give them auto retarget on the Torpedos)
                    -- AA has been nerfed as well in the past.
                ...
                if bTorpBombers then
                    ...
                    if iEnemyTorpDefenceCount > 0 then
                        ...
                        if M28Utilities.bLoudModActive then 
                            iTotalStrikeDamageWanted = math.max(math.min(iTotalStrikeDamageWanted * 4, iEnemyTorpDefenceCount * 400), iTotalStrikeDamageWanted * 1.8)
                            ...
                            iTotalStrikeDamageWanted = math.max(math.min(iTotalStrikeDamageWanted * 3, iEnemyTorpDefenceCount * 250), iTotalStrikeDamageWanted * 1.25)
                        end
                    else
...
function ManageGunships(iTeam, iAirSubteam)
    ...
    if M28Utilities.IsTableEmpty(tAvailableGunships) == false then
        ...
        local iOurGunshipAA = M28Team.tAirSubteamData[iAirSubteam][M28Team.refiOurGunshipAAThreat]
        ...
        if not(bRetreatFromAhwassa) and (not(iClosestSnipeTarget) or iClosestSnipeTarget >= 400) then
            ...
            if M28Utilities.IsTableEmpty(tEnemyGroundOrGunshipTargets) then
                ...
                elseif M28Utilities.bQuietModActive then
                    iGunshipThreatFactorForSameZone = 1.9 --Gunships no longer get slowed down/lose fuel when taking damage, so won't be quite as bad in QCE; Az also mentioned their damage should be similar to FAF levels, although concern from replays seen so far is effective DPS is significantly lower due to targeting
                end

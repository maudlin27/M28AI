...
function RecordGroundThreatForWaterZone(tWZData, tWZTeamData, iTeam, iPond, iWaterZone)
    ...
    if M28Utilities.IsTableEmpty(tWZTeamData[M28Map.subreftoLZOrWZAlliedUnits]) then
        ...
        --GetAirThreatLevel(tUnits,                                                                                               bEnemyUnits, bIncludeAirToAir, bIncludeGroundToAir, bIncludeAirToGround, bIncludeNonCombatAir, bIncludeAirTorpedo, bBlueprintThreat)
        tWZTeamData[M28Map.subrefWZThreatAlliedAA] = M28UnitInfo.GetAirThreatLevel(tWZTeamData[M28Map.subreftoLZOrWZAlliedUnits], false,           false,          true,                   false,              false,              false,              false)
        ...
        if bDebugMessages == true then LOG(sFunctionRef..': subrefWZThreatAlliedAntiNavy='..tWZTeamData[M28Map.subrefWZThreatAlliedAntiNavy]..'; subrefWZThreatAlliedSubmersible='..tWZTeamData[M28Map.subrefWZThreatAlliedSubmersible]..'; subrefWZThreatAlliedSurface='..tWZTeamData[M28Map.subrefWZThreatAlliedSurface]..'; subrefWZThreatAlliedAA='..tWZTeamData[M28Map.subrefWZThreatAlliedAA]..'; subrefWZThreatAlliedMAA='..tWZTeamData[M28Map.subrefWZThreatAlliedMAA]..'; AA threat re-running='..M28UnitInfo.GetAirThreatLevel(tWZTeamData[M28Map.subreftoLZOrWZAlliedUnits], true,           false,          true,                   false,              false,              false,              false)) end
                                ...
                                if bCheckForBuildingsToAttack then
                                    ...
                                    if oBuildingToAttack and M28Utilities.GetDistanceBetweenPositions(oBuildingToAttack:GetPosition(), oUnit:GetPosition()) > math.max((oUnit[M28UnitInfo.refiDFRange] or 0), (oUnit[M28UnitInfo.refiIndirectRange] or 0)) then
                                        ...
                                        if iDefencesHeadroom >= 10 then
                                            ...
                                            local tPriorityTargets = EntityCategoryFilterDown(iPriorityCategory, tEnemyCombatNearFrontUnit)
    ...
    if not(bHaveRunFromAir) or M28Utilities.IsTableEmpty(tAvailableSubmarines) == false or M28Utilities.IsTableEmpty(tAvailableCombatUnits) == false or M28Utilities.IsTableEmpty(tMissileShips) == false then
        ...
        if oNearestEnemyNonHoverToFriendlyBase and M28Utilities.IsTableEmpty(tAvailableSubmarines) == false then
            ...
            if bAttackAndKite then
                ...
                for iUnit, oUnit in tAvailableSubmarines do
                    ...
                    if oUnit[M28UnitInfo.refiAntiNavyRange] > iScenario1AntiNavyRangeThreshold then
                        ...
                        elseif bMoveBlockedNotAttackMove and oUnit[M28UnitInfo.refbLastShotBlocked] and (GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or -100)) >= 10 and GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastCheck] or -100) < 6 then
                            ...
                            if oUnit[M28UnitInfo.refbEasyBrain] or bMoveAntiNavyForwardsAsCantSee or bEnemyHasNoCombatUnits or iScenario1AntiNavyRangeThreshold <= 5 or (iScenario1AntiNavyRangeThreshold + 10 < oUnit[M28UnitInfo.refiAntiNavy] and (oNearestEnemyNonHoverToFriendlyBase[M28UnitInfo.oNearestEnemyNonHoverToFriendlyBase] or 0) == 0) or (not(M28Conditions.CloseToEnemyUnit(oUnit:GetPosition(), tWZTeamData[M28Map.reftoNearestCombatEnemies], oUnit[M28UnitInfo.refiAntiNavyRange] * 0.94, iTeam, false,                nil,                    nil,                                    oUnit)) and (not(M28UnitInfo.IsUnitValid(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck])) or (oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.refiAntiNavyRange] or 0) == 0 or M28Utilities.GetDistanceBetweenPositions(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.reftLastKnownPositionByTeam][iTeam], oUnit:GetPosition()) >= oUnit[M28UnitInfo.refiAntiNavyRange] or not(M28UnitInfo.CanSeeUnit(oUnit:GetAIBrain(), oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck], false)))) then
                ...
                if M28Conditions.WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iNearbyFriendlySubThreat, iAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, true                             ,iModForEnemyScenario2Threat ) then
        ...
        if oNearestEnemyToFriendlyBase and M28Utilities.IsTableEmpty(tAvailableCombatUnits) == false then
            ...
            if not(bAreInScenario1) then
                if M28Utilities.IsTableEmpty(tCombatUnitsOfUse) == false then
                    ...
                    --WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iNearbyFriendlySubThreat, iAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, bConsideringSubmarinesNotSurface, iOptionalThreatAbsolutePercentIncrease)
                    local bAreInScenario2 = M28Conditions.WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iNearbyFriendlySubThreat, iAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, false                               ,iModForEnemyScenario2Threat, iEnemyNearbySubmersibleThreat, iOurAntiNavyThreat)
            ...
            if M28Utilities.IsTableEmpty(tWZData[M28Map.subrefWZOtherWaterZones]) == false then
                ...
                for iEntry, tWZSubtable in tWZData[M28Map.subrefWZOtherWaterZones] do
                    ...
                    if bDontCheckInPlayableArea or M28Conditions.IsLocationInPlayableArea(tAltWZData[M28Map.subrefMidpoint]) then
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering whether to send MAA to assist alt WZ iAltWZ='..iAltWZ..'; tAltTeamWZData[M28Map.subrefWZMAAThreatWanted]='..(tAltTeamWZData[M28Map.subrefWZMAAThreatWanted] or 'nil')..'; tAltTeamWZData[M28Map.subrefWZThreatAlliedAA]='..(tAltTeamWZData[M28Map.subrefWZThreatAlliedAA] or 'nil')..'; Air to ground threat in this alt WZ='..(tAltTeamWZData[M28Map.refiEnemyAirToGroundThreat] or 'nil')..'; Our allied surface threat in the WZ considering MAA orders='..(tWZTeamData[M28Map.subrefWZThreatAlliedSurface] or 'nil')..'; subrefWZbCoreBase='..tostring(tAltTeamWZData[M28Map.subrefWZbCoreBase] or false)) end
...
function GetNearestEnemyUnitsAndUpdateUnitTables(iPond, iWaterZone, tWZData, tWZTeamData, iTeam, tEnemySurfaceUnits, tEnemyNonHoverUnits)
    ...
    local iCurDist, iCurMidpointDist, iCurRangeAdjustedDist
    local iOverallClosestNonHoverRangeDist = 100000
    ...
    function ConsiderIfUnitIsClosest(oUnit, bDealingWithAdjZone)
        ...
        if not(bDealingWithAdjZone) or M28UnitInfo.IsUnitValid(oUnit) then
            if not(M28Map.bIsCampaignMap) or M28Conditions.IsLocationInPlayableArea(oUnit:GetPosition()) then
                ...
                iCurRangeAdjustedDist = iCurDist - (oUnit[M28UnitInfo.refiCombatRange] or 0)
                ...
                if not(M28UnitInfo.IsUnitUnderwater(oUnit)) then
                    if iCurRangeAdjustedDist < iClosestSurfaceToMidpointByRange then
                        iClosestSurfaceToMidpointByRange = iCurRangeAdjustedDist
                        oClosestSurfaceToMidpointByRange = oUnit
                        if iCurRangeAdjustedDist < iOverallClosestSurfaceRangeDist then
                            iOverallClosestSurfaceRangeDist = iCurRangeAdjustedDist
                            oOverallClosestSurfaceRangeUnit = oUnit
                        end
                    end

                    if iCurDist < iClosestSurfaceDist then
                        if bDealingWithAdjZone then
                ...
                if not(EntityCategoryContains(categories.HOVER, oUnit.UnitId)) then
                    if iCurRangeAdjustedDist < iClosestNonHoverToMidpointByRange then
                        iClosestNonHoverToMidpointByRange = iCurRangeAdjustedDist
                        oClosestNonHoverToMidpointByRange = oUnit
                        if iCurRangeAdjustedDist < iOverallClosestNonHoverRangeDist then
                            iOverallClosestNonHoverRangeDist = iCurRangeAdjustedDist
                            oOverallClosestNonHoverRangeUnit = oUnit
                        end
                    end

                    if iCurDist < iClosestNonHoverDist then
                        if bDealingWithAdjZone then
                            if not(iCurMidpointDist) then iCurMidpointDist = M28Utilities.GetDistanceBetweenPositions(tWZData[M28Map.subrefMidpoint], oUnit[M28UnitInfo.reftLastKnownPositionByTeam][iTeam]) end
    ...
    for _, iAdjWZ in tWZData[M28Map.subrefWZAdjacentWaterZones] do
        ...
        if M28Utilities.IsTableEmpty(M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][iAdjWZ][M28Map.subrefWZTeamData][iTeam][M28Map.subrefTEnemyUnits]) == false then
            ...
            iClosestSurfaceDist = 100000
            iClosestSurfaceToMidpointDist = 100000
            iClosestNonHoverToMidpointDist = 100000
            iClosestSurfaceToMidpointByRange = 100000
            oClosestSurfaceToMidpointByRange = nil
            iClosestNonHoverToMidpointByRange = 100000
    ...
    if iOverallClosestNonHoverRangeDist < iOverallClosestSurfaceRangeDist then
    ...
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
    return iClosestDist, iClosestSurfaceDist, iClosestNonHoverDist, oNearestEnemyToFriendlyBase, oNearestEnemySurfaceToFriendlyBase, oNearestEnemyNonHoverToFriendlyBase

...
function GetNearestWaterRallyPoint(tWZData, iTeam, iPond, iWaterZone)
    ...
    if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subrefiRallyPointWaterZonesByPond][iPond]) == false then
        ...
        if not(iClosestWZRef) then
            ...
            --Campaign adj if this would be outside playable area
            if M28Map.bIsCampaignMap and not(M28Conditions.IsLocationInPlayableArea(M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][iClosestWZRef])) then
                if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subrefiRallyPointWaterZonesByPond][iPond]) == false then
                    for iEntry, iAltWZ in M28Team.tTeamData[iTeam][M28Team.subrefiRallyPointWaterZonesByPond][iPond] do
                        local tAltWZData = M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][iAltWZ]
                        iCurDist = M28Utilities.GetDistanceBetweenPositions(tWZData[M28Map.subrefMidpoint], tAltWZData[M28Map.subrefMidpoint])
        ...
        for iWaterZone, tWZData in M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones] do
            ...
            if tWZTeamData[M28Map.subrefWZbCoreBase] then
                if bDontCheckPlayableArea or M28Conditions.IsLocationInPlayableArea(tWZData[M28Map.subrefMidpoint]) then
                    --Check this WZ either has the main pond naval factory in it, or we have more threat than enemy
                    if (tWZTeamData[M28Map.subrefWZThreatAlliedAntiNavy] >= (tWZTeamData[M28Map.subrefWZThreatEnemySubmersible] or 0) and tWZTeamData[M28Map.subrefWZThreatAlliedSurface] >= tWZTeamData[M28Map.subrefWZThreatEnemyVsSurface])
                            or (M28UnitInfo.IsUnitValid(tWZTeamData[M28Map.refoLastNavalFacAssisted]) and tWZTeamData[M28Map.refoLastNavalFacAssisted]:GetFractionComplete() == 1) then
                    ...
                    elseif bDebugMessages == true then LOG(sFunctionRef..': Core base WS hsa too much enemy threat in it so will look for a dif WZ to retreat to')
                    end
                end
            end
        end
...
function RecordGroundThreatForWaterZone(tWZData, tWZTeamData, iTeam, iPond, iWaterZone)
    ...
    --EnemyCombatTotal is basically enemy threat vs surface since that includes anti-surface and anti-navy threat, have left separate just in case change mind later
    tWZTeamData[M28Map.subrefTThreatEnemyCombatTotal] = math.max(tWZTeamData[M28Map.subrefWZThreatEnemyVsSurface],  tWZTeamData[M28Map.subrefWZThreatEnemySubmersible])
    ...
    if tWZTeamData[M28Map.subrefWZThreatAlliedSurface] == 0 or tWZTeamData[M28Map.subrefWZThreatAlliedSubmersible] == 0 then
        ...
        tWZTeamData[M28Map.subrefWZTThreatAllyCombatTotal] = math.max(tWZTeamData[M28Map.subrefWZThreatAlliedSurface], tWZTeamData[M28Map.subrefWZThreatAlliedSubmersible], M28UnitInfo.GetCombatThreatRating(tWZTeamData[M28Map.subreftoLZOrWZAlliedUnits], false,       false,              false,                      false,  true,           false,              false,              false,          false))
    end
...
function ShieldUnitsInWaterZone(tTeamTargetWZData, tShieldsToAssign, bAssignAllShields)
    ...
    if M28Utilities.IsTableEmpty(tTeamTargetWZData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) and M28Utilities.IsTableEmpty(tTeamTargetWZData[M28Map.reftoWZUnitsWantingMobileShield]) then
        ...
        if M28Utilities.IsTableEmpty(tShieldsToAssign) == false then
            ...
            if bAssignAllShields then
                ...
                if M28Utilities.IsTableEmpty(tTeamTargetWZData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then
                    for iTarget, oTarget in tTeamTargetWZData[M28Map.subrefWZTAlliedSurfaceCombatUnits] do
...
function StealthUnitsInWaterZone(tTeamTargetWZData, tStealthsToAssign, bAssignAllStealths)
    ...
    if M28Utilities.IsTableEmpty(tTeamTargetWZData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) and M28Utilities.IsTableEmpty(tTeamTargetWZData[M28Map.reftoWZUnitsWantingMobileStealth]) then
...
function ManageMobileShieldsInWaterZone(tWZData, tWZTeamData, iTeam, iPond, iWaterZone, tMobileShields)
    ...
    if M28Utilities.IsTableEmpty(tShieldsToAssign) == false then
        ...
        if not(tWZTeamData[M28Map.refbWZWantsMobileShield]) and M28Utilities.IsTableEmpty(tWZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then iClosestWZNotWantingShieldButWithUnits = iWaterZone end
        if M28Utilities.IsTableEmpty(tWZData[M28Map.subrefWZOtherWaterZones]) == false then
            for iEntry, tPathingDetails in tWZData[M28Map.subrefWZOtherWaterZones] do
                ...
                elseif not(iClosestWZNotWantingShieldButWithUnits) then
                    if tTeamTargetWZData[M28Map.subrefbWZWantsSupport] and M28Utilities.IsTableEmpty(tTeamTargetWZData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then iClosestWZNotWantingShieldButWithUnits = tPathingDetails[M28Map.subrefWZAWZRef] end
                    if not(iClosestWZWithAnyCombatUnits) and M28Utilities.IsTableEmpty(tTeamTargetWZData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then iClosestWZWithAnyCombatUnits = tPathingDetails[M28Map.subrefWZAWZRef] end
                end
...
function ManageMobileStealthsInWaterZone(tWZData, tWZTeamData, iTeam, iPond, iWaterZone, tMobileStealths)
    ...
    if M28Utilities.IsTableEmpty(tStealthsToAssign) == false then
        ...
        local iClosestWZWithAnyCombatUnits
        if tWZTeamData[M28Map.subrefbWZWantsSupport] and not(tWZTeamData[M28Map.refbWZWantsMobileStealth]) and M28Utilities.IsTableEmpty(tWZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then iClosestWZNotWantingStealthButWithUnits = iWaterZone end
        if M28Utilities.IsTableEmpty(tWZData[M28Map.subrefWZOtherWaterZones]) == false then
            for iEntry, tPathingDetails in tWZData[M28Map.subrefWZOtherWaterZones] do
                local tTeamTargetWZData = M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][tPathingDetails[M28Map.subrefWZAWZRef]][M28Map.subrefWZTeamData][iTeam]
                ...
                elseif not(iClosestWZNotWantingStealthButWithUnits) then
                    if tTeamTargetWZData[M28Map.subrefbWZWantsSupport] and M28Utilities.IsTableEmpty(tTeamTargetWZData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then iClosestWZNotWantingStealthButWithUnits = tPathingDetails[M28Map.subrefWZAWZRef] end
                    if not(iClosestWZWithAnyCombatUnits) and M28Utilities.IsTableEmpty(tTeamTargetWZData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then iClosestWZWithAnyCombatUnits = tPathingDetails[M28Map.subrefWZAWZRef] end
                end
...
function MoveUnassignedLandUnits(tWZData, tWZTeamData, iPond, iWaterZone, iTeam, tAmphibiousUnits)
    ...
    if M28Utilities.IsTableEmpty(tiUnitsInZoneByAmphibiousLabel) == false then
        for iAmphibiousLabel, tAmphibiousLabelUnits in tiUnitsInZoneByAmphibiousLabel do
            ...
            local iLZToSupport, iWZToSupport
            ...
            if M28Utilities.IsTableEmpty(tWZData[M28Map.subrefAdjacentLandZones]) == false then
                local bDontCheckPlayableArea = not(M28Map.bIsCampaignMap)
                for iEntry, tSubtable in tWZData[M28Map.subrefAdjacentLandZones] do
                    local tAltLZData = M28Map.tAllPlateaus[tSubtable[M28Map.subrefWPlatAndLZNumber][1]][M28Map.subrefPlateauLandZones][tSubtable[M28Map.subrefWPlatAndLZNumber][2]]
            ...
            if not(iLZToSupport) then
                        ...
                        if bDontCheckPlayableArea or M28Conditions.IsLocationInPlayableArea(tAltWZData[M28Map.subrefMidpoint]) then
                            if M28Utilities.IsTableEmpty(tAltWZData[M28Map.subrefAdjacentLandZones]) == false then
                                for iEntry, tSubtable in tAltWZData[M28Map.subrefAdjacentLandZones] do
                                    local tAltLZData = M28Map.tAllPlateaus[tSubtable[M28Map.subrefWPlatAndLZNumber][1]][M28Map.subrefPlateauLandZones][tSubtable[M28Map.subrefWPlatAndLZNumber][2]]
                                    if bDebugMessages == true then LOG(sFunctionRef..': Considering adjacent land zone '..tSubtable[M28Map.subrefWPlatAndLZNumber][2]..' in plateau '..tSubtable[M28Map.subrefWPlatAndLZNumber][1]..'; Does this zone want support='..tostring(tAltLZData[M28Map.subrefLZTeamData][iTeam][M28Map.subrefbLZWantsSupport])..'; Dist to enemy base='..M28Utilities.GetDistanceBetweenPositions(tWZTeamData[M28Map.reftClosestEnemyBase], tAltLZData[M28Map.subrefMidpoint])..'; iMaxDistWantedToEnemyBase='..iMaxDistWantedToEnemyBase) end
                                    if bDontCheckPlayableArea or M28Conditions.IsLocationInPlayableArea(tAltLZData[M28Map.subrefMidpoint]) then
                                        if (bDontCheckForPacifism or not(tAltLZData[M28Map.subrefbPacifistArea])) and tAltLZData[M28Map.subrefLZTeamData][iTeam][M28Map.subrefbLZWantsSupport] then
                                            if iAmphibiousLabel == tAltLZData[M28Map.refiMidpointAmphibiousLabel] then
                                                iLZToSupport = tSubtable[M28Map.subrefWPlatAndLZNumber][2]
                                                iPlateau = tSubtable[M28Map.subrefWPlatAndLZNumber][1]
                                                break
                                        ...
                                        elseif not(iCloserLandZoneToEnemy) and not(iLZToSupport) and iEnemyPlateau == tSubtable[M28Map.subrefWPlatAndLZNumber][1] and M28Utilities.GetDistanceBetweenPositions(tWZTeamData[M28Map.reftClosestEnemyBase], tAltLZData[M28Map.subrefMidpoint]) < iMaxDistWantedToEnemyBase then
                            ...
                            if bDebugMessages == true then LOG(sFunctionRef..': Considering if we want to support a WZ instead, iAdjWZ='..tWZSubtable[M28Map.subrefWZAWZRef]..'; Amphbious label of midpoint='..(NavUtils.GetLabel(M28Map.refPathingTypeAmphibious, tAltWZData[M28Map.subrefMidpoint]) or 'nil')..'; iAmphibiousLabel='..iAmphibiousLabel) end
                            if NavUtils.GetLabel(M28Map.refPathingTypeAmphibious, tAltWZData[M28Map.subrefMidpoint]) == iAmphibiousLabel then
                                local tAltWZTeamData = tAltWZData[M28Map.subrefWZTeamData][iTeam]
                                if tAltWZTeamData[M28Map.subrefbWZWantsSupport] then
                                    --Can we attack units in this zone?
                                    bHaveAntiNavy = false
                                    bHaveHover = false
                                    for iUnit, oUnit in tAmphibiousLabelUnits do
                                        ...
                                        if not(bHaveHover) and (oUnit[M28UnitInfo.refiDFRange] or 0) > 0 and EntityCategoryContains(categories.HOVER * categories.DIRECTFIRE, oUnit.UnitId) then
                                            bHaveHover = true
                                            if bHaveAntiNavy then break end
                                        end
                                    end
                                    if (bHaveAntiNavy or not(tWZTeamData[M28Map.subrefbWZOnlySubmersibleEnemies])) and (bHaveHover or not(tWZTeamData[M28Map.subrefbWZOnlyHoverEnemies])) then
                                        iWZToSupport = tWZSubtable[M28Map.subrefWZAWZRef]
                                        if bDebugMessages == true then LOG(sFunctionRef..': Want to support this WZ') end
                                        break
                                    end
                                end
                            end
                ...
                if iWZToSupport and iLZToSupport then iWZToSupport = nil end --set to nil here as we want to prioritise land zone over WZ if land zone didnt need to use the below redundancy of 'closest LZ to enemy base'
                if bDebugMessages == true then LOG(sFunctionRef..': iLZToSupport after checking adjacent LZ to all other WZ='..(iLZToSupport or 'nil')..'; iWZToSupport='..(iWZToSupport or 'nil')) end
                if not(iLZToSupport) then

                    --Also get LZToSupport
                    if iCloserLandZoneToEnemy then
            ...
            if iWZToSupport then
                --Flag to use the amphibious unit for combat purposes (and reset flag in 10s); also send order to travel to the WZ; however in future might want to consider similar check on attackwitheverything; that said wouldve expected such logic to have already applied when managing combat units in WZ
                local tAltWZData = M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][iWZToSupport]
                local tHoverDestination = tAltWZData[M28Map.subrefMidpoint]
                local tAmphibiousDestination = tAltWZData[M28Map.subrefMidpoint]

                for iUnit, oUnit in tAmphibiousLabelUnits do
                    if not(oUnit[M28UnitInfo.refbTreatAmphibiousUnitAsSubmarine]) then --we also have similar logic in M28Land logic that sends amphibious units to a WZ to support it
                        M28Utilities.DelayChangeVariable(oUnit, M28UnitInfo.refbTreatAmphibiousUnitAsSubmarine, nil, 120)
                    ...
                    if EntityCategoryContains(M28UnitInfo.refCategoryAmphibious, oUnit.UnitId) then
                        ...
                        if not(IgnoreOrderDueToStuckUnit(oUnit)) then
                            M28Orders.IssueTrackedMove(oUnit, tAmphibiousDestination, iOrderReissueDistToUse, false, 'NMAToWZ'..iWZToSupport..'Fr'..iWaterZone)
                        end
                    else
                        ...
                        if not(IgnoreOrderDueToStuckUnit(oUnit)) then
                            M28Orders.IssueTrackedMove(oUnit, tHoverDestination, iOrderReissueDistToUse, false, 'NMHToWZ'..iWZToSupport..'Fr'..iWaterZone)
                        end
                    end
                end

            elseif not(iLZToSupport) then M28Utilities.ErrorHandler('Unable to find a LZ needing support for water zone '..iWaterZone..'; Pond '..iPond..'; only expected on maps with naval start points', true)
            else
                ...
                if not(bAttackWithEverything) then
                    ...
                    if M28Utilities.IsTableEmpty(tLZData[M28Map.subrefLZAdjacentLandZones]) == false then
                        for iEntry, iAdjLZ in tLZData[M28Map.subrefLZAdjacentLandZones] do
                            ...
                            if iBestEnemyDFRange >= 70 then --If enemy has PD then if its T1-T2 we can probably ignore if its in adjacent zone unless enemy has LR units
                                iEnemyCombatThreat = iEnemyCombatThreat + (tAltLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 0)
                            else
                                iEnemyCombatThreat = iEnemyCombatThreat + (tAltLZTeamData[M28Map.subrefLZThreatEnemyMobileDFTotal] or 0)
                            end
                            iOurCombatThreat = iOurCombatThreat + (tAltLZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] or 0)
                            if bDebugMessages == true then LOG(sFunctionRef..': Considering iAdjLZ='..iAdjLZ..'; iEnemyCombatThreat cumulative='..iEnemyCombatThreat..'; iOurCombatThreat='..iOurCombatThreat..'; iBestEnemyDFRange='..iBestEnemyDFRange) end
                        end
                    end
                ...
                if bAttackWithEverything then
                    --Record we are attacking the land zone with amphibious/hover units
                    local iMoveType = M28Map.subrefiLZOrWZTMovingToOtherZone
                    if M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subrefTEnemyUnits]) == false then iMoveType = M28Map.subrefiLZOrWZTAttackingUnit end
                    M28Land.RecordDFLandZoneTarget(tWZTeamData, iPlateau, iWaterZone, iTeam, iLZToSupport, iMoveType, true)
                else
...
function ManageSpecificWaterZone(aiBrain, iTeam, iPond, iWaterZone)
    ...
    if bContinue then
        ...
        if tWZTeamData[M28Map.refbNoSubSupportPoint] then tWZTeamData[M28Map.refbNoSubSupportPoint] = nil end
        if tWZTeamData[M28Map.refbNoSurfaceSupportPoint] then tWZTeamData[M28Map.refbNoSurfaceSupportPoint] = nil end
        ...
        tWZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits] = {}
        ...
        if M28Utilities.IsTableEmpty(tWZTeamData[M28Map.subreftoLZOrWZAlliedUnits]) == false then
                    ...
                    elseif oUnit[M28Land.refiRaidingTargetZone] then
                        ...
                        elseif EntityCategoryContains(M28UnitInfo.refCategoryAllAmphibiousAndNavy * categories.MOBILE, oUnit.UnitId) then
                            ...
                            elseif EntityCategoryContains(M28UnitInfo.refCategoryMAA + M28UnitInfo.refCategoryNavalAA + M28UnitInfo.refCategoryMobileLand + M28UnitInfo.refCategoryNavalSurface + M28UnitInfo.refCategorySubmarine - categories.COMMAND - M28UnitInfo.refCategoryRASSACU, oUnit.UnitId) then
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': Considering if have available combat or MAA unit, oUnit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; oUnit[refiCurrentWZAssignmentValue]='..(oUnit[refiCurrentWZAssignmentValue] or 'nil')..'; oUnit[refiCurrentAssignmentWaterZone]='..(oUnit[refiCurrentAssignmentWaterZone] or 'nil')..'; LZ assignment='..repru(oUnit[M28Land.refiCurrentAssignmentPlateauAndLZ])..'; Time since last WZ assignment='..(GetGameTimeSeconds() - (oUnit[refiTimeOfLastWZAssignment] or 0))..'; Does unit contain MAA category='..tostring(EntityCategoryContains(M28UnitInfo.refCategoryMAA + M28UnitInfo.refCategoryNavalAA, oUnit.UnitId))..'; refiCurrentAssignmentPlateauAndLZ='..repru(oUnit[M28Land.refiCurrentAssignmentPlateauAndLZ])) end
                                if (oUnit[refiCurrentWZAssignmentValue] or 0) < iCurWZValue or (oUnit[refiCurrentAssignmentWaterZone] == iWaterZone or (GetGameTimeSeconds() - (oUnit[refiTimeOfLastWZAssignment] or 0) >= 10 and not(oUnit[M28Land.refiCurrentAssignmentPlateauAndLZ]))) then
                                    ...
                                    iCurShield, iMaxShield = M28UnitInfo.GetCurrentAndMaximumShield(oUnit, true)
                                    if iMaxShield > 0 and iCurShield < iMaxShield * 0.6 and iMaxShield > oUnit:GetMaxHealth() * 0.8 then --primarily fatboy, but in theory could affect SACUs
                                        table.insert(tOtherUnitsToRetreat, oUnit)
                                        oUnit[refiCurrentWZAssignmentValue] = 100000
                                        if bDebugMessages == true then LOG(sFunctionRef..': Want to retreat unit so shield can recharge') end
                                    else
                                        bIncludeUnit = true
                                        ...
                                        elseif ((oUnit[M28UnitInfo.refiDFRange] or 0) > 0 or (oUnit[M28UnitInfo.refiAntiNavyRange] or 0) > 0) and EntityCategoryContains(M28UnitInfo.refCategoryNavalSurface + categories.HOVER - M28UnitInfo.refCategoryLandExperimental * M28UnitInfo.refCategoryAmphibious - M28UnitInfo.refCategoryLandCombat * categories.AMPHIBIOUS + M28UnitInfo.refCategorySeraphimDestroyer + categories.uas0401, oUnit.UnitId) then
                                            ...
                                            table.insert(tWZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits], oUnit)
                                        ...
                                        elseif EntityCategoryContains(M28UnitInfo.refCategoryMissileShip, oUnit.UnitId) then
                                            ...
                                            --Mobile sonar - dont move
                                        elseif EntityCategoryContains(M28UnitInfo.refCategorySonar, oUnit) then
                                            if bDebugMessages == true then LOG(sFunctionRef..': Have mobile sonar unit, will just ignore giving it orders') end
                                        else
                                            --Campaign - if we have Cybran amphibious tanks that have no land zone to support but do have a water zone to support, then switch to using them as subs (see also the logic in unassignedlandunits which will do the same) - this flag gets reset every 120s so unit is only temporarily considered a submarine
                                            if oUnit[M28UnitInfo.refbTreatAmphibiousUnitAsSubmarine] and (oUnit[M28UnitInfo.refiAntiNavyRange] or 0) > 0 then
                                                table.insert(tAvailableSubmarines, oUnit)
                                            else
            ...
            if M28Utilities.IsTableEmpty(tWZData[M28Map.subrefWZOtherWaterZones]) == false and bConsiderAdjacentCombat then
                ...
                for iEntry, tWZSubtable in tWZData[M28Map.subrefWZOtherWaterZones] do
                    ...
                    if bDebugMessages == true then LOG(sFunctionRef..': iPond='..iPond..'; Base WZ='..iWaterZone..'; iAdjWZ='..iAdjWZ..'; WZTValue='..repru(tAltWZTeam[M28Map.subrefWZTValue])..'; Is table of allied combat units empty='..tostring(M28Utilities.IsTableEmpty(tAltWZTeam[M28Map.subrefWZTAlliedSurfaceCombatUnits]))) end
                    if not(bWaterZoneOrAdjHasUnitsWantingScout) and M28Utilities.IsTableEmpty(tAltWZTeam[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then bWaterZoneOrAdjHasUnitsWantingScout = true end
                    if tAltWZTeam[M28Map.subrefWZTValue] < iCurWZValue and tAltWZTeam[M28Map.subrefTThreatEnemyCombatTotal] <= 50 and (((tAltWZTeam[M28Map.subrefWZThreatAlliedSubmersible] or 0) > 0 or (tAltWZTeam[M28Map.subrefWZThreatAlliedSurface] or 0) > 0) and M28Utilities.IsTableEmpty(tAltWZTeam[M28Map.subreftoLZOrWZAlliedUnits]) == false) then
                        for iUnit, oUnit in tAltWZTeam[M28Map.subreftoLZOrWZAlliedUnits] do
                            ...
                            if not(oUnit.Dead) and not(oUnit[refbActiveRaider]) and not(oUnit[M28Land.refiRaidingTargetZone]) and ((oUnit[refiCurrentWZAssignmentValue] or 0) < iCurWZValue or (oUnit[refiCurrentAssignmentWaterZone] == iWaterZone)) and oUnit:GetFractionComplete() == 1 then
                                if oUnit[M28UnitInfo.refiSACUWaterZoneTarget] then
                                    ...
                                    --Combat unit related
                                elseif bConsiderAdjacentCombat and ((oUnit[M28UnitInfo.refiAntiNavyRange] or 0) > 0 or ((oUnit[M28UnitInfo.refiDFRange] or 0) > 0 and EntityCategoryContains(M28UnitInfo.refCategoryNavalSurface + categories.HOVER, oUnit.UnitId))) and not(EntityCategoryContains(M28UnitInfo.refCategoryCruiser, oUnit.UnitId)) then
                                    if (oUnit[M28UnitInfo.refiAntiNavyRange] or 0) > 0 and (EntityCategoryContains(M28UnitInfo.refCategorySubmarine - M28UnitInfo.refCategorySeraphimDestroyer - categories.uas0401, oUnit.UnitId) or oUnit[M28UnitInfo.refbTreatAmphibiousUnitAsSubmarine]) then
                                        table.insert(tAvailableSubmarines, oUnit)
                                    else
                                ...
                                elseif EntityCategoryContains(M28UnitInfo.refCategoryMAA + M28UnitInfo.refCategoryNavalAA - M28UnitInfo.refCategorySubmarine, oUnit.UnitId) and ((oUnit[M28UnitInfo.refiDFRange] or 0) < 10 or not(EntityCategoryContains(M28UnitInfo.refCategoryDestroyer + M28UnitInfo.refCategoryFrigate + M28UnitInfo.refCategoryBattleship + M28UnitInfo.refCategoryBattlecruiser, oUnit.UnitId))) then
                                    table.insert(tAvailableMAA, oUnit)
                ...
                for iPond, tPondSubtable in M28Map.tPondDetails do
                    if M28Utilities.IsTableEmpty(tPondSubtable[M28Map.subrefPondWaterZones]) == false then
                        for iWaterZone, tWZData in tPondSubtable[M28Map.subrefPondWaterZones] do
                            ...
                            elseif M28UnitInfo.IsUnitValid(tWZTeamData[M28Map.refoLastNavalFacAssisted]) and M28UnitInfo.GetUnitTechLevel(tWZTeamData[M28Map.refoLastNavalFacAssisted]) >= M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyNavalFactoryTech] then

                                local tBrainStartPosition = tPondSubtable[M28Map.subrefBuildLocationByStartPosition][tWZTeamData[M28Map.refoLastNavalFacAssisted]:GetAIBrain():GetArmyIndex()]
                                local tStartWZData, tStartWZTeamData = M28Map.GetLandOrWaterZoneData(tBrainStartPosition, true, iTeam)
                                --if bDebugMessages == true then LOG(sFunctionRef..': tBrainStartPosition='..repru(tBrainStartPosition)..'; refoLastNavalFacAssisted='..tWZTeamData[M28Map.refoLastNavalFacAssisted].UnitId..M28UnitInfo.GetUnitLifetimeCount(tWZTeamData[M28Map.refoLastNavalFacAssisted])..'; tStartWZTeamData is nil='..tostring(tStartWZTeamData == nil)..'; tWZTeamData[M28Map.reftClosestFriendlyBase]='..repru(tWZTeamData[M28Map.reftClosestFriendlyBase])..'; Factory brain owner='..(tWZTeamData[M28Map.refoLastNavalFacAssisted]:GetAIBrain().Nickname or 'nil')) end
                                if bDebugMessages == true then LOG(sFunctionRef..': We have a factory we have assisted that is our teams highest naval tech level, will check if it is closer to core base than our main naval build location for the brain owner, or that we lack any naval factory for our brain owner, dist from this WZ to closest base='..M28Utilities.GetDistanceBetweenPositions(tWZTeamData[M28Map.reftClosestFriendlyBase], tWZData[M28Map.subrefMidpoint])..'; Dist from main build location='..M28Utilities.GetDistanceBetweenPositions(tBrainStartPosition, tWZTeamData[M28Map.reftClosestFriendlyBase])..'; is tStartWZTeamData[M28Map.refoLastNavalFacAssisted] valid='..tostring(M28UnitInfo.IsUnitValid(tStartWZTeamData[M28Map.refoLastNavalFacAssisted]))) end
                                if not(M28UnitInfo.IsUnitValid(tStartWZTeamData[M28Map.refoLastNavalFacAssisted])) or M28Utilities.GetDistanceBetweenPositions(tWZTeamData[M28Map.reftClosestFriendlyBase], tWZData[M28Map.subrefMidpoint]) < M28Utilities.GetDistanceBetweenPositions(tBrainStartPosition, tWZTeamData[M28Map.reftClosestFriendlyBase]) then
                                    if bDebugMessages == true then LOG(sFunctionRef..': Will make this a core WZ base') end
...
function ConsiderOrdersForUnitsWithNoTarget(tWZData, iPond, iWaterZone, iTeam, tSubmarinesWithNoTarget, tCombatUnitsWithNoTarget, tMissileShips)
    ...
    if M28Utilities.IsTableEmpty(tWZData[M28Map.subrefWZOtherWaterZones]) == false then
        ...
        for iEntry, tWZSubtable in tWZData[M28Map.subrefWZOtherWaterZones] do
            ...
            if bDontCheckPlayableArea or (M28Conditions.IsLocationInPlayableArea(tAltWZData[M28Map.subrefMidpoint]) and M28Conditions.CanTravelToDestinationWithinMapBounds(tWZData[M28Map.subrefMidpoint], tAltWZData[M28Map.subrefMidpoint], M28Map.refPathingTypeNavy)) then
    ...
    if M28Utilities.IsTableEmpty(tUnitsWithOnlyAntiNavy) == false then
        ...
        if iSubmersibleWZToSupport then
            RecordWaterZoneTarget(tWZTeamData, iWaterZone, iTeam, true, iSubmersibleWZToSupport, M28Map.subrefiLZOrWZTMovingToOtherZone, false)
    ...
    if M28Utilities.IsTableEmpty(tUnitsWithNoAntiNavy) == false then
        if not(iNoAntiNavyWZToSupport) then
            ...
            tWZTeamData[M28Map.refbNoSurfaceSupportPoint] = true
        else
    ...
    if (iWZToSupport or iNoAntiNavyWZToSupport) and (M28Utilities.IsTableEmpty(tUnitsWithAntiNavyAndSurface) == false or M28Utilities.IsTableEmpty(tUnitsWithNoAntiNavy) == false) then
        RecordWaterZoneTarget(tWZTeamData, iWaterZone, iTeam, false, (iWZToSupport or iNoAntiNavyWZToSupport), M28Map.subrefiLZOrWZTMovingToOtherZone, false)
    end
...
function AssignBombardmentActions(tWZData, iPond, iWaterZone, iTeam, tPotentialBombardmentUnits, tbOptionalPlateauAndLandZonesToConsider, iOptionalRaidBaseWZ, bOptionalDontTreatAsBombardment)
    ...
    local tWZTeamData = tWZData[M28Map.subrefWZTeamData][iTeam]
    --Campaign specific - if have pacifist land zones adjacent to here then dont do bomardment unless none of the adjacent land zones are pacifist zones and the unit is trying to attack somewhere that isnt pacifist
    if M28Overseer.bPacifistModeActive and M28Conditions.AdjacentToPacifistZone(0, iWaterZone) then
        ...
        local tRallyPoint = GetNearestWaterRallyPoint(tWZData, iTeam, iPond, iWaterZone)
        local bConsiderIfTargetingNonPacifistZone = true
        if M28Utilities.IsTableEmpty(tWZData[M28Map.subrefAdjacentLandZones]) == false then
            bConsiderIfTargetingNonPacifistZone = true
            for iEntry, tSubtable in tWZData[M28Map.subrefAdjacentLandZones] do
                if M28Conditions.IsZoneAPacifistZone(tSubtable[M28Map.subrefWPlatAndLZNumber][1], tSubtable[M28Map.subrefWPlatAndLZNumber][2]) then
                    bConsiderIfTargetingNonPacifistZone = false
                    break
                end
            end
        end
        for iCurUnit = table.getn(tPotentialBombardmentUnits), 1, -1 do
            local oUnit = tPotentialBombardmentUnits[iCurUnit]
            if not(bConsiderIfTargetingNonPacifistZone) or ((oUnit[M28Orders.reftiLastOrders][1][M28Orders.subreftOrderPosition] and M28Conditions.IsLocationInAPacifistZone( oUnit[M28Orders.reftiLastOrders][1][M28Orders.subreftOrderPosition])) or (M28UnitInfo.IsUnitValid(oUnit[M28Orders.reftiLastOrders][1][M28Orders.subrefoOrderUnitTarget]) and M28Conditions.IsLocationInAPacifistZone( oUnit[M28Orders.reftiLastOrders][1][M28Orders.subrefoOrderUnitTarget]:GetPosition()))) then
    ...
    if M28Utilities.IsTableEmpty(tPotentialBombardmentUnits) == false then
        ...
        for iUnit, oUnit in tPotentialBombardmentUnits do
            ...
            if (oUnit[M28UnitInfo.refiIndirectRange] or 0) > 0 or (oUnit[M28UnitInfo.refiDFRange] or 0) > 0 then
                ...
                iCurDist = M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), tClosestEnemyBase)
        ...
        if bDebugMessages == true then LOG(sFunctionRef..': Deciding whether to include enemy t1 mexes in potential bombardment categories, subrefWZTThreatAllyCombatTotal='..tWZTeamData[M28Map.subrefWZTThreatAllyCombatTotal]..'; iOurBestIndirectRange='..iOurBestIndirectRange) end
        ...
        if aiBrain then
                            ...
                            if bTargetInDesiredZone then
                                ...
                                if iCurDist < iClosestMexDist then
                                    ...
                                    if M28Utilities.IsTableEmpty(tEnemyBuildings) == false then
                                        if (not(M28Overseer.bPacifistModeActive) or not(M28Conditions.IsCloseToPacifistUnit(tMexInfo[M28Map.subrefMexLocation], 120))) then
            ...
            --Get the target
            local tBombardmentMainTarget, iBombardmentMainTargetDistToMidpoint, bBombardmentMainTargetIsClosestBase
            if iClosestMexRef then
                if bDebugMessages == true then LOG(sFunctionRef..': Setting tBombardmentMainTarget to be closestmex') end
                tBombardmentMainTarget = {tClosestMex[1], tClosestMex[2], tClosestMex[3]}
            else
                --Is closest enemy base in playable area and not a pacifist zone?
                if bDontCheckPlayableArea or M28Conditions.IsLocationInPlayableArea(tWZTeamData[M28Map.reftClosestEnemyBase]) then
                    if not(M28Overseer.bPacifistModeActive) or not(M28Conditions.IsLocationInPlayableArea(tWZTeamData[M28Map.reftClosestEnemyBase])) then
                        if bDebugMessages == true then LOG(sFunctionRef..': Setting tBombardmentMainTarget to be closestenemybase') end
                        ...
                        iBombardmentMainTargetDistToMidpoint = M28Utilities.GetDistanceBetweenPositions(tBombardmentMainTarget, tWZData[M28Map.subrefMidpoint])
                        bBombardmentMainTargetIsClosestBase = true
                    end
                end
            end
            ...
            if (tbOptionalPlateauAndLandZonesToConsider and not(iClosestMexRef)) or (M28Map.bIsCampaignMap and (not(iClosestMexRef) or not(M28Conditions.IsLocationInPlayableArea(tBombardmentMainTarget)))) then
                                ...
                                if not(tbOptionalPlateauAndLandZonesToConsider) or tbOptionalPlateauAndLandZonesToConsider[iAdjPlateau][iAdjLZ] then
                                    ...
                                    if bDontCheckPlayableArea or M28Conditions.IsLocationInPlayableArea(tAdjLZData[M28Map.subrefMidpoint]) then
                                        ...
                                        if tAdjLZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ] or tAdjLZTeamData[M28Map.subrefThreatEnemyStructureTotalMass] >= 30 then
                                            if not(M28Overseer.bPacifistModeActive) or not(tAdjLZData[M28Map.subrefbPacifistArea]) then
                ...
                if tbOptionalPlateauAndLandZonesToConsider and iOptionalRaidBaseWZ then
                    ...
                    if M28UnitInfo.IsUnitValid(tRaidWZTeamData[M28Map.refoLastRaidTarget]) then
                        if not(M28Overseer.bPacifistModeActive) or not(M28Conditions.IsLocationInAPacifistZone(tRaidWZTeamData[M28Map.refoLastRaidTarget]:GetPosition())) then
                            if not(oClosestEnemyUnit) then
                ...
                if oClosestEnemyUnit then
                    ...
                    bBombardmentMainTargetIsClosestBase = false
                    iBombardmentMainTargetDistToMidpoint = M28Utilities.GetDistanceBetweenPositions(tBombardmentMainTarget, tWZData[M28Map.subrefMidpoint])
                    ...
                    if bDebugMessages == true then LOG(sFunctionRef..': Setting tBombardmentMainTarget to be cloesst enemy unit, Will target closest enemy unit '..oClosestEnemyUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oClosestEnemyUnit)..'; Position of this='..repru(oClosestEnemyUnit:GetPosition())..'; oClosestEnemyUnit[reftBlockedShotLocationByPond]='..repru(oClosestEnemyUnit[reftBlockedShotLocationByPond])..'; iPond='..iPond) end
                ...
                elseif tbOptionalPlateauAndLandZonesToConsider then
                    ...
                    --Adjust to the nearest location on map to the bombardment target or the enemy base

                    --end
                    local iInterval = 30
                    local iDistFromBase = M28Utilities.GetDistanceBetweenPositions(tWZData[M28Map.subrefMidpoint], (tBombardmentMainTarget or tWZTeamData[M28Map.reftClosestEnemyBase]))
                    local iMaxDist = math.floor(iDistFromBase / iInterval) * iInterval
                    local iAngleFromTargetToMidpoint = M28Utilities.GetAngleFromAToB(tWZData[M28Map.subrefMidpoint], (tBombardmentMainTarget or tWZTeamData[M28Map.reftClosestEnemyBase]))
                    ...
                    for iCurDist = iInterval, iMaxDist, iInterval do
                        local tRevisedTarget = M28Utilities.MoveInDirection((tBombardmentMainTarget or tWZTeamData[M28Map.reftClosestEnemyBase]), iAngleFromTargetToMidpoint, iCurDist, true, false, true)
                        ...
                        if tRevisedTarget and M28Conditions.IsLocationInPlayableArea(tRevisedTarget) then
                            if bDebugMessages == true then
                                LOG(sFunctionRef..': Adjusting bombardment target, setting tBombardmentMainTarget to be revised target, iCurDist='..iCurDist..'; tRevisedTarget='..repru(tRevisedTarget)..'; tBombardmentMainTarget before adjust='..repru(tBombardmentMainTarget)..'; Map playable area='..repru(M28Map.rMapPlayableArea))
            ...
            local bDoneExtraCheckForShieldsNearTarget = false --used if bombardment unit is closer to taret than oClosestFriendlyUnitToEnemyBase
            local bCheckForShieldsNearTarget
            local iClosestFriendlyUnitDistToPriority
            if bDebugMessages == true then LOG(sFunctionRef..': Deciding whether to just look at tEnemyCombatNearFrontUnit for enemy shields and TMD - if indirect range is bigger then will include units almost in range of indirect units, iSearchRange='..iSearchRange..'; iOurBestIndirectRange='..iOurBestIndirectRange..'; is tEnemyCombatNearFrontUnit empty='..tostring(M28Utilities.IsTableEmpty(tEnemyCombatNearFrontUnit))..'; oClosestFriendlyUnitToEnemyBase='..(oClosestFriendlyUnitToEnemyBase.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oClosestFriendlyUnitToEnemyBase) or 'nil')) end
                ...
                if M28Utilities.IsTableEmpty(tEnemyCombatNearFrontUnit) == false then
                    ...
                    if bDebugMessages == true then
                        if M28Utilities.IsTableEmpty(tEnemyShieldsAndTMD) == false then
                            for iRecorded, oRecorded in tEnemyShieldsAndTMD do
                                LOG(sFunctionRef..': Cycling through tEnemyShieldsAndTMD, oRecorded='..oRecorded.UnitId..M28UnitInfo.GetUnitLifetimeCount(tEnemyShieldsAndTMD)..'; dist to oClosestFriendlyUnitToEnemyBase='..M28Utilities.GetDistanceBetweenPositions(oClosestFriendlyUnitToEnemyBase:GetPosition(), oRecorded:GetPosition()))
                            end
                        else
                            LOG(sFunctionRef..': tEnemyShieldsAndTMD is empty')
                        end
                    end
                end
            else
            ...
            local tEnemyLowerPriorityBuildings = EntityCategoryFilterDown(M28UnitInfo.refCategoryStructure - M28UnitInfo.refCategoryPD - M28UnitInfo.refCategoryFixedT2Arti - M28UnitInfo.refCategoryFixedShield, tEnemyCombatNearFrontUnit)
            ...
            if M28Utilities.IsTableEmpty(tEnemyCombatNearFrontUnit) then
                if bDebugMessages == true then LOG(sFunctionRef .. ': No enemy structures within a range of iSearchRange='..iSearchRange..'; Min IF range='..iIndirectMinRange..'; iDFMinRange='..iDFMinRange..'; iOurBestIndirectRange='..(iOurBestIndirectRange or 'nil') .. ' of the unit closest to the enemy base, oClosestFriendlyUnitToEnemyBase='..(oClosestFriendlyUnitToEnemyBase.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oClosestFriendlyUnitToEnemyBase) or 'nil')) end
                bCheckForBuildingsToAttack = false
            elseif bDebugMessages == true then
                for iUnit, oUnit in tEnemyCombatNearFrontUnit do
                    LOG(sFunctionRef..': Going through tEnemyCombatNearFrontUnit, oUnit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; Dist to closestFriendlyUnitToEnemyBase='..M28Utilities.GetDistanceBetweenPositions(oClosestFriendlyUnitToEnemyBase:GetPosition(), oUnit:GetPosition()))
                end
            end
            ...
            if tBlockedShotBaseMoveLocation and tBombardmentMainTarget then
            ...
            for iUnit, oUnit in tPotentialBombardmentUnits do
                ...
                if not (M28UnitInfo.IsUnitUnderwater(oUnit)) then
                    ...
                    local bUpdatedBSShieldBombardment = false
                    ...
                    if bRetreatUnit then
                        ...
                        if (oUnit[M28UnitInfo.refiDFRange] or 0) >= iDFMinRange or (oUnit[M28UnitInfo.refiIndirectRange] or 0) >= iIndirectMinRange or ((oUnit[M28UnitInfo.refiDFRange] or 0) >= iDFLowerMinRange and M28UnitInfo.GetUnitHealthPercent(oUnit) >= 0.5) then
                            ...
                            if (oUnit[M28UnitInfo.refiDFRange] or 0) > (oUnit[M28UnitInfo.refiIndirectRange] or 0) and (oUnit[M28UnitInfo.refbLastShotBlocked] and (GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or -100)) >= 10 and GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiLastWeaponEvent] or -100) < 6) and (tBlockedShotBaseMoveLocation or (tBombardmentMainTarget and (oUnit[M28UnitInfo.refiDFRange] or 0) > 0 and oClosestEnemyUnit and not(oClosestEnemyUnit[reftBlockedShotLocationByPond]) and EntityCategoryContains(M28UnitInfo.refCategoryStructure, oClosestEnemyUnit.UnitId) and M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), tBombardmentMainTarget) - 5 <= (oUnit[M28UnitInfo.refiDFRange] or 0) and M28Utilities.GetRoughDistanceBetweenPositions(tBombardmentMainTarget, oClosestEnemyUnit:GetPosition()) <= 5))  then
                                ...
                                bCheckForShieldsNearTarget = true
                                ...
                                if bCheckForBuildingsToAttack then
                                    ...
                                    if M28Utilities.IsTableEmpty(tEnemyCombatNearFrontUnit) == false then
                                        for iPriority, oPriority in tEnemyCombatNearFrontUnit do
                                            if ((oPriority[M28UnitInfo.refiMissileShotBlockedCount] or 0) <= 10 or not(oUnit[M28UnitInfo.reftoTargetBlockedMissileCountByEntityId]) or (oUnit[M28UnitInfo.reftoTargetBlockedMissileCountByEntityId][oPriority.EntityId] or 0) == 0) and (oPriority[M28UnitInfo.refiTargetShotBlockedCount] or 0) < 15 then
                                                ...
                                                if oUnit[M28UnitInfo.refiIndirectRange] > (oUnit[M28UnitInfo.refiDFRange] or 0) and iClosestDist < iClosestImmobileDist and (not(EntityCategoryContains(categories.MOBILE, oPriority.UnitId)) or M28UnitInfo.GetUnitSpeed(oPriority) == 0) then
                                    ...
                                    if oBuildingOrPriorityToAttack then
                                        if not(iClosestFriendlyUnitDistToPriority) then iClosestFriendlyUnitDistToPriority = M28Utilities.GetDistanceBetweenPositions(oClosestFriendlyUnitToEnemyBase:GetPosition(), oBuildingOrPriorityToAttack:GetPosition()) end
                                        iCurDist = M28Utilities.GetDistanceBetweenPositions(oBuildingOrPriorityToAttack:GetPosition(), oUnit:GetPosition())
                                        if iCurDist  > math.max((oUnit[M28UnitInfo.refiDFRange] or 0), (oUnit[M28UnitInfo.refiIndirectRange] or 0)) then
                                    ...
                                    if bCheckForShieldsNearTarget or not(oBuildingOrPriorityToAttack) then
                                        --Consider overwriting with a priority target (e.g. TMD and shields)
                                        if bDebugMessages == true then LOG(sFunctionRef..': Considering whether to ignore closest building if enoguh defence headroom, iDefencesHeadroom='..(iDefencesHeadroom or 'nil')..'; bCheckForDangerousDefences='..tostring(bCheckForDangerousDefences)..'; Is tEnemyShieldsAndTMD empty='..tostring(M28Utilities.IsTableEmpty(tEnemyShieldsAndTMD))..'; bDoneExtraCheckForShieldsNearTarget='..tostring(bDoneExtraCheckForShieldsNearTarget)) end
                                        if (iDefencesHeadroom >= 10 or not(bCheckForDangerousDefences)) then

                                            if not(bDoneExtraCheckForShieldsNearTarget) and oBuildingOrPriorityToAttack then
                                                if not(iClosestFriendlyUnitDistToPriority) then iClosestFriendlyUnitDistToPriority = M28Utilities.GetDistanceBetweenPositions(oClosestFriendlyUnitToEnemyBase:GetPosition(), oBuildingOrPriorityToAttack:GetPosition()) end
                                                if not(iCurDist) then iCurDist = M28Utilities.GetDistanceBetweenPositions(oBuildingOrPriorityToAttack:GetPosition(), oUnit:GetPosition()) end
                                                if bDebugMessages == true then LOG(sFunctionRef..': IF this unit is closer to target tahn our closest unit, then update table of shields and tmd, iCurDist='..iCurDist..'; iClosestFriendlyUnitDistToPriority='..iClosestFriendlyUnitDistToPriority) end
                                                if iCurDist <= iClosestFriendlyUnitDistToPriority - 4 then
                                                    --Look for shields near oBuildingOrPriorityToAttack
                                                    bDoneExtraCheckForShieldsNearTarget = true
                                                    local bAlreadyRecorded
                                                    local toShieldsAndTMDNearTarget = aiBrain:GetUnitsAroundPoint(M28UnitInfo.refCategoryFixedShield + M28UnitInfo.refCategoryTMD, oBuildingOrPriorityToAttack:GetPosition(), 25, 'Enemy')
                                                    if bDebugMessages == true then LOG(sFunctionRef..': Just done getunitsaroundpoint, Is toShieldsAndTMDNearTarget empty='..tostring(M28Utilities.IsTableEmpty(toShieldsAndTMDNearTarget))) end
                                                    if M28Utilities.IsTableEmpty(toShieldsAndTMDNearTarget) == false then
                                                        if M28Utilities.IsTableEmpty(tEnemyShieldsAndTMD) then tEnemyShieldsAndTMD = toShieldsAndTMDNearTarget
                                                        else
                                                            for iEnemyShield, oEnemyShield in toShieldsAndTMDNearTarget do
                                                                bAlreadyRecorded = false
                                                                for iRecordedShield, oRecordedShield in tEnemyShieldsAndTMD do
                                                                    if oRecordedShield == oEnemyShield then bAlreadyRecorded = true break end
                                                                end
                                                                if bDebugMessages == true then LOG(sFunctionRef..': Already recorded shield  bAlreadyRecorded='..tostring(bAlreadyRecorded)..'; oEnemyShield='..oEnemyShield.UnitId..M28UnitInfo.GetUnitLifetimeCount(oEnemyShield)) end
                                                                if not(bAlreadyRecorded) then
                                                                    if bDebugMessages == true then LOG(sFunctionRef..'; Adding oEnemyShield='..oEnemyShield.UnitId..M28UnitInfo.GetUnitLifetimeCount(oEnemyShield)..' to table of enemy shields and TMD as it is near the target and not in our table to consider') end
                                                                    table.insert(tEnemyShieldsAndTMD, oEnemyShield)
                                                                end
                                                            end
                                                        end
                                                    end
                                            ...
                                            if M28Utilities.IsTableEmpty(tEnemyShieldsAndTMD) == false then
                                                ...
                                                if EntityCategoryContains(M28UnitInfo.refCategoryMissileShip, oUnit.UnitId) then
                                                    tPriorityTargets = EntityCategoryFilterDown(M28UnitInfo.refCategoryFixedShield + M28UnitInfo.refCategoryTMD, tEnemyShieldsAndTMD)
                                                else
                                                    tPriorityTargets = EntityCategoryFilterDown(M28UnitInfo.refCategoryFixedShield, tEnemyShieldsAndTMD)
                                                end
                                                if bDebugMessages == true then LOG(sFunctionRef..': Considering targeting TMD and shields, is tPriorityTargets empty='..tostring(M28Utilities.IsTableEmpty(tPriorityTargets))..'; size of tPriorityTargets='..table.getn(tPriorityTargets)..'; size of tEnemyShieldsAndTMD='..table.getn(tEnemyShieldsAndTMD)) end
                                                if M28Utilities.IsTableEmpty(tPriorityTargets) == false then
                                                    for iCurPriority, oPriority in tPriorityTargets do
                                                        if bDebugMessages == true then LOG(sFunctionRef..': Considering shield (or  TMD for missile ships), oPriority='..oPriority.UnitId..M28UnitInfo.GetUnitLifetimeCount(oPriority)..'; oPriority[M28UnitInfo.refiMissileShotBlockedCount]='..(oPriority[M28UnitInfo.refiMissileShotBlockedCount] or 'nil')..'; Is oUnit[M28UnitInfo.reftoTargetBlockedMissileCountByEntityId] nil='..tostring(oUnit[M28UnitInfo.reftoTargetBlockedMissileCountByEntityId] == nil)..'; oPriority[M28UnitInfo.refiTargetShotBlockedCount]='..(oPriority[M28UnitInfo.refiTargetShotBlockedCount] or 'nil')..'; iCurPriority='..iCurPriority) end
                                                        if ((oPriority[M28UnitInfo.refiMissileShotBlockedCount] or 0) <= 10 or not(oUnit[M28UnitInfo.reftoTargetBlockedMissileCountByEntityId]) or (oUnit[M28UnitInfo.reftoTargetBlockedMissileCountByEntityId][oPriority.EntityId] or 0) == 0) and (oUnit[M28UnitInfo.reftoTargetBlockedMissileCountByEntityId] or (oPriority[M28UnitInfo.refiTargetShotBlockedCount] or 0) < 15) then
                                                            iDistToPriority = M28Utilities.GetDistanceBetweenPositions(oPriority:GetPosition(), oUnit:GetPosition())
                                                            iModDistToPriority = iDistToPriority
                                                            ...
                                                            if iMaxShield > 0 and iCurShield < math.min(iMaxShield * 0.1, 1000) then
                                                                if GetGameTimeSeconds() >= 45*60 then iModDistToPriority = iModDistToPriority - math.max((oPriority:GetBlueprint().Defense.Shield.ShieldSize or 0) * 0.5, math.min(20, iModDistToPriority * 0.2))
                                                                else iModDistToPriority = iModDistToPriority - math.min(20, iModDistToPriority * 0.2)
                                                                end
                                                                if bDebugMessages == true then LOG(sFunctionRef..': Considering a low health shield so will reduce dist to make it more likely we attack it, iModDistToPriority='..iModDistToPriority..'; iDistToPriority='..iDistToPriority..'; iClosestDist='..iClosestDist) end
                                                            end
                                                            if bDebugMessages == true then LOG(sFunctionRef..': Dist between unit and oPriority='..M28Utilities.GetDistanceBetweenPositions(oPriority:GetPosition(), oUnit:GetPosition())..'; iModDistToPriority='..iModDistToPriority..'; iCurShield='..iCurShield..'; iMaxShield='..iMaxShield..'; is ModDist < closest dist='..tostring(iModDistToPriority < iClosestDist)) end
                                                            if iModDistToPriority < iClosestDist then
                                                                if bDebugMessages == true then LOG(sFunctionRef..': Dist between oBuildingOrPriorityToAttack and oPriority='..M28Utilities.GetDistanceBetweenPositions(oBuildingOrPriorityToAttack:GetPosition(), oPriority:GetPosition())..'; bDontCheckIfTargetUnderwater='..tostring(bDontCheckIfTargetUnderwater)..'; Is underwater='..tostring(M28UnitInfo.IsUnitUnderwater(oPriority))..'; Is shot blocked='..tostring(M28Logic.IsShotBlocked(oUnit, oPriority))..'; Time of last unblocked shot='..(GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or -100))) end
                                                                if bDontCheckIfTargetUnderwater or not(M28UnitInfo.IsUnitUnderwater(oPriority)) then
                                                                    ...
                                                                    then
                                                                        if not((oUnit[M28UnitInfo.refiDFRange] or 0) > (oUnit[M28UnitInfo.refiIndirectRange] or 0) and ((oUnit[M28UnitInfo.refbLastShotBlocked] and (GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or -100)) >= 10 and GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastCheck] or -100) < 6 and iDistToPriority <= oUnit[M28UnitInfo.refiDFRange] and GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastCheck] or 0) <= 4) or M28Logic.IsShotBlocked(oUnit, oPriority))) then
                                                                            ...
                                                                            --Fixed shield targeted by battleship - set zone flag
                                                                            if not(bUpdatedBSShieldBombardment) and EntityCategoryContains(M28UnitInfo.refCategoryBattleship, oUnit.UnitId) and EntityCategoryContains(M28UnitInfo.refCategoryFixedShield, oPriority.UnitId) then
                                                                                local tTargetLZOrWZData, tTargetLZOrWZTeamData = M28Map.GetLandOrWaterZoneData(oPriority:GetPosition(), true, iTeam)
                                                                                local iEnemyShielding = (tTargetLZOrWZTeamData[M28Map.subrefThreatEnemyShield] or 0)
                                                                                if M28Utilities.IsTableEmpty(oPriority[M28Building.reftoShieldsProvidingCoverage]) == false then
                                                                                    iEnemyShielding = math.max(iEnemyShielding, (oPriority[M28UnitInfo.refiUnitMassCost] or M28UnitInfo.GetUnitMassCost(oPriority)) + M28UnitInfo.GetMassCostOfUnits(oPriority[M28Building.reftoShieldsProvidingCoverage], true))
                                                                                else
                                                                                    iEnemyShielding = math.max(iEnemyShielding, (oPriority[M28UnitInfo.refiUnitMassCost] or M28UnitInfo.GetUnitMassCost(oPriority)))
                                                                                end

                                                                                if not(M28Map.tPondDetails[iPond][M28Map.refiLastBombardmentBSShieldTargetValueByTeam]) then M28Map.tPondDetails[iPond][M28Map.refiLastBombardmentBSShieldTargetValueByTeam] = {} end
                                                                                if not(M28Map.tPondDetails[iPond][M28Map.refiLastBombardmentBSShieldTargetTimeByTeam]) then M28Map.tPondDetails[iPond][M28Map.refiLastBombardmentBSShieldTargetTimeByTeam] = {} end
                                                                                local iTime = math.floor(GetGameTimeSeconds())
                                                                                if M28Map.tPondDetails[iPond][M28Map.refiLastBombardmentBSShieldTargetTimeByTeam] >= iTime then
                                                                                    M28Map.tPondDetails[iPond][M28Map.refiLastBombardmentBSShieldTargetValueByTeam][iTeam] = math.max((M28Map.tPondDetails[iPond][M28Map.refiLastBombardmentBSShieldTargetValueByTeam][iTeam] or 0), iEnemyShielding)
                                                                                else
                                                                                    M28Map.tPondDetails[iPond][M28Map.refiLastBombardmentBSShieldTargetValueByTeam][iTeam] = iEnemyShielding
                                                                                end
                                                                                bUpdatedBSShieldBombardment = true
                                                                                M28Map.tPondDetails[iPond][M28Map.refiLastBombardmentBSShieldTargetTimeByTeam][iTeam] = math.floor(GetGameTimeSeconds())
                                                                                if bDebugMessages == true then LOG(sFunctionRef..': iEnemyShielding='..iEnemyShielding) end
                                                                            end
                                                                        else
                                                                            if bDebugMessages == true then LOG(sFunctionRef..': Shot blocked so wont change oBuildingOrPriorityToAttack afterall, iDistToPriority='..iDistToPriority..'; our unit refiDFRange='..(oUnit[M28UnitInfo.refiDFRange] or 0)..'; iRangeThreshold='..iRangeThreshold) end
                                                                            if iDistToPriority < (oUnit[M28UnitInfo.refiDFRange] or 0) - iRangeThreshold then
                                                                                table.insert(toBlockedBuildingsAndDistWithinRange, {oPriority, (oUnit[M28UnitInfo.refiDFRange] or 0) - iDistToPriority})
                                                                            end
                                                                        end
                                    ...
                                    if bBombardmentMainTargetIsClosestBase and not(oBuildingOrPriorityToAttack) and M28Utilities.IsTableEmpty(tEnemyLowerPriorityBuildings) == false then
                                        --check lower priority buildings for potential targets
                                        if bDebugMessages == true then LOG(sFunctionRef..': Will check lower priority buildings for if we want to attack them instead') end
                                        for iLowerPriority, oLowerPriority in tEnemyLowerPriorityBuildings do
                                            iCurDist = M28Utilities.GetDistanceBetweenPositions(oLowerPriority:GetPosition(), oUnit:GetPosition())
                                            if iCurDist < iClosestDist then
                                                iClosestDist = iCurDist
                                                if iCurDist - math.min((iDefencesHeadroom or 30), 30) <= math.max((oUnit[M28UnitInfo.refiIndirectRange] or 0), (oUnit[M28UnitInfo.refiDFRange] or 0)) then
                                                    if not((oUnit[M28UnitInfo.refiDFRange] or 0) > (oUnit[M28UnitInfo.refiIndirectRange] or 0) and ((oUnit[M28UnitInfo.refbLastShotBlocked] and (GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or -100)) >= 10 and GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastCheck] or -100) < 6 and iDistToPriority <= oUnit[M28UnitInfo.refiDFRange] and GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastCheck] or 0) <= 4) or M28Logic.IsShotBlocked(oUnit, oLowerPriority))) then
                                                        if bDebugMessages == true then LOG(sFunctionRef..': Changing oBuildingOrPriorityToAttack to be this lower priority unit, iCurDist='..iCurDist) end
                                                        oBuildingOrPriorityToAttack = oLowerPriority
                                                    else
                                                        if bDebugMessages == true then LOG(sFunctionRef..': Shot blocked so wont change oBuildingOrPriorityToAttack to lower priority afterall, iCurDist='..iCurDist..'; our df range='..(oUnit[M28UnitInfo.refiDFRange] or 'nil')..'; iRangeThreshold='..iRangeThreshold) end
                                                        if iCurDist < (oUnit[M28UnitInfo.refiDFRange] or 0) - iRangeThreshold then
                                                            table.insert(toBlockedBuildingsAndDistWithinRange, {oLowerPriority, (oUnit[M28UnitInfo.refiDFRange] or 0) - iCurDist})
                                                        end
                                                    end
                                                end
                                            end
                                    ...
                                    if not(oBuildingOrPriorityToAttack) and M28Utilities.IsTableEmpty(toBlockedBuildingsAndDistWithinRange) == false then
                                        ...
                                        for iEntry, tUnitAndDist in toBlockedBuildingsAndDistWithinRange do
                                            ...
                                            if tUnitAndDist[2] > iFurthestInRange then
                                                ...
                                                if bDebugMessages == true then LOG(sFunctionRef..': tMoveViaPoint='..repru(tMoveViaPoint)..'; Distance from tMoveViaPoint and this unit='..(tUnitAndDist[2]-1)..'; Cur poisition='..repru(oUnit:GetPosition())..'; Is shot blocked from the via point='..tostring(M28Logic.IsShotBlocked(oUnit, tUnitAndDist[1], false, tMoveViaPoint))..'; iPond='..iPond..'; Naval path label for move point='..NavUtils.GetTerrainLabel(M28Map.refPathingTypeNavy, tMoveViaPoint)..'; if we move to via point are we blocked='..tostring(M28Logic.IsShotBlocked(oUnit, tUnitAndDist[1], false, tMoveViaPoint))) end
                            ...
                            if oBuildingOrPriorityToAttack and (not(tBombardmentMainTarget) or bBombardmentMainTargetIsClosestBase) then
                                if bDebugMessages == true then LOG(sFunctionRef..': Setting tBombardmentMainTarget to be oBuildingOrPriorityToAttack') end
                                tBombardmentMainTarget =  oBuildingOrPriorityToAttack:GetPosition()
                                bBombardmentMainTargetIsClosestBase = false
                                iBombardmentMainTargetDistToMidpoint = M28Utilities.GetDistanceBetweenPositions(tBombardmentMainTarget, tWZData[M28Map.subrefMidpoint])
                            end
                            if tBombardmentMainTarget then
                                if not (oBuildingOrPriorityToAttack) or (bBlockedSoMove and tBlockedShotActualMoveLocation) then
                                    ...
                                    if bBlockedSoMove and tBlockedShotActualMoveLocation then
                                        ...
                                        if bConsiderGroundAttack and tBombardmentMainTarget then
                                        ...
                                        if ((oUnit[M28UnitInfo.refbLastShotBlocked] and (GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or -100)) >= 10 and GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastCheck] or -100) < 6) or (bIgnoreLowThreats and EntityCategoryContains(categories.TECH3 + M28UnitInfo.refCategoryMissileShip, oUnit.UnitId))) and tBombardmentMainTarget and M28Utilities.GetDistanceBetweenPositions(tBombardmentMainTarget, oUnit:GetPosition()) > math.max((oUnit[M28UnitInfo.refiDFRange] or 0), (oUnit[M28UnitInfo.refiIndirectRange] or 0)) then
                                        ...
                                        if bDebugMessages == true then
                                            LOG(sFunctionRef..': Will bombard the target wit hthis unit.  Considering whether to move or attack for unit '..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; bIgnoreLowThreats='..tostring(bIgnoreLowThreats)..'; oUnit[M28UnitInfo.refbLastShotBlocked]='..tostring(oUnit[M28UnitInfo.refbLastShotBlocked])..'; bEnemyUnitsNearlyInRange='..tostring(bEnemyUnitsNearlyInRange)..'; M28UnitInfo.GetUnitHealthPercent(oUnit)='..M28UnitInfo.GetUnitHealthPercent(oUnit)..'; tBombardmentMainTarget='..repru(tBombardmentMainTarget)..'; tBlockedShotBaseMoveLocation='..repru(tBlockedShotBaseMoveLocation)..'; main shot blocked test='..tostring((oUnit[M28UnitInfo.refbLastShotBlocked] and (GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or -100)) >= 10 and GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastCheck] or -100) < 6))..'; Time-refiTimeOfLastUnblockedShot='..(GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or -100))..'; Time-refiTimeOfLastCheck='..GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastCheck] or -100))
                                ...
                                --No bombardment location, e.g. campaign pacifist logic
                                if not(M28Overseer.bPacifistModeActive) then M28Utilities.ErrorHandler('Dont have bombardment target but not pacifist') end
                                M28Orders.IssueTrackedMove(oUnit, tNonBombardmentRallyPoint, 10, false, 'NoBombTarget', false)
                            end
                        else
...
function IncludeThreatOfAdjacentZone(iAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, tBaseWZToUse, tAdjWZTeamData, iAdjWZ, bIncludeAdjacentAlliedSubmersibleThreat, bIncludeAdjacentEnemyAntiNavyThreat, bIncludeAdjacentAlliedCombatThreat, bIncludeAdjacentEnemyCombatThreat, iEnemyFactorOverride)
    ...
    if M28Utilities.IsTableEmpty(tAdjWZTeamData[M28Map.subrefTEnemyUnits]) == false or M28Utilities.IsTableEmpty(tAdjWZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then
        local iEnemyFactor = iEnemyFactorOverride or 0.2
        local iAlliedFactor = 0.8
        if bIncludeAdjacentAlliedSubmersibleThreat then iAdjacentAlliedSubmersibleThreat = iAdjacentAlliedSubmersibleThreat + (tAdjWZTeamData[M28Map.subrefWZThreatAlliedSubmersible] or 0) end --* iAlliedFactor end --removed reduction in v317 as we are working out as separate variables those subs with a common target as us and factoring into threat decisions based on whether they are close to our front sub or not
        ...
        if tAdjWZTeamData[M28Map.subrefWZTThreatAllyLauncherDefenceTotal] > 0 then
            if bIncludeAdjacentAlliedSubmersibleThreat then iAdjacentAlliedSubmersibleThreat = iAdjacentAlliedSubmersibleThreat - math.min(tAdjWZTeamData[M28Map.subrefWZTThreatAllyLauncherDefenceTotal], (tAdjWZTeamData[M28Map.subrefWZThreatAlliedSubmersible] or 0)) end --* iAlliedFactor end
...
function RecordWaterZoneTarget(tCurrentLZOrWZTeamData, iLandOrWaterZone, iTeam, bSubmersibleNotSurface, iWaterZoneTarget, iAttackType, bIsLandZone)
    --Send nil value to clear previously recorded values
    local bDebugMessages = false if M28Profiler.bGlobalDebugOverride == true then   bDebugMessages = true end
    local sFunctionRef = 'RecordWaterZoneTarget'
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerStart)

    if bDebugMessages == true then LOG(sFunctionRef..': Start of code for iLandOrWaterZone='..(iLandOrWaterZone or 'nil')..'; iTeam='..iTeam..'; bSubmersibleNotSurface='..tostring(bSubmersibleNotSurface or false)..'; iWaterZoneTarget='..(iWaterZoneTarget or 'nil')..'; iAttackType='..(iAttackType or 'nil')..'; bIsLandZone='..tostring(bIsLandZone or false)) end
    if not(iWaterZoneTarget) then
        if bSubmersibleNotSurface and tCurrentLZOrWZTeamData[M28Map.subreftiWaterZoneTargetedByOurSubmersibleCombat] then
            local iZoneWeAreTargeting = tCurrentLZOrWZTeamData[M28Map.subreftiWaterZoneTargetedByOurSubmersibleCombat]
            local tTargetingWaterZoneTeamData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iZoneWeAreTargeting]][M28Map.subrefPondWaterZones][iZoneWeAreTargeting][M28Map.subrefWZTeamData][iTeam]
            if tTargetingWaterZoneTeamData then
                if bDebugMessages == true then LOG(sFunctionRef..': About to update for iZoneWeAreTargeting='..(iZoneWeAreTargeting or 'nil')..'; is tTargetingWaterZoneTeamData nil='..tostring(tTargetingWaterZoneTeamData == nil)) end
                if bIsLandZone then
                    if tTargetingWaterZoneTeamData[M28Map.subreftiLandZonesTargetingThisWithAmphibious] then
                        tTargetingWaterZoneTeamData[M28Map.subreftiLandZonesTargetingThisWithAmphibious][iLandOrWaterZone] = nil
                    end
                else
                    if tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSubmersibleNavy] then
                        tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSubmersibleNavy][iLandOrWaterZone] = nil
                    end
                end
            else
                M28Utilities.ErrorHandler('invalid WZ ref, iLandOrWaterZone='..(iLandOrWaterZone or 'nil')..'; subreftiLandZoneTargetedByOurCombat='..(tCurrentLZOrWZTeamData[M28Map.subreftiWaterZoneTargetedByOurSurfaceCombat] or 'nil'))
            end
            tCurrentLZOrWZTeamData[M28Map.subreftiWaterZoneTargetedByOurSubmersibleCombat] = nil
        elseif not(bSubmersibleNotSurface) and tCurrentLZOrWZTeamData[M28Map.subreftiWaterZoneTargetedByOurSurfaceCombat] then
            local iZoneWeAreTargeting = tCurrentLZOrWZTeamData[M28Map.subreftiWaterZoneTargetedByOurSurfaceCombat]
            local tTargetingWaterZoneTeamData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iZoneWeAreTargeting]][M28Map.subrefPondWaterZones][iZoneWeAreTargeting][M28Map.subrefWZTeamData][iTeam]
            if tTargetingWaterZoneTeamData then
                if bDebugMessages == true then LOG(sFunctionRef..': About to update for iZoneWeAreTargeting='..(iZoneWeAreTargeting or 'nil')..'; is tTargetingWaterZoneTeamData nil='..tostring(tTargetingWaterZoneTeamData == nil)) end
                if bIsLandZone then
                    ...
                    if tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSurfaceNavy] then
                        tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSurfaceNavy][iLandOrWaterZone] = nil
                    end
                end
            else
                M28Utilities.ErrorHandler('invalid WZ ref, iLandOrWaterZone='..(iLandOrWaterZone or 'nil')..'; subreftiLandZoneTargetedByOurCombat='..(tCurrentLZOrWZTeamData[M28Map.subreftiWaterZoneTargetedByOurSurfaceCombat] or 'nil'))
            end
            tCurrentLZOrWZTeamData[M28Map.subreftiWaterZoneTargetedByOurSurfaceCombat] = nil
        end
    else
        if bSubmersibleNotSurface then
            tCurrentLZOrWZTeamData[M28Map.subreftiWaterZoneTargetedByOurSubmersibleCombat] = iWaterZoneTarget
            local tTargetingWaterZoneTeamData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iWaterZoneTarget]][M28Map.subrefPondWaterZones][iWaterZoneTarget][M28Map.subrefWZTeamData][iTeam]
            if bIsLandZone then
                M28Utilities.ErrorHandler('Havent added code for this scenario yet')
            else
                if not(tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSubmersibleNavy]) then tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSubmersibleNavy] = {} end
                tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSubmersibleNavy][iLandOrWaterZone] = iAttackType
            end
        else --surface or land
            tCurrentLZOrWZTeamData[M28Map.subreftiWaterZoneTargetedByOurSurfaceCombat] = iWaterZoneTarget
            local tTargetingWaterZoneTeamData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iWaterZoneTarget]][M28Map.subrefPondWaterZones][iWaterZoneTarget][M28Map.subrefWZTeamData][iTeam]
            if bIsLandZone then
                if not(tTargetingWaterZoneTeamData[M28Map.subreftiLandZonesTargetingThisWithAmphibious]) then tTargetingWaterZoneTeamData[M28Map.subreftiLandZonesTargetingThisWithAmphibious] = {} end
                tTargetingWaterZoneTeamData[M28Map.subreftiLandZonesTargetingThisWithAmphibious][iLandOrWaterZone] = iAttackType
            else
                if not(tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSurfaceNavy]) then tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSurfaceNavy] = {} end
                if bDebugMessages == true then LOG(sFunctionRef..': iWaterZoneTarget='..iWaterZoneTarget..'; is tTargetingWaterZoneTeamData nil='..tostring(tTargetingWaterZoneTeamData == nil)..'; iLandOrWaterZone='..(iLandOrWaterZone or 'nil')..'; tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSurfaceNavy]='..reprs(tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSurfaceNavy])..'; iAttackType='..(iAttackType or 'nil')) end
                tTargetingWaterZoneTeamData[M28Map.subreftiWaterZonesTargetingThisWithSurfaceNavy][iLandOrWaterZone] = iAttackType
            end
        end
    end
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
...
function ManageCombatUnitsInWaterZone(tWZData, tWZTeamData, iTeam, iPond, iWaterZone, tAvailableCombatUnits, tAvailableSubmarines, tUnavailableUnitsInThisWZ, tMissileShips)
    ...
    local tUnassignedLandUnits
    if bDebugMessages == true then LOG(sFunctionRef..': start of code for time '..GetGameTimeSeconds()..', iTeam='..iTeam..'; iPond='..iPond..'; iWaterZone='..iWaterZone..'; Is table of available combat units empty='..tostring(M28Utilities.IsTableEmpty(tAvailableCombatUnits))..'; Is tAvailableSubmarines empty='..tostring(M28Utilities.IsTableEmpty(tAvailableSubmarines))..'; Are there enemy units in this or adjacent WZ='..tostring(tWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentWZ])..'; Is table of missile ships empty='..tostring(M28Utilities.IsTableEmpty(tMissileShips))..'; subrefWZiSuicideIntoEnemyCombatThreat='..(tWZTeamData[M28Map.subrefWZiSuicideIntoEnemyCombatThreat] or 'nil')) end
    ...
    if M28Utilities.IsTableEmpty(tWZData[M28Map.subrefWZAdjacentWaterZones]) == false then
        for _, iAltWZ in tWZData[M28Map.subrefWZAdjacentWaterZones] do
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Enemy air to ground threat in iAltWZ='..iAltWZ..'='..(tAltWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 'nil')..'; subrefWZThreatAlliedMAA='..(tAltWZTeamData[M28Map.subrefWZThreatAlliedMAA] or 0)) end
    ...
    if not(bHaveRunFromAir) or M28Utilities.IsTableEmpty(tAvailableSubmarines) == false or M28Utilities.IsTableEmpty(tAvailableCombatUnits) == false or M28Utilities.IsTableEmpty(tMissileShips) == false then
        ...
        function GetAdjacentNearbyThreatsWithSameZoneTarget(iLikelyTargetWaterZone, bGetSurfaceAndLandThreat, bGetSubmersibleThreat)
            local tLikelyTargetWZTeamData = M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][iLikelyTargetWaterZone][M28Map.subrefWZTeamData][iTeam]
            local iNearbySameTargetAdjacentSubmersibleAntiNavyThreat = 0
            local iNearbySameTargetAdjacentNonSubmersibleThreat = 0

            if bDebugMessages == true then LOG(sFunctionRef..': We are likely to be targeting nearest enemy in WZ='..(iLikelyTargetWaterZone or 'nil')..'; is subreftiWaterZonesTargetingThisWithSurfaceNavy empty='..tostring(M28Utilities.IsTableEmpty(tLikelyTargetWZTeamData[M28Map.subreftiWaterZonesTargetingThisWithSurfaceNavy]))) end
            --Surface and land threats
            if bGetSurfaceAndLandThreat and tLikelyTargetWZTeamData then
                local tbAdjZoneUnitsInAvailableCombatUnits
                if M28Utilities.IsTableEmpty(tLikelyTargetWZTeamData[M28Map.subreftiWaterZonesTargetingThisWithSurfaceNavy]) == false and M28Utilities.IsTableEmpty(tWZData[M28Map.subrefWZAdjacentWaterZones]) == false then
                    local tbZonesAdjacentToThis = {}
                    for _, iAdjWZ in tWZData[M28Map.subrefWZAdjacentWaterZones] do
                        tbZonesAdjacentToThis[iAdjWZ] = true
                    end
                    local bIncludeCurZoneDF = false
                    for iOtherWZ, iAttackingType in tLikelyTargetWZTeamData[M28Map.subreftiWaterZonesTargetingThisWithSurfaceNavy] do
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering iOtherWZ='..iOtherWZ..'; tbZonesAdjacentToThis[iOtherWZ]='..tostring(tbZonesAdjacentToThis[iOtherWZ] or false)..'; iLikelyTargetZone='..iLikelyTargetWaterZone) end
                        if tbZonesAdjacentToThis[iOtherWZ] and not(iLikelyTargetWaterZone == iOtherWZ) then
                            local tAdjWZTeamData = M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][iOtherWZ][M28Map.subrefWZTeamData][iTeam]
                            if bDebugMessages == true then LOG(sFunctionRef..': subrefWZTThreatAllyCombatTotal for otherWZ='..(tAdjWZTeamData[M28Map.subrefWZTThreatAllyCombatTotal] or 'nil')..'; OtherWZ subrefWZTValue='..(tAdjWZTeamData[M28Map.subrefWZTValue] or 'nil')..'; ThisWaterZone subrefWZTValue='..(tWZTeamData[M28Map.subrefWZTValue] or 'nil')) end
                            if tAdjWZTeamData[M28Map.subrefWZThreatAlliedSurface] > 0 and M28Utilities.IsTableEmpty(tAdjWZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then
                                --Include this zone unless we think we are giving orders to its units in availablecombatunits
                                bIncludeCurZoneDF = true
                                if tAdjWZTeamData[M28Map.subrefWZTValue] < tWZTeamData[M28Map.subrefWZTValue] then
                                    ...
                                    if not(tbAdjZoneUnitsInAvailableCombatUnits) then
                                        ...
                                        if M28Utilities.IsTableEmpty(tAvailableCombatUnits) == false then
                                            for iUnit, oUnit in tAvailableCombatUnits do
                                                if oUnit[refiCurrentAssignmentWaterZone] == iWaterZone and not(oUnit[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] == iWaterZone) then tbAdjZoneUnitsInAvailableCombatUnits[oUnit[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam]] = true end
                                            end
                                        end
                                    end
                                    ...
                                    if tbAdjZoneUnitsInAvailableCombatUnits[iOtherWZ] then bIncludeCurZoneDF = false end
                                end
                                if bIncludeCurZoneDF then
                                    if iAttackingType == M28Map.subrefiLZOrWZTMovingToOtherZone then
                                        ...
                                        local toSurfaceCombatUnitsToInclude = {}
                                        if M28Utilities.IsTableEmpty(tAdjWZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then
                                            local iClosestFriendlyToOtherZoneDist = 10000
                                            local oClosestFriendlyToOtherZone
                                            local tOtherZoneMidpoint = M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][iOtherWZ][M28Map.subrefMidpoint]
                                            local iCurDist
                                            if M28Utilities.IsTableEmpty(tAvailableCombatUnits) == false then
                                                for iUnit, oUnit in tAvailableCombatUnits do
                                            ...
                                            if oClosestFriendlyToOtherZone then
                                                for iUnit, oUnit in tAdjWZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits] do
                                                    if not(oUnit.Dead) then
                                                        --Are we within 70 of being in range? then include
                                                        ...
                                                        if M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oClosestFriendlyToOtherZone:GetPosition()) - oUnit[M28UnitInfo.refiCombatRange] <= 30 then
                                                            if EntityCategoryContains(M28UnitInfo.refCategoryNavalSurface + categories.HOVER, oUnit.UnitId) then
                                                                table.insert(toSurfaceCombatUnitsToInclude, oUnit)
                                                            end
                                                        end
                                                    end
                                                end
                                                if M28Utilities.IsTableEmpty(toSurfaceCombatUnitsToInclude) == false then
                                                    iNearbySameTargetAdjacentNonSubmersibleThreat = iNearbySameTargetAdjacentNonSubmersibleThreat + M28UnitInfo.GetCombatThreatRating(toSurfaceCombatUnitsToInclude, false, false, false, false, true, false)
                                                end
                                            end
                                        end
                                    else
                                        iNearbySameTargetAdjacentNonSubmersibleThreat = iNearbySameTargetAdjacentNonSubmersibleThreat + M28UnitInfo.GetCombatThreatRating(tAdjWZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits], false, false, false, false, true, false)
                                    end
                                    if bDebugMessages == true then LOG(sFunctionRef..': Including surface threat of friendly naval units in iOtherWZ='..iOtherWZ..', iNearbySameTargetAdjacentNonSubmersibleThreat='..iNearbySameTargetAdjacentNonSubmersibleThreat) end
                                end
                            end
                        end
                    end
                end
                --Include surface threat of any units already in the target zone as well
                if tLikelyTargetWZTeamData[M28Map.subrefWZThreatAlliedSurface] > 0 and not(iLikelyTargetWaterZone == iWaterZone) then
                    if not(tbAdjZoneUnitsInAvailableCombatUnits) then
                        tbAdjZoneUnitsInAvailableCombatUnits = {}
                        if M28Utilities.IsTableEmpty(tAvailableCombatUnits) == false then
                            for iUnit, oUnit in tAvailableCombatUnits do
                                if oUnit[refiCurrentWZAssignmentValue] == iWaterZone and not(oUnit[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] == iWaterZone) then tbAdjZoneUnitsInAvailableCombatUnits[oUnit[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam]] = true end
                            end
                        end
                    end
                    if not(tbAdjZoneUnitsInAvailableCombatUnits[iLikelyTargetWaterZone]) then
                        iNearbySameTargetAdjacentNonSubmersibleThreat = iNearbySameTargetAdjacentNonSubmersibleThreat + (tLikelyTargetWZTeamData[M28Map.subrefWZThreatAlliedSurface] or 0)

                        if bDebugMessages == true then LOG(sFunctionRef..': Including threat of friendly Naval units in iLikelyTargetWaterZone, iNearbySameTargetAdjacentSubmersibleAntiNavyThreat after this='..iNearbySameTargetAdjacentSubmersibleAntiNavyThreat..'; iNearbySameTargetAdjacentNonSubmersibleThreat='..iNearbySameTargetAdjacentNonSubmersibleThreat) end
                    end
                end
                if bDebugMessages == true then LOG(sFunctionRef..': iNearbySameTargetAdjacentSubmersibleAntiNavyThreat after including adj zones with the same targetWZ as us='..iNearbySameTargetAdjacentSubmersibleAntiNavyThreat..'; iNearbySameTargetAdjacentNonSubmersibleThreat='..iNearbySameTargetAdjacentNonSubmersibleThreat) end
                ...
                if bGetSurfaceAndLandThreat and M28Utilities.IsTableEmpty(tLikelyTargetWZTeamData[M28Map.subreftiLandZonesTargetingThisWithAmphibious]) == false and M28Utilities.IsTableEmpty(tWZData[M28Map.subrefAdjacentLandZones]) == false then
                    ...
                    for iOtherLZ, iAttackingType in tLikelyTargetWZTeamData[M28Map.subreftiLandZonesTargetingThisWithAmphibious] do
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering iOtherLZ='..iOtherLZ..'; tbWaterZonesAdjacentToThis[iOtherLZ]='..tostring(tbLandZonesAdjacentToThis[iOtherLZ] or false)..'; iLikelyTargetWaterZone='..iLikelyTargetWaterZone) end
                        if tbLandZonesAdjacentToThis[iOtherLZ] then
                            local tAdjLZTeamData = M28Map.tAllPlateaus[iPlateau][M28Map.subrefPlateauLandZones][iOtherLZ][M28Map.subrefLZTeamData][iTeam]
                            if bDebugMessages == true then LOG(sFunctionRef..': subrefWZThreatAlliedSurface for otherLZ='..(tAdjLZTeamData[M28Map.subrefWZThreatAlliedSurface] or 'nil')..'; is subrefWZTAlliedSurfaceCombatUnits empty='..tostring(M28Utilities.IsTableEmpty(tAdjLZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits]))) end
                            if tAdjLZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] > 0 and M28Utilities.IsTableEmpty(tAdjLZTeamData[M28Map.subrefLZTAlliedCombatUnits]) == false then
                                if iAttackingType == M28Map.subrefiLZOrWZTMovingToOtherZone then
                                    --We might have units that are far away, so only include if they are relatively close to our available units, and arent skirmishers
                                    local toUnitsToInclude = {}
                                    local toPotentialUnits = EntityCategoryFilterDown(M28UnitInfo.refCategoryAmphibiousCombat, tAdjLZTeamData[M28Map.subrefLZTAlliedCombatUnits])
                                        ...
                                        if oClosestFriendlyToOtherZone then
                                            for iUnit, oUnit in toPotentialUnits do
                                                if not(oUnit.Dead) then
                                                    ...
                                                    if bDebugMessages == true then LOG(sFunctionRef..': Dist between LZ unit='..M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oClosestFriendlyToOtherZone:GetPosition())..'; CombatRange='..oUnit[M28UnitInfo.refiCombatRange]..'; Dist until in range='..M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oClosestFriendlyToOtherZone:GetPosition()) - oUnit[M28UnitInfo.refiCombatRange]) end
                                                    if M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oClosestFriendlyToOtherZone:GetPosition()) - oUnit[M28UnitInfo.refiCombatRange] <= 30 then
                                            ...
                                            if M28Utilities.IsTableEmpty(toUnitsToInclude) == false then
                                                if bDebugMessages == true then LOG(sFunctionRef..': Generic combat threat of units that are including='..M28UnitInfo.GetCombatThreatRating(toUnitsToInclude, false)) end
                                                --iNearbySameTargetAdjacentSubmersibleAntiNavyThreat = iNearbySameTargetAdjacentSubmersibleAntiNavyThreat + M28UnitInfo.GetCombatThreatRating(toUnitsToInclude, false, false, false, false, false, true, false)
                                                iNearbySameTargetAdjacentNonSubmersibleThreat = iNearbySameTargetAdjacentNonSubmersibleThreat + M28UnitInfo.GetCombatThreatRating(toUnitsToInclude, false, false, false, false, true, false)
                                            end
                                        end
                                    end
                                else
                                    --Get threat of hover/amphibious units
                                    local toUnitsToInclude = EntityCategoryFilterDown(M28UnitInfo.refCategoryAmphibiousCombat, tAdjLZTeamData[M28Map.subrefLZTAlliedCombatUnits])
                                    --iNearbySameTargetAdjacentSubmersibleAntiNavyThreat = iNearbySameTargetAdjacentSubmersibleAntiNavyThreat + M28UnitInfo.GetCombatThreatRating(toUnitsToInclude, false, false, false, false, false, true, false)
                                    iNearbySameTargetAdjacentNonSubmersibleThreat = iNearbySameTargetAdjacentNonSubmersibleThreat + M28UnitInfo.GetCombatThreatRating(toUnitsToInclude, false, false, false, false, true, false)
                                end
                                if bDebugMessages == true then LOG(sFunctionRef..': Including threat of friendly amphbiious and hover units in iOtherLZ='..iOtherLZ..', iNearbySameTargetAdjacentSubmersibleAntiNavyThreat after this='..iNearbySameTargetAdjacentSubmersibleAntiNavyThreat..'; iNearbySameTargetAdjacentNonSubmersibleThreat='..iNearbySameTargetAdjacentNonSubmersibleThreat) end
                            end
                        end
                    end
                end
            ...
            --Submersible threats:
            if bGetSubmersibleThreat and tLikelyTargetWZTeamData then
                local tbAdjZoneUnitsInAvailableCombatUnits
                if M28Utilities.IsTableEmpty(tLikelyTargetWZTeamData[M28Map.subreftiWaterZonesTargetingThisWithSubmersibleNavy]) == false and M28Utilities.IsTableEmpty(tWZData[M28Map.subrefWZAdjacentWaterZones]) == false then
                    local tbZonesAdjacentToThis = {}
                    for _, iAdjWZ in tWZData[M28Map.subrefWZAdjacentWaterZones] do
                        tbZonesAdjacentToThis[iAdjWZ] = true
                    end
                    if bDebugMessages == true then LOG(sFunctionRef..': Looking at adjacent WZs, tWZData[M28Map.subrefWZAdjacentWaterZones]='..repru(tWZData[M28Map.subrefWZAdjacentWaterZones])..'; tbZonesAdjacentToThis='..repru(tbZonesAdjacentToThis)) end
                    local bIncludeCurZoneDF = false
                    for iOtherWZ, iAttackingType in tLikelyTargetWZTeamData[M28Map.subreftiWaterZonesTargetingThisWithSubmersibleNavy] do
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering iOtherWZ='..iOtherWZ..'; tbZonesAdjacentToThis[iOtherWZ]='..tostring(tbZonesAdjacentToThis[iOtherWZ] or false)..'; iLikelyTargetZone='..iLikelyTargetWaterZone) end
                        if tbZonesAdjacentToThis[iOtherWZ] and not(iLikelyTargetWaterZone == iOtherWZ) then
                            local tAdjWZTeamData = M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][iOtherWZ][M28Map.subrefWZTeamData][iTeam]
                            if bDebugMessages == true then LOG(sFunctionRef..': Considering submersibles, subrefWZThreatAlliedSubmersible for otherWZ='..(tAdjWZTeamData[M28Map.subrefWZThreatAlliedSubmersible] or 'nil')..'; OtherWZ subrefWZTValue='..(tAdjWZTeamData[M28Map.subrefWZTValue] or 'nil')..'; ThisWaterZone subrefWZTValue='..(tWZTeamData[M28Map.subrefWZTValue] or 'nil')..'; is subreftoLZOrWZAlliedUnits empty='..tostring(M28Utilities.IsTableEmpty(tAdjWZTeamData[M28Map.subreftoLZOrWZAlliedUnits]))) end
                            if tAdjWZTeamData[M28Map.subrefWZThreatAlliedSubmersible] > 0 and M28Utilities.IsTableEmpty(tAdjWZTeamData[M28Map.subreftoLZOrWZAlliedUnits]) == false then
                                --Include this zone unless we think we are giving orders to its units in availablecombatunits
                                bIncludeCurZoneDF = true
                                if tAdjWZTeamData[M28Map.subrefWZTValue] < tWZTeamData[M28Map.subrefWZTValue] then
                                    --Higher risk we may be giving orders to units in the adjacent zone
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': bIncludeCurZoneDF='..tostring(bIncludeCurZoneDF or false)) end
                                if bIncludeCurZoneDF then
                                    if bDebugMessages == true then LOG(sFunctionRef..': iAttackingType='..iAttackingType..'; subrefiLZOrWZTMovingToOtherZone='..M28Map.subrefiLZOrWZTMovingToOtherZone) end
                                    if iAttackingType == M28Map.subrefiLZOrWZTMovingToOtherZone then
                                        --We might have units that are far away, so only include if they are relatively close to our available units, and arent skirmishers
                                        local toSubmersibleUnitsToInclude = {}
                                        if (tAdjWZTeamData[M28Map.subrefWZThreatAlliedSubmersible] or 0) > 0 and M28Utilities.IsTableEmpty(tAdjWZTeamData[M28Map.subreftoLZOrWZAlliedUnits]) == false then
                                            local iClosestFriendlyToOtherZoneDist = 10000
                                            local oClosestFriendlyToOtherZone
                                            local tOtherZoneMidpoint = M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][iOtherWZ][M28Map.subrefMidpoint]
                                            local iCurDist
                                            ...
                                            if bDebugMessages == true then LOG(sFunctionRef..': Will check for units close to our sub that are in the other zone, oClosestFriendlyToOtherZone='..(oClosestFriendlyToOtherZone.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oClosestFriendlyToOtherZone) or 'nil')) end
                                            if oClosestFriendlyToOtherZone then
                                                for iUnit, oUnit in tAdjWZTeamData[M28Map.subreftoLZOrWZAlliedUnits] do
                                                    if not(oUnit.Dead) then
                                                        --Are we within 70 of being in range? then include
                                                        if bDebugMessages == true then LOG(sFunctionRef..': Dist between units='..M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oClosestFriendlyToOtherZone:GetPosition())..'; CombatRange='..oUnit[M28UnitInfo.refiCombatRange]..'; Dist until in range='..M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oClosestFriendlyToOtherZone:GetPosition()) - oUnit[M28UnitInfo.refiCombatRange]) end
                                                        if M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oClosestFriendlyToOtherZone:GetPosition()) - oUnit[M28UnitInfo.refiCombatRange] <= 30 then
                                                            if (oUnit[M28UnitInfo.refiAntiNavyRange] or 0) > 0 and EntityCategoryContains(M28UnitInfo.refCategorySubmarine + M28UnitInfo.refCategoryAmphibious, oUnit.UnitId) then
                                                                table.insert(toSubmersibleUnitsToInclude, oUnit)
                                                            end
                                                        end
                                                    end
                                                end
                                                if M28Utilities.IsTableEmpty(toSubmersibleUnitsToInclude) == false then
                                                    if bDebugMessages == true then LOG(sFunctionRef..': Combat threat of units that are including='..M28UnitInfo.GetCombatThreatRating(toSubmersibleUnitsToInclude, false)) end
                                                    iNearbySameTargetAdjacentSubmersibleAntiNavyThreat = iNearbySameTargetAdjacentSubmersibleAntiNavyThreat +  M28UnitInfo.GetCombatThreatRating(toSubmersibleUnitsToInclude, false, false, false, false, false, true, false)
                                                end
                                            end
                                        end
                                    else
                                        if bDebugMessages == true then LOG(sFunctionRef..': Will include threat of units in the other zone into iNearbySameTargetAdjacentSubmersibleAntiNavyThreat, iNearbySameTargetAdjacentSubmersibleAntiNavyThreat before including='..iNearbySameTargetAdjacentSubmersibleAntiNavyThreat..'; Threat of adj zone units='..M28UnitInfo.GetCombatThreatRating(tAdjWZTeamData[M28Map.subreftoLZOrWZAlliedUnits], false, false, false, false, false, true, false)) end
                                        --                                                                                    GetCombatThreatRating(tUnits,                                             bEnemyUnits, bJustGetMassValue, bIndirectFireThreatOnly, bAntiNavyOnly, bAddAntiNavy, bSubmersibleOnly, bLongRangeThreatOnly, bBlueprintThreat, bSurfaceThreatOnly)
                                        iNearbySameTargetAdjacentSubmersibleAntiNavyThreat = iNearbySameTargetAdjacentSubmersibleAntiNavyThreat + M28UnitInfo.GetCombatThreatRating(tAdjWZTeamData[M28Map.subreftoLZOrWZAlliedUnits], false, false,              false,                 false,          false,          true,           false)
                                    end
                                    if bDebugMessages == true then LOG(sFunctionRef..': Including threat of friendly naval units in iOtherWZ='..iOtherWZ..', iNearbySameTargetAdjacentSubmersibleAntiNavyThreat after this='..iNearbySameTargetAdjacentSubmersibleAntiNavyThreat..'; iNearbySameTargetAdjacentNonSubmersibleThreat='..iNearbySameTargetAdjacentNonSubmersibleThreat) end
                                end
                            end
                        end
                    end
                end
                --Include submersible threat of any units already in the target zone as well
                if tLikelyTargetWZTeamData[M28Map.subrefWZThreatAlliedSubmersible] > 0 and not(iLikelyTargetWaterZone == iWaterZone) then
                    if not(tbAdjZoneUnitsInAvailableCombatUnits) then
                        tbAdjZoneUnitsInAvailableCombatUnits = {}
                        if M28Utilities.IsTableEmpty(tAvailableSubmarines) == false then
                            for iUnit, oUnit in tAvailableSubmarines do
                    ...
                    if not(tbAdjZoneUnitsInAvailableCombatUnits[iLikelyTargetWaterZone]) then
                        iNearbySameTargetAdjacentSubmersibleAntiNavyThreat = iNearbySameTargetAdjacentSubmersibleAntiNavyThreat + (tLikelyTargetWZTeamData[M28Map.subrefWZThreatAlliedSubmersible] or 0)

                        if bDebugMessages == true then LOG(sFunctionRef..': Including threat of friendly Naval units in iLikelyTargetWaterZone, iNearbySameTargetAdjacentSubmersibleAntiNavyThreat after this='..iNearbySameTargetAdjacentSubmersibleAntiNavyThreat) end
                    end
                end
                if bDebugMessages == true then LOG(sFunctionRef..': iNearbySameTargetAdjacentSubmersibleAntiNavyThreat after including adj zones with the same targetWZ as us='..iNearbySameTargetAdjacentSubmersibleAntiNavyThreat..'; iNearbySameTargetAdjacentNonSubmersibleThreat='..iNearbySameTargetAdjacentNonSubmersibleThreat) end
            end
            if bDebugMessages == true then LOG(sFunctionRef..': Finished calculating, bGetSurfaceAndLandThreat='..tostring(bGetSurfaceAndLandThreat)..'; bGetSubmersibleThreat='..tostring(bGetSubmersibleThreat)..'; iNearbySameTargetAdjacentSubmersibleAntiNavyThreat after including adj zones with the same targetWZ as us='..iNearbySameTargetAdjacentSubmersibleAntiNavyThreat..'; iNearbySameTargetAdjacentNonSubmersibleThreat='..iNearbySameTargetAdjacentNonSubmersibleThreat) end
            if bGetSubmersibleThreat and bGetSurfaceAndLandThreat then
                return iNearbySameTargetAdjacentSubmersibleAntiNavyThreat, iNearbySameTargetAdjacentNonSubmersibleThreat
            elseif bGetSubmersibleThreat then
                return iNearbySameTargetAdjacentSubmersibleAntiNavyThreat
            else return iNearbySameTargetAdjacentNonSubmersibleThreat
            end
        end
        ...
        if M28Utilities.IsTableEmpty(tAvailableSubmarines) == false then
            ...
            --Re-run basedo n frontsub target, as front sub may not actually be front sub if we are targeting something in a different direction to the enemy base - will only do this if front sub isnt in combat and is relatively far from its destination
            local tFrontSubCurTarget = oFrontSub[M28Orders.reftiLastOrders][1][M28Orders.subreftOrderPosition]
            if not(tFrontSubCurTarget) and oFrontSub[M28Orders.reftiLastOrders][1][M28Orders.subrefoOrderUnitTarget] and not(oFrontSub[M28Orders.reftiLastOrders][1][M28Orders.subrefoOrderUnitTarget].Dead) then tFrontSubCurTarget = oFrontSub[M28Orders.reftiLastOrders][1][M28Orders.subrefoOrderUnitTarget]:GetPosition() end
            if M28Utilities.IsTableEmpty(tFrontSubCurTarget) == false then
                if bDebugMessages == true then LOG(sFunctionRef..': Checking if want to consider a different front sub, oFrontSub before check='..oFrontSub.UnitId..M28UnitInfo.GetUnitLifetimeCount(oFrontSub)..'; Angle to tFrontSubCurTarget from front sub='..M28Utilities.GetAngleFromAToB(oFrontSub:GetPosition(), tFrontSubCurTarget)..'; Angle to enemy base from front sub='..M28Utilities.GetAngleFromAToB(oFrontSub:GetPosition(), tWZTeamData[M28Map.reftClosestEnemyBase])..'; Dist to front sub target='..M28Utilities.GetDistanceBetweenPositions(oFrontSub:GetPosition(), tFrontSubCurTarget)..'; Time since last fired weapon='..GetGameTimeSeconds() - (oFrontSub[M28UnitInfo.refiLastWeaponEvent] or 0)) end
                if GetGameTimeSeconds() - (oFrontSub[M28UnitInfo.refiLastWeaponEvent] or 0) >= 12 and M28Utilities.GetDistanceBetweenPositions(oFrontSub:GetPosition(), tFrontSubCurTarget) >= 10 and M28Utilities.GetAngleDifference(M28Utilities.GetAngleFromAToB(oFrontSub:GetPosition(), tFrontSubCurTarget), M28Utilities.GetAngleFromAToB(oFrontSub:GetPosition(), tWZTeamData[M28Map.reftClosestEnemyBase])) >= 45 then
                    local iClosestSubToTargetDist = M28Utilities.GetDistanceBetweenPositions(oFrontSub:GetPosition(), tFrontSubCurTarget)
                    for iUnit, oUnit in tAvailableSubmarines do
                        iCurSubDist = M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), tFrontSubCurTarget)
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering if other subs are closer to front sub target, oUnit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; iCurSubDist='..iCurSubDist) end
                        if iCurSubDist < iClosestSubToTargetDist then
                            oFrontSub = oUnit
                            iClosestSubToTargetDist = iCurSubDist
                        end
                    end
                    if bDebugMessages == true then LOG(sFunctionRef..': oFrontSub after check='..oFrontSub.UnitId..M28UnitInfo.GetUnitLifetimeCount(oFrontSub)) end
                end
            end
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Considering whether to make far away subs move to be close to the front sub, oFrontSub='..oFrontSub.UnitId..M28UnitInfo.GetUnitLifetimeCount(oFrontSub)) end
        ...
        elseif (iBestAvailableSubmarineRange >= 80 or tWZTeamData[M28Map.subrefWZBestAlliedDFRange] >= 80) and tWZTeamData[M28Map.subrefWZTThreatAllyCombatTotal] >= 2000 then
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Have long ranged untis so considering enemies in doubly adjacent zones') end
        end
        if bDebugMessages == true then LOG(sFunctionRef..': oNearestEnemyToFriendlyBase after checking nearby zones='..(oNearestEnemyToFriendlyBase.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oNearestEnemyToFriendlyBase) or 'nil')..' for iWaterZone='..iWaterZone) end
        ...
        if not(bCalcAntiNavyThreatNormally) then
            ...
            if M28Utilities.IsTableEmpty(tTargetZoneWZTeamData[M28Map.subrefWZTAlliedSurfaceCombatUnits]) == false then
        ...
        if oNearestEnemyNonHoverToFriendlyBase and M28Utilities.IsTableEmpty(tAvailableSubmarines) == false then
            ...
            if bAttackAndKite then
                ...
                tWZTeamData[M28Map.subrefWZbSurfaceInScenario2PrevCycle] = false --updating for next cycle
                ...
                if bDebugMessages == true then LOG(sFunctionRef..': Deciding if we want to attack enemy even if outranged, iAdjacentAlliedSubmersibleThreat='..iAdjacentAlliedSubmersibleThreat..'; iAdjacentEnemyAntiNavyThreat='..iAdjacentEnemyAntiNavyThreat..'; iAdjacentAlliedCombatThreat='..iAdjacentAlliedCombatThreat..'; iAdjacentEnemyCombatThreat='..iAdjacentEnemyCombatThreat..'; iModForEnemyScenario2Threat='..iModForEnemyScenario2Threat..'; Want to attack='..tostring(M28Conditions.WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat, iAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, true                             ,iModForEnemyScenario2Threat ))..'; tWZTeamData[M28Map.subrefWZiSuicideIntoEnemyCombatThreat]='..(tWZTeamData[M28Map.subrefWZiSuicideIntoEnemyCombatThreat] or 'nil')) end
                local iNearbyAdjZoneCommonTargetSubmersibleThreat = 0
                if oNearestEnemyNonHoverToFriendlyBase[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] then iNearbyAdjZoneCommonTargetSubmersibleThreat = GetAdjacentNearbyThreatsWithSameZoneTarget(oNearestEnemyNonHoverToFriendlyBase[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam], false, true) end
                --WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat,                                                                                                        iCurZoneAndAdjacentAlliedSubmersibleThreat,                    iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, bConsideringSubmarinesNotSurface, iOptionalThreatAbsolutePercentIncrease, iEnemyNearbySubmersibleThreat, iAvailableAntiNavyThreat, iCommonZoneTargetSurfaceThreat, bNearbyEnemiesAreHostileCivilians)
                tWZTeamData[M28Map.subrefWZbSubsInScenario2] = M28Conditions.WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat + (tWZTeamData[M28Map.subrefWZiSuicideIntoEnemyCombatThreat] or 0) + iNearbyAdjZoneCommonTargetSubmersibleThreat, iAdjacentAlliedSubmersibleThreat - iNearbyAdjZoneCommonTargetSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, true                             ,iModForEnemyScenario2Threat,                nil,                            nil,                    nil,                            (oNearestEnemyNonHoverToFriendlyBase[M28UnitInfo.refbHostileImmobileCivilian] or oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.refbHostileImmobileCivilian]))
                ...
                if tWZTeamData[M28Map.subrefWZbSubsInScenario2] then
                    tWZTeamData[M28Map.subrefWZbSubsPreviouslyInScenario2] = true
                elseif tWZTeamData[M28Map.subrefWZbSubsPreviouslyInScenario2] then
                    --If we were in scenario 2 last cycle remain in it for 1 more cycle in case units have just moved to an adjacent WZ that will target the same location
                    tWZTeamData[M28Map.subrefWZbSubsInScenario2] = true
                    tWZTeamData[M28Map.subrefWZbSubsPreviouslyInScenario2] = false
                    if bDebugMessages == true then LOG(sFunctionRef..': WIll remain in scenario 2 for subs for 1 more cycle in case some units left this zone for an adjacentzone that will attack the same target') end
                else
                    if bDebugMessages == true then LOG(sFunctionRef..': Subs Not in scenario 2 and werent in previous cycle either') end
                end
                if tWZTeamData[M28Map.subrefWZbSubsInScenario2] then
                    --SCENARIO 2 - We are either near our core naval factory or we have a greater threat than the enemy - attack
                    if oNearestEnemyNonHoverToFriendlyBase[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] then RecordWaterZoneTarget(tWZTeamData, iWaterZone, iTeam, true, oNearestEnemyNonHoverToFriendlyBase[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam], M28Map.subrefiLZOrWZTAttackingUnit) end
                    ...
                    if bDebugMessages == true then LOG(sFunctionRef..': Dont have enough threat to attack, will see if we want to consolidate in this zone, iAvailableSubmersibleThreat='..iAvailableSubmersibleThreat..'; iAdjacentAlliedSubmersibleThreat='..iAdjacentAlliedSubmersibleThreat..'; Want to attack with navy='..tostring(M28Conditions.WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat, iAvailableSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, true, iModForEnemyScenario2Threat ))) end
                    --WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat, iCurZoneAndAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, bConsideringSubmarinesNotSurface, iOptionalThreatAbsolutePercentIncrease, iEnemyNearbySubmersibleThreat, iAvailableAntiNavyThreat, iCommonZoneTargetSurfaceThreat, bNearbyEnemiesAreHostileCivilians)
                    if iAvailableSubmersibleThreat > iAdjacentAlliedSubmersibleThreat * 1.1 and iAvailableSubmersibleThreat > (tWZTeamData[M28Map.subrefWZThreatEnemyAntiNavy] or 0) * 2 and M28Conditions.WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat, iAvailableSubmersibleThreat,              iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, true,                                iModForEnemyScenario2Threat             , nil                           , nil                   , nil                           , (oNearestEnemyNonHoverToFriendlyBase[M28UnitInfo.refbHostileImmobileCivilian] or oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.refbHostileImmobileCivilian]) ) then
                        sMessage = 'WSConsR'
                    ...
                    for iUnit, oUnit in tAvailableSubmarines do
                        --Only retreat units from this WZ
                        if oUnit[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] == iWaterZone then
                            if bDebugMessages == true then LOG(sFunctionRef..': Cycling through tavailablesubmarines, will retreat oUnit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)) end
                            oUnit[M28UnitInfo.refiTimeLastTriedRetreating] = iCurTime
        ...
        if bDebugMessages == true then LOG(sFunctionRef..': Considering nonsub combat units, oNearestEnemyToFriendlyBase='..(oNearestEnemyToFriendlyBase.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oNearestEnemyToFriendlyBase) or 'nil')..'; Is table of available combat units empty='..tostring(M28Utilities.IsTableEmpty(tAvailableCombatUnits))..'; oNearestEnemySurfaceToFriendlyBase='..(oNearestEnemySurfaceToFriendlyBase.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oNearestEnemySurfaceToFriendlyBase) or 'nil')..'; iWaterZone='..iWaterZone) end
        if oNearestEnemyToFriendlyBase and M28Utilities.IsTableEmpty(tAvailableCombatUnits) == false then
            ...
            function KitingRetreatOfUnit(oUnit)
                --Enemy has DF units and they are already in our range so retreat
                if bDebugMessages == true then LOG(sFunctionRef..': KitingRetreatOfUnit: Are close to enemies, Unit combat range='..oUnit[M28UnitInfo.refiCombatRange]..'; Unit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; oEnemyToFocusOn='..(oEnemyToFocusOn.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oEnemyToFocusOn) or 'nil')) end
                ...
                if tWZTeamData[M28Map.subrefWZbCoreBase] and GetGameTimeSeconds() - 2 - (oUnit[M28UnitInfo.refiLastWeaponEvent] or -100) > oUnit[M28UnitInfo.refiTimeBetweenDFShots] and M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), tRallyPoint) <= 16 then
                    if bDebugMessages == true then LOG(sFunctionRef..': oUnit is at position '..repru(oUnit:GetPosition())..'; oEnemyToFocusOn is at position '..repru(oEnemyToFocusOn:GetPosition())..'; IsDestroyed='..tostring(IsDestroyed(oEnemyToFocusOn))..'; .Dead='..tostring(oEnemyToFocusOn.Dead or false)) end
                    ...
                    if M28UnitInfo.IsUnitValid(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]) then
                        ...
                        if M28Utilities.GetAngleDifference(iAngleToNearestEnemy, iAngleToRally) <= 80 or M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), tRallyPoint) <= 9 then
                            tRetreatLocation = M28Utilities.MoveInDirection(oUnit:GetPosition(), iAngleToNearestEnemy + 180, 10, true, false, false)
                            if tRetreatLocation then
                                local iKitingSegmentX, iKitingSegmentZ = M28Map.GetPathingSegmentFromPosition(tRetreatLocation)
                                if M28Map.tPondBySegment[iKitingSegmentX][iKitingSegmentZ] == iPond and (not(M28Map.bIsCampaignMap) or M28Conditions.IsLocationInPlayableArea(tRetreatLocation)) then
                    ...
                    if oUnit[M28UnitInfo.refbAttackMoveInsteadOfKiting] then
                        ...
                        --Attackmove if almost at rally point so arent stuck doing nothing while taking fire
                    elseif tRallyPoint and M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), tRallyPoint) <= 12 then
                        oUnit[M28UnitInfo.refiTimeLastTriedRetreating] = iCurTime
                        M28Orders.IssueTrackedAttackMove(oUnit, (tRetreatLocation or tRallyPoint), 3, false, 'NKAMRet'..iWaterZone, false)
                        if bDebugMessages == true then LOG(sFunctionRef..': Almost at retreat location so will attack move') end
                    else
                        oUnit[M28UnitInfo.refiTimeLastTriedRetreating] = iCurTime
            ...
            if oNearestEnemyToFriendlyBase == oNearestEnemySurfaceToFriendlyBase and (tWZTeamData[M28Map.subrefiNearbyEnemyLongRangeDFThreat] or 0) == 0 and tWZTeamData[M28Map.subrefWZBestAlliedDFRange] > math.max((oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.refiDFRange] or 0), oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.refiAntiNavyRange] or 0) + 6 then
                ...
                if M28Utilities.IsTableEmpty(tNearestEnemyWZTeamData[M28Map.reftoNearestCombatEnemies]) == false then
                    for iEnemy, oEnemy in tNearestEnemyWZTeamData[M28Map.reftoNearestCombatEnemies] do
                        ...
                        if (oEnemy[M28UnitInfo.refiDFRange] or 0) > iEnemyRangeThreshold or (oEnemy[M28UnitInfo.refiAntiNavyRange] or 0) > iEnemyRangeThreshold then
                ...
                if iClosestDistLessRange * -1 + 9 > iEnemyRangeThreshold then
                    iFriendlyDFRangeThresholdBasedOnAbove = iClosestDistLessRange * -1 + 9
                ...
                elseif not(bAreInScenario1) and iFriendlyDFRangeThresholdBasedOnAbove + 5 < iEnemyBestRange and iFriendlyDFRangeThresholdBasedOnAbove <= tWZTeamData[M28Map.subrefWZBestAlliedDFRange] then
                    iFriendlyDFScenario1RangeThresholdOverride = iFriendlyDFRangeThresholdBasedOnAbove
                    bAreInScenario1 = true
                ...
                if bDebugMessages == true then
                    LOG(sFunctionRef..': Finished checking if we think closest enemy unit is vulnerable to a kiting attack, bAreInScenario1='..tostring(bAreInScenario1 or false)..'; iFriendlyDFScenario1RangeThresholdOverride='..(iFriendlyDFScenario1RangeThresholdOverride or 'nil'))
                    if M28UnitInfo.IsUnitValid(oNearestEnemyToFriendlyBase) then LOG(sFunctionRef..': nearest enemy unit state='..M28UnitInfo.GetUnitState(oNearestEnemyToFriendlyBase)) end
                end
            end
            if bAreInScenario1 then
                ...
                for iUnit, oUnit in tCombatUnitsOfUse do
                    if not(oUnit.Dead) then --rare redundancy
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering if our unit '..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..' outrangs enemy, iRangeToUseForChecks='..iRangeToUseForChecks..'; iEnemyBestAntiNavyRange='..iEnemyBestAntiNavyRange..'; iEnemyBestUnderwaterRange='..iEnemyBestUnderwaterRange..'; refbLastShotBlocked='..tostring(oUnit[M28UnitInfo.refbLastShotBlocked] or false)..'; Time of last blocked shot='..(oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or 'nil')) end

                        if iFriendlyDFScenario1RangeThresholdOverride and iRangeToUseForChecks >= iFriendlyDFScenario1RangeThresholdOverride and (EntityCategoryContains(categories.HOVER, oUnit.UnitId) or iRangeToUseForChecks >= math.max((oEnemyToFocusOn[M28UnitInfo.refiDFRange] or 0), oEnemyToFocusOn[M28UnitInfo.refiAntiNavyRange] or 0)) or (iRangeToUseForChecks > (iEnemyAntiNavyOverride or iEnemyBestUnderwaterRange) and (not(iFriendlyDFScenario1RangeThresholdOverride) or iRangeToUseForChecks >= iEnemyBestRange)) then
                            table.insert(tUnitsToSupport, oUnit)
                            --Consider kiting logic unless want to use shot blocked override logic
                            if bMoveBlockedNotAttackMove and oUnit[M28UnitInfo.refbLastShotBlocked] and (GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or -100)) >= 10 and GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastCheck] or -100) < 6 then
                                if bDebugMessages == true then LOG(sFunctionRef..': Blocked shot, oEnemyToFocusOn='..oEnemyToFocusOn.UnitId..M28UnitInfo.GetUnitLifetimeCount(oEnemyToFocusOn)..'; Blocked count='..(oEnemyToFocusOn[M28UnitInfo.refiTargetShotBlockedCount] or 0)) end
                                ...
                                if oUnit[M28UnitInfo.refbEasyBrain] and oEnemyToFocusOn then
                                    ...
                                    --                                                                                                                                                                                          CloseToEnemyUnit(tStartPosition, tUnitsToCheck,               iDistThreshold,                                   iTeam, bIncludeEnemyDFRange, iAltThresholdToDFRange, oUnitIfConsideringAngleAndLastShot, oOptionalFriendlyUnitToRecordClosestEnemy, iOptionalDistThresholdForStructure, bIncludeEnemyAntiNavyRange)
                                elseif M28Utilities.IsTableEmpty(tWZTeamData[M28Map.reftoNearestCombatEnemies]) or (bMoveAntiNavyForwardsAsCantSee and (oUnit[M28UnitInfo.refiAntiNavyRange] or 0) > 0) or
                                        (not (M28Conditions.CloseToEnemyUnit(oUnit:GetPosition(), tEnemiesToConsider, iRangeToUseForChecks - math.min(4, iRangeToUseForChecks * 0.1), iTeam, false, nil, nil, oUnit))
                                                --Tried below for battleship duel but didnt seem to help so just commented out
                                                --and ((iRangeToUseForChecks <= 80 or oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.refiDFRange] or 0) + 30 <= iRangeToUseForChecks or oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck].Dead or not(M28Conditions.CloseToEnemyUnit(oUnit:GetPosition(), { oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck] }, iRangeToUseForChecks - math.min(4, iRangeToUseForChecks * 0.1) + 1, iTeam,    true,                  iRangeToUseForChecks - math.min(4, iRangeToUseForChecks * 0.1) + 1,                    oUnit,                                oUnit, nil, true)))
                                        ) then

                                    if bDebugMessages == true then
                                        LOG(sFunctionRef..': Not in range of enemy yet (or not far enough in range), and we outrange enemy; oUnit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; Antinavy range='..(oUnit[M28UnitInfo.refiAntiNavyRange] or 'nil')..'; DF range='..(oUnit[M28UnitInfo.refiDFRange] or 'nil')..'; oEnemyToFocusOn='..(oEnemyToFocusOn.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oEnemyToFocusOn) or 'nil')..'; oEnemyToFocusOn antinavy range='..(oEnemyToFocusOn[M28UnitInfo.refiAntiNavyRange] or 'nil')..'; Distance to the nearest enemy to midpoint='..M28Utilities.GetDistanceBetweenPositions(oEnemyToFocusOn[M28UnitInfo.reftLastKnownPositionByTeam][iTeam], oUnit:GetPosition())..'; Enemy unit actual position='..repru(oEnemyToFocusOn:GetPosition())..'; Enemy last recorded position='..repru(oEnemyToFocusOn[M28UnitInfo.reftLastKnownPositionByTeam][iTeam])..'; Our unit position='..repru(oUnit:GetPosition())..'; WZ midpoint position='..repru(tWZData[M28Map.subrefMidpoint])..'; bMoveAntiNavyForwardsAsCantSee='..tostring(bMoveAntiNavyForwardsAsCantSee or false)..'; M28Utilities.IsTableEmpty(tWZTeamData[M28Map.reftoNearestCombatEnemies])='..tostring(M28Utilities.IsTableEmpty(tWZTeamData[M28Map.reftoNearestCombatEnemies]))..'; iRangeToUseForChecks='..iRangeToUseForChecks)
                                    ...
                                    if oUnit[M28UnitInfo.refiCombatRange] >= 100 and not(oEnemyToFocusOn.Dead) then
                                        if M28UnitInfo.CanSeeUnit(oUnit:GetAIBrain(), oEnemyToFocusOn) and M28Utilities.GetDistanceBetweenPositions(oEnemyToFocusOn:GetPosition(), oUnit:GetPosition()) <= oUnit[M28UnitInfo.refiCombatRange] - 6 then --dont want to chase after enemy necessarily
                                            ...
                                            if M28Utilities.IsTableEmpty(tNearbyUnitsOfInterest) == false then
                                                    ...
                                                    if M28Utilities.IsTableEmpty(tNearbyT2ArtiAndExperimentalStructure) == false then oEnemyToTarget = M28Utilities.GetNearestUnit(tNearbyT2ArtiAndExperimentalStructure, oUnit:GetPosition())
                                                        ...
                                                        for iEnemyTarget, oEnemyTarget in tNearbyUnitsOfInterest do
                                                            ...
                                                            if oEnemyToTarget then
                                                                if bDebugMessages == true then LOG(sFunctionRef..': Have an enemy in our range so will do manual attack order, oEnemyToTarget='..oEnemyToTarget.UnitId..M28UnitInfo.GetUnitLifetimeCount(oEnemyToTarget)) end
                                                                M28Orders.IssueTrackedAttack(oUnit, oEnemyToTarget, false, 'LRHVEn', false)
                                                            else
                                                                if bDebugMessages == true then LOG(sFunctionRef..': No enemy to do manual attack order, so will attackmove to oEnemyToFocusOn, IgnoreOrderDueToStuckUnit='..tostring(IgnoreOrderDueToStuckUnit(oUnit) or false)) end
                                                                if not(IgnoreOrderDueToStuckUnit(oUnit)) then
                                                ...
                                                if bDebugMessages == true then LOG(sFunctionRef..': Will attack move to oEnemyToFocusOn unless we are stuck, IgnoreOrderDueToStuckUnit='..tostring(IgnoreOrderDueToStuckUnit(oUnit) or false)) end
                                                if not(IgnoreOrderDueToStuckUnit(oUnit)) then
                                        ...
                                        if bDebugMessages == true then LOG(sFunctionRef..': Will do an attack move to oEnemyToFocusOn unless stuck, IgnoreOrderDueToStuckUnit='..tostring(IgnoreOrderDueToStuckUnit(oUnit) or false)) end
                                        if not(IgnoreOrderDueToStuckUnit(oUnit)) then
                                    ...
                                    if bDebugMessages == true then LOG(sFunctionRef..': Will do a kiting retreat of this unit, refoClosestEnemyFromLastCloseToEnemyUnitCheck='..(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck].UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]) or 'nil')..' with DF range='..(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.refiDFRange] or 'nil')) end
                        ...
                        if not(tWZTeamData[M28Map.subrefWZbSubsInScenario2]) then
                            for _, iAdjWZ in tWZData[M28Map.subrefWZAdjacentWaterZones] do
                                ...
                                if tAdjWZTeamData[M28Map.subrefWZbSubsInScenario2] then
                                    ...
                                    if (not(oNearestEnemyNonHoverToFriendlyBase) and tAdjWZTeamData[M28Map.subrefWZThreatEnemySubmersible] > 0) or (oNearestEnemyNonHoverToFriendlyBase and not(oNearestEnemyNonHoverToFriendlyBase.Dead) and M28Utilities.GetDistanceBetweenPositions(oNearestEnemyNonHoverToFriendlyBase:GetPosition(), M28Map.tPondDetails[M28Map.tiPondByWaterZone[iAdjWZ]][M28Map.subrefPondWaterZones][iAdjWZ][M28Map.subrefMidpoint]) < M28Utilities.GetDistanceBetweenPositions(tWZData[M28Map.subrefMidpoint], M28Map.tPondDetails[M28Map.tiPondByWaterZone[iAdjWZ]][M28Map.subrefPondWaterZones][iAdjWZ][M28Map.subrefMidpoint])) then
                                        if (not(oNearestEnemyNonHoverToFriendlyBase) or oNearestEnemyNonHoverToFriendlyBase.Dead) and M28Utilities.IsTableEmpty(tAdjWZTeamData[M28Map.subrefTEnemyUnits]) == false then
                                            local iCurDist
                                        ...
                                        if oNearestEnemyNonHoverToFriendlyBase and not(oNearestEnemyNonHoverToFriendlyBase.Dead) then
            ...
            if bAreInScenario1 then
                tWZTeamData[M28Map.subrefWZbSurfaceInScenario2PrevCycle] = false
            else
                if M28Utilities.IsTableEmpty(tCombatUnitsOfUse) == false then
                    ...
                    local iAvailableCombatThreat
                    local iCommonZoneTargetFriendlySurfaceThreat = 0
                    if oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] then iCommonZoneTargetFriendlySurfaceThreat = GetAdjacentNearbyThreatsWithSameZoneTarget(oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam], true, false) end
                    --WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat, iCurZoneAndAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat,                                                                iAdjacentEnemyCombatThreat, bConsideringSubmarinesNotSurface, iOptionalThreatAbsolutePercentIncrease, iEnemyNearbySubmersibleThreat, iAvailableAntiNavyThreat,                                                         iCommonZoneTargetSurfaceThreat, bNearbyEnemiesAreHostileCivilians)
                    bAreInScenario2 = M28Conditions.WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat, iAdjacentAlliedSubmersibleThreat,            iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat + (tWZTeamData[M28Map.subrefWZiSuicideIntoEnemyCombatThreat] or 0), iAdjacentEnemyCombatThreat, false                               ,iModForEnemyScenario2Threat, iEnemyNearbySubmersibleThreat, iOurAntiNavyThreat + (tWZTeamData[M28Map.subrefWZiSuicideIntoEnemyCombatThreat] or 0), iCommonZoneTargetFriendlySurfaceThreat, (oNearestEnemyNonHoverToFriendlyBase[M28UnitInfo.refbHostileImmobileCivilian] or oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.refbHostileImmobileCivilian]))
                    if bDebugMessages == true then LOG(sFunctionRef..': iCommonZoneTargetFriendlySurfaceThreat='..iCommonZoneTargetFriendlySurfaceThreat..'; bAreInScenario2 from initial check='..tostring(bAreInScenario2)) end
                    --If our available combat threat isnt that much then switch to not attacking
                    if bAreInScenario2 or tWZTeamData[M28Map.subrefWZbSurfaceInScenario2PrevCycle] then
                        iAvailableCombatThreat = M28UnitInfo.GetCombatThreatRating(tCombatUnitsOfUse, false,          false,          false,                  false,          true,           false,          false)
                        if iAvailableCombatThreat + (tWZTeamData[M28Map.subrefWZiSuicideIntoEnemyCombatThreat] or 0) + iCommonZoneTargetFriendlySurfaceThreat < iAdjacentEnemyCombatThreat then
                            bAreInScenario2 = false
                            if bDebugMessages == true then LOG(sFunctionRef..': Our combat threat available is less than nearby enemy, and we dont outrange enemy, so wont attack afterall') end
                        end
                    end
                    if bAreInScenario2 and iEnemyBestCombatRange >= 45 and M28Utilities.IsTableEmpty(tWZTeamData[M28Map.reftoNearestCombatEnemies]) == false then
                        ...
                        if M28Utilities.IsTableEmpty(toEnemiesFromAdjacentLandZone) == false then
                            ...
                            if iAdjacentLandThreat > 100 and iAdjacentLandThreat > iAdjacentEnemyCombatThreat * 0.1 then
                                ...
                                bAreInScenario2 = M28Conditions.WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat, iAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, false                               ,iModForEnemyScenario2Threat, iEnemyNearbySubmersibleThreat, iOurAntiNavyThreat, iCommonZoneTargetFriendlySurfaceThreat, (oNearestEnemyNonHoverToFriendlyBase[M28UnitInfo.refbHostileImmobileCivilian] or oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.refbHostileImmobileCivilian]))
                    ...
                    if bDebugMessages == true then LOG(sFunctionRef..': Finished checking if are in scenario2, bAreInScenario2='..tostring(bAreInScenario2)..'; subrefWZbSurfaceInScenario2PrevCycle='..tostring(tWZTeamData[M28Map.subrefWZbSurfaceInScenario2PrevCycle] or false)..'; tWZTeamData[M28Map.subrefWZTThreatAllyCombatTotal]='..tWZTeamData[M28Map.subrefWZTThreatAllyCombatTotal]..'; tWZTeamData[M28Map.subrefTThreatEnemyCombatTotal]='..tWZTeamData[M28Map.subrefTThreatEnemyCombatTotal]..'; Core WZ base='..tostring(tWZTeamData[M28Map.subrefWZbCoreBase])..'; iAdjacentAlliedCombatThreat='..iAdjacentAlliedCombatThreat..'; iAdjacentEnemyCombatThreat='..iAdjacentEnemyCombatThreat..'; threat of tAvailableCombatUnits='..M28UnitInfo.GetCombatThreatRating(tCombatUnitsOfUse, false,          false,          false,                  false,          true,           false,          false)) end
                    if bAreInScenario2 then
                        tWZTeamData[M28Map.subrefWZbSurfaceInScenario2PrevCycle] = true
                    elseif tWZTeamData[M28Map.subrefWZbSurfaceInScenario2PrevCycle] then
                        --If we were in scenario 2 last cycle remain in it for 1 more cycle in case units have just moved to an adjacent WZ that will target the same location
                        bAreInScenario2 = true
                        tWZTeamData[M28Map.subrefWZbSurfaceInScenario2PrevCycle] = false
                        if bDebugMessages == true then LOG(sFunctionRef..': WIll remain in scenario 2 for 1 more cycle in case some units left this zone for an adjacentzone that will attack the same target') end
                    else
                        if bDebugMessages == true then LOG(sFunctionRef..': Not in scenario 2 and werent in previous cycle either') end
                    end
                    ...
                    if bAreInScenario2 then
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': Since are in scenario 2 will look to record water zone attack target if there is an enemy unit, is oNearestEnemySurfaceToFriendlyBase valid='..tostring(M28UnitInfo.IsUnitValid(oNearestEnemySurfaceToFriendlyBase))..'; Assigned WZ for nearest surface enem='..( oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] or 'nil')) end
                        if M28UnitInfo.IsUnitValid(oNearestEnemySurfaceToFriendlyBase) then
                            bNearestEnemySurfaceIsVisible = M28UnitInfo.CanSeeUnit(aiBrain, oNearestEnemySurfaceToFriendlyBase)
                            if oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] then
                                if bDebugMessages == true then LOG(sFunctionRef..': Will record the water zone target for this unit so other zones can factor in to their calculations') end
                                RecordWaterZoneTarget(tWZTeamData, iWaterZone, iTeam, false, oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam], M28Map.subrefiLZOrWZTAttackingUnit)
                            end
                        end
                        ...
                        local bConsolidateScenario2Units = false
                        --If dont have overwhelming force then consider consolidating if units arent already in range of the enemy
                        if not(iAvailableCombatThreat) then --redundancy
                            iAvailableCombatThreat = M28UnitInfo.GetCombatThreatRating(tCombatUnitsOfUse, false,          false,          false,                  false,          true,           false,          false)
                        end
                        if iAvailableCombatThreat * 0.5 < iAdjacentEnemyCombatThreat then
                            --Get closest friendly to enemy
                            local bEnemyAlreadyAbleToAttackUsOrWeCanAttackEnemy = false
                            local iEnemyRangeThreshold = math.max(iEnemyBestRange - 5, (oNearestEnemyToFriendlyBase[M28UnitInfo.refiDFRange] or 0), (oNearestEnemyToFriendlyBase[M28UnitInfo.refiAntiNavyRange] or 0))
                            local oClosestFriendlyToEnemyInclRange
                            local iClosestFriendlyToEnemyDistLessRange = 10000
                            local iCurDist, iCurRange
                            for iUnit, oUnit in tCombatUnitsOfUse do
                                iCurDist = M28Utilities.GetDistanceBetweenPositions(oNearestEnemyToFriendlyBase:GetPosition(), oUnit:GetPosition())
                                iCurRange = math.max((oUnit[M28UnitInfo.refiDFRange] or 0), (oUnit[M28UnitInfo.refiAntiNavyRange] or 0))
                                if iCurDist <= iCurRange or iCurDist <= iEnemyRangeThreshold then
                                    bEnemyAlreadyAbleToAttackUsOrWeCanAttackEnemy = true
                                    break
                                else
                                    if iCurDist - iCurRange < iClosestFriendlyToEnemyDistLessRange then
                                        iClosestFriendlyToEnemyDistLessRange = iCurDist - iCurRange
                                        oClosestFriendlyToEnemyInclRange = oUnit
                                    end
                                end
                            end
                            if not(bEnemyAlreadyAbleToAttackUsOrWeCanAttackEnemy) and oClosestFriendlyToEnemyInclRange then
                                --How much threat is near our unit closest to the enemy
                                local iNearbyMassInCombatUnits = 0
                                local iMaxDistLessRangeWanted = 60 -math.max(oClosestFriendlyToEnemyInclRange[M28UnitInfo.refiDFRange] or 0), (oClosestFriendlyToEnemyInclRange[M28UnitInfo.refiAntiNavyRange] or 0)

                                for iUnit, oUnit in tCombatUnitsOfUse do
                                    iCurDist = M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oClosestFriendlyToEnemyInclRange:GetPosition())
                                    if iCurDist <= 60 or iCurDist - math.max((oUnit[M28UnitInfo.refiDFRange] or 0), (oUnit[M28UnitInfo.refiAntiNavyRange] or 0))  <= iMaxDistLessRangeWanted then
                                        iNearbyMassInCombatUnits = iNearbyMassInCombatUnits + (oUnit[M28UnitInfo.refiUnitMassCost] or M28UnitInfo.GetUnitMassCost(oUnit))
                                    end
                                end
                                if bDebugMessages == true then LOG(sFunctionRef..': iNearbyMassInCombatUnits='..iNearbyMassInCombatUnits..'; iAdjacentEnemyCombatThreat='..iAdjacentEnemyCombatThreat..'; iAvailableCombatThreat='..iAvailableCombatThreat..'; iCommonZoneTargetFriendlySurfaceThreat='..iCommonZoneTargetFriendlySurfaceThreat) end
                                if iNearbyMassInCombatUnits < iAdjacentEnemyCombatThreat and iNearbyMassInCombatUnits < (iAvailableCombatThreat + iCommonZoneTargetFriendlySurfaceThreat)*0.5 then
                                    bConsolidateScenario2Units = true
                                    if bDebugMessages == true then LOG(sFunctionRef..': Will consolidate scenario 2 units rather than attacking, but still treat it as though we are attacking the target zone') end
                                    for iUnit, oUnit in tCombatUnitsOfUse do
                                        if EntityCategoryContains(categories.HOVER, oUnit.UnitId) then iOrderReissueDistToUse = iResisueOrderDistanceHover
                                        else iOrderReissueDistToUse = iReissueOrderDistanceStandard
                                        end
                                        if bDebugMessages == true then LOG(sFunctionRef..'; Will consolidate, oUnit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; Dist to oClosestFriendlyToEnemyInclRange='..M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oClosestFriendlyToEnemyInclRange:GetPosition())) end
                                        if M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oClosestFriendlyToEnemyInclRange:GetPosition()) >= 10 then
                                            M28Orders.IssueTrackedAggressiveMove(oUnit, tRallyPoint, iOrderReissueDistToUse, false, 'Sc2ConsA'..iWaterZone)
                                        else
                                            M28Orders.IssueTrackedMove(oUnit, tRallyPoint, iOrderReissueDistToUse, false, 'Sc2ConsM'..iWaterZone)
                                        end
                                    end
                                end
                            end
                        end
                        if not(bConsolidateScenario2Units) then
                            ...
                            for iUnit, oUnit in tCombatUnitsOfUse do
                                if not(oUnit.Dead) then --strange error where this rarely triggers for a dead unit, so adding extra redundancy here
                                    ...
                                    elseif bMoveBlockedNotAttackMove and oUnit[M28UnitInfo.refbLastShotBlocked] and (GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastUnblockedShot] or -100)) >= 10 and GetGameTimeSeconds() - (oUnit[M28UnitInfo.refiTimeOfLastCheck] or -100) < 6 then
                                        ...
                                        --If we are close to the last known position such that we will be able to see there is no longer a unit there, then update this unit's position for next cycle
                                        if bCheckIfNearestUnitVisible and not(bUpdateNearestUnit) and M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oNearestEnemyToFriendlyBase[M28UnitInfo.reftLastKnownPositionByTeam][iTeam]) <= 18 then bUpdateNearestUnit = true end
                                    elseif bOutrangeClosestEnemy and not(oUnit[M28UnitInfo.refbEasyBrain]) and (iDistUntilEnemyOutOfOurRange >= 2 or (M28Conditions.CloseToEnemyUnit(oUnit:GetPosition(), tEnemySurfaceUnits, (oUnit[M28UnitInfo.refiDFRange] or 10) - 2, iTeam, false, nil, oUnit, oUnit, nil, true, false, nil) and not(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck] == oEnemyToFocusOn) and M28UnitInfo.IsUnitValid(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]) and oUnit[M28UnitInfo.refiDFRange] > math.max((oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.refiDFRange] or 0), (oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.refiAntiNavyRange] or 0)) and M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.reftLastKnownPositionByTeam][iTeam]))) then

                                        local oEnemyToKiteFrom
                                        if iDistUntilEnemyOutOfOurRange >= 2 or not(M28UnitInfo.IsUnitValid(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]) ) then oEnemyToKiteFrom = oEnemyToFocusOn
                                        else
                                            oEnemyToKiteFrom = oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]
                                            iDistUntilEnemyOutOfOurRange = oUnit[M28UnitInfo.refiDFRange] - M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oEnemyToKiteFrom:GetPosition())
                                        end
                                        ...
                                        local tLocalisedKitingPosition = M28Utilities.MoveInDirection(oUnit:GetPosition(), M28Utilities.GetAngleFromAToB(oEnemyToKiteFrom:GetPosition(), oUnit:GetPosition()), math.min(5, math.max(1, iDistUntilEnemyOutOfOurRange - 0.25)), true, false, bCheckPlayableArea)
                                        if bDebugMessages == true then LOG(sFunctionRef..': Will do kiting retreat if we can move back in the same pond, naval (pond) label of kiting position='..(NavUtils.GetTerrainLabel(M28Map.refPathingTypeNavy, tLocalisedKitingPosition) or 'nil')..'; current iPond we are in='..iPond..'; oEnemyToKiteFrom='..oEnemyToKiteFrom.UnitId..M28UnitInfo.GetUnitLifetimeCount(oEnemyToKiteFrom)..'; iDistUntilEnemyOutOfOurRange='..iDistUntilEnemyOutOfOurRange) end
                                        if NavUtils.GetTerrainLabel(M28Map.refPathingTypeNavy, tLocalisedKitingPosition) == iPond then
                                            ...
                                            if not(IgnoreOrderDueToStuckUnit(oUnit)) then
                                                if M28Conditions.GroundAttackTargetUnitInsteadOfAttackMove(oUnit, oEnemyToKiteFrom) then
                                                    M28Orders.IssueTrackedGroundAttack(oUnit, oEnemyToKiteFrom:GetPosition(), 1, false, 'WATagWE'..iWaterZone, false, oEnemyToKiteFrom)
                                                else
                                                    M28Orders.IssueTrackedAggressiveMove(oUnit, oEnemyToKiteFrom[M28UnitInfo.reftLastKnownPositionByTeam][iTeam], iOrderReissueDistToUse, false, 'WATWE'..iWaterZone)
                                                end
                                            end
                                        end
                                    else
                        ...
                        --GetCombatThreatRating(tUnits,         bEnemyUnits, bJustGetMassValue, bIndirectFireThreatOnly, bAntiNavyOnly, bAddAntiNavy, bSubmersibleOnly, bLongRangeThreatOnly, bBlueprintThreat)
                        if not( iAvailableCombatThreat) then iAvailableCombatThreat = M28UnitInfo.GetCombatThreatRating(tCombatUnitsOfUse, false,          false,          false,                  false,          true,           false,          false) end --redundancy
                        ...
                        if not(bAntiTorpLauncherMode) and M28Utilities.IsTableEmpty(tCombatUnitsOfUse) == false then
                            ...
                            if bDebugMessages == true then LOG(sFunctionRef..': Dont have enough threat to attack with surface naval units, will see if we want to consolidate in this zone, iAvailableCombatThreat='..iAvailableCombatThreat..'; iAdjacentAlliedCombatThreat='..iAdjacentAlliedCombatThreat..'; Want to attack with navy='..tostring(M28Conditions.WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat, iAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAvailableCombatThreat, iAdjacentEnemyCombatThreat, true, iModForEnemyScenario2Threat, iEnemyNearbySubmersibleThreat,  iOurAntiNavyThreat ))) end
                            --WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat, iCurZoneAndAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAdjacentAlliedCombatThreat, iAdjacentEnemyCombatThreat, bConsideringSubmarinesNotSurface, iOptionalThreatAbsolutePercentIncrease, iEnemyNearbySubmersibleThreat, iAvailableAntiNavyThreat, iCommonZoneTargetSurfaceThreat, bNearbyEnemiesAreHostileCivilians)
                            if iAvailableCombatThreat > iAdjacentAlliedCombatThreat and M28Conditions.WantToAttackWithNavyEvenIfOutranged(tWZData, tWZTeamData, iTeam, iWaterZone, iNearbyFriendlySubThreat, iAdjacentAlliedSubmersibleThreat, iAdjacentEnemyAntiNavyThreat, iAvailableCombatThreat, iAdjacentEnemyCombatThreat,                true,                               iModForEnemyScenario2Threat,            iEnemyNearbySubmersibleThreat,  iOurAntiNavyThreat,         nil,                        (oNearestEnemyNonHoverToFriendlyBase[M28UnitInfo.refbHostileImmobileCivilian] or oNearestEnemySurfaceToFriendlyBase[M28UnitInfo.refbHostileImmobileCivilian])) then
                                sMessage = 'WSfConsR'
                                tSubRallyPoint = {tWZData[M28Map.subrefMidpoint][1], tWZData[M28Map.subrefMidpoint][2], tWZData[M28Map.subrefMidpoint][3]}
                                bConsiderAmphibiousRally = false
                            ...
                            for iUnit, oUnit in tCombatUnitsOfUse do
                                if oUnit[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] == iWaterZone then
                                    if bDebugMessages == true then LOG(sFunctionRef..': Will retreat combat unit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; bConsiderAmphibiousRally for amphibious units='..tostring(bConsiderAmphibiousRally)..'; Is this amphibious/hover unit='..tostring(EntityCategoryContains(M28UnitInfo.refCategoryAmphibious + categories.HOVER, oUnit.UnitId))) end
                                    if EntityCategoryContains(categories.HOVER, oUnit.UnitId) then iOrderReissueDistToUse = iResisueOrderDistanceHover
                                    else iOrderReissueDistToUse = iReissueOrderDistanceStandard
                                    end
                                    oUnit[M28UnitInfo.refiTimeLastTriedRetreating] = iCurTime
                                    if bConsiderAmphibiousRally and EntityCategoryContains(M28UnitInfo.refCategoryAmphibious + categories.HOVER, oUnit.UnitId) then --redundancy - wouldnt expect a sub to be amphibious
                                        if iAmphibiousRallyPlateau then
                                            ForkThread(M28Land.BackupUnitTowardsRallyIfAvailable, oUnit, tAmphibiousRallyPoint, iAmphibiousRallyPlateau, sMessage..'_AmpR'..iWaterZone, false, nil, nil, false)
                                        else
        ...
        if bDebugMessages == true then LOG(sFunctionRef..': Is table of subs with no target empty='..tostring(M28Utilities.IsTableEmpty(tSubmarinesWithNoTarget))..'; Is table of combat units with no target empty='..tostring(M28Utilities.IsTableEmpty(tCombatUnitsWithNoTarget))..'; oNearestEnemyNonHoverToFriendlyBase='..(oNearestEnemyNonHoverToFriendlyBase.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oNearestEnemyNonHoverToFriendlyBase) or 'nil')) end
...
function ManageMAAInWaterZone(tWZData, tWZTeamData, iTeam, iPond, iWaterZone, tAvailableMAA)
    ...
    if M28Utilities.IsTableEmpty(tWZTeamData[M28Map.reftoNearestCombatEnemies]) and not(bHaveNearbyLandUnits) then
        ...
        local iBaseRunThreshold = 30
        if (tWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) > 0 and tWZTeamData[M28Map.refiEnemyAirToGroundThreat] < tWZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] and tWZTeamData[M28Map.subrefWZTThreatAllyCombatTotal] > 0.2 * tWZTeamData[M28Map.subrefWZThreatAlliedMAA]  then
            iBaseRunThreshold = 19
            if bDebugMessages == true then LOG(sFunctionRef..': Reducing run threshold due to enemy air to ground') end
        end
        local iT3RunThreshold = iBaseRunThreshold + 8 --aircraft carriers are slower, turn slower, and larger
        ...
        for iUnit, oUnit in tAvailableMAA do
            if not(oUnit.Dead) then --to stop rare error
                if EntityCategoryContains(categories.TECH3 + categories.EXPERIMENTAL, oUnit.UnitId) then iRunThreshold = iT3RunThreshold else iRunThreshold = iBaseRunThreshold end
                --Run if within 30 of being in range of enemy combat
                if bDebugMessages == true then
                    LOG(sFunctionRef..': MAA '..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..' - checking if is too close to enemy, is close='..tostring(M28Conditions.CloseToEnemyUnit(oUnit:GetPosition(), tWZTeamData[M28Map.reftoNearestCombatEnemies], iRunThreshold, iTeam, true                    ,nil                        ,nil                            ,nil                                        ,nil                                    ,true))..'; iRunThreshold='..iRunThreshold..'; will run back if are too close; will list out enemy units and distance to us in a moment; Is unit too close to T2 arti='..tostring(DoesUnitWantToRunFromT2Arti(oUnit, 10))..'; bHaveNearbyWaterEnemies='..tostring(bHaveNearbyWaterEnemies)..'; iRunThreshold='..iRunThreshold)
                ...
                --                                              CloseToEnemyUnit(tStartPosition, tUnitsToCheck,                             iDistThreshold, iTeam, bIncludeEnemyDFRange, iAltThresholdToDFRange, oUnitIfConsideringAngleAndLastShot, oOptionalFriendlyUnitToRecordClosestEnemy, iOptionalDistThresholdForStructure, bIncludeEnemyAntiNavyRange)
                if (bHaveNearbyWaterEnemies and (M28Conditions.CloseToEnemyUnit(oUnit:GetPosition(), tWZTeamData[M28Map.reftoNearestCombatEnemies], iRunThreshold, iTeam, true                    ,nil                        ,nil                            ,oUnit                                        ,nil                                    ,true) or (oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.refiDFRange] and not(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck].Dead) and M28Utilities.GetDistanceBetweenPositions(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]:GetPosition(), oUnit:GetPosition()) - oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.refiDFRange] <= iRunThreshold + 10 and M28UnitInfo.GetUnitSpeed(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]) >= 2.8 and M28Utilities.GetAngleDifference(M28UnitInfo.GetUnitFacingAngle(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]), M28Utilities.GetAngleFromAToB(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]:GetPosition(), oUnit:GetPosition())) <= 30)))
                    ...
                    if bDebugMessages == true and oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.refiDFRange] then LOG(sFunctionRef..': Dist until in range of refoClosestEnemyFromLastCloseToEnemyUnitCheck='..M28Utilities.GetDistanceBetweenPositions(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]:GetPosition(), oUnit:GetPosition()) - oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck][M28UnitInfo.refiDFRange]..'; Speed of that unit='..M28UnitInfo.GetUnitSpeed(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck])..'; Facing angle='..M28UnitInfo.GetUnitFacingAngle(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck])..'; Angle from it to us='..M28Utilities.GetAngleFromAToB(oUnit[M28UnitInfo.refoClosestEnemyFromLastCloseToEnemyUnitCheck]:GetPosition(), oUnit:GetPosition())) end
    ...
    if M28Utilities.IsTableEmpty(tAvailableSubjectToAA) == false then
        ...
        if iEnemyAirToGroundThreat > iOurAAThreat and not(bRetreatAllMAA) then
            local iCategoriesToAvoid

            iCategoriesToAvoid = M28UnitInfo.refCategoryAirToGround - M28UnitInfo.refCategoryBomber * categories.EXPERIMENTAL --dont want to avoid exp bombers since they struggle to turn around and want to maximise damage on them; only avoid t3 strats (who also struggle to turn around) if significant enemy threat
            if iEnemyAirToGroundThreat < tWZTeamData[M28Map.subrefWZThreatAlliedMAA] then
                iCategoriesToAvoid = iCategoriesToAvoid -M28UnitInfo.refCategoryBomber * categories.TECH3
                if bDebugMessages == true then LOG(sFunctionRef..': Wont avoid strat bombers as our MAA threat exceeds total enemy threat') end
            end
    ...
    if M28Utilities.IsTableEmpty(tMAAToAdvance) == false then
        ...
        if M28Utilities.IsTableEmpty(tWZData[M28Map.subrefOtherLandAndWaterZonesByDistance]) == false then
            ...
            local bDontCheckPlayableArea = not(M28Map.bIsCampaignMap)
            if not(tWZTeamData[M28Map.subrefbDangerousEnemiesInAdjacentWZ]) and iBestRange >= 70 and tWZTeamData[M28Map.refiModDistancePercent] < 0.5 and M28Utilities.IsTableEmpty(tWZData[M28Map.subrefWZAdjacentWaterZones]) == false and not(M28Utilities.bCPUPerformanceMode) then
                iDistanceThreshold = iDistanceThreshold + 50
            end
            ...
            local tbLandZonesConsideredByPlateau = {}
            function ConsiderTargetingUnitsOfInterest(tClosestEnemyBuildingsOfInterest)
            ...
            for iEntry, tSubtable in tWZData[M28Map.subrefOtherLandAndWaterZonesByDistance] do
                if tSubtable[M28Map.subrefiDistance] > iDistanceThreshold then
                    if bDebugMessages == true then LOG(sFunctionRef..': Zone is too far away, tSubtable[M28Map.subrefiDistance]='..tSubtable[M28Map.subrefiDistance]..'; iDistanceThreshold='..iDistanceThreshold) end
                    break
                else
                    ...
                    if tSubtable[M28Map.subrefbIsWaterZone] then --will check if it's in the same pond later on
                        ...
                        --Commented out below as want to consider with lower thresholds in the adjacent zone check
                        --if not(tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefiPlateauOrPond]]) then tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefiPlateauOrPond]] = {} end
                        --tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefiPlateauOrPond]][tSubtable[M28Map.subrefiLandOrWaterZoneRef]] = true
                    end
                    ...
                    if (bDontCheckIfInCampaignArea or M28Conditions.IsLocationInPlayableArea(tAltLZOrWZData[M28Map.subrefMidpoint])) and iBestRange >= 80 and (tSubtable[M28Map.subrefiPlateauOrPond] == iPond or not(tSubtable[M28Map.subrefbIsWaterZone])) and iZoneCountFromFirstBuilding <= 8 then
                        if (bDontCheckPlayableArea or M28Conditions.IsLocationInPlayableArea(tAltLZOrWZData[M28Map.subrefMidpoint])) and (tAltLZOrWZTeamData[M28Map.subrefThreatEnemyStructureTotalMass] or 0) >= 200 then
                            ...
                            if M28Utilities.IsTableEmpty(tClosestEnemyBuildingsOfInterest) == false then
                                ...
                                --Get the closest enemy unit to thie midpoint of this zone if we have combat aa:
                                ConsiderTargetingUnitsOfInterest(tClosestEnemyBuildingsOfInterest)
                            end
                        end
                    end
            ...
            --Check land zones adjacent to adjacent water zones if no enemies in both this zone and adjacent WZs; and this time consider t1 mexes
            if not(oClosestEnemyStructureOfInterest) and iDistanceThreshold < 250 and M28Utilities.IsTableEmpty(tWZData[M28Map.subrefWZAdjacentWaterZones]) == false and tWZTeamData[M28Map.subrefTThreatEnemyCombatTotal] == 0 then
                if M28Utilities.IsTableEmpty(tWZData[M28Map.subrefAdjacentLandZones]) == false then
                    for iEntry, tSubtable in tWZData[M28Map.subrefAdjacentLandZones] do
                        if not(tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefWPlatAndLZNumber][1]][tSubtable[M28Map.subrefWPlatAndLZNumber][2]]) then
                            if not(tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefWPlatAndLZNumber][1]]) then tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefWPlatAndLZNumber][1]] = {} end
                            tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefWPlatAndLZNumber][1]][tSubtable[M28Map.subrefWPlatAndLZNumber][2]] = true
                            local tAltLZ = M28Map.tAllPlateaus[tSubtable[M28Map.subrefWPlatAndLZNumber][1]][M28Map.subrefPlateauLandZones][tSubtable[M28Map.subrefWPlatAndLZNumber][2]]
                            local tAltLZTeamData = tAltLZ[M28Map.subrefLZTeamData][iTeam]
                            if bDebugMessages == true then LOG(sFunctionRef..': Considering land zone adjacent to this water zone, Plateau='..tSubtable[M28Map.subrefWPlatAndLZNumber][1]..'; LZ='..tSubtable[M28Map.subrefWPlatAndLZNumber][2]) end
                            if tAltLZTeamData[M28Map.subrefThreatEnemyStructureTotalMass] > 10 and M28Utilities.IsTableEmpty(tAltLZTeamData[M28Map.subrefTEnemyUnits]) == false then
                                ConsiderTargetingUnitsOfInterest(tAltLZTeamData[M28Map.subrefTEnemyUnits])
                            end
                        end
                    end
                end
                for _, iAdjWZ in tWZData[M28Map.subrefWZAdjacentWaterZones] do
                    local tAdjWZData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iAdjWZ]][M28Map.subrefPondWaterZones]
                    if bDebugMessages == true then LOG(sFunctionRef..': Considering if LZs adjacent to iAdjWZ='..iAdjWZ..'; is table empty='..tostring(M28Utilities.IsTableEmpty(tAdjWZData[M28Map.subrefAdjacentLandZones]))) end
                    if M28Utilities.IsTableEmpty(tAdjWZData[M28Map.subrefAdjacentLandZones]) == false then
                        local tAdjWZTeamData = tAdjWZData[iAdjWZ][M28Map.subrefWZTeamData][iTeam]
                        if bDebugMessages == true then LOG(sFunctionRef..': subrefTThreatEnemyCombatTotal for this adjWZ='..( tAdjWZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 'nil')) end
                        if tAdjWZTeamData[M28Map.subrefTThreatEnemyCombatTotal] == 0 then
                            for iEntry, tSubtable in tAdjWZData[M28Map.subrefAdjacentLandZones] do
                                if not(tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefWPlatAndLZNumber][1]][tSubtable[M28Map.subrefWPlatAndLZNumber][2]]) then
                                    if not(tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefWPlatAndLZNumber][1]]) then tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefWPlatAndLZNumber][1]] = {} end
                                    tbLandZonesConsideredByPlateau[tSubtable[M28Map.subrefWPlatAndLZNumber][1]][tSubtable[M28Map.subrefWPlatAndLZNumber][2]] = true
                                    local tAltLZ = M28Map.tAllPlateaus[tSubtable[M28Map.subrefWPlatAndLZNumber][1]][M28Map.subrefPlateauLandZones][tSubtable[M28Map.subrefWPlatAndLZNumber][2]]
                                    local tAltLZTeamData = tAltLZ[M28Map.subrefLZTeamData][iTeam]
                                    if bDebugMessages == true then LOG(sFunctionRef..': Considering if enemy bulidings in land zone adjacent to adjacent water zone '..iAdjWZ..'; Plateau='..tSubtable[M28Map.subrefWPlatAndLZNumber][1]..'; LZ='..tSubtable[M28Map.subrefWPlatAndLZNumber][2]) end
                                    if tAltLZTeamData[M28Map.subrefThreatEnemyStructureTotalMass] > 10 and M28Utilities.IsTableEmpty(tAltLZTeamData[M28Map.subrefTEnemyUnits]) == false then
                                        ConsiderTargetingUnitsOfInterest(tAltLZTeamData[M28Map.subrefTEnemyUnits])
                                    end
                                end
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Finsihed checking nearby and adjacent land zones for buildings, oClosestEnemyStructureOfInterest='..(oClosestEnemyStructureOfInterest.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oClosestEnemyStructureOfInterest) or 'nil')) end
            if oClosestEnemyStructureOfInterest then
                ...
                local iCurDist, iAngleFromStructureToUnit
                for iUnit, oUnit in tCombatAAByOrigRef do
                    ...
                    if iCurDist > oUnit[M28UnitInfo.refiCombatRange] then
                        ...
                        iAngleFromStructureToUnit =  M28Utilities.GetAngleFromAToB(oEnemyStructureToTarget:GetPosition(), oUnit:GetPosition())
                        for iDistInterval = 0, 50, 10 do
                            local tPossibleFiringPoint = M28Utilities.MoveInDirection(oEnemyStructureToTarget:GetPosition(), iAngleFromStructureToUnit, oUnit[M28UnitInfo.refiCombatRange] - iDistInterval, true, false, false)
                            if bDebugMessages == true then
                                local iColour
                                LOG(sFunctionRef..': If we moved into range of this unit would we be in this pond? iDistInterval='..iDistInterval..'; iPond='..iPond..'; Navy label of tPossibleFiringPoint='..(NavUtils.GetTerrainLabel(M28Map.refPathingTypeNavy, tPossibleFiringPoint) or 'nil'))
                                if NavUtils.GetTerrainLabel(M28Map.refPathingTypeNavy, tPossibleFiringPoint) == iPond then iColour = 1 else iColour = 2 end
                                M28Utilities.DrawLocation(tPossibleFiringPoint, iColour)
                            end
                            if NavUtils.GetTerrainLabel(M28Map.refPathingTypeNavy, tPossibleFiringPoint) == iPond then
                                bHaveValidTarget = true
                                break
                            elseif iDistInterval == 0 or iDistInterval == 50 then
                                for iAngleAdjust = -45, 45, 90 do
                                    tPossibleFiringPoint = M28Utilities.MoveInDirection(oEnemyStructureToTarget:GetPosition(), iAngleAdjust + M28Utilities.GetAngleFromAToB(oEnemyStructureToTarget:GetPosition(), oUnit:GetPosition()), oUnit[M28UnitInfo.refiCombatRange] - iDistInterval, true, false, false)
                                    if bDebugMessages == true then
                                        LOG(sFunctionRef..': If we moved at iAngleAdjust into range of this unit would we be in this pond? iAngleAdjust='..iAngleAdjust..'; iDistInterval='..iDistInterval..'; iPond='..iPond..'; Navy label of tPossibleFiringPoint='..(NavUtils.GetTerrainLabel(M28Map.refPathingTypeNavy, tPossibleFiringPoint) or 'nil'))
                                        local iColour
                                        if NavUtils.GetTerrainLabel(M28Map.refPathingTypeNavy, tPossibleFiringPoint) == iPond then iColour = 1 else iColour = 2 end
                                        M28Utilities.DrawLocation(tPossibleFiringPoint, iColour)
                                    end
                            ...
                            if not(tbPlateauAndLandZonesConsidered[iCurPlateau][iCurLandZone]) then
                                ...
                                if bDontCheckPlayableArea or M28Conditions.IsLocationInPlayableArea(tCurLZData[M28Map.subrefMidpoint]) then
                                    ...
                                    if M28Utilities.IsTableEmpty(tCurLZTeamData[M28Map.subrefTEnemyUnits]) == false then
                                        if not(M28Overseer.bPacifistModeActive) or not(M28Conditions.IsZoneAPacifistZone(iCurPlateau, iCurLandZone)) then
...
function GetNearestEnemyUnitsAndUpdateUnitTables(iPond, iWaterZone, tWZData, tWZTeamData, iTeam, tEnemySurfaceUnits, tEnemyNonHoverUnits, bOnlyConsiderDoublyAdjacentZones)
    ...
    function ConsiderIfUnitIsClosest(oUnit, bDealingWithAdjZone)
        ...
        if bDebugMessages == true then LOG(sFunctionRef..': Considering oUnit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; bDealingWithAdjZone='..tostring(bDealingWithAdjZone or false)..'; Is unit valid='..tostring(M28UnitInfo.IsUnitValid(oUnit))..'; bOnlyConsiderDoublyAdjacentZones='..tostring(bOnlyConsiderDoublyAdjacentZones or false)..'; Unit mass cost='..(oUnit[M28UnitInfo.refiUnitMassCost] or 'nil')..'; Unit assigned WZ='..(oUnit[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] or 'nil')..'; Is this zone pacifist='..tostring(M28Conditions.IsZoneAPacifistZone(0, oUnit[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam]))) end
        if not(oUnit.Dead) and (not(bOnlyConsiderDoublyAdjacentZones) or (oUnit[M28UnitInfo.refiUnitMassCost] or M28UnitInfo.GetUnitMassCost(oUnit)) >= 250)
                ...
                and (not(M28Overseer.bPacifistModeActive) or oUnit[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam] == iWaterZone or not(M28Conditions.IsZoneAPacifistZone(0, oUnit[M28UnitInfo.reftAssignedWaterZoneByTeam][iTeam])) or not(M28Conditions.IsCloseToPacifistUnit(oUnit:GetPosition(), 120))) then
            ...
            if not(M28Map.bIsCampaignMap) or M28Conditions.IsLocationInPlayableArea(oUnit:GetPosition()) then
                ...
                refiCurRangeAdjustedDist = refiCurDist - (oUnit[M28UnitInfo.refiDFRange] or 0) --changed in v317 to use DF range, as otherwise it resulted in us thinking we could attack an enemy cruiser due to it being 'closer' than an enemy destroyer'
                refiCurAntiNavyRangeAdjustedDist = refiCurDist - (oUnit[M28UnitInfo.refiAntiNavyRange] or 0)
                refiCurMidpointDist = -1
                if bDebugMessages == true then LOG(sFunctionRef..': ConsiderIfUnitIsClosest: oUnit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; refiCurDist to friendly base='..refiCurDist..'; Actual dist based on actual position='..M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), tWZTeamData[M28Map.reftClosestFriendlyBase])..'; oUnit[M28UnitInfo.refiCombatRange]='..(oUnit[M28UnitInfo.refiCombatRange] or 'nil')..'; refiClosestDist ='..refiClosestDist..'; refiCurRangeAdjustedDist='..refiCurRangeAdjustedDist..'; refiClosestNonHoverToMidpointByRange='..refiClosestNonHoverToMidpointByRange..'; bDealingWithAdjZone='..tostring(bDealingWithAdjZone or false)) end
    ...
    for _, iAdjWZ in tWZData[M28Map.subrefWZAdjacentWaterZones] do
        --iAdjWZ = tWZSubtable[M28Map.subrefWZAWZRef]
        if not(bOnlyConsiderDoublyAdjacentZones) then
            if bDebugMessages == true then LOG(sFunctionRef..': About to consider units in zone iAdjWZ='..iAdjWZ) end
...
function ManageSACUsInWaterZone(tSACUs, tWZData, tWZTeamData, iPond, iWaterZone, iTeam)
    ...
    if M28Conditions.IsTableOfUnitsStillValid(tSACUs) then --redundancy for rare error
...
function ManageLandRaidersInWaterZone(tWZData, tWZTeamData, iTeam, iPond, iWaterZone, tRaiders)
    ...
    if M28Utilities.IsTableEmpty(tEnemiesToConsiderRunningFrom) then tRaidersToAdvance = tRaiders
        ...
        local bRaiderMovingOn, iCurDist, iClosestDist, oClosestImmobile, bRunning, tBackupRallyPoint
        for iRaider, oRaider in tRaiders do
            if not(oRaider.Dead) then
                if M28Conditions.CloseToEnemyUnit(oRaider:GetPosition(), tEnemiesToConsiderRunningFrom, math.max(5, (oRaider[M28UnitInfo.refiCombatRange]-25) * 0.3), iTeam, true, 3, oRaider, oRaider, oRaider[M28UnitInfo.refiCombatRange] - 4, false)
                    ...
                    if bRaiderMovingOn then
                        ...
                        if tTargetZoneLZData[M28Map.subrefMidpoint] then
                            ...
                            M28Orders.IssueTrackedAttackMove(oRaider, tWZTeamData[M28Map.reftClosestEnemyBase], 3, false, 'RaidBckMvLZ'..oRaider[M28Land.refiRaidingTargetZone], false)
                        end
                    end
                end
            end

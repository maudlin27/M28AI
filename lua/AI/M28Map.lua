...
iLandZoneSegmentSize = 5 --Gets updated by the SetupLandZones - the size of one side of the square that is the lowest resolution land zones go to; each segment that is land pathable gets assigned to a land zone
    --Land zone subrefs (against tAllPlateaus[iPlateau][subrefPlateauLandZones][iLandZone]):
        ...
        subrefLZTeamData = 'Subteam' --tAllPlateaus[iPlateau][subrefPlateauLandZones][iLandZone][subrefLZTeamData] - Table for all the data by team for a plateau's land zone
            ...
            subrefiNearbyEnemyLongRangeDFThreat = 'NrEnLRT' --Number equalling the threat value; intended for fatboys who can outrange adjacent land zones - for enemies that arent in this zone but have a long range and are close to being in range of this zone; units are added to here as a result of AddUnitToLongRangeThreatTable (which adds to the team baesd table, which is then checked to add to subrefoNearbyEnemyLongRangeDFThreats, which then determines subrefiNearbyEnemyLongRangeDFThreat
            subrefoNearbyEnemyLongRangeDFThreats = 'NrEnLRU' --As above, but a table of the units (likely a table of fatboys; t2 arti are kept in a separate table)
            subrefiNearbyEnemyLongRangeIFThreat = 'NrEnLRIT' --Number equalling threat value of long ranged indirectfire units
            subrefoNearbyEnemyLongRangeIFThreats = 'NREnLRIU' --As above, but for indirectfire units (e.g. mobile arti)
            ...
            subrefLZOrWZThreatAllyGroundAA = 'AAATotal' --Same ref used for WZ
...
--subrefPondNearbyBrains = 'PondNearbyBrains'
    ...
    subrefPondWaterZones = 'PondWZ' --e.g. access the water zone data tables via M28Map.tPondDetails[iPond][M28Map.subrefPondWaterZones][iWaterZone], where iWaterZone is the NavUtils refPathingTypeNavy pathing result
        ...
        subrefWZTeamData = 'PWZTeam' --Used to house team related data for a particular water zone
            ...
            subrefWZiSuicideIntoEnemyCombatThreat = 'WZSuc' --true if we want units here to act as if they are in scenario 2 rather than retreating
            ...
            subrefWZThreatEnemyVsSurface = 'EnSurf'
            ...
            --subrefWZThreatAlliedAA = 'AAATotal' --replaced with subrefLZOrWZThreatAllyGroundAA
...
function SetupPlayableAreaAndSegmentSizes(rCampaignPlayableAreaOverride)
    ...
    if not(bMapLandSetupComplete) then --e.g. if this is a campaign we may want to change playable area size
        --iTableSizeCap = math.pow(SegmentCount,2); SegmentCount = iTotalSize / SegmentSize; math.pow((TotalSize/SegmentSize),2) = iTableSizeCap; SemgentSize = TotalSize/Sqrt(iTableSizeCap)
...
function RecordPlateauReclaimSegmentsMidpointAndRadius(iPlateau, sPathing, tSegmentPositionMin, tSegmentPositionMax)
    ...
    tAllPlateaus[iPlateau][subrefPlateauMaxRadius] = math.pow((math.pow(iXRadius,2)+math.pow(iZRadius,2)),0.5)
end
...
function AssignSegmentsNearMexesToLandZones()
    ...
    --Record plateaus containing start positions (in case they dont have a mex in them but we still want to use as our core base)
    local tiStartPositionsByPlateau = {}
    for iBrain, oBrain in ArmyBrains do
        if not(M28Conditions.IsCivilianBrain(oBrain)) then
            local tStartPosition = GetPlayerStartPosition(oBrain)
            local iCurPlateau = NavUtils.GetLabel(refPathingTypeHover, tStartPosition)
            if iCurPlateau then
                if not(tiStartPositionsByPlateau[iCurPlateau]) then tiStartPositionsByPlateau[iCurPlateau] = {} end
                table.insert(tiStartPositionsByPlateau[iCurPlateau], {tStartPosition[1], tStartPosition[2], tStartPosition[3]})
            end
        end
    end
    ...
    for iPlateau, tPlateauSubtable in tAllPlateaus do
        ...
        if M28Utilities.IsTableEmpty(tPlateauSubtable[subrefPlateauMexes]) == false or tiStartPositionsByPlateau[iPlateau] then
            ...
            if iBaseQueueCount == 0 and M28Utilities.IsTableEmpty(tiStartPositionsByPlateau[iPlateau]) == false then
                for iEntry, tStartPosition in tiStartPositionsByPlateau[iPlateau] do
        ...
        for iBrain, oBrain in M28Team.tTeamData[iTeam][M28Team.subreftoEnemyBrains] do
            if not(oBrain.M28IsDefeated) then
                ...
                if bIgnoreThisBase then
                    ...
                    table.insert(toIgnoredEnemyBrains, oBrain)
        ...
        if M28Utilities.IsTableEmpty(tbTeamsToUpdateForChangesInAlliesOrEnemies) == false then
            for iBrain, oBrain in ArmyBrains do
                if oBrain.CampaignAI or M28Conditions.IsCivilianBrain(oBrain) then
                    if bDebugMessages == true then LOG(sFunctionRef..': Considering oBrain='..oBrain.Nickname..'; .M28Team='..(oBrain.M28Team or 'nil')..'; bRecordedAllPlayers='..tostring(M28Team.bRecordedAllPlayers or false)..';  oBrain.M28AI='..tostring(oBrain.M28AI or false)..'; Time='..GetGameTimeSeconds()) end

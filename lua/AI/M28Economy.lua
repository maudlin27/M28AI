...
toRecordedEconomyUnits = 'M28ERcU' --resource generating units that we have recorded for the player (so if any are destroyed and we dont pick it up we can update)
...
iSpecialNonPowerNonT1MexEngiAction = 'M28ENonPwrAction'
...
function UpgradeUnit(oUnitToUpgrade, bUpdateUpgradeTracker, iOptionalWait)
    ...
    if bContinue then
        ...
        if sUpgradeID and M28UnitInfo.IsUnitValid(oUnitToUpgrade) then
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': About to issue ugprade to unit '..oUnitToUpgrade.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnitToUpgrade)..'; Current state='..M28UnitInfo.GetUnitState(oUnitToUpgrade)..'; Work progress='..(oUnitToUpgrade:GetWorkProgress() or 'nil')..'; Is unit upgrading='..tostring(oUnitToUpgrade:IsUnitState('Upgrading'))..'; Fraction complete='..oUnitToUpgrade:GetFractionComplete()..'; bDelayedUpgradeTrackingCheck='..tostring(bDelayedUpgradeTrackingCheck)..'; M28Overseer.bUnitRestrictionsArePresent='..tostring(M28Overseer.bUnitRestrictionsArePresent or false)..'; CanBuild check='..tostring(oUnitToUpgrade:CanBuild(sUpgradeID))) end
                    ...
                    if not(bHaveOtherFactoriesOfSameTech) then
                        ...
                        elseif M28Utilities.IsTableEmpty(tFactoryLZData[M28Map.subrefLZAdjacentLandZones]) == false then
                            ...
                            for _, iAdjLZ in tFactoryLZData[M28Map.subrefLZAdjacentLandZones] do
                                ...
                                iNearbyMexCount = iNearbyMexCount + (tAdjLZData[M28Map.subrefLZOrWZMexCount] or 0)
                            end
        ...
        if oBP.General.UpgradesTo and not(oBP.General.UpgradesTo == '') then
            ...
            if oUpgradedBP then
                ...
                if tLZTeamData[M28Map.refiModDistancePercent] >= 0.3 then
                    ...
                    if tLZTeamData[M28Map.refiModDistancePercent] >= 0.45 then --dont really want to get if near middle of map
                        if iUnitTechLevel >= 2 then
                            iDelay = 120
                            if aiBrain[refiGrossEnergyBaseIncome] >= 400 then
                                iResourceFactorAdjust = 10
                            else
                                iResourceFactorAdjust = 8
                            end
                        else
                            iDelay = 60
        ...
        if aiBrain.M28AI then
            ...
            if (bDestroyed and oUnit[refoBrainRecordedForEconomy] == aiBrain) or (not(bDestroyed) and not(oUnit[refoBrainRecordedForEconomy] == aiBrain)) then
                ...
                if bDestroyed then
                    ...
                    if not(aiBrain[toRecordedEconomyUnits]) then aiBrain[toRecordedEconomyUnits] = {} end
                    table.insert(aiBrain[toRecordedEconomyUnits], oUnit)
                ...
                if bDebugMessages == true then LOG(sFunctionRef..': Updated gross and net resources for iMassGen='..iMassGen..'; iEnergyGen='..iEnergyGen..'; aiBrain[refiNetMassBaseIncome]='..aiBrain[refiNetMassBaseIncome]..'; aiBrain[refiGrossMassBaseIncome]='..aiBrain[refiGrossMassBaseIncome]..'; refiNetEnergyBaseIncome='..aiBrain[refiNetEnergyBaseIncome]..'; refiGrossEnergyBaseIncome='..aiBrain[refiGrossEnergyBaseIncome]) end
            end
...
function RefreshEconomyGrossValues(aiBrain)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': refreshing gross income for every unit we own time='..GetGameTimeSeconds()..'; size of tEconomyUnits='..table.getn(tEconomyUnits)..'; Gross E before update='..aiBrain[refiGrossEnergyBaseIncome]) end
    if tEconomyUnits then
        local iConstructedEconomyUnits = 0
        for iUnit, oUnit in tEconomyUnits do
            if oUnit:GetFractionComplete() == 1 then
                if not(oUnit[refoBrainRecordedForEconomy] == aiBrain) then
                    UpdateGrossIncomeForUnit(oUnit) --if we havent recorded the unit then will record it (this has its own check re brain, per above; added above for minor optimisation)
                end
                iConstructedEconomyUnits = iConstructedEconomyUnits + 1
            end
        end
        --Check for destroyed units we haven't picked up yet
        if aiBrain[toRecordedEconomyUnits] and iConstructedEconomyUnits < table.getn(aiBrain[toRecordedEconomyUnits]) then
...
function ConsiderReclaimingPower(iTeam, oPowerJustBuilt)
    ...
    elseif iPowerTechLevel == 3 then
        ...
        if bDebugMessages == true then LOG(sFunctionRef..': Considering if want to reclaim t2 power having just built t3 power, subrefbActiveT2PowerReclaimer='..tostring(M28Team.tTeamData[iTeam][M28Team.subrefbActiveT2PowerReclaimer] or false)) end
        if not(M28Team.tTeamData[iTeam][M28Team.subrefbActiveT2PowerReclaimer]) then
            ...
            if (not(M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] >= 600 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] * 0.65 * M28Team.tTeamData[iTeam][M28Team.refiHighestBrainResourceMultiplier] and (iT3PowerEquivalent >= M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] or M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] >= 2000 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] * M28Team.tTeamData[iTeam][M28Team.refiHighestBrainResourceMultiplier]))
            --LOUD version require more power first, as a big reason for reclaiming will be to free up space in base late-game
            or (iT3PowerEquivalent >= 5 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] and M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] > 0 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] >= 1200 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] * M28Team.tTeamData[iTeam][M28Team.refiHighestBrainResourceMultiplier])
             then
                M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
    ...
    if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subreftoFriendlyActiveM28Brains]) == false then
        ...
        if bStallingMass then
            if not(bImminentThreat) then
                ...
                if M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyLandFactoryTech] == 3 then
                    tCategoryAndEngineerTables[1] = { M28UnitInfo.refCategorySMD, M28UnitInfo.refCategoryEngineerStation, iSpecialSurplusUpgradeCategory, M28UnitInfo.refCategorySpecialFactory, M28UnitInfo.refCategoryEngineer, M28UnitInfo.refCategoryQuantumGateway, categories.SUBCOMMANDER, M28UnitInfo.refCategoryNavalFactory * categories.TECH1, M28UnitInfo.refCategoryLandFactory * categories.TECH1, M28UnitInfo.refCategoryLandFactory * categories.TECH2, M28UnitInfo.refCategoryEngineer, M28UnitInfo.refCategoryNavalFactory, M28UnitInfo.refCategoryLandFactory * categories.TECH3, categories.COMMAND, M28UnitInfo.refCategoryTML, M28UnitInfo.refCategoryEngineer, iSpecialHQCategory }
                elseif M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyLandFactoryTech] == 2 then
                    tCategoryAndEngineerTables[1] = { M28UnitInfo.refCategorySMD, M28UnitInfo.refCategoryEngineerStation, iSpecialSurplusUpgradeCategory, M28UnitInfo.refCategorySpecialFactory, M28UnitInfo.refCategoryEngineer, M28UnitInfo.refCategoryQuantumGateway, categories.SUBCOMMANDER, M28UnitInfo.refCategoryNavalFactory * categories.TECH1, M28UnitInfo.refCategoryLandFactory * categories.TECH1, M28UnitInfo.refCategoryEngineer, M28UnitInfo.refCategoryNavalFactory, M28UnitInfo.refCategoryLandFactory * categories.TECH2, M28UnitInfo.refCategoryLandFactory * categories.TECH3, categories.COMMAND, M28UnitInfo.refCategoryTML, M28UnitInfo.refCategoryEngineer, iSpecialHQCategory }
                else
                    tCategoryAndEngineerTables[1] = { M28UnitInfo.refCategorySMD, M28UnitInfo.refCategoryEngineerStation, iSpecialSurplusUpgradeCategory, M28UnitInfo.refCategorySpecialFactory, M28UnitInfo.refCategoryEngineer, M28UnitInfo.refCategoryQuantumGateway, categories.SUBCOMMANDER, M28UnitInfo.refCategoryEngineer, M28UnitInfo.refCategoryNavalFactory, M28UnitInfo.refCategoryLandFactory * categories.TECH1, M28UnitInfo.refCategoryLandFactory * categories.TECH2, M28UnitInfo.refCategoryLandFactory * categories.TECH3, categories.COMMAND, M28UnitInfo.refCategoryTML, M28UnitInfo.refCategoryEngineer, iSpecialHQCategory }
                end

                tCategoryAndEngineerTables[2] = { { M28Engineer.refActionBuildQuantumOptics, M28Engineer.refActionBuildHive, M28Engineer.refActionBuildT3Radar, M28Engineer.refActionBuildGameEnder, M28Engineer.refActionBuildLandExperimental, M28Engineer.refActionBuildSecondExperimental, M28Engineer.refActionNavalSpareAction, M28Engineer.refActionBuildT2Sonar, M28Engineer.refActionBuildThirdPower, M28Engineer.refActionBuildSecondAirFactory, M28Engineer.refActionBuildSecondLandFactory, M28Engineer.refActionBuildLandFactory, M28Engineer.refActionSAMCreep, M28Engineer.refActionBuildNavalFactory, M28Engineer.refActionBuildAirFactory, M28Engineer.refActionBuildT1Sonar, M28Engineer.refActionBuildT2Radar, M28Engineer.refActionBuildT1Radar, M28Engineer.refActionBuildQuantumGateway, M28Engineer.refActionBuildSecondPower, M28Engineer.refActionManageGameEnderTemplate, M28Engineer.refActionBuildShield, M28Engineer.refActionBuildSecondShield, M28Engineer.refActionUpgradeBuilding, M28Engineer.refActionBuildPower },
                                                  { M28Engineer.refActionBuildExperimental, M28Engineer.refActionAssistAirFactory, M28Engineer.refActionAssistNavalFactory, M28Engineer.refActionAssistUpgrade, M28Engineer.refActionBuildEnergyStorage,  M28Engineer.refActionBuildT3MassFab, M28Engineer.refActionBuildSecondMassStorage, M28Engineer.refActionAssistMexUpgrade, M28Engineer.refActionSpare },
                                                  { M28Engineer.refActionBuildMassStorage, M28Engineer.refActionBuildAirStaging, M28Engineer.refActionBuildTML, M28Engineer.refActionFortifyFirebase, M28Engineer.refActionBuildSMD, M28Engineer.refActionBuildEmergencyArti }}
            ...
            if not(bImminentThreat) then
                if M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.3 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] >= 200 then
                    ...
                    tCategoryAndEngineerTables[2] = { { M28Engineer.refActionBuildQuantumOptics, M28Engineer.refActionBuildT3MassFab, M28Engineer.refActionBuildHive, M28Engineer.refActionBuildT3Radar, M28Engineer.refActionBuildGameEnder, M28Engineer.refActionBuildLandExperimental, M28Engineer.refActionBuildSecondExperimental, M28Engineer.refActionNavalSpareAction, M28Engineer.refActionBuildT2Sonar, M28Engineer.refActionBuildT1Sonar, M28Engineer.refActionBuildT2Radar, M28Engineer.refActionBuildT1Radar, M28Engineer.refActionBuildExperimental, M28Engineer.refActionBuildQuantumGateway, M28Engineer.refActionBuildEnergyStorage, M28Engineer.refActionBuildAirStaging, M28Engineer.refActionManageGameEnderTemplate, M28Engineer.refActionBuildShield, M28Engineer.refActionBuildSecondShield, M28Engineer.refActionBuildThirdPower, M28Engineer.refActionBuildSecondAirFactory, M28Engineer.refActionBuildAirFactory, M28Engineer.refActionBuildSecondLandFactory, M28Engineer.refActionSAMCreep, M28Engineer.refActionBuildNavalFactory, M28Engineer.refActionAssistMexUpgrade, M28Engineer.refActionUpgradeBuilding, M28Engineer.refActionAssistUpgrade },
                                                      ...
                                                      { M28Engineer.refActionBuildLandFactory, M28Engineer.refActionBuildSMD, M28Engineer.refActionBuildEmergencyArti, iSpecialNonPowerNonT1MexEngiAction } }
                else
            ...
            if M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] <= 100 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] <= 16 + 10 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] * M28Team.tTeamData[iTeam][M28Team.refiHighestBrainBuildMultiplier] then
        ...
        --LOG('TEMPCODE bImminentThreat='..tostring(bImminentThreat)..'; size of tCategoryAndEngineerTables[1]='..table.getn(tCategoryAndEngineerTables[1])..'; Time='..GetGameTimeSeconds())
                            ...
                            elseif iCategoryRef == iSpecialSurplusUpgradeCategory then
                                ...
                                if M28Conditions.IsTableOfUnitsStillValid(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes]) then
                                    ...
                                    if iMexesToPause > 0 and (M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] >= 14 or (M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] > 10 and M28Conditions.GetHighestOtherTeamT3MexCount(iTeam) > M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3])) then
                                        ...
                                        elseif M28Map.iMapSize >= 1000 and M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] > 0 then iMexPerPlayer = 0.5 + (oBrain[refiGrossMassBaseIncome] / 6)*0.4
                                    ...
                                    if M28UnitInfo.IsUnitValid(oUnit) and oUnit:GetFractionComplete() == 1 then  --Only consider unit if it has been constructed
                                        if not(bPauseNotUnpause) then
                                            ...
                                            elseif iCategoryRef == M28UnitInfo.refCategoryEngineer then
                                                ...
                                                for iActionCount, iActionRef in tEngineerActionSubtable do
                                                    if iActionRef == oUnit[M28Engineer.refiAssignedAction] or iActionRef == iSpecialNonPowerNonT1MexEngiAction then
                                                        bApplyActionToUnit = true
                                                        ...
                                                        if oUnit[M28Engineer.refiAssignedAction] == M28Engineer.refActionBuildPower and
                                                                (M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.7 or iActionRef == iSpecialNonPowerNonT1MexEngiAction --cond 1b
                                                        ...
                                                        elseif iActionRef == iSpecialNonPowerNonT1MexEngiAction and (oUnit[M28Engineer.refiAssignedAction] == M28Engineer.refActionBuildMex or oUnit:GetWorkProgress() >= 0.99 or ((oUnit:GetFocusUnit().UnitId and (EntityCategoryContains(M28UnitInfo.refCategoryPower, oUnit:GetFocusUnit().UnitId)) and (oUnit:GetWorkProgress() > 0.4 or EntityCategoryContains(categories.TECH1, oUnit:GetFocusUnit().UnitId))))) then
                                                            bApplyActionToUnit = false
                                                            if oUnit:GetFocusUnit().UnitId and (oUnit:GetWorkProgress() > 0.4 or oUnit:GetFocusUnit():GetFractionComplete() > 0.4) and EntityCategoryContains(M28UnitInfo.refCategoryPower * categories.TECH1, oUnit:GetFocusUnit().UnitId) and (oUnit[M28UnitInfo.refbPaused] or (oUnit.IsPaused and oUnit:IsPaused())) then bUnpauseThisUnit = true end
                                                            if bDebugMessages == true then LOG(sFunctionRef..': Building or repairing power or t1 mex during a pause non power stall exception, work progress='..oUnit:GetWorkProgress()..'; Focus unit ID='..(oUnit:GetFocusUnit().UnitId or 'nil')..'; refbPaused='..tostring(oUnit[M28UnitInfo.refbPaused] or false)..'; IsPaused='..tostring(oUnit:IsPaused())..'; bUnpauseThisUnit='..tostring(bUnpauseThisUnit)) end
    ...
    if not(iTimeToWait) then
        if iMexTechLevel == 1 then
            ...
            --Eco slot in FAF - consider upgrading first mex relatively early on if lots of mexes in start zone and have a resource multiplier
                --Have further eco slot condition a bit lower down that is slower than this for where we dont have lots of mexes
            ...
            elseif tLZOrWZData[M28Map.subrefLZOrWZMexCount] >= 4 and tLZOrWZTeamData[M28Map.refiModDistancePercent] <= 0.25 and (aiBrain[refiOurHighestFactoryTechLevel] >= 2 or aiBrain[M28Overseer.refbPrioritiseHighTech]) and aiBrain[refiGrossMassBaseIncome] >= 5 and tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] >= tLZOrWZData[M28Map.subrefLZOrWZMexCount] - 2 and tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] + tLZOrWZTeamData[M28Map.subrefMexCountByTech][2] + tLZOrWZTeamData[M28Map.subrefMexCountByTech][3] >= math.min(5, tLZOrWZData[M28Map.subrefLZOrWZMexCount] - 1) and not(tLZOrWZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ]) and tLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat] == 0 and not(tLZOrWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ]) and not(tLZOrWZTeamData[M28Map.subrefbDangerousEnemiesInAdjacentWZ]) and GetGameTimeSeconds() >= 600 and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades]) then
                --Wait 1 sec to check updated mex count
                iTimeToWait = 5
            ...
            elseif (M28Map.iMapSize >= 1000 or aiBrain[M28Overseer.refbPrioritiseHighTech]) and tLZOrWZTeamData[M28Map.subrefLZbCoreBase] then
                ...
                if aiBrain[M28Overseer.refbPrioritiseHighTech] then iTimeToWait = 6.5 * 60
                else
                    iTimeToWait = 7 * 60
                end
            end
            if M28UnitInfo.GetUnitLifetimeCount(oMex) == 1 and (tLZOrWZTeamData[M28Map.refbBaseInSafePosition] or aiBrain[M28Overseer.refbPrioritiseHighTech]) and tLZOrWZData[M28Map.subrefLZOrWZMexCount] >= 3 then
                if bDebugMessages == true then LOG(sFunctionRef..': First mex in eco slot but dont have large number of mexes in this zone, will ensure time to wait is low, if not already lower, iTimeToWait pre adj='..iTimeToWait) end
                iTimeToWait = math.min(iTimeToWait, 250 - GetGameTimeSeconds())
            end
        elseif iMexTechLevel == 2 then
            if aiBrain[refiGrossMassBaseIncome] < 10 and (not(M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) or iMexesOnMap > 20 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount]) then
                if aiBrain[M28Overseer.refbPrioritiseHighTech] then
                    iTimeToWait = 7 * 60 + 4 * 60 * (10-aiBrain[refiGrossMassBaseIncome]) / 10
                else
        ...
        elseif bT3MexCanBeUpgraded and (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and iMexTechLevel == 3 then
            ...
            local iMexesInZone = (tLZData[M28Map.subrefLZOrWZMexCount] or 0)
            ...
            if iMexesInZone < 4 and aiBrain[refiGrossMassBaseIncome] <= 40 and iMexesOnMap > 60 then
                iTimeToWait = iTimeToWait + 2 * 60 * (4 - (tLZData[M28Map.subrefLZOrWZMexCount] or 0))
            elseif iMexesInZone < 4 and aiBrain[refiGrossMassBaseIncome] <= 20 and iMexesOnMap <= 60 then
                iTimeToWait = iTimeToWait + 1 * 60 * (4 - (tLZData[M28Map.subrefLZOrWZMexCount] or 0))
            end
        end
    ...
    if M28UnitInfo.IsUnitValid(oMex) then
        ...
        if not(M28Team.tTeamData[iTeam][M28Team.refbFocusOnT1Spam]) then
            ...
            if M28Conditions.SafeToUpgradeUnit(oMex) and not(M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refbPrioritiseProduction]) then
                            ...
                            if iPlateauOrZero > 0 then
                                if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentLandZones]) == false then
                                    for _, iAdjLZ in tLZOrWZData[M28Map.subrefAdjacentLandZones] do
                                        ...
                                        iMexesChecked = iMexesChecked + tAdjLZData[M28Map.subrefLZOrWZMexCount]
                                    end
                                end
                            else
                                if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentWaterZones]) == false then
                                    for _, iAdjWZ in tLZOrWZData[M28Map.subrefAdjacentWaterZones] do
                                        ...
                                        iMexesChecked = iMexesChecked + tAdjWZData[M28Map.subrefLZOrWZMexCount]
                                    end
                                end
                            end
                ...
                if not(M28Conditions.ZoneWantsT1Spam(tLZOrWZTeamData, iTeam)) and (bUpgradeDueToHowLongHadMex or not(M28Conditions.TeamHasLowMass(iTeam)) or iMexTechLevel == 3 or ((tLZOrWZData[M28Map.subrefLZOrWZMexCount] or 0) >= 3 and (tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] or 0) == 0 and (tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] > 0 or aiBrain[refiGrossMassBaseIncome] >= 8 or (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) or (((tLZOrWZTeamData[M28Map.refbBaseInSafePosition] and (not(M28Team.tTeamData[iTeam][M28Team.subrefbRushT3AirInAirSlot]) or aiBrain[refiOurHighestAirFactoryTech] >= 3)) or aiBrain[M28Overseer.refbPrioritiseHighTech]) and not(tLZOrWZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ]) and (tLZOrWZTeamData[M28Map.subrefMexCountByTech][3] > 0 or (tLZOrWZTeamData[M28Map.subrefMexCountByTech][2] > 0 and tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] + tLZOrWZTeamData[M28Map.subrefMexCountByTech][2] >= tLZOrWZData[M28Map.subrefLZOrWZMexCount])))))) then

                    --Are there enough mexes that we want to consider upgrading?
                    if bDebugMessages == true then
                        LOG(sFunctionRef..': Safe to upgrade, will proceed if enoguh mexes in this zone to support an upgrade, or dealing with a 2 mex plateau, tLZOrWZData[M28Map.subrefLZOrWZMexCount]='..tLZOrWZData[M28Map.subrefLZOrWZMexCount]..'; Plateau mex count='..M28Map.tAllPlateaus[iPlateauOrZero][M28Map.subrefPlateauTotalMexCount]..'; tLZOrWZTeamData[M28Map.refiModDistancePercent]='..tLZOrWZTeamData[M28Map.refiModDistancePercent]..'; Team upgrading mex count='..table.getn(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes] or {})..'; M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass]='..M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass])
                    ...
                    if iMexTechLevel == 3 or
                        ...
                        if (iMexTechLevel ==3 and (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive)) or (M28Conditions.TeamHasLowMass(iTeam) or (tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] or 0) == 0 and not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingEnergy]) and
                                (not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass]) or
                                        ...
                                        (tLZOrWZData[M28Map.subrefLZOrWZMexCount] >= 3 and table.getn(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes]) < M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] and aiBrain[refiGrossMassBaseIncome] >= 3.5 * iMexTechLevel and (iMexTechLevel == 1 or  table.getn(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes]) < M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] * 0.5)) or
                                        ((tLZOrWZData[M28Map.subrefLZOrWZMexCount] >= 4 or tLZOrWZData[M28Map.subrefLZbCoreBase]) and M28Conditions.GetHighestOtherTeamT3MexCount(iTeam) > M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3])
                                )) then
        ...
        if not(EntityCategoryContains(categories.TECH1, oJustBuilt.UnitId)) or aiBrain[M28Overseer.refbPrioritiseHighTech] then
            ...
            if not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingEnergy]) or (iMexTechLevel >= 3 and aiBrain:GetEconomyStoredRatio('ENERGY') >= 0.95) then
                ...
                if M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refbPrioritiseProduction] and M28Conditions.TeamHasLowMass(iTeam) and (not(tLZOrWZTeamData[M28Map.refbBaseInSafePosition]) or tLZOrWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] or ((tLZOrWZTeamData[M28Map.subrefMexCountByTech][iMexTechLevel - 1] or 0) == 0) and not(M28Utilities.bQuietModActive)) then
                    ...
                    if iMexTechLevel >= 3 or
                            (iMexTechLevel > 1 or (tLZOrWZTeamData[M28Map.subrefLZbCoreBase] and tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] >= tLZOrWZData[M28Map.subrefLZOrWZMexCount] and tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] == 0) and (tLZOrWZData[M28Map.subrefLZOrWZMexCount] > 1 or tLZOrWZTeamData[M28Map.subrefLZbCoreBase]) and
                                ...
                                elseif M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subreftoLZOrWZAlliedUnits]) == false then --redundancy
                                    ...
                                    if M28Utilities.bQuietModActive then
                                        if iMexTechLevel <= 2 then
                                            ...
                                            --Tech 3 mex
                                            iMexCategory = M28UnitInfo.refCategoryT1Mex + M28UnitInfo.refCategoryT2Mex
                                            if (oJustBuilt:GetBlueprint().General.UpgradesTo or '') == '' then
                                                --Likely dealing with a t3.5 mex not a t3 mex, double-check
                                                local sUpgradesFrom = oJustBuilt:GetBlueprint().General.UpgradesFrom
                                                if sUpgradesFrom and EntityCategoryContains(categories.TECH3, sUpgradesFrom) then
                                                    iMexCategory = M28UnitInfo.refCategoryMex - categories[oJustBuilt.UnitId]
                                                end
                                            end
                                        end
                                    else
                                    ...
                                    if not(bAlreadyUpgraded) and M28Utilities.bQuietModActive and not((oJustBuilt:GetBlueprint().General.UpgradesTo or '') == '') and (tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] or 0) < 2 then
                                        UpgradeUnit(oJustBuilt, true)
                                        bAlreadyUpgraded = true
                                    end
                                end
...
function JustBuiltT2PlusPowerOrExperimentalInZone(oPGen)
    ...
    if tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] == 0 and (tLZOrWZData[M28Map.subrefLZOrWZMexCount] or 0) > 0 and tLZOrWZTeamData[M28Map.subrefMexCountByTech][3] < tLZOrWZData[M28Map.subrefLZOrWZMexCount] then
...
function ConsiderChangingCoreBase(aiBrain, iOriginalPlateau, iOriginalZone)
    --Assumed has been called after our air fac (generlaly) or last land fac in a core base or override base in the zone was destroyed, suggesting core base is in trouble
    local bDebugMessages = false if M28Profiler.bGlobalDebugOverride == true then   bDebugMessages = true end
    ...
    if aiBrain.M28AI then
        ...
        local iSValueWanted
        if bDebugMessages == true then LOG(sFunctionRef..': About to start loop for aiBrain='..aiBrain.Nickname..'; refiOurHighestAirFactoryTech='..aiBrain[refiOurHighestAirFactoryTech]..'; iOriginalPlateau='..iOriginalPlateau..'; iOriginalZone='..iOriginalZone..'; subrefiActiveM28BrainCount='..M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount]..'; Time='..GetGameTimeSeconds()..'; aiBrain[refbActiveCoreBaseReassessment]='..tostring(aiBrain[refbActiveCoreBaseReassessment] or false)) end
        if not(aiBrain[refbActiveCoreBaseReassessment]) then
            aiBrain[refbActiveCoreBaseReassessment] = true
            while aiBrain[refiOurHighestAirFactoryTech] == 0 do
                --Only consider changing if we lack land facts in our core base
                if bDebugMessages == true then LOG(sFunctionRef..': Start of loop, subrefiActiveM28BrainCount='..M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount]..'; Time='..GetGameTimeSeconds()) end
                if M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] <= 1 then
                    bLookForDifferentZone = true
                    if M28Utilities.IsTableEmpty(tOrigLZTeamData[M28Map.subreftoLZOrWZAlliedUnits]) then
                        if bDebugMessages == true then LOG(sFunctionRef..': Have no units in orig base so will look for alternative') end
                    else
                    ...
                    if bLookForDifferentZone then
                        iSValueWanted = (tOrigLZTeamData[M28Map.subrefLZSValue] or 0)
                        local toLandFactories = aiBrain:GetListOfUnits(M28UnitInfo.refCategoryLandFactory + M28UnitInfo.refCategoryAirFactory, false, true)
                        ...
                        if M28Utilities.IsTableEmpty(toLandFactories) == false then
                            ...
                            for iFactory, oFactory in toLandFactories do
                                ...
                                if not(tbZoneConsideredByPlateau[iCurPlateau][iCurZone]) then
                                    ...
                                    if bDebugMessages == true then LOG(sFunctionRef..': Considering iCurPlateau='..iCurPlateau..'; iCurZone='..iCurZone..'; iSValueWanted='..iSValueWanted..'; subrefLZSValue='..tCurLZTeamData[M28Map.subrefLZSValue]..'; subrefLZTThreatAllyCombatTotal='..tCurLZTeamData[M28Map.subrefLZTThreatAllyCombatTotal]..'; refiEnemyAirToGroundThreat='..tCurLZTeamData[M28Map.refiEnemyAirToGroundThreat]..'; subrefMexCountByTech='..repru(tCurLZTeamData[M28Map.subrefMexCountByTech])..'; tCurLZData[M28Map.subrefLZOrWZMexCount]='..tCurLZData[M28Map.subrefLZOrWZMexCount]..'; subrefTThreatEnemyCombatTotal='..tCurLZTeamData[M28Map.subrefTThreatEnemyCombatTotal]) end
                                    if tCurLZTeamData[M28Map.subrefLZSValue] > iSValueWanted and ((tCurLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 0) == 0 or tCurLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] * 5 < tCurLZTeamData[M28Map.subrefLZTThreatAllyCombatTotal]) and ((tCurLZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) == 0 or tCurLZTeamData[M28Map.refiEnemyAirToGroundThreat] * 10 < tCurLZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA]) and (iSValueWanted <= 50 or tCurLZTeamData[M28Map.subrefMexCountByTech][1] + tCurLZTeamData[M28Map.subrefMexCountByTech][2] + tCurLZTeamData[M28Map.subrefMexCountByTech][3] >= tCurLZData[M28Map.subrefLZOrWZMexCount]) then
            ...
            if iNewBasePlateau and iNewBaseLandZone then
                ...
                --Clear override flag from old (but not the actual core zone flag just in case we could get into a situation with no core zone)
                tOrigLZTeamData[M28Map.subrefbCoreBaseOverride] = nil
                if bDebugMessages == true then LOG(sFunctionRef..': have flagged for iNewBasePlateau='..iNewBasePlateau..'iNewBaseLandZone='..iNewBaseLandZone..' to be a core base, removed flag from iOriginalPlateau='..iOriginalPlateau..'; iOriginalZone='..iOriginalZone) end
            end
            if bDebugMessages == true then LOG(sFunctionRef..': End of monitoring, Time='..GetGameTimeSeconds()) end
            aiBrain[refbActiveCoreBaseReassessment] = false
        end
    end
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
end

    ...
    if sBlueprintToBuild == nil then
        ...
        if EntityCategoryContains(M28UnitInfo.refCategoryMex + M28UnitInfo.refCategoryHydro + M28UnitInfo.refCategoryMassStorage, sBlueprintToBuild) then
            ...
            if M28Utilities.IsTableEmpty(tPotentialBuildLocations) then
                ...
                if EntityCategoryContains(M28UnitInfo.refCategoryFixedShield * categories.TECH3, sBlueprintToBuild) and (M28Map.bIsCampaignMap or not(M28Map.refbCloseToUnitCap)) then iCycleSizeLimit = 4 end
...
function GetBPMinTechAndUnitForFixedShields(tLZTeamData, iTeam, bCoreZone, bHaveLowMass, bWantMorePower, bConsideringSecondShield, iLandZoneRef, iPlateau)
    ...
    if M28Utilities.IsTableEmpty(tLZTeamData[M28Map.reftoLZUnitWantingFixedShield]) == false then
        ...
        if M28Map.bIsCampaignMap and bHaveLowMass and GetGameTimeSeconds() <= 1200 and not(ScenarioInfo.Ariel and ScenarioInfo.UEF and ScenarioInfo.Colonies) then --First 20m
...
function ConsiderMinorLandZoneEngineerAssignment(tLZTeamData, iTeam, iPlateau, iLandZone, tEngineers)
    ...
    if iPlateau == 25 and (iLandZone == 23 or iLandZone == 20) and tLZTeamData[M28Map.subrefLZCoreExpansion] == nil then bDebugMessages = true end

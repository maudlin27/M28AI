...
function GetACUEarlyGameOrders(aiBrain, oACU)
    ...
    if not(bQueuedEnhancement) then
        ...
        if iCurLandFacs >= 1 and iPlateauOrZero > 0 and (iPlateauOrZero == 0 or not(bACUWantsToRun) or M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefTEnemyUnits]) == false) and AttackNearestEnemyWithACU(iPlateauOrZero, iLZOrWZ, tLZOrWZData, tLZOrWZTeamData, oACU, iSearchDistance) then
            ...
            if iCurLandFacs >= 2 and GetGameTimeSeconds() >= 240 / aiBrain[M28Economy.refiBrainBuildRateMultiplier] and aiBrain[M28Economy.refiGrossEnergyBaseIncome] >= 20 and aiBrain[M28Economy.refiGrossMassBaseIncome] >= 1.4 and (oACU[refiUpgradeCount] > 0 or (aiBrain[M28Economy.refiGrossEnergyBaseIncome] >= 24 and aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryAirFactory) > 0) or bACUWantsToRun) then
                if bDebugMessages == true then LOG(sFunctionRef..': Will clear initial build order flag as are nearby enemies so not getting a chance to build everything but have several factories and decent eco, setting refbDoingInitialBuildOrder to be false') end
        ...
        if bProceedWithLogic then
            --Get nearest enemy DF or IF unit if almost in range of us
            local oNearestEnemy
            local iClosestEnemyLessRangeDist = 10
            local iCurEnemyLessRangeDist
            function ConsiderEnemies(tEnemyUnits)
                if tEnemyUnits[1] then
                    for iUnit, oUnit in tEnemyUnits do
                        if not(oUnit.Dead) and (oUnit[M28UnitInfo.refiCombatRange] or 0) >= 4 then
                            iCurEnemyLessRangeDist = M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oACU:GetPosition()) - oUnit[M28UnitInfo.refiCombatRange]
                            if iCurEnemyLessRangeDist < iClosestEnemyLessRangeDist and not(EntityCategoryContains(M28UnitInfo.refCategoryLandScout - M28UnitInfo.refCategoryCombatScout, oUnit.UnitId)) then
                                iClosestEnemyLessRangeDist = iCurEnemyLessRangeDist
                                oNearestEnemy = oUnit
                            end
                        end
                    end
            ...
            ConsiderEnemies(tLZOrWZTeamData[M28Map.subrefTEnemyUnits])
            if not(oNearestEnemy) and M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefLZAdjacentLandZones]) == false then
                for _, iAdjLZ in tLZOrWZData[M28Map.subrefLZAdjacentLandZones] do
                    local tAdjLZTeamData = M28Map.tAllPlateaus[iPlateauOrZero][M28Map.subrefPlateauLandZones][iAdjLZ][M28Map.subrefLZTeamData][iTeam]
                    ConsiderEnemies(tAdjLZTeamData[M28Map.subrefTEnemyUnits])
                end
            end

            --Are we already building something?
            if bDebugMessages == true then LOG(sFunctionRef..': ACU unit state='..M28UnitInfo.GetUnitState(oACU)..'; oNearestEnemy='..(oNearestEnemy.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oNearestEnemy) or 'nil')) end
            --If are repariing then check we have a focus unit due to bug where unit state can show as repairing when ACU is idling
            if not(oACU:IsUnitState('Building')) and (not(oACU:IsUnitState('Repairing')) or (oACU:GetWorkProgress() == 0 and not(M28UnitInfo.IsUnitValid(oACU:GetFocusUnit())))) and not(oACU:IsUnitState('Capturing')) and (aiBrain:GetEconomyStoredRatio('MASS') <= 0.95 or not(oACU:IsUnitState('Reclaiming'))) then
                if bDebugMessages == true and oNearestEnemy then
                    LOG(sFunctionRef..': oNearestEnemy y height='..oNearestEnemy:GetPosition()[2]..'; M28Map.iMapWaterHeight='..M28Map.iMapWaterHeight..'; Is enemy underwater='..tostring(M28UnitInfo.IsUnitUnderwater(oNearestEnemy))..'; iPlateauOrZero='..iPlateauOrZero)
                end
                if oNearestEnemy and (not(oNearestEnemy:GetPosition()[2] < M28Map.iMapWaterHeight) or (not(M28UnitInfo.IsUnitUnderwater(oNearestEnemy)) and iPlateauOrZero > 0)) then --Attack enemy that is almost in range of us
                    if AttackNearestEnemyWithACU(iPlateauOrZero, iLZOrWZ, tLZOrWZData, tLZOrWZTeamData, oACU, math.max(10, iClosestEnemyLessRangeDist + oNearestEnemy[M28UnitInfo.refiCombatRange])) then
                        if bDebugMessages == true then LOG(sFunctionRef..': Attacking enemy with normal logic') end
                    elseif oACU[M28UnitInfo.refbLastShotBlocked] or iPlateauOrZero == 0 then
                        M28Orders.IssueTrackedMove(oACU, oNearestEnemy:GetPosition(), 5, false, 'EarlyEnBlM', false)
                    else
                        M28Orders.IssueTrackedAttackMove(oACU, oNearestEnemy:GetPosition(), 5, false, 'EarlyEnAM', false)
                    end
                else
                    ...
                    --Lifetime check - want 1 higher than nubmer we want to ahve built
                    if not(aiBrain[M28Map.refbCanPathToEnemyBaseWithLand]) and aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryNavalFactory) < 3 and (aiBrain[M28Economy.refiOurHighestNavalFactoryTech] == 0 or M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryMobileNavalSurface + M28UnitInfo.refCategorySubmarine) < 4 or (iApproachingEnemyACUThreat >= 700 and aiBrain:GetCurrentUnits(M28UnitInfo.refCategorySubmarine) == 0 and M28Map.iMapSize <= 512 and iApproachingEnemyACUThreat <= 1400 and (tLZOrWZTeamData[M28Map.subrefWZThreatAlliedAntiNavy] or 0) <= 200)) and (iPlateauOrZero == 0 or M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentWaterZones]) == false) and aiBrain[M28Economy.refiOurHighestLandFactoryTech] >= 1 and (aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryFactory) >= 2 or M28Map.iMapSize <= 256) and aiBrain[M28Economy.refiGrossEnergyBaseIncome] >= 12 * aiBrain[M28Economy.refiBrainResourceMultiplier] and (aiBrain[M28Economy.refiOurHighestAirFactoryTech] >= 1 or M28Map.iMapSize <= 500) then
                        ...
                        if iPlateauOrZero == 0 then
                            ...
                            if (M28Team.tTeamData[iTeam][M28Team.refiPriorityPondValues][M28Map.tiPondByWaterZone[iLZOrWZ]] or 0) >= 15 then
                                ...
                                elseif aiBrain[M28Economy.refiOurHighestNavalFactoryTech] == 0 then
                                    ...
                                    bGettingNavalFacInstead = true
                                    --Assist naval factory if havent built many naval units and it is nearby
                                elseif M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryMobileNavalSurface + M28UnitInfo.refCategorySubmarine) < 2 then
                                    --Is there a naval fac in this WZ?
                                    local oNearbyNavalFacToAssist, iCurDist
                                    local iNearestNavalFacDist = 50
                                    if M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subreftoLZOrWZAlliedUnits]) == false then
                                        local tFactories = EntityCategoryFilterDown(M28UnitInfo.refCategoryNavalFactory, tLZOrWZTeamData[M28Map.subreftoLZOrWZAlliedUnits])
                                        if M28Utilities.IsTableEmpty(tFactories) == false then
                                            for iUnit, oUnit in tFactories do
                                                if M28UnitInfo.IsUnitValid(oUnit) then
                                                    ...
                                                    if iCurDist < iNearestNavalFacDist then
                                                        iNearestNavalFacDist = iCurDist
                                                        oNearbyNavalFacToAssist = oUnit
                                                    end
                                                end
                                            end
                                        end
                                    end
                                    if bDebugMessages == true then LOG(sFunctionRef..': oNearbyNavalFacToAssist='..(oNearbyNavalFacToAssist.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oNearbyNavalFacToAssist) or 'nil')) end
                                    if oNearbyNavalFacToAssist then
                                        if oNearbyNavalFacToAssist:GetFractionComplete() < 1 then
                                            M28Orders.IssueTrackedRepair(oACU, oNearbyNavalFacToAssist, false, 'ACUAstNR', false)
                                        else
                                            M28Orders.IssueTrackedGuard(oACU, oNearbyNavalFacToAssist, false, 'ACUAstNG', false)
                                        end
                                        bGettingNavalFacInstead = true
                                    end
                                end
                            ...
                            elseif aiBrain[M28Economy.refiGrossEnergyBaseIncome] < iMinEnergyPerTickWanted then
                                ...
                                if not(bHydroBuildOrder) then
                                    ...
                                    elseif aiBrain[M28Economy.refiGrossEnergyBaseIncome] < math.max(6, 2 * (tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] + tLZOrWZTeamData[M28Map.subrefMexCountByTech][2] * 3 + tLZOrWZTeamData[M28Map.subrefMexCountByTech][3] * 9)) * iResourceMod then
                                        ...
                                        elseif (M28Utilities.bQuietModActive or M28Utilities.bLoudModActive) and (aiBrain[M28Economy.refiNetEnergyBaseIncome] >= 4 or aiBrain:GetEconomyTrend('ENERGY') >= 4 or (aiBrain:GetEconomyStoredRatio('ENERGY') >= 0.65 and math.max(aiBrain[M28Economy.refiNetEnergyBaseIncome], aiBrain:GetEconomyTrend('ENERGY')) >= 2.5)) and aiBrain[M28Economy.refiGrossEnergyBaseIncome] >= 18 and aiBrain[M28Economy.refiNetMassBaseIncome] <= -0.2 and aiBrain:GetEconomyStoredRatio('ENERGY') >= 0.4 and aiBrain:GetEconomyStored('MASS') <= 1 then
                                            if bDebugMessages == true then LOG(sFunctionRef..': Finish build order early as we cant spend the mass, setting refbDoingInitialBuildOrder to be false') end
                                            oACU[refbDoingInitialBuildOrder] = false
                                    ...
                                    if M28Utilities.IsTableEmpty(oACU[M28Orders.reftiLastOrders]) and oACU[refbDoingInitialBuildOrder] then
                                        ...
                                        if M28Utilities.IsTableEmpty(oACU[M28Orders.reftiLastOrders]) then
                                            ...
                                            if M28Utilities.IsTableEmpty(oACU[M28Orders.reftiLastOrders]) then
                                                oACU[refbDoingInitialBuildOrder] = false
                                                if bDebugMessages == true then LOG(sFunctionRef..': No orders so setting refbDoingInitialBuildOrder to be false') end
                                            end
                                        end
                                    end
                                        ...
                                        if GetGameTimeSeconds() <= 180 and aiBrain[M28Economy.refiGrossEnergyBaseIncome] < 10 * iResourceMod and M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefHydroLocations]) == false then
                                            ...
                                            if not(M28Conditions.DoesACUHaveValidOrder(oACU)) then
                                                ...
                                                if not(M28Conditions.DoesACUHaveValidOrder(oACU)) then
                                                    ACUActionBuildPower(aiBrain, oACU)
                                                    if not(M28Conditions.DoesACUHaveValidOrder(oACU)) then
                                                        if not(M28Map.bIsCampaignMap) or GetGameTimeSeconds() <= 540 then
                                                            oACU[refbDoingInitialBuildOrder] = false
                                                            if bDebugMessages == true then LOG(sFunctionRef..': No valid order so setting refbDoingInitialBuildOrder to be false') end
                                                        end
                                                    end
                                                end
                                            end
                                ...
                                if iCurLandFactories + aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryAirFactory) < iFactoryCap then
                                    ...
                                    elseif aiBrain:GetEconomyStoredRatio('MASS') >= 0.6 and (aiBrain[M28Economy.refiNetMassBaseIncome] > 0.2 or aiBrain:GetEconomyStoredRatio('MASS') >= 0.9) and M28Conditions.HaveLowPower(iTeam) and aiBrain[M28Economy.refiOurHighestFactoryTechLevel] <= 1 then
                                        ...
                                        ACUActionBuildPower(aiBrain, oACU)
                                        --If lots of reclaim, have some mass, and no energy, then also get power
                                    elseif aiBrain:GetEconomyStored('MASS') >= 60 and aiBrain:GetEconomyStored('ENERGY') < 60 then
                                        if bDebugMessages == true then LOG(sFunctionRef..': Have more mass stored than energy so will get power') end
                                        ACUActionBuildPower(aiBrain, oACU)
                                    else
                                        local iMexInLandZone = 0
                                        if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefLZOrWZMexLocations]) == false then iMexInLandZone = table.getn(tLZOrWZData[M28Map.subrefLZOrWZMexLocations]) end
                                            ...
                                            if M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies]) == false and iPlateauOrZero > 0 then
                                                ...
                                                if iApproachingACUThreat >= 600 then
                                                    if M28Conditions.CloseToEnemyUnit(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftoNearestDFEnemies], 15, iTeam, true, nil, nil, nil, nil, nil) then
                                                        bAbortLogic = true
                                                        oACU[refbDoingInitialBuildOrder] = false
                                                        if bDebugMessages == true then LOG(sFunctionRef..': Are close to enemy DF unit so will attack enemy and end early game order, setting refbDoingInitialBuildOrder to be false') end
                                                    ...
                                                    elseif AttackNearestEnemyWithACU(iPlateauOrZero, iLZOrWZ, tLZOrWZData, tLZOrWZTeamData, oACU, 90)  then
                                                        if bDebugMessages == true then LOG(sFunctionRef..': will attack nearest enemy and end early game order, so setting refbDoingInitialBuildOrder to be false') end
                                                        bAbortLogic = true
                                                        oACU[refbDoingInitialBuildOrder] = false
                                                    end
                                                end
                                            ...
                                            if not(bAbortLogic) then
                                                ...
                                                elseif M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefMexUnbuiltLocations]) == false and tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] + tLZOrWZTeamData[M28Map.subrefMexCountByTech][2] + tLZOrWZTeamData[M28Map.subrefMexCountByTech][3] < iMexInLandZone and (not(M28Overseer.bNoRushActive) or not(M28Conditions.NoRushPreventingHydroOrMex(tLZOrWZData, true))) then
                                                    ...
                                                    if bTryBuildingMex then
                                                        ...
                                                        if bDebugMessages == true then LOG(sFunctionRef..': Building mex means we take a detour so leave it for engineers and end our initial build order, setting refbDoingInitialBuildOrder to be false') end
                                                        oACU[refbDoingInitialBuildOrder] = false
                                                    end
                                                elseif aiBrain[M28Economy.refiGrossEnergyBaseIncome] < 20 and aiBrain:GetEconomyStoredRatio('ENERGY') < 0.99 and (aiBrain[M28Economy.refiGrossEnergyBaseIncome] < 15 or aiBrain:GetEconomyStoredRatio('MASS') >= 0.1) then
                                                    ...
                                                    oACU[refbDoingInitialBuildOrder] = false
                                                    if bDebugMessages == true then LOG(sFunctionRef..': Finishing initial build order, setting refbDoingInitialBuildOrder to be false') end
                                                end
                                            end
                                        end
                                    end
                        ...
                        if (iLZOrWZ or 0) == 0 then
                            ...
                            if iCurNavalFactories == 0 then
                                ...
                                elseif M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefMexUnbuiltLocations]) == false and (not(M28Overseer.bNoRushActive) or not(M28Conditions.NoRushPreventingHydroOrMex(tLZOrWZData, true))) then
                                    ...
                                    oACU[refbDoingInitialBuildOrder] = false
                                    if bDebugMessages == true then LOG(sFunctionRef..': No mex or hydro to build, setting refbDoingInitialBuildOrder to be false') end
                                end
                            end
                        end
                ...
                if oNearestEnemy and (not(oNearestEnemy:GetPosition()[2] < M28Map.iMapWaterHeight) or (not(M28UnitInfo.IsUnitUnderwater(oNearestEnemy)) and iPlateauOrZero > 0)) and (not(M28UnitInfo.IsUnitValid(oACUFocusUnit)) or oACU:GetWorkProgress() >= 0.8) then --Attack enemy that is almost in range of us
                    if bDebugMessages == true then LOG(sFunctionRef..': Will ahve ACU attack nearby enemy instead') end
                    if AttackNearestEnemyWithACU(iPlateauOrZero, iLZOrWZ, tLZOrWZData, tLZOrWZTeamData, oACU, math.max(10, iClosestEnemyLessRangeDist + oNearestEnemy[M28UnitInfo.refiCombatRange])) then
                        if bDebugMessages == true then LOG(sFunctionRef..': Attacking enemy with normal logic') end
                    elseif oACU[M28UnitInfo.refbLastShotBlocked] then
                        M28Orders.IssueTrackedMove(oACU, oNearestEnemy:GetPosition(), 5, false, 'EarlyEnBlM', false)
                    else
                        M28Orders.IssueTrackedAttackMove(oACU, oNearestEnemy:GetPosition(), 5, false, 'EarlyEnAM', false)
                    end
                else
                    local iPlateauOrPond = iPlateauOrZero
                    if iPlateauOrPond == 0 then iPlateauOrPond = M28Map.tiPondByWaterZone[iLZOrWZ] end
                    ...
                    if not(M28UnitInfo.IsUnitValid(oACUFocusUnit)) and aiBrain:GetEconomyStoredRatio('MASS') >= 0.95 and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subreftoLZOrWZAlliedUnits]) == false and M28Utilities.IsTableEmpty(EntityCategoryFilterDown(M28UnitInfo.refCategoryFactory, tLZOrWZTeamData[M28Map.subreftoLZOrWZAlliedUnits])) == false then
                        ...
                        --Stalling mass and wreck in our build range - reclaim it
                    elseif aiBrain:GetEconomyStored('MASS') <= 5 and oACU:GetWorkProgress() < 0.9 and (tLZOrWZData[M28Map.subrefTotalSignificantMassReclaim] or 0) >= 40 and M28Engineer.GetEngineerToReclaimNearbyArea(oACU, 1, tLZOrWZTeamData, iPlateauOrPond, iLZOrWZ, false, true, 15, iPlateauOrZero == 0, true) then
                        if bDebugMessages == true then LOG(sFunctionRef..': Stalling mass so switching to get reclaim') end
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': End of code, is ACU table of last orders empty='..tostring(M28Utilities.IsTableEmpty(oACU[M28Orders.reftiLastOrders]))..'; Does ACU have valid order='..tostring(M28Conditions.DoesACUHaveValidOrder(oACU))..'; refbDoingInitialBuildOrder='..tostring(oACU[refbDoingInitialBuildOrder] or false)) end
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
end
...
function GetUpgradePathForACU(oACU, bWantToDoTeleSnipe)
    ...
    elseif oACU[refbStartedUnderwater] then
        ...
        --Cybran ACU on naval non-campaign map where enemy isnt at T3 yet
    elseif oACU.UnitId == 'url0001' and not(aiBrain[M28Map.refbCanPathToEnemyBaseWithLand]) and not(M28Map.bIsCampaignMap) and aiBrain[M28Map.refbCanPathToEnemyBaseWithAmphibious] and M28Team.tTeamData[aiBrain.M28Team][M28Team.subrefiHighestFriendlyNavalFactoryTech] < 3 and M28Team.tTeamData[aiBrain.M28Team][M28Team.subrefiHighestEnemyNavyTech] < 3 then
        ...
        for iUpgradeWanted, sUpgradeWanted in oACU[reftPreferredUpgrades] do
            ...
            elseif bCheckForRestrictions then
                ...
                if tRestrictedEnhancements[sUpgradeWanted] then
                    ...
                    bInvalidUpgrade = true
                elseif oBP.Enhancements[sUpgradeWanted].Prerequisite and tRestrictedEnhancements[oBP.Enhancements[sUpgradeWanted].Prerequisite] then
        ...
        if bDontConsiderAnyUpgrades then
            ...
            if bDebugMessages == true then
                ...
                if sUpgradeWanted then
                    ...
                    LOG(sFunctionRef..': Is this restricted='..tostring(import("/lua/enhancementcommon.lua").GetRestricted()[sUpgradeWanted]))
                end
            end
        end
    end
    --Redudnancy for missions where they add enhancement restrictions after we have chosen our upgrade path
    if sUpgradeWanted and import("/lua/enhancementcommon.lua").GetRestricted()[sUpgradeWanted] then
        sUpgradeWanted = nil
        if M28Utilities.IsTableEmpty(oACU[reftPreferredUpgrades]) == false then
            local tRestrictions = import("/lua/enhancementcommon.lua").GetRestricted()
            for iCurUpgradeWanted = table.getn(oACU[reftPreferredUpgrades]), 1, -1 do
                if tRestrictions[oACU[reftPreferredUpgrades]] then
                    table.remove(oACU[reftPreferredUpgrades], iCurUpgradeWanted)
                end
            end
        end
    end
...
function DoesACUWantToRun(iPlateau, iLandZone, tLZData, tLZTeamData, oACU)
    ...
    elseif tLZTeamData[M28Map.subrefLZbCoreBase] and (M28UnitInfo.GetUnitHealthPercent(oACU) >= 0.2 and (oACU[refbUseACUAggressively] or (M28UnitInfo.GetUnitHealthPercent(oACU) >= 0.4 and ((oACU[refiUpgradeCount] or 0) > 0 or not(M28Team.tTeamData[iTeam][M28Team.refbEnemyHasUpgradedACU])) and (M28UnitInfo.GetUnitHealthPercent(oACU) >= 0.75 or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZData[M28Map.subrefMidpoint]) <= 70)) or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZData[M28Map.subrefMidpoint]) <= 15)) and ((M28UnitInfo.GetUnitHealthPercent(oACU) >= 0.5 or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZTeamData[M28Map.reftClosestEnemyBase]) > 5 + M28Utilities.GetDistanceBetweenPositions(tLZData[M28Map.subrefMidpoint],tLZTeamData[M28Map.reftClosestEnemyBase])))
            --Check we have a base left - being decent level of combat units, or owe have fac or PD that is completed in zone
            and (M28UnitInfo.GetUnitHealthPercent(oACU) >= 0.95 or tLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] < 300 or tLZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] > 200 + M28UnitInfo.GetCombatThreatRating({ oACU }, false, false) or M28Conditions.GetNumberOfConstructedUnitsMeetingCategoryInZone(tLZTeamData, M28UnitInfo.refCategoryFactory + M28UnitInfo.refCategoryPD + M28UnitInfo.refCategoryExperimentalLevel) > 0)
                    ...
                    and (iHealthPercent <= 0.3 or iMaxShield == 0 or iCurShield / iMaxShield + iHealthPercent <= iPercentMod)
                     then
                        ...
                        if iPlateau > 0 and iLandZone > 0 then
                            ...
                            if not(bWantToRun) then
                                ...
                                if (iEnemyAirToGroundNearbyThreat > (iFriendlyAAThreat or 0) * 2.5 or iEnemyAirToGroundNearbyThreat >= 1000) and not(bAdjacentToCoreLZ) and (iMaxShield == 0 or tLZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] < tLZTeamData[M28Map.refiEnemyAirToGroundThreat] * 0.5) and (iEnemyAirToGroundNearbyThreat > 150 or not(oACU[refbUseACUAggressively])) and (iEnemyAirToGroundNearbyThreat >= 300 or M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] >= 400 or iDistToFriendlyBase >= 300 or iPercentageToFriendlyBase > 0.5 or M28UnitInfo.GetUnitHealthPercent(oACU) <= 0.75) then
                                    ...
                                    if bHighHealthWithStealth and (oACU[refbUseACUAggressively] or not(M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar])) then
                                        ...
                                        if bDebugMessages == true then LOG(sFunctionRef..': iEnemyNearbyThreat='..iEnemyNearbyThreat..'; iACUThreat='..iACUThreat..'; iEnemyMobileNearbyDFThreat='..iEnemyMobileNearbyDFThreat..'; bAgainstEnemyACUAndMightWin='..tostring(bAgainstEnemyACUAndMightWin or false)..'; iPercentageToFriendlyBase='..iPercentageToFriendlyBase..'; bAdjacentToCoreLZ='..tostring(bAdjacentToCoreLZ)..'; iAllyNearbyThreat='..iAllyNearbyThreat..'; iMaxLRThreat='..iMaxLRThreat..'; ACU cur health+shield='..M28UnitInfo.GetUnitCurHealthAndShield(oACU)..'; ACU cur health+shield%='..M28UnitInfo.GetUnitHealthAndShieldPercent(oACU)..'; GetHighestOtherTeamT2AndT3MexCount='..M28Conditions.GetHighestOtherTeamT2AndT3MexCount(iTeam)..'; our refiMexCountByTech: T3='..(M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3] or 0)..'; T2='..(M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][2] or 0)..'; Row1 Cond='..tostring(iEnemyNearbyThreat > math.min(math.max(iACUThreat * 2, iACUThreat * 0.75 + iAllyNearbyThreat), 4000 + 2000 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount]) and (iEnemyNearbyThreat > math.max(iACUThreat * 3, iACUThreat + iAllyNearbyThreat) or ((not(bAgainstEnemyACUAndMightWin) and iPercentageToFriendlyBase >= 0.35 and not(bAdjacentToCoreLZ) and not(oACU[refbUseACUAggressively])))))..'; Row2 Cond='..tostring((iACUThreat < iEnemyMobileNearbyDFThreat * 1.1 or iACUThreat * 3 < iEnemyNearbyThreat))..'; Row 3 Cond='..tostring((iACUThreat < 1500 or oACU[refiUpgradeCount] <= 2 or M28UnitInfo.GetUnitHealthAndShieldPercent(oACU) <= 0.9 or M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or M28UnitInfo.GetUnitCurHealthAndShield(oACU) <= 18000 or GetGameTimeSeconds() >= 1500 or tLZTeamData[M28Map.refiModDistancePercent] >= 0.75 or (tLZTeamData[M28Map.refiModDistancePercent] >= 0.3 + 0.1 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount])))..'; Row4 Cond='..tostring((iACUThreat < 1700 or oACU[refiUpgradeCount] < 3 or M28UnitInfo.GetUnitHealthAndShieldPercent(oACU) <= 0.95 or tLZTeamData[M28Map.refiModDistancePercent] >= 0.55 or M28Map.iMapSize >=700 or M28Conditions.GetHighestOtherTeamT2AndT3MexCount(iTeam) * 1.3 + 1 < (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3] or 0) + (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][2] or 0)))) end
                                        --Run if enemy has a really large threat (regardless of if we think we can beat it) subject to some exceptions
                                        if iEnemyNearbyThreat > math.min(math.max(iACUThreat * 2, iACUThreat * 0.75 + iAllyNearbyThreat), 4000 + 2000 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount]) and (iEnemyNearbyThreat > math.max(iACUThreat * 3, iACUThreat + iAllyNearbyThreat) or ((not(bAgainstEnemyACUAndMightWin) and iPercentageToFriendlyBase >= 0.35 and not(bAdjacentToCoreLZ) and not(oACU[refbUseACUAggressively])))) --row1 cond
                                                and (iACUThreat < iEnemyMobileNearbyDFThreat * 1.1 or iACUThreat * 3 < iEnemyNearbyThreat) --row2 cond
                                                --In teamgames want ACU with shield to be more aggressive
                                                and (iACUThreat < 1500 or oACU[refiUpgradeCount] <= 2 or M28UnitInfo.GetUnitHealthAndShieldPercent(oACU) <= 0.9 or M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or M28UnitInfo.GetUnitCurHealthAndShield(oACU) <= 18000 or GetGameTimeSeconds() >= 1500 or tLZTeamData[M28Map.refiModDistancePercent] >= 0.75 or (tLZTeamData[M28Map.refiModDistancePercent] >= 0.3 + 0.1 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount])) --row3 cond
                                                and (iACUThreat < 1700 or oACU[refiUpgradeCount] < 3 or M28UnitInfo.GetUnitHealthAndShieldPercent(oACU) <= 0.95 or tLZTeamData[M28Map.refiModDistancePercent] >= 0.55 or M28Map.iMapSize >=700 or M28Conditions.GetHighestOtherTeamT2AndT3MexCount(iTeam) * 1.3 + 1 < (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3] or 0) + (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][2] or 0)) --row4 cond
                                        then
                                            ...
                                            --Increase ACU factor in teamgame or if behind on eco if have shield
                                            if oACU[refiUpgradeCount] >= 3 and iACUThreat >= 1500 and M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] == 0 and ((not(M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar]) and M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] >= 2) or M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] < 1.1 * aiBrain[M28Economy.refiBrainResourceMultiplier] * M28Conditions.GetEnemyTeamActualMassIncome(iTeam)) and ((oACU.MyShield.GetHealth and iHealthPercent >= 0.98 and M28UnitInfo.GetUnitHealthAndShieldPercent(oACU) >= 0.9) or (oACU:GetMaxHealth() >= 26000 and iHealthPercent >= 0.9)) then
                                            ...
                                            if bDebugMessages == true then LOG(sFunctionRef..': Will consider if should run, iACUThreat='..iACUThreat..'; iACUFactor='..iACUFactor..'; tLZTeamData[M28Map.subrefLZTThreatAllyCombatTotal]='..(tLZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] or 'nil')..'; iEnemyNearbyThreat='..iEnemyNearbyThreat..'; Ally mobile DF total='..(tLZTeamData[M28Map.subrefLZThreatAllyMobileDFTotal] or 0)..'; iAllyNearbyThreat='..iAllyNearbyThreat..'; iAllyNearbyThreatFactor='..iAllyNearbyThreatFactor) end

                                            --NOTE: subrefLZTThreatAllyCombatTotal includes the ACU threat
                                            if not(oACU[refbUseACUAggressively]) and (iACUThreat * iACUFactor + iAllyNearbyThreat * iAllyNearbyThreatFactor < iEnemyNearbyThreat) and iEnemyMobileNearbyDFThreat >= iACUThreat * iACUFactor
                                                    --Slightly increase ACU factor if have shield/lots of upgrade on a small map with low mod distance and not significantly ahead on eco
                                                    and (iACUThreat < 1700 or oACU[refiUpgradeCount] < 3 or tLZTeamData[M28Map.refiModDistancePercent] >= 0.55 or M28Map.iMapSize >=700 or iACUThreat * (iACUFactor + 0.3) + iAllyNearbyThreat < iEnemyNearbyThreat or M28UnitInfo.GetUnitHealthAndShieldPercent(oACU) <= 0.95 or M28Conditions.GetHighestOtherTeamT2AndT3MexCount(iTeam) * 1.3 + 1 < (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3] or 0) + (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][2] or 0))
                                            ...
                                            elseif iACUThreat * 10 < math.max(iAllyNearbyThreat, iEnemyNearbyThreat) and (iACUThreat < 2500 or M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar]) then
                                                bWantToRun = true
                                                if bDebugMessages == true then LOG(sFunctionRef..': Enemy or ally threat so great relative to ACU that too dangerous to risk ACU') end
                                            elseif not(bWantToRun) and not(oACU[refbUseACUAggressively]) and M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] and (iHealthPercent < 0.75 or iACUThreat * 7 < iEnemyNearbyThreat or (iACUThreat * 5 < iEnemyNearbyThreat and (iAllyNearbyThreat > iEnemyNearbyThreat or not(tLZTeamData[M28Map.subrefLZbCoreBase])))) and iACUThreat * math.min(1, iACUFactor) < iEnemyNearbyThreat * 1.25 and (iACUThreat * 5 < iEnemyNearbyThreat or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZTeamData[M28Map.reftClosestFriendlyBase]) >= 25)
                                            ...
                                            then
                                                ...
                                                if bDebugMessages == true then LOG(sFunctionRef..': Considering exception to wanting to run if gun ACU with high health, iACUThreat='..iACUThreat..'; iHealthPercent='..iHealthPercent..'; refbUseACUAggressively='..tostring(oACU[refbUseACUAggressively] or false)..'; refbAssassinationOrSimilar='..tostring(M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or false)..'; dist to friendly base='..M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZTeamData[M28Map.reftClosestFriendlyBase])..'; subrefiTeamGrossMass='..M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass]..'; GetEnemyTeamActualMassIncome='..M28Conditions.GetEnemyTeamActualMassIncome(iTeam)) end
                                                if bWantToRun and iHealthPercent >= 0.98 and iACUThreat >= 1300 and oACU[refiUpgradeCount] >= 1 and (oACU[refbUseACUAggressively] or not(M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar]) or (oACU[refiUpgradeCount] >= 3 and iACUThreat >= 1600 and not(M28Team.tTeamData[aiBrain.M28Team][M28Team.refbDangerousForACUs]) and M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZTeamData[M28Map.reftClosestFriendlyBase]) <= 180 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] < 1.1 * aiBrain[M28Economy.refiBrainResourceMultiplier] * M28Conditions.GetEnemyTeamActualMassIncome(iTeam))) then
                                                --Get closest enemy
                                                local oClosestEnemy
                                                ...
                                                if iClosestEnemy <= 110 and oClosestEnemy and (iClosestEnemy <= 75 or (M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZTeamData[M28Map.reftClosestFriendlyBase]) <= 200 - iClosestEnemy and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] < 1.1 * aiBrain[M28Economy.refiBrainResourceMultiplier] * M28Conditions.GetEnemyTeamActualMassIncome(iTeam))) then --We are close enough that we could consider attacking nearest enemies, or are close enough to our base that we can risk going a bit further away
                            ...
                            --No land zone - if we have no order then return to base if not doing early build order on a naval map
                            ...
                            if oACU[M28Orders.refiOrderCount] == 0 then
                                ...
                                if M28Utilities.IsTableEmpty(tOrderTarget) == false then
                                    ...
                                    if NavUtils.GetTerrainHeight(tOrderTarget[1], tOrderTarget[3]) < M28Map.iMapWaterHeight then
                                        ...
                                        --Are we not underwater, such that nearby enemy land units can fire at us, and we are damaged? in which case keep running
                                        if bDebugMessages == true then LOG(sFunctionRef..': bWantToRun after checking antinavy threat='..tostring(bWantToRun)..'; iAntiNavyThreat='..iAntiNavyThreat..'; iHealthPercent='..iHealthPercent..'; Is underwater='..tostring(M28UnitInfo.IsUnitUnderwater(oACU))) end
                                        if not(bWantToRun) and iHealthPercent <= 0.75 and not(M28UnitInfo.IsUnitUnderwater(oACU)) and M28Utilities.IsTableEmpty(tLZData[M28Map.subrefAdjacentLandZones]) == false then
                                            local iEnemyMassInRange = 0
                                            local iMaxEnemyInRange = M28UnitInfo.GetCombatThreatRating({ oACU }, false)
                                            for iEntry, tSubtable in tLZData[M28Map.subrefAdjacentLandZones] do
                                                ...
                                                local tAltLZTeamData = tAltLZData[M28Map.subrefLZTeamData][iTeam]
                                                if M28Utilities.IsTableEmpty(tAltLZTeamData[M28Map.reftoNearestDFEnemies]) == false then
                                                    --Will have a bit of double-counting with this, but dont need ti to be precise
                                                    for iEnemy, oEnemy in tAltLZTeamData[M28Map.reftoNearestDFEnemies] do
                                                        if not(oEnemy.Dead) and M28Utilities.GetDistanceBetweenPositions(oEnemy:GetPosition(), oACU:GetPosition()) - (oEnemy[M28UnitInfo.refiDFRange] or 0) <= 3 then
                                                            iEnemyMassInRange = iEnemyMassInRange + M28UnitInfo.GetUnitMassCost(oEnemy)
                                                            if iEnemyMassInRange >= iMaxEnemyInRange then bWantToRun = true break end
                                                        end
                                                    end
                                                end
                                                if bDebugMessages == true then LOG(sFunctionRef..': iEnemyMassInRange after considering iAdjLZ='..tSubtable[M28Map.subrefWPlatAndLZNumber][2]..'='..iEnemyMassInRange..'; iMaxEnemyInRange threshold='..iMaxEnemyInRange) end
                                            end
                                        end
                                    end
                                end
...
function DoesACUWantToReturnToCoreBase(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': Condition1 (dangerousforACUs): M28Team.tTeamData[iTeam][M28Team.refbDangerousForACUs]='..tostring(M28Team.tTeamData[iTeam][M28Team.refbDangerousForACUs])..'; Cond1a='..tostring(((GetGameTimeSeconds() >= 1080 and (GetGameTimeSeconds() >= 2100 or iACUHealthPercentage < 0.95 or (GetGameTimeSeconds() >= 1380 and M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] > 0) or (M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] and (oACU[refiUpgradeCount] <= 1 or (oACU[refiUpgradeCount] == 2 and EntityCategoryContains(categories.AEON, oACU.UnitId)) or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 140))))))..';Cond1b='..tostring( M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] > 0)..'; Cond1c='..tostring((M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] and GetGameTimeSeconds() >= 300 and (oACU:GetAIBrain()[M28Economy.refiOurHighestFactoryTechLevel] >= 3 or tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.45 or (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3] or 0) >= 3 or ((M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][2] or 0) >= 5 and M28Conditions.GetHighestOtherTeamT2AndT3MexCount(iTeam) * 1.25 < (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3] or 0) +  (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][2] or 0))) and (M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] > 1.1 * oACU:GetAIBrain()[M28Economy.refiBrainResourceMultiplier] * M28Conditions.GetEnemyTeamActualMassIncome(iTeam) or oACU[refiUpgradeCount] <= 1 or (oACU[refiUpgradeCount] == 2 and EntityCategoryContains(categories.AEON, oACU.UnitId)) or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 140)))..'; cond1d='..tostring(iACUHealthPercentage < 0.9)..'; cond1e='..tostring(iACUHealthPercentage < 0.9)..'; cond1f='..tostring((tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.25 and (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.6 or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 150) and (M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or M28Utilities.GetDistanceBetweenPositions(tLZOrWZData[M28Map.subrefMidpoint], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 150 + 50 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount])))..'; cond1c broken out in more detail:refiOurHighestFactoryTechLevel='..oACU:GetAIBrain()[M28Economy.refiOurHighestFactoryTechLevel]..'; Moddist%='..tLZOrWZTeamData[M28Map.refiModDistancePercent]..'; T3 mexes='..M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3]..';T2 mexes='..M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][2]..'; Enemy T2+T3 mexes='..M28Conditions.GetHighestOtherTeamT2AndT3MexCount(iTeam)..'; Our fac and enemy fac or our LC t3='..tostring(oACU:GetAIBrain()[M28Economy.refiOurHighestFactoryTechLevel] >= 3 and (M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyGroundTech] >= 3 or M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech] >= 3 or M28Conditions.GetTeamLifetimeBuildCount(iTeam, M28UnitInfo.refCategoryLandCombat * categories.TECH3) >= 2))..'; Dist to base='..M28Utilities.GetDistanceBetweenPositions(tLZOrWZData[M28Map.subrefMidpoint], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase])..'; Air snipe condition='..tostring(((M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] >= math.max(500, 500 * M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech], (tLZOrWZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] or 0) + (tLZOrWZTeamData[M28Map.subrefLZThreatAllyMAA] or 0))..'; High upgrade ACU new row 1 cond for everything='..tostring((M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or oACU[refiUpgradeCount] < 3 or (oACU[refiUpgradeCount] == 3 and EntityCategoryContains(categories.AEON, oACU.UnitId)) or not(tLZOrWZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ]) or iACUHealthPercentage < 0.95 or (M28UnitInfo.GetCombatThreatRating({ oACU}, false) or 0) <= 3000 or tLZOrWZTeamData[M28Map.refiModDistancePercent] > 0.4 or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) > 100 + 40 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] and ((tLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) > (tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0)))))))) end
    --Heavily upgraded ACU with a team with enemies nearby that we could target - be more aggressive
    if (M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or oACU[refiUpgradeCount] < 3 or (oACU[refiUpgradeCount] == 3 and EntityCategoryContains(categories.AEON, oACU.UnitId)) or (not(tLZOrWZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ]) and not(tLZOrWZTeamData[M28Map.subrefLZbCoreBase])) or iACUHealthPercentage < 0.95 or (M28UnitInfo.GetCombatThreatRating({ oACU}, false) or 0) <= 2300 or tLZOrWZTeamData[M28Map.refiModDistancePercent] > 0.4 or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) > 100 + 40 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] or ((tLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) > (tLZOrWZTeamData[M28Map.subrefiThreatEnemyGroundAA] or 0))) and
            ((M28Team.tTeamData[iTeam][M28Team.refbDangerousForACUs] and
                ((GetGameTimeSeconds() >= 1080 and (GetGameTimeSeconds() >= 2100 or iACUHealthPercentage < 0.95 or (GetGameTimeSeconds() >= 1380 and M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] > 0) or (M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] and (oACU[refiUpgradeCount] <= 1 or (oACU[refiUpgradeCount] == 2 and EntityCategoryContains(categories.AEON, oACU.UnitId)) or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 140)))) --1a
                        or M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] > 0 --1b
                        or (M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] and GetGameTimeSeconds() >= 300 and (oACU:GetAIBrain()[M28Economy.refiOurHighestFactoryTechLevel] >= 3 or tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.45 or (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3] or 0) >= 3 or ((M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][2] or 0) >= 5 and M28Conditions.GetHighestOtherTeamT2AndT3MexCount(iTeam) * 1.25 < (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3] or 0) +  (M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][2] or 0))) and (M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] > 1.1 * oACU:GetAIBrain()[M28Economy.refiBrainResourceMultiplier] * M28Conditions.GetEnemyTeamActualMassIncome(iTeam) or oACU[refiUpgradeCount] <= 1 or (oACU[refiUpgradeCount] == 2 and EntityCategoryContains(categories.AEON, oACU.UnitId)) or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 140)) --1c
                        or M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftEnemyLandExperimentals]) == false --1d
                        or iACUHealthPercentage < 0.9 --1e
                        or (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.25 and (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.6 or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 150) and (M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or M28Utilities.GetDistanceBetweenPositions(tLZOrWZData[M28Map.subrefMidpoint], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 150 + 50 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount]))) --1f
            ...
            or iACUHealthPercentage <= 0.4
            or ((tLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) >= math.max(400, math.min(tLZOrWZTeamData[M28Map.subrefLZThreatAllyMAA], 1500)) and (not(M28Map.bIsCampaignMap) or not((tLZOrWZTeamData[M28Map.subrefLZbCoreBase] and tLZOrWZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] > 0 and iACUHealthPercentage >= 0.9))))
            --Is enemy planning a potential air snipe and we are exposed?
            or ((not(tLZOrWZTeamData[M28Map.subrefLZbCoreBase]) and M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] >= math.max(500, 500 * M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech], (tLZOrWZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] or 0) + (tLZOrWZTeamData[M28Map.subrefLZThreatAllyMAA] or 0))) --3rd row
                and (not(M28Map.bIsCampaignMap) or not((tLZOrWZTeamData[M28Map.subrefLZbCoreBase] and (tLZOrWZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] > 0 or (iACUHealthPercentage >= 0.4 and M28Conditions.GetFriendlyMAAInThisAndAdjacentLandZones(tLZOrWZData, tLZOrWZTeamData, iPlateauOrZero, iTeam) >= M28Conditions.GetNearbyEnemyAirToGroundThreat(tLZOrWZData, tLZOrWZTeamData, iTeam, 120, 2000))) and iACUHealthPercentage >= 0.9))) --4th row
                ...
                and (M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] > 1000 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] or (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.45 and (M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 180 or iACUHealthPercentage <= 0.75) and (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.55 or (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.45 and M28Map.iMapSize >= 1000))) or iACUHealthPercentage <= 0.95 or oACU[refiUpgradeCount] <= 1 or M28Conditions.GetNearbyEnemyAirToGroundThreat(tLZOrWZData, tLZOrWZTeamData, iTeam, 175, 0) > math.max(100, (tLZOrWZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] or 0) * 2.5, M28Conditions.GetFriendlyMAAInThisAndAdjacentLandZones(tLZOrWZData, tLZOrWZTeamData, iPlateauOrZero, iTeam))))
                and ((M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech] >= 3 or GetGameTimeSeconds() >= 1080 or M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] >= 3000 or iACUHealthPercentage <= 0.9 or tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.6 or iPlateauOrZero == 0) and (M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or not(M28Conditions.IsFurthestACUToFriendlyBase(oACU, tLZOrWZTeamData, iTeam))) or --7th row
        ...
        if bDebugMessages == true then LOG(sFunctionRef..': Is dangerous for ACU or low health or large enemy air to ground threat so returning to base; First condition row='..tostring(((M28Team.tTeamData[iTeam][M28Team.refbDangerousForACUs] and (GetGameTimeSeconds() >= 1080 or tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.35 or M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] > 0 or M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftEnemyLandExperimentals]) == false or iACUHealthPercentage < 0.9 or (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.25 and M28Utilities.GetDistanceBetweenPositions(tLZOrWZData[M28Map.subrefMidpoint], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 175))) or iACUHealthPercentage <= 0.4 or (tLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) >= 400) or iACUHealthPercentage <= 0.4 or (tLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) >= 400)..'; Eception to first row condition='..tostring(not(M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftEnemyLandExperimentals]) and not(M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar]) and GetGameTimeSeconds() <= 1200 and iACUHealthPercentage >= 0.95 and oACU[refiUpgradeCount] >= 2 and (M28UnitInfo.GetCombatThreatRating({ oACU}, false) or 0) >= 3500 and M28Utilities.GetDistanceBetweenPositions(tLZOrWZData[M28Map.subrefMidpoint], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) <= 250))..'; Is furthest ACU from friendly base='..tostring(M28Conditions.IsFurthestACUToFriendlyBase(oACU, tLZOrWZTeamData, iTeam))..'; 3rd row condition='..tostring((not(tLZOrWZTeamData[M28Map.subrefLZbCoreBase]) and M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] >= math.max(500, 500 * M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech], (tLZOrWZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] or 0) + (tLZOrWZTeamData[M28Map.subrefLZThreatAllyMAA] or 0))))..'; 4th row condition='..tostring((M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or (not(M28Team.tAirSubteamData[oACU:GetAIBrain().M28AirSubteam][M28Team.refbHaveAirControl]) and (M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] > 1200 or M28Team.iPlayersAtGameStart <= 4 or M28Conditions.GetHighestAirToGroundThreatForIndividualEnemyBrain(iTeam) > 400))))..'; 5th row condition='..tostring((M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] > 1000 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] or (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.45 and (M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 180 or iACUHealthPercentage <= 0.75) and (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.55 or (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.45 and M28Map.iMapSize >= 1000))) or iACUHealthPercentage <= 0.95 or oACU[refiUpgradeCount] <= 1 or M28Conditions.GetNearbyEnemyAirToGroundThreat(tLZOrWZData, tLZOrWZTeamData, iTeam, 175, 0) > math.max(100, (tLZOrWZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] or 0) * 2.5)))..'; MAA in this and adj zone='..M28Conditions.GetFriendlyMAAInThisAndAdjacentLandZones(tLZOrWZData, tLZOrWZTeamData, iPlateauOrZero, iTeam)..'; Total enemy air to ground threat='..M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat]..'; Mod dist='..tLZOrWZTeamData[M28Map.refiModDistancePercent]..'; Enemy air to ground factor for check='..(0.35 + math.min(math.max(-0.15,tLZOrWZTeamData[M28Map.refiModDistancePercent] - 0.4), 0.2))..'; 7th row='..tostring(((M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech] >= 3 or GetGameTimeSeconds() >= 1080 or M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] >= 3000 or iACUHealthPercentage <= 0.9 or tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.6 or iPlateauOrZero == 0) and (M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar] or not(M28Conditions.IsFurthestACUToFriendlyBase(oACU, tLZOrWZTeamData, iTeam)))))..'; En land tech='..M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyGroundTech]..'; En air tech='..M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech]..'; ACU health%='..iACUHealthPercentage..'; Plateau='..iPlateauOrZero..'; Is table of enemy land exp empty='..tostring(M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftEnemyLandExperimentals]))) end
        M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
        return true
    else
        ...
        if M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] >= math.max(math.min(1000, 400 + math.max(250 * (M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] or 1), (M28Team.iPlayersAtGameStart or 1) * 100)), math.min(750, (tLZOrWZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] or 0) * 2), math.min(4000, (M28Team.tAirSubteamData[oACU:GetAIBrain().M28AirSubteam][M28Team.subrefiOurAirAAThreat] or 0))) and M28Team.tAirSubteamData[oACU:GetAIBrain().M28AirSubteam][M28Team.refbFarBehindOnAir] and ((oACU[refiUpgradeCount] or 0) < 3 or not(oACU:GetHealth() >= 15000 - 500 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] or (oACU.MyShield.GetHealth and oACU.MyShield:GetHealth()) >= 7000))
                ...
                and (tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.4 or (not(M28Map.bIsCampaignMap) and tLZOrWZTeamData[M28Map.refiModDistancePercent] >= 0.2 and M28Utilities.GetDistanceBetweenPositions(tLZOrWZData[M28Map.subrefMidpoint], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 180 and (oACU[refiUpgradeCount] < 2 or tLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat] > 0 or iACUHealthPercentage <= 0.8 or (M28Map.iMapSize >= 1000 and M28Utilities.GetDistanceBetweenPositions(tLZOrWZData[M28Map.subrefMidpoint], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase]) >= 150))))
        then
            ...
            if oACU:IsUnitState('Upgrading') then
                if iPlateauOrZero > 0 and iACUHealthPercentage <= 0.99 and oACU:GetWorkProgress() <= 0.8 and (tLZOrWZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ] or oACU:GetWorkProgress() <= 0.6) and (not(tLZOrWZTeamData[M28Map.subrefLZbCoreBase]) or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZData[M28Map.subrefMidpoint]) >= 35) and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies]) == false then
                    ...
                    if not(M28Conditions.SafeToUpgradeUnit(oACU)) then
                        ...
                        local iCurDist
                        function AddNearbyEnemyThreats(tCurLZTeamData, bIsWaterZone)
                            if M28Utilities.IsTableEmpty(tCurLZTeamData[M28Map.subrefTEnemyUnits]) == false then
                                for iEnemy, oEnemy in tCurLZTeamData[M28Map.subrefTEnemyUnits] do
                                    if not(oEnemy.Dead) and oEnemy[M28UnitInfo.refiCombatRange] > 10 and (not(bIsWaterZone) or ((oEnemy[M28UnitInfo.refiDFRange] or 0) > 0 and not(M28UnitInfo.IsUnitUnderwater(oEnemy)))) then
                        ...
                        if tLZOrWZTeamData[M28Map.subrefbDangerousEnemiesInAdjacentWZ] and M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentWaterZones]) == false then
                            for iEntry, tSubtable in tLZOrWZData[M28Map.subrefAdjacentWaterZones] do
                                local iAdjWZ = tSubtable[M28Map.subrefAWZRef]
                                local tAdjWZTeamData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iAdjWZ]][M28Map.subrefPondWaterZones][iAdjWZ][M28Map.subrefWZTeamData][iTeam]
                                AddNearbyEnemyThreats(tAdjWZTeamData, true)
                            end
                        end
            ...
            if (oACU[M28UnitInfo.refiAntiNavyRange] or 0) > 10 and iACUHealthPercentage >= 0.75 and (M28Team.tTeamData[iTeam][M28Team.refiEnemyTorpBombersThreat] <= 600 or (M28Team.tTeamData[iTeam][M28Team.refiEnemyTorpBombersThreat] <= 5000 and M28Team.tAirSubteamData[oACU:GetAIBrain().M28AirSubteam][M28Team.refbHaveAirControl])) then
            ...
            if bWaterIsDangerous then
                ...
                if bWantACUToRun and (oACU[refiUpgradeCount] or 0) >= 1 then
                    local iHealthPercent = iACUHealthPercentage
...
function AttackNearestEnemyWithACU(iPlateau, iLandZone, tLZData, tLZTeamData, oACU, iOptionalDistThresholdOverride, bReturnClosestOutOfThresholdDistance)
    --Attack move to the nearest enemy if we arent in range of it or retreat if we are in range of it
    --returns true if given order; returns false if not, and optionally returns the closest enemy dist
    local bDebugMessages = false if M28Profiler.bGlobalDebugOverride == true then   bDebugMessages = true end
    ...
    local iClosestOutOfThresholdDistance = 10000
    if (oACU[M28UnitInfo.refiDFRange] or 0) > 0 and iPlateau > 0 then
        local iCurDist
                    ...
                    for iUnit, oUnit in tAdjLZTeamData[M28Map.subrefTEnemyUnits] do
                        if M28UnitInfo.IsUnitValid(oUnit) then
                            ...
                            if not(EntityCategoryContains(M28UnitInfo.refCategoryLandScout - categories.SERAPHIM, oUnit.UnitId)) then
                                ...
                                elseif iCurDist < iClosestOutOfThresholdDistance then
                                    iClosestOutOfThresholdDistance = iCurDist
                                end
                            end
                        end
                    end
        ...
        if M28Utilities.IsTableEmpty(tUnitsToTarget) == false then
            ...
            if tLZTeamData[M28Map.subrefLZbCoreBase] and tLZTeamData[M28Map.subrefLZThreatEnemyBestStructureDFRange] > oACU[M28UnitInfo.refiDFRange] and (GetGameTimeSeconds() <= 300 or (GetGameTimeSeconds() <= 600 and M28Map.bIsCampaignMap)) then
                ...
                if M28Utilities.IsTableEmpty(tPDToTarget) == false then
            ...
            function ConsiderUnitsToTarget(tCurUnitsToTarget)
                for iUnit, oUnit in tCurUnitsToTarget do
                    if M28UnitInfo.IsUnitValid(oUnit) and not(M28UnitInfo.IsUnitUnderwater(oUnit)) then
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': Checking if unit in zone we recently ran from, oUnit='..(oUnit.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oUnit) or 'nil')..'; iUnitPlateau='..(iUnitPlateau or 'nil')..'; iUnitZone='..(iUnitZone or 'nil')) end
                        if iUnitPlateau > 0 and iUnitZone > 0 and not((iUnitZone == iLandZone and iUnitPlateau == iPlateau)) and (oACU[reftiTimeLastRanFromZoneByPlateau][iUnitPlateau][iUnitZone] and GetGameTimeSeconds() - oACU[reftiTimeLastRanFromZoneByPlateau][iUnitPlateau][iUnitZone] < 30) or (oACU[refiLastPlateauAndZoneToAttackUnitIn][2] == iUnitZone and oACU[refiLastPlateauAndZoneToAttackUnitIn][1] == iUnitPlateau and GetGameTimeSeconds() - (oACU[refiTimeLastToldToAttackUnitInOtherZone] or -100) < 30) then
                        ...
                        if not(bUnitInFurtherAwayZoneWeRanFrom) then
                            ...
                            if iCurDist then
                                ...
                                elseif iCurDist < iClosestUnshieldedDist and not(M28Logic.IsTargetUnderShield(aiBrain, oUnit, 3000, false, false, false, false, false)) then
                                    oClosestUnshielded = oUnit
                                    iClosestUnshieldedDist = iCurDist
                                    if bReturnClosestOutOfThresholdDistance and iCurDist < iClosestOutOfThresholdDistance then iClosestOutOfThresholdDistance = iCurDist end
                                elseif bReturnClosestOutOfThresholdDistance and iCurDist < iClosestOutOfThresholdDistance then iClosestOutOfThresholdDistance = iCurDist
                                end
                            end
                        end
                    end
            ...
            ConsiderUnitsToTarget(tUnitsToTarget)
            ...
            --Target non-DF units in this zone if we failed to find DF units nearby to target but do have some nearby
            if not(oEnemyToTarget) and M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subrefTEnemyUnits]) == false and M28Utilities.IsTableEmpty(tLZTeamData[M28Map.reftoNearestDFEnemies]) == false then
                local tAltUnitsToTarget = EntityCategoryFilterDown(categories.ALLUNITS - M28UnitInfo.refCategoryLandScout + M28UnitInfo.refCategoryCombatScout - categories.DIRECTFIRE, tLZTeamData[M28Map.subrefTEnemyUnits])
                if bDebugMessages == true then LOG(sFunctionRef..': Failed to find normal target from enemy DF units (e.g. they are in zones we have recently retreated from) but have enemies in this zone that we can consider targeting as well, is tAltUnitsToTarget empty='..tostring(M28Utilities.IsTableEmpty(tAltUnitsToTarget))) end
                if M28Utilities.IsTableEmpty(tAltUnitsToTarget) == false then
                    ConsiderUnitsToTarget(tAltUnitsToTarget)
                end
            end
            ...
            if oEnemyToTarget then
                ...
                if oACU[M28UnitInfo.refbLastShotBlocked] then
                    ...
                    local bEnemyHasLongerRangedUnits
                    ...
                    --Shot blocked override
                    if bWantKitingRetreat and ((bEnemyHasLongerRangedUnits and iEnemyT1ArtiAndDFThreatCloseToOurRange <= iEnemyThreatThresholdForDetailedCheck * 0.5)
                            or (oACU[refiUpgradeCount] >= 2 and iOurACUHealthPercent >= 0.95 and (oACU[refiUpgradeCount] >= 3 or tLZTeamData[M28Map.refiModDistancePercent] <= 0.4 or M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZTeamData[M28Map.reftClosestFriendlyBase]) <= 100 + M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] * 50))) then
                        if M28Logic.IsShotBlocked(oACU, oEnemyToTarget, false, nil) then
                            bWantKitingRetreat = false
                        ...
                        elseif iEnemyT1ArtiAndDFThreatCloseToOurRange >= 250 then
                            ...
                            elseif iUnitPlateau == 0 and iCurDist <= oACU[M28UnitInfo.refiDFRange] + 2 then
                                ...
                                if M28Conditions.GiveAttackMoveAsWeaponStuck(oACU) then
                                    ...
                                    if bDebugMessages == true then LOG(sFunctionRef..': Weapon stuck so doing attack move') end
                                else
    ...
    if oEnemyToTarget then return true
    else
        if bReturnClosestOutOfThresholdDistance then return false, iClosestOutOfThresholdDistance
        else return false
        end
    end
end
...
function AssistBuildingUpgradeOrStorageConstruction(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, iOptionalMaxSearchDist)
...
function GetACUOrder(aiBrain, oACU)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': Near start of code for brain '..oACU:GetAIBrain().Nickname..', ACU='..oACU.UnitId..M28UnitInfo.GetUnitLifetimeCount(oACU)..'; time='..GetGameTimeSeconds()..'; oACU[refbDoingInitialBuildOrder]='..tostring(oACU[refbDoingInitialBuildOrder])..'; ACU unit state='..M28UnitInfo.GetUnitState(oACU)..'; work progress='..oACU:GetWorkProgress()..'; iPlateau='..(iPlateauOrZero or 'nil')..'; iLandZone='..(iLandOrWaterZone or 'nil')..'; Can ACU use overcharge='..tostring(M28Conditions.CanUnitUseOvercharge(oACU:GetAIBrain(), oACU))..'; ACU position='..repru(oACU:GetPosition())..'; ACU Orders (before updates)='..reprs(oACU[M28Orders.reftiLastOrders])..'; Is special micro active='..tostring(oACU[M28UnitInfo.refbSpecialMicroActive] or false)..'; Time to stop micro='..(oACU[M28UnitInfo.refiGameTimeToResetMicroActive] or 'nil')..'; Brian nickname='..aiBrain.Nickname..'; reftSpecialObjectiveMoveLocation='..repru(oACU[reftSpecialObjectiveMoveLocation])..'; Enemy combat threat='..(tLZOrWZTeamData[M28Map.subrefTThreatEnemyCombatTotal] or 'nil')..'; Is table of unbuilt mexes empty='..tostring(M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefMexUnbuiltLocations]))..'; refbFocusOnT1Spam='..tostring(M28Team.tTeamData[iTeam][M28Team.refbFocusOnT1Spam])..'; oACU[M28UnitInfo.refbUsingDefaultWeaponPriority]='..tostring(oACU[M28UnitInfo.refbUsingDefaultWeaponPriority] or false)..'; Is table of snipe targets empty='..tostring(M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.toActiveSnipeTargets])))
        ...
        if oFocusUnit.UnitId then LOG('Focus unit='..oFocusUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oFocusUnit))
        else LOG('No focus unit')
        end
    end
    ...
    if oACU:IsUnitState('Upgrading') then
        ...
        if not(oACU[refbDoingInitialBuildOrder]) then
            ...
            if DoesACUWantToReturnToCoreBase(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU) then
                if bDebugMessages == true then LOG(sFunctionRef..': ACU wants to run to core base, considering if need emergency support and, if ACU health is low, will consider cancelling, ACU health%='..M28UnitInfo.GetUnitHealthPercent(oACU)..'; use acu aggressively='..tostring(oACU[refbUseACUAggressively] or false)..'; ACU expected to die before completing='..tostring(ACUExpectedToDieBeforeCompletingUpgrade(oACU))) end
                ...
                --Core base - more restrictive than if not in core base
                if tLZOrWZTeamData[M28Map.subrefLZbCoreBase] and iACUHealthPercent <= 0.9 and (1 - iACUHealthPercent) + 0.05 > oACU:GetWorkProgress() and ACUExpectedToDieBeforeCompletingUpgrade(oACU) then
                    if bDebugMessages == true then LOG(sFunctionRef..': In core base but still want to cancel ACU upgrade') end
                    M28Orders.IssueTrackedMove(oACU, M28Map.GetPlayerStartPosition(oACU:GetAIBrain()), 5, false, 'CCRun')
                ...
                elseif not(tLZOrWZTeamData[M28Map.subrefWZbContainsUnderwaterStart]) and (not(tLZOrWZTeamData[M28Map.subrefLZbCoreBase]) or (iACUHealthPercent <= 0.99 and tLZOrWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] and M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZData[M28Map.subrefMidpoint]) >= 35 and DoesACUWantToRun(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU))) then

                    if (iACUHealthPercent <= 0.99 and ((1 - iACUHealthPercent) + 0.1 > oACU:GetWorkProgress()) or M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar]) or ACUExpectedToDieBeforeCompletingUpgrade(oACU)  then
            ...
            if not(bCanceledUpgrade) and tLZOrWZTeamData[M28Map.subrefLZbCoreBase] and not(tLZOrWZTeamData[M28Map.subrefWZbContainsUnderwaterStart]) and M28UnitInfo.GetUnitHealthPercent(oACU) <= 0.7 and tLZOrWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] and M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tLZOrWZData[M28Map.subrefMidpoint]) >= 35 and DoesACUWantToRun(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU) then
                local iACUHealthPercent = M28UnitInfo.GetUnitHealthPercent(oACU)
                if iACUHealthPercent < 0.6 and (1 - iACUHealthPercent) + math.max(0.1, math.min(0.4, 1 - iACUHealthPercent)) > oACU:GetWorkProgress() and ((1 - iACUHealthPercent) + 0.1 > oACU:GetWorkProgress() or ACUExpectedToDieBeforeCompletingUpgrade(oACU)) then
                    --Do we no longer consider this location safe?
                    if bDebugMessages == true then LOG(sFunctionRef..': ACU health relatively low, iACUHealthPercent='..iACUHealthPercent..'; Work progress='..oACU:GetWorkProgress()..'; woudl it be safe to start an ugprade here='..tostring(M28Conditions.SafeToUpgradeUnit(oACU))) end
    ...
    elseif GiveOverchargeOrderIfRelevant(tLZOrWZData, tLZOrWZTeamData, oACU, iPlateauOrZero, iLandOrWaterZone) then
        ...
        if oACU[refbDoingInitialBuildOrder] then
            ...
            elseif (iLandOrWaterZone or 0) > 0 and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefTEnemyUnits]) == false then
                ...
                if not(bCampaignRetreatInstead) then
                    ...
                    elseif iPlateauOrZero == 0 and M28UnitInfo.GetUnitHealthPercent(oACU) >= 0.75 then
            ...
            if oACU[refbDoingInitialBuildOrder] and GetGameTimeSeconds() >= 60 and aiBrain[M28Economy.refiOurHighestFactoryTechLevel] >= 3 and aiBrain[M28Economy.refiGrossEnergyBaseIncome] >= 15 and aiBrain[M28Economy.refiGrossMassBaseIncome] >= 1.5 then
                oACU[refbDoingInitialBuildOrder] = false
                if bDebugMessages == true then LOG(sFunctionRef..': Clearing the initial build order flag for ACU earlier than normal as we already have tech 3, setting refbDoingInitialBuildOrder to be false') end
            end
        ...
        if bProceedWithLogic then
            ...
            if (iPlateauOrZero > 0 and not(tLZOrWZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ]) and (tLZOrWZTeamData[M28Map.subrefLZbCoreBase] or not(tLZOrWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ]))) and (M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies]) or not(DoesACUWantToRun(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU))) then
                ...
                if aiBrain[M28Economy.refiOurHighestLandFactoryTech] == 0 or (aiBrain[M28Economy.refiOurHighestAirFactoryTech] == 0 and (aiBrain[M28Economy.refiGrossMassBaseIncome] >= 6 * aiBrain[M28Economy.refiBrainBuildRateMultiplier] or aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryMex - categories.TECH1) >= 2)) or (not(tLZOrWZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ]) and aiBrain:GetEconomyStoredRatio('MASS') >= 0.8 and (aiBrain[M28Economy.refiGrossEnergyBaseIncome] > 150 or (aiBrain:GetEconomyStoredRatio('ENERGY') >= 0.99 and aiBrain[M28Economy.refiNetEnergyBaseIncome] > 0)) and (aiBrain[M28Economy.refiGrossMassBaseIncome] > 2 or aiBrain[M28Economy.refiNetMassBaseIncome] > 0) and aiBrain:GetEconomyStoredRatio('ENERGY') >= 0.95 and (GetGameTimeSeconds() <= 360 or M28Conditions.WantMoreFactories(iTeam, iPlateauOrZero, iLandOrWaterZone, true))) then
                ...
                if bWantAnotherFactory then
                    bProceedWithLogic = false
                    --If enemies are almost in range of us then attack them first instead, as otherwise will just keep building factory
                    local oNearestEnemy
                    local iClosestEnemyLessRangeDist = 10
                    local iCurEnemyLessRangeDist
                    function ConsiderEnemies(tEnemyUnits)
                        if tEnemyUnits[1] then
                            for iUnit, oUnit in tEnemyUnits do
                                if not(oUnit.Dead) and (oUnit[M28UnitInfo.refiCombatRange] or 0) >= 4 then
                                    iCurEnemyLessRangeDist = M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oACU:GetPosition()) - oUnit[M28UnitInfo.refiCombatRange]
                                    if iCurEnemyLessRangeDist < iClosestEnemyLessRangeDist then
                                        iClosestEnemyLessRangeDist = iCurEnemyLessRangeDist
                                        oNearestEnemy = oUnit
                                    end
                                end
                    ...
                    if bDebugMessages == true then LOG(sFunctionRef..': Wont try building factory if enemy almost in range of us, oNearestEnemy='..(oNearestEnemy.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oNearestEnemy) or 'nil')) end
                    if oNearestEnemy then
                        if not(oACU[M28UnitInfo.refbLastShotBlocked]) then
                            M28Orders.IssueTrackedAttackMove(oACU, oNearestEnemy:GetPosition(), 3, false, 'RebuildEnAM', false)
                        else
                            M28Orders.IssueTrackedMove(oACU, oNearestEnemy:GetPosition(), 3, false, 'RebuildEnM', false)
                        end
                    else
            ...
            elseif iPlateauOrZero == 0 and (tLZOrWZTeamData[M28Map.subrefWZbContainsUnderwaterStart] or M28Map.bIsCampaignMap) and aiBrain[M28Economy.refiOurHighestNavalFactoryTech] == 0 and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefTEnemyUnits]) then
                --Check if have adjacent land zone with enemy units with a decent range, so we dont get stuck building factory that they kill
                local bNearbyDangerousLandUnits = false
                local tNearestZoneWithEnemies
                if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentLandZones]) == false and M28UnitInfo.GetUnitHealthPercent(oACU) > 0.75 then
                    ...
                    for iEntry, tSubtable in tLZOrWZData[M28Map.subrefAdjacentLandZones] do
                        ...
                        local tAdjLZTeamData = tAdjLZData[M28Map.subrefLZTeamData][iTeam]
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering adjacent land zone LZ='..tSubtable[M28Map.subrefWPlatAndLZNumber][2]..'; subrefLZThreatEnemyBestMobileIndirectRange='..(tAdjLZTeamData[M28Map.subrefLZThreatEnemyBestMobileIndirectRange] or 0)..'; subrefLZThreatEnemyBestMobileDFRange='..(tAdjLZTeamData[M28Map.subrefLZThreatEnemyBestMobileDFRange] or 0)) end
                        if (tAdjLZTeamData[M28Map.subrefiNearbyEnemyLongRangeDFThreat] or 0) > 0 or (tAdjLZTeamData[M28Map.subrefiNearbyEnemyLongRangeIFThreat] or 0) > 0 or (tAdjLZTeamData[M28Map.subrefLZThreatEnemyBestMobileIndirectRange] or 0) > 30 or (tAdjLZTeamData[M28Map.subrefLZThreatEnemyBestMobileDFRange] or 0) > 30 then
                            bNearbyDangerousLandUnits = true
                            tNearestZoneWithEnemies = M28Map.tAllPlateaus[tSubtable[M28Map.subrefWPlatAndLZNumber][1]][M28Map.subrefPlateauLandZones][tSubtable[M28Map.subrefWPlatAndLZNumber][2]]
                            if bDebugMessages == true then LOG(sFunctionRef..': Setting bNearbyDangerousLandUnits=true due to iAdjLZ='..tSubtable[M28Map.subrefWPlatAndLZNumber][2]..' having land units with decent range') end
                            break
                        end
                    end
                end
                if not(bNearbyDangerousLandUnits) then
                    ...
                    bProceedWithLogic = false
                    --Attack enemies on land if we have no factories so we can try and clear a space or go out with a bang
                elseif tNearestZoneWithEnemies and aiBrain[M28Economy.refiOurHighestFactoryTechLevel] == 0 and M28UnitInfo.GetUnitHealthPercent(oACU) > 0.9 then
                    if bDebugMessages == true then LOG(sFunctionRef..': Will try attacking nearest zone with enemies') end
                    bProceedWithLogic = false
                    M28Orders.IssueTrackedMove(oACU, tNearestZoneWithEnemies[M28Map.subrefMidpoint], 6, false, 'ACSuicLZ', false)
                end
            end
            ...
            if bProceedWithLogic then
                ...
                if HaveTelesnipeAction(oACU, tLZOrWZData, tLZOrWZTeamData, aiBrain, iTeam, iPlateauOrZero, iLandOrWaterZone) then
                    ...
                    function GetPositionToRunFromNearestEnemy()
                        ...
                        return tPositionToRunTo, oClosestEnemy
                    end
                    ...
                    elseif not(oACU:IsUnitState('Building')) and DoesACUWantToReturnToCoreBase(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU) then
                        if  tLZOrWZTeamData[M28Map.subrefLZbCoreBase] and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies]) == false then
                            if not(ConsiderRunningToGETemplate(oACU, tLZOrWZData, tLZOrWZTeamData, iPlateauOrZero)) then
                                local tRallyPoint, oClosestEnemy = GetPositionToRunFromNearestEnemy()
                                local iDistUntilACUCanAttackNearestEnemy
                                if oClosestEnemy then iDistUntilACUCanAttackNearestEnemy = M28Utilities.GetDistanceBetweenPositions(oClosestEnemy:GetPosition(), oACU:GetPosition()) - (oACU[M28UnitInfo.refiDFRange] or 0)
                                else iDistUntilACUCanAttackNearestEnemy = 1000
                                end
                                if bDebugMessages == true then LOG(sFunctionRef..': Will run from enarest enemy as are in core base unless we have run out of places to run and want to attack the enemy as it is almost in our range, Dist to closest enemy='..M28Utilities.GetDistanceBetweenPositions(oClosestEnemy:GetPosition(), oACU:GetPosition())..'; iDistUntilACUCanAttackNearestEnemy='..iDistUntilACUCanAttackNearestEnemy..'; Dist to rally='..M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tRallyPoint)) end
                                if iDistUntilACUCanAttackNearestEnemy < 6 and iDistUntilACUCanAttackNearestEnemy >= -1 and M28Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), tRallyPoint) < 12 - iDistUntilACUCanAttackNearestEnemy then
                                    M28Orders.IssueTrackedAggressiveMove(oACU, oClosestEnemy:GetPosition(), 3, false, 'CantRun')
                                else
                        ...
                        if not(ConsiderRunningToGETemplate(oACU, tLZOrWZData, tLZOrWZTeamData, iPlateauOrZero)) then
                            ...
                            if tLZOrWZTeamData[M28Map.subrefLZbCoreBase] and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies]) == false and (M28UnitInfo.GetUnitHealthPercent(oACU) < 0.9 or M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar]) then
                                ...
                                if iLandOrWaterZone > 0 and iPlateauOrZero > 0 then
                                    ...
                                    --Are in water - if we make landfall will we just die, and are better staying in water? Check for nearby enemy antinavy first as if enemy has antinavy then should surface
                                    if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentLandZones]) == false and M28UnitInfo.GetUnitHealthPercent(oACU) < 0.75 then
                                        local iNearbyAntiNavy = (tLZOrWZTeamData[M28Map.subrefWZThreatEnemyAntiNavy] or 0)
                                        if iNearbyAntiNavy == 0 and M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefWZAdjacentWaterZones]) == false then
                                            for iEntry, tSubtable in tLZOrWZData[M28Map.subrefWZAdjacentWaterZones] do
                                                local iAdjWZ = tSubtable[M28Map.subrefAWZRef]
                                                iNearbyAntiNavy = iNearbyAntiNavy + (M28Map.tPondDetails[M28Map.tiPondByWaterZone[iAdjWZ]][M28Map.subrefPondWaterZones][iAdjWZ][M28Map.subrefWZTeamData][iTeam][M28Map.subrefWZThreatEnemyAntiNavy] or 0)
                                                if iNearbyAntiNavy > 0 then break end
                                            end
                                        end
                                        if bDebugMessages == true then LOG(sFunctionRef..': iNearbyAntiNavy='..iNearbyAntiNavy..'; is subrefAdjacentLandZones empty (it will be if this is a LZ)='..tostring(M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentLandZones]))) end
                                        if iNearbyAntiNavy == 0 then
                                            --Does enemy have combat on an adjacent land zone
                                            if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentLandZones]) == false then
                                                local iNearbyEnemyNetLandThreat = 0
                                                for iEntry, tSubtable in tLZOrWZData[M28Map.subrefAdjacentLandZones] do
                                                    local tAdjLZ = M28Map.tAllPlateaus[tSubtable[M28Map.subrefWPlatAndLZNumber][1]][M28Map.subrefPlateauLandZones][tSubtable[M28Map.subrefWPlatAndLZNumber][2]]
                                                    local tAdjLZTeamData = tAdjLZ[M28Map.subrefLZTeamData][iTeam]
                                                    iNearbyEnemyNetLandThreat = iNearbyEnemyNetLandThreat + (tAdjLZTeamData[M28Map.subrefLZThreatEnemyMobileDFTotal] or 0) + (tAdjLZTeamData[M28Map.subrefThreatEnemyDFStructures] or 0) - (tAdjLZTeamData[M28Map.subrefLZTThreatAllyCombatTotal] or 0)
                                                end
                                                if bDebugMessages == true then LOG(sFunctionRef..': iNearbyEnemyNetLandThreat='..iNearbyEnemyNetLandThreat..'; Our ACU threat='..M28UnitInfo.GetCombatThreatRating({ oACU}, false, false)) end
                                                if iNearbyEnemyNetLandThreat > 500 and iNearbyEnemyNetLandThreat > M28UnitInfo.GetCombatThreatRating({ oACU}, false, false) then
                                                    if bDebugMessages == true then LOG(sFunctionRef..': Will go to water rally instead as worried ACU will die if it makes landfall') end
                                                    bGoToWaterRally = true
                                                end
                                            end
                                        end
                                    end
                                    if bGoToWaterRally then
                                        tRallyPoint = (M28Navy.GetNearestWaterRallyPoint(tLZOrWZData, iTeam, M28Map.tiPondByWaterZone[iLandOrWaterZone], iLandOrWaterZone) or M28Map.GetPlayerStartPosition(oACU:GetAIBrain()))
                                        if bDebugMessages == true then LOG(sFunctionRef..': Will retreat to nearest water rally instead of base, angle to this='..M28Utilities.GetAngleFromAToB(oACU:GetPosition(), tRallyPoint)) end
                                    else
                            ...
                            if not(bHaveNearbyExperimentalOrder) and (not(bConsiderMexesAndReclaim) or (not(ConsiderBuildingMex(tLZOrWZData, tLZOrWZTeamData, oACU, 15)) and not(ConsiderNearbyReclaimForACUOrEngineer(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, M28UnitInfo.GetUnitHealthPercent(oACU) < 0.75, 20)))) then
                                ...
                                if not(bGoToWaterRally) and M28Team.tTeamData[iTeam][M28Team.refbDangerousForACUs] and M28Conditions.BaseIsSafeToRetreatTo(tLZOrWZTeamData[M28Map.reftClosestFriendlyBase], iTeam) then
                                    if bDebugMessages == true then LOG(sFunctionRef..': Dangerous for ACUs and base is safe to retreat to, so changing rally point to the closest friendly base') end
                                    tRallyPoint = {tLZOrWZTeamData[M28Map.reftClosestFriendlyBase][1], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase][2], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase][3]}
                                end
                        ...
                        if oACU[refiBuildTech] > 1 and ((oACU[refiBuildTech] >= aiBrain[M28Economy.refiOurHighestFactoryTechLevel] and ((oACU[refiBuildTech] > aiBrain[M28Economy.refiOurHighestFactoryTechLevel] and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies])) or not(tLZOrWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ]))) or (oACU[M28UnitInfo.refiDFRange] or 0) <= 24) and HaveActionForACUAsEngineer(oACU, tLZOrWZData, tLZOrWZTeamData, iPlateauOrZero, iLandOrWaterZone) then
                            ...
                            if (M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.toActiveSnipeTargets]) or not(HaveACUSnipeAction(oACU, iTeam, iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData))) and not(HaveRecentCommonACUTarget(oACU)) and not(AttackNearestEnemyWithACU(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU)) then
                                ...
                                elseif iPlateauOrZero > 0 and M28Team.tAirSubteamData[aiBrain.M28AirSubteam][M28Team.refbNoAvailableTorpsForEnemies] and M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentWaterZones]) == false and ConsiderAttackingNearbyNavalUnits(tLZOrWZData, tLZOrWZTeamData, oACU, iWaterZoneEnemyRangeOverride) then
                                    ...
                                    if oACU[refbWantsPriorityUpgrade] and (not(oACU:IsUnitState('Building')) and not(oACU:IsUnitState('Capturing')) and (not(oACU:IsUnitState('Repairing')) or (oACU:IsUnitState('Guarding') and oACU:GetWorkProgress() > 0 and not(EntityCategoryContains(categories.MOBILE - categories.COMMAND - M28UnitInfo.categories.SUBCOMMANDER, oACU:GetFocusUnit().UnitId)))) and (not(oACU:IsUnitState('Reclaiming')) or aiBrain:GetEconomyStoredRatio('MASS') >= 0.8) and not(oACU:IsUnitState('Capturing'))) then
                                            ...
                                            if not(ConsiderRunningToGETemplate(oACU, tLZOrWZData, tLZOrWZTeamData, iPlateauOrZero)) then
                                                ...
                                                if not(bHaveMovedToBeUnderShield) then
                                                    ...
                                                    if aiBrain:GetEconomyStoredRatio('MASS') < 0.3 and (not(bAreBuildingOrReclaiming) or aiBrain:GetEconomyStored('MASS') < 25) and ((oACU[refiUpgradeCount] or 0) == 0 or oACU:HasEnhancement('AdvancedEngineering')) and M28UnitInfo.GetUnitHealthPercent(oACU) >= 0.9 and
                                                        ...
                                                        local bNoNearbyLRThreats = (iPlateauOrZero == 0 or M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subreftoAllNearbyEnemyT2ArtiUnits])) and (tLZOrWZTeamData[M28Map.subrefiNearbyEnemyLongRangeDFThreat] or 0) == 0
                                                        if (tLZOrWZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) >= 150 and bNoNearbyLRThreats and M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech] < 3 and (not(bAreBuildingOrReclaiming) or oACU:GetWorkProgress() >= 0.9 or (oACU:GetFocusUnit().UnitId and not(EntityCategoryContains(categories.ANTIAIR, oACU:GetFocusUnit().UnitId)))) then
                                                        ...
                                                        elseif not(bAreBuildingOrReclaiming) then
                                                            ...
                                                            if bNoNearbyLRThreats and iPlateauOrZero == 0 and not(bBuildingOrAssistingPowerOrFactory) and (oACU[M28UnitInfo.refiAntiNavyRange] or 0) < 10 then
                                                            ...
                                                            if not(bBuildingOrAssistingPowerOrFactory) then
                                                                ...
                                                                --If have orders for ACU but no land zone then wait until it is in a land zone again assuming it is moving
                                                                if not(iLandOrWaterZone) or (iPlateauOrZero == 0 and not(tLZOrWZTeamData[M28Map.subrefWZbContainsUnderwaterStart])) then
                                                                    if bDebugMessages == true then LOG(sFunctionRef..': ACU isnt in a land zone, if has no orders will tell it to retreat to start position, oACU[M28Orders.refiOrderCount]='..(oACU[M28Orders.refiOrderCount] or 'nil')..'; ACU unit state='..M28UnitInfo.GetUnitState(oACU)..'; reprs of last order='..reprs(oACU[M28Orders.reftiLastOrders][oACU[M28Orders.refiOrderCount]])..'; ACU speed='..M28UnitInfo.GetUnitSpeed(oACU)) end
                                                                    --If ACU is guarding it might be assisiting a naval fac, so this is to stop it staying assisting that naval fac forever (i.e. whenever naval fac finishes a unit should get to here, but with acu still in guarding unit state)
                                                                    if (oACU[M28Orders.refiOrderCount] or 0) == 0 or (oACU:IsUnitState('Guarding') and M28UnitInfo.GetUnitSpeed(oACU) <= 0.1)  then
                                                                    ...
                                                                    if M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefTEnemyUnits]) == false and iPlateauOrZero > 0 and AttackNearestEnemyWithACU(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU) then
                                                                        ...
                                                                        if oACU[refbDoingInitialBuildOrder] and (M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftEnemyLandExperimentals]) or not(DoesACUWantToRun(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU))) then
                                                                            ...
                                                                            if tLZOrWZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ] and (oACU:IsUnitState('Building') or oACU:IsUnitState('Capturing') or oACU:IsUnitState('Repairing') or (oACU:IsUnitState('Guarding') and oACU:GetWorkProgress() > 0 and not(EntityCategoryContains(categories.MOBILE - categories.COMMAND - M28UnitInfo.categories.SUBCOMMANDER, oACU:GetFocusUnit().UnitId))) or (oACU:IsUnitState('Reclaiming') and aiBrain:GetEconomyStoredRatio('MASS') <= 0.6)) then iNearbyEnemyAttackRangeThreshold = 4
                                                                            elseif tLZOrWZTeamData[M28Map.subrefLZbCoreBase] or tLZOrWZTeamData[M28Map.subrefLZSValue] >= 200 then
                                                                                iNearbyEnemyAttackRangeThreshold = 20
                                                                            end
                                                                            --Be more aggressive if have several upgrades already
                                                                            if oACU[refiUpgradeCount] >= 2 and (oACU[refiUpgradeCount] >= 3 or not(EntityCategoryContains(categories.AEON, oACU.UnitId))) and M28UnitInfo.GetUnitHealthPercent(oACU) >= 0.95 and not(M28Team.tTeamData[iTeam][M28Team.refbAssassinationOrSimilar]) then iNearbyEnemyAttackRangeThreshold = iNearbyEnemyAttackRangeThreshold + 15 end
                                                                            --Check adjacent zones for closest enemy in case are any within range
                                                                            local oClosestEnemy, oCurEnemy
                                                                            local iClosestEnemy = (oACU[M28UnitInfo.refiDFRange] or 0) + iNearbyEnemyAttackRangeThreshold
                                                                            local iCurDist
                                                                            if iPlateauOrZero > 0 and M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefLZAdjacentLandZones]) == false then
                                                                                for _, iAdjLZ in tLZOrWZData[M28Map.subrefLZAdjacentLandZones] do
                                                                                    local tAdjLZTeamData = M28Map.tAllPlateaus[iPlateauOrZero][M28Map.subrefPlateauLandZones][iAdjLZ][M28Map.subrefLZTeamData][iTeam]
                                                                                    if M28Utilities.IsTableEmpty(tAdjLZTeamData[M28Map.subrefTEnemyUnits]) == false then
                                                                            ...
                                                                            --No nearby enemies subject to oClosestEnemy, consider building power if have low level of it and lots of mass
                                                                            if bDebugMessages == true then LOG(sFunctionRef..': Considering if we want more power for high mass levels, or want to attack nearby enemies, or get nearby reclaim, Is table of nearest DF enemies empty='..tostring(M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies]))..'; M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy]='..M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy]..'; not(M28Conditions.HaveLowMass(aiBrain)='..tostring(not(M28Conditions.HaveLowMass(aiBrain)))..'; Want more power='..tostring(M28Conditions.WantMorePower(iTeam))..'; tLZOrWZData[M28Map.subrefTotalSignificantMassReclaim]='..(tLZOrWZData[M28Map.subrefTotalSignificantMassReclaim] or 'nil')..'; oClosestEnemy='..(oClosestEnemy.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oClosestEnemy) or 'nil')..'; iNearbyEnemyAttackRangeThreshold='..iNearbyEnemyAttackRangeThreshold) end
                                                                            --Ignore scouts that arent in range
                                                                            if oClosestEnemy and (iClosestEnemy <= oACU[M28UnitInfo.refiDFRange] or not(EntityCategoryContains(M28UnitInfo.refCategoryLandScout - M28UnitInfo.refCategoryCombatScout, oClosestEnemy.UnitId))) and AttackNearestEnemyWithACU(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU) then
                                                                                if bDebugMessages == true then LOG(sFunctionRef..': Attacking nearby closest enemy') end
                                                                                --Closetoenemy check - in theory shouldnt trigger, but left in as a redundancy
                                                                            ...
                                                                            elseif bNoNearbyLRThreats and M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies]) and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] <= 50 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.3 and not(M28Conditions.HaveLowMass(aiBrain)) and (M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyFactoryTech] == 1 or (M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyFactoryTech] == 2 and oACU.HasEnhancement and oACU:HasEnhancement('AdvancedEngineering'))) and (M28Conditions.WantMorePower(iTeam) or M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.7) then
                                                                                ACUActionBuildPower(aiBrain, oACU)
                                                                            ...
                                                                            elseif tLZOrWZData[M28Map.subrefTotalSignificantMassReclaim] >= 40 and ConsiderNearbyReclaimForACUOrEngineer(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, true, nil, nil, nil) then
                                                                                ...
                                                                                if not(oACU:IsUnitState('Building')) and not(oACU:IsUnitState('Capturing')) and (not(oACU:IsUnitState('Repairing')) or (oACU:IsUnitState('Guarding') and oACU:GetWorkProgress() > 0 and not(EntityCategoryContains(categories.MOBILE - categories.COMMAND - M28UnitInfo.categories.SUBCOMMANDER, oACU:GetFocusUnit().UnitId)))) and (not(oACU:IsUnitState('Reclaiming')) or aiBrain:GetEconomyStoredRatio('MASS') >= 0.8) then
                                                                                    --Do we want to get an upgrade?

                                                                                    local sUpgradeToGet, bIgnoreOtherUpgradeConditions
                                                                                    if bNoNearbyLRThreats then sUpgradeToGet, bIgnoreOtherUpgradeConditions = GetACUUpgradeWanted(oACU, nil, tLZOrWZData, tLZOrWZTeamData, iPlateauOrZero) end
                                                                                    if bDebugMessages == true then LOG(sFunctionRef..': Do we have an upgrade we want to get? sUpgradeToGet='..(sUpgradeToGet or 'nil')..'; bNoNearbyLRThreats='..tostring(bNoNearbyLRThreats or false)) end
                                                                                    if sUpgradeToGet then
                                                                                        --Do we have a friendly nearby ugprading ACU? If so then assist its upgrade first before getting ours
                                                                                        if bIgnoreOtherUpgradeConditions or not(AssistNearbyUpgradingACU(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, 40)) then
                                                                                            --Are we safe to get the upgrade here? if not then retreat
                                                                                        ...
                                                                                        if not(ConsiderBuildingMex(tLZOrWZData, tLZOrWZTeamData, oACU, iMaxSearchRange)) then
                                                                                            ...
                                                                                            if iPlateauOrZero > 0 or not(tLZOrWZTeamData[M28Map.subrefWZbContainsUnderwaterStart]) then
                                                                                                ...
                                                                                                if bNoNearbyLRThreats and not(ConsiderGettingExtraLandFactory(tLZOrWZData, tLZOrWZTeamData, oACU, iPlateauOrZero, iLandOrWaterZone)) then
                                                                                                    ...
                                                                                                    if ConsiderNearbyReclaimForACUOrEngineer(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, true) or (M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subrefTEnemyUnits]) == false and AttackNearestEnemyWithACU(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU)) then
                                                                                                        ...
                                                                                                        if tLZOrWZData[M28Map.subrefTotalSignificantMassReclaim] >= iMinTotalReclaimThreshold and not(tLZOrWZTeamData[M28Map.subrefLZbCoreBase]) and tLZOrWZTeamData[M28Map.subrefLZSValue] <= 200 and aiBrain:GetEconomyStoredRatio('MASS') <= 0.6 and ConsiderNearbyReclaimForACUOrEngineer(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, false, iMinIndivReclaimThreshold) then
                                                                                                            ...
                                                                                                            local bAttackNearestEnemy, iClosestOutOfThresholdEnemy = AttackNearestEnemyWithACU(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, nil, true)
                                                                                                            if bAttackNearestEnemy then
                                                                                                                ...
                                                                                                                elseif iClosestOutOfThresholdEnemy <= 120 and oACU[refiUpgradeCount] >= 2 and M28UnitInfo.GetUnitHealthPercent(oACU) >= 0.98 and tLZOrWZData[M28Map.subrefTotalSignificantMassReclaim] <= 150 then
                                                                                                                    if bDebugMessages == true then LOG(sFunctionRef..': Dont want to look for reclaim outside of our build range as have nearby enemy that is outside normal range but close enough we want to attack') end
                                                                                                                    bOnlyConsiderInBuildRange = true
                                                                                                                end
                                                                                                                ...
                                                                                                                if ( tLZOrWZData[M28Map.subrefTotalSignificantMassReclaim] < 40 or not(ConsiderNearbyReclaimForACUOrEngineer(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, true))) and ( (tLZOrWZData[M28Map.subrefTotalSignificantMassReclaim] >= 40 or not(ConsiderNearbyReclaimForACUOrEngineer(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, bOnlyConsiderInBuildRange))) or (not(bOnlyConsiderInBuildRange) and not(ConsiderNearbyReclaimForACUOrEngineer(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, false))))  then
                                                                                                                    ...
                                                                                                                    if not(AssistNearbyUpgradingACU(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU)) then
                                                                                                                        ...
                                                                                                                        if M28Conditions.HaveEngineersOrFactoriesInZone(tLZOrWZTeamData) or (oACU[refiUpgradeCount] >= 2 and not(tLZOrWZTeamData[M28Map.subrefLZbCoreBase]) and tLZOrWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] and GetGameTimeSeconds() >= 180 and (oACU[M28UnitInfo.refiDFMassThreatOverride]) >= 4000) or not(ConsiderBuildingMex(tLZOrWZData, tLZOrWZTeamData, oACU)) then
                                                                                                                            if tLZOrWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] and (iClosestOutOfThresholdEnemy or 1000) <= 120 and AttackNearestEnemyWithACU(iPlateauOrZero, iLandOrWaterZone, tLZOrWZData, tLZOrWZTeamData, oACU, iClosestOutOfThresholdEnemy) then
                                                                                                                                --Do nothing - will have given the order
                                                                                                                                if bDebugMessages == true then LOG(sFunctionRef..':Attacking slightly further away enemies') end
                                                                                                                            else
        ...
        if iPlateauOrZero > 0 and aiBrain:GetEconomyStored('MASS') > 0 and not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass]) and (tLZOrWZTeamData[M28Map.subrefLZbCoreBase] or tLZOrWZTeamData[M28Map.subrefMexCountByTech][2] + tLZOrWZTeamData[M28Map.subrefMexCountByTech][3] >= 4) then
                ...
                if iExistingLRPDThreat <= 1500 or (iExistingLRPDThreat <= 3000 and not(M28Conditions.HaveLowPower(iTeam)) and not(M28Conditions.TeamHasLowMass(iTeam))) then
                    ...
                    if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentWaterZones]) == false then
                        for iEntry, tSubtable in tLZOrWZData[M28Map.subrefAdjacentWaterZones] do
                            local iAdjWZ = tSubtable[M28Map.subrefAWZRef]
                            local tAdjWZTeamData = M28Map.tPondDetails[M28Map.tiPondByWaterZone[iAdjWZ]][M28Map.subrefPondWaterZones][iAdjWZ][M28Map.subrefWZTeamData][iTeam]
                            iNearbyEnemyThreat = iNearbyEnemyThreat + math.max(0, (tAdjWZTeamData[M28Map.subrefWZThreatEnemyVsSurface] or 0) - (tAdjWZTeamData[M28Map.subrefWZThreatEnemyAntiNavy] or 0))
                        end
                    end
            ...
            if not(bGivenOrder) then
                ...
                elseif oACU[refiBuildTech] >= 3 and tLZOrWZTeamData[M28Map.subrefMexCountByTech][3] >= tLZOrWZData[M28Map.subrefLZOrWZMexCount] and (tLZOrWZTeamData[M28Map.subrefLZbCoreBase] or (M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] > 2 and tLZOrWZTeamData[M28Map.refiModDistancePercent] <= 0.15 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.7 and not(M28Conditions.HaveLowPower(iTeam))))
                        and (M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] > 0 or M28Conditions.GetTeamLifetimeBuildCount(iTeam, M28UnitInfo.refCategoryLandCombat + M28UnitInfo.refCategoryAirNonScout * categories.TECH3) >= 50)
                then
...
function ACUExpectedToDieBeforeCompletingUpgrade(oACU)
    local sFunctionRef = 'ACUExpectedToDieBeforeCompletingUpgrade'
    local bDebugMessages = false if M28Profiler.bGlobalDebugOverride == true then   bDebugMessages = true end
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerStart)

    if M28Utilities.IsTableEmpty(oACU[reftiUpgradingHealthData]) or not(oACU[refiHealthWhenStartedUpgrade]) or not(oACU[M28UnitInfo.refsLastEnhancementStarted]) then
        if bDebugMessages == true then LOG(sFunctionRef..': Lack data so returning false') end
        M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
        return false --redundancy
    else
        local iTotalRecordedCycles = table.getn(oACU[reftiUpgradingHealthData])
        if oACU[reftiUpgradingHealthData][iTotalRecordedCycles] + 100 <= oACU[refiHealthWhenStartedUpgrade] then
            local iHealthLoss = oACU[reftiUpgradingHealthData][1] - oACU[reftiUpgradingHealthData][iTotalRecordedCycles]
            if iHealthLoss > 100 then
                --How fast have we lost health?
                local iHealthLossPerCycle = iHealthLoss / math.max(5, (iTotalRecordedCycles - 1))
                --How long for us to complete the upgrade if we dont pause?
                local iTimeToCompleteUpgrade = 250 --redundancy
                local tUpgrades = oACU:GetBlueprint().Enhancements
                if tUpgrades[oACU[M28UnitInfo.refsLastEnhancementStarted]].BuildTime then
                    local iBuildRate = (oACU:GetBlueprint().Economy.BuildRate or 10)
                    if oACU.HasEnhancement then
                        if oACU:HasEnhancement('AdvancedEngineering') then
                            iBuildRate = (oACU:GetBlueprint().Enhancements.AdvancedEngineering.BuildRate or 42)
                        elseif oACU:HasEnhancement('T3Engineering') then
                            iBuildRate = (oACU:GetBlueprint().Enhancements.T3Engineering.BuildRate or 100)
                        end
                    end
                    iBuildRate = iBuildRate * (oACU:GetAIBrain()[M28Economy.refiBrainBuildRateMultiplier] or 1)
                    iTimeToCompleteUpgrade = (tUpgrades[oACU[M28UnitInfo.refsLastEnhancementStarted]].BuildTime * (1 - oACU:GetWorkProgress())) / iBuildRate
                end
                --Will we complete with a few seconds to spare?
                if bDebugMessages == true then LOG(sFunctionRef..': iHealthLossPerCycle='..iHealthLossPerCycle..'; iTimeToCompleteUpgrade='..iTimeToCompleteUpgrade..'; oACU[reftiUpgradingHealthData][iTotalRecordedCycles]='..oACU[reftiUpgradingHealthData][iTotalRecordedCycles]..'; Health at record 1='..oACU[reftiUpgradingHealthData][1]) end
                if iHealthLossPerCycle * (iTimeToCompleteUpgrade + math.min(30, iTimeToCompleteUpgrade * 0.2)) >= oACU[reftiUpgradingHealthData][iTotalRecordedCycles] then
                    if bDebugMessages == true then LOG(sFunctionRef..': We think we will die or come close so want to abort') end
                    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                    return true
                end
            end
        elseif bDebugMessages == true then LOG(sFunctionRef..': Health isnt lower than when we first started upgrade so will continue')
        end

    end
    if bDebugMessages == true then LOG(sFunctionRef..': We dont expect to die before completing the upgrade') end
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
    return false
end

function UpdateACUUpgradingHealth(oACU)
    if M28Utilities.IsTableEmpty(oACU[reftiUpgradingHealthData]) or table.getn(oACU[reftiUpgradingHealthData]) < 6 then
        --Nothing to remove
    else
        table.remove(oACU[reftiUpgradingHealthData], 1)
    end

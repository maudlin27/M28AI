...
function AdjustBlueprintForOverrides(aiBrain, oFactory, sBPIDToBuild, tLZTeamData, iFactoryTechLevel)
    ...
    if M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.subrefBlueprintBlacklist][sBPIDToBuild] then
        ...
        elseif sBPIDToBuild == 'uel0303' or sBPIDToBuild == 'brnt3bt' and not(ScenarioInfo.Options.M28PrioritiseBPs == 2) then --UEFLightT3DF
            if M28Team.tTeamData[aiBrain.M28Team][M28Team.refbEnemyHasHeavyLandT3] and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryLandCombat * (categories.uel0303 + categories.brnt3bt)) >= 30 then
        ...
        elseif sBPIDToBuild == 'ual0303' and not(ScenarioInfo.Options.M28PrioritiseBPs == 2) then --AeonLightT3DF
            if M28Team.tTeamData[aiBrain.M28Team][M28Team.refbEnemyHasHeavyLandT3] and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryLandCombat * categories.ual0303) >= 30 then
        ...
        elseif sBPIDToBuild == 'wsl0308' or sBPIDToBuild == 'ssl0311' and not(ScenarioInfo.Options.M28PrioritiseBPs == 2) then --SeraphimLightT3DF
            if M28Team.tTeamData[aiBrain.M28Team][M28Team.refbEnemyHasHeavyLandT3] and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryLandCombat * (categories.wsl0308 + categories.ssl0311)) >= 30 then
        ...
        if not(iBaseCategoryWanted) then
            if not(bDontGetCombat) and tLZTargetTeamData[M28Map.subrefbLZWantsSupport] then
                ...
                if bInSameIsland then
                    ...
                    --T2 Snipers only at T2 Phase then switch into T3 Snipers at T3 Phase!
                    ...
                    if M28Utilities.bLoudModActive and iFactoryTechLevel == 2 and categories.ual0204 and EntityCategoryContains(categories.AEON, oFactory.UnitId) then
                        ...
                        if iFactoryTechLevel == 2 and iBaseCategoryWanted == M28UnitInfo.refCategorySkirmisher * iTechCategory then
                            local iSkirmisherLCOfTech = M28Conditions.GetLifetimeBuildCount(oFactory:GetAIBrain(), M28UnitInfo.refCategorySkirmisher + categories.ual0204)
                            local iDFTankLCOfTech = M28Conditions.GetLifetimeBuildCount(oFactory:GetAIBrain(), M28UnitInfo.refCategoryDFTank * (categories.TECH2 + categories.TECH3))
                            if iSkirmisherLCOfTech < iDFTankLCOfTech then
...
function GetBlueprintToBuildForAirFactory(aiBrain, oFactory)
    ...
    if bHaveLowPower then
        ...
        iCurrentConditionToTry = iCurrentConditionToTry + 1
        if M28Team.tTeamData[iTeam][M28Team.refiEnemyAirAAThreat] == 0 and tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] and aiBrain[M28Economy.refiGrossEnergyBaseIncome] >= 30 and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryBomber + M28UnitInfo.refCategoryGunship) <= 12 then
        ...
        if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.toActiveSnipeTargets]) == false then
            ...
            if oACUToSnipe then
                if M28Utilities.bLoudModActive and not(M28Utilities.bLCEActive) and iFactoryTechLevel >= 3 and ConsiderBuildingCategory(M28UnitInfo.refCategoryGunship) then return sBPIDToBuild
                elseif ConsiderBuildingCategory(M28UnitInfo.refCategoryBomber) then
        ...
        if tLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] > 0 then
            if bDebugMessages == true then LOG(sFunctionRef..': Nearby enemy threat so will try and build gunship and bombers') end
            if ConsiderBuildingCategory(M28UnitInfo.refCategoryGunship) then return sBPIDToBuild end
            if ConsiderBuildingCategory(M28UnitInfo.refCategoryBomber) then return sBPIDToBuild end
        end
        ...
        if aiBrain[M28Overseer.refbCloseToUnitCap] and iFactoryTechLevel < math.min(3, M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyAirFactoryTech]) then
            ...
            if M28Utilities.IsTableEmpty(tLZData[M28Map.subrefLZAdjacentLandZones]) == false then
                for iEntry, iAdjLZ in tLZData[M28Map.subrefLZAdjacentLandZones] do
                    ...
                    if tAdjLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] > 0 and (tAdjLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] > 10 or not(tLZTeamData[M28Map.refbBaseInSafePosition])) then
                        if bDebugMessages == true then LOG(sFunctionRef..': Adjacent zone has enemy threat so will try and build gunship and bombers') end
                        if ConsiderBuildingCategory(M28UnitInfo.refCategoryGunship) then return sBPIDToBuild end
                        if ConsiderBuildingCategory(M28UnitInfo.refCategoryBomber) then return sBPIDToBuild end
                    end
                end
            ...
            if iFactoryTechLevel < math.min(3, M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyAirFactoryTech]) and (iFactoryTechLevel == 1 or (not(M28Team.tAirSubteamData[iAirSubteam][M28Team.refbNoAvailableTorpsForEnemies]) and not(tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ]))) then
                ...
                --Bombers if dont have at least 3
                iCurrentConditionToTry = iCurrentConditionToTry + 1
                local iCurBombers = aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryBomber)
                if iCurBombers < 2 then
                    if bDebugMessages == true then LOG(sFunctionRef..': Fewer than 2 Bombers so will try and get Bombers') end
                    if ConsiderBuildingCategory(M28UnitInfo.refCategoryBomber) then return sBPIDToBuild end
                end

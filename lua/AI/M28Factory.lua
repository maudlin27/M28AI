...
refiTimeLastDestroyedFactoryForMass = 'M28FaLstCtrlk' --gametimeseconds we last destroyed a factory for mass
...
function GetBlueprintThatCanBuildOfCategory(aiBrain, iCategoryCondition, oFactory, bGetSlowest, bGetFastest, bGetCheapest, iOptionalCategoryThatMustBeAbleToBuild, bIgnoreTechDifferences, iOptionalMaxSkirtSize, bGetMostExpensive)
    ...
    if not(oFactory.CanBuild) then
        ...
        for _, sBlueprint in tBlueprints do
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': About to see if factory '..oFactory.UnitId..M28UnitInfo.GetUnitLifetimeCount(oFactory)..' with fraction complete='..oFactory:GetFractionComplete()..' can build blueprint '..(sBlueprint or 'nil')..'; CanBuild='..tostring(oFactory:CanBuild(sBlueprint))..'; iArmyIndex='..(iArmyIndex or 'nil')) end
    ...
    if not(iBaseCategoryWanted) and (not(bDontGetCombat) and (tLZTargetTeamData[M28Map.subrefbLZWantsIndirectSupport] and not(bDontGetIndirect)) and tLZTargetTeamData[M28Map.subrefbLZWantsSupport]) or (GetGameTimeSeconds() - (tLZTargetTeamData[M28Map.subrefiTimeOfMMLFiringNearTMDOrShield] or -100) <= 30 and (not(M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) or tLZTargetTeamData[M28Map.refiModDistancePercent] <= 0.35 or M28UnitInfo.GetUnitTechLevel(oFactory) >= 3) and (not(bDontGetCombat) or not(tBaseLZTeamData[M28Map.subrefLZbCoreBase]) or oFactory[refiTotalBuildCount] <= 5 or (M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] == 0 and M28Conditions.GetLifetimeBuildCount(oFactory:GetAIBrain(), M28UnitInfo.refCategoryIndirectT3 + M28UnitInfo.refCategoryLandCombat * categories.TECH3) < 8) or oFactory:GetAIBrain():GetCurrentUnits(M28UnitInfo.refCategoryIndirect * categories.TECH2) < 30 - 10 * math.min(2.5, M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount]))) then
        ...
        if bWantMAANotIndirect then
            ...
            if not(tLZTargetTeamData[M28Map.subrefLZbCoreBase]) and M28Utilities.IsTableEmpty(tLZTargetTeamData[M28Map.subreftEnemyFirebasesInRange]) == false then
                ...
                --Upgrade if are at T1 and enemy has T2 PD, and avoid getting too much indirect if no nearby enemy PD or T2 arti threat
                if oFactory[refiTotalBuildCount] >= 3 then
                    ...
                    local bMayHaveTooMuchIndirect = (not(tBaseLZTeamData == tLZTargetTeamData) and tBaseLZTeamData[M28Map.subrefLZThreatAllyMobileIndirectTotal] + tLZTargetTeamData[M28Map.subrefLZThreatAllyMobileIndirectTotal] >= 1200 and (M28Utilities.IsTableEmpty(tLZTargetTeamData[M28Map.subreftEnemyFirebasesInRange]) or M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refiEnemyMobileDFThreatNearOurSide] > M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refiAllyMobileDFThreatNearOurSide] and M28Team.tTeamData[iTeam][M28Team.subrefiAlliedIndirectThreat] >= 5000 and (M28Team.tTeamData[iTeam][M28Team.subrefiAlliedIndirectThreat] >= 8000 or GetGameTimeSeconds() - (tLZTargetTeamData[M28Map.subrefiTimeOfMMLFiringNearTMDOrShield] or 0) >= 30)) and M28Team.tTeamData[iTeam][M28Team.subrefiAlliedIndirectThreat] > 0.5 * M28Team.tTeamData[iTeam][M28Team.subrefiAlliedDFThreat])
                    if bDebugMessages == true then LOG(sFunctionRef..': Considering further override to not get indirect, bMayHaveTooMuchIndirect='..tostring(bMayHaveTooMuchIndirect)..'; tBaseLZTeamData[M28Map.subrefLZThreatAllyMobileIndirectTotal]='..tBaseLZTeamData[M28Map.subrefLZThreatAllyMobileIndirectTotal]..'; tLZTargetTeamData[M28Map.subrefLZThreatAllyMobileIndirectTotal]='..tLZTargetTeamData[M28Map.subrefLZThreatAllyMobileIndirectTotal]..'; Total team subrefiAlliedDFThreat='..M28Team.tTeamData[iTeam][M28Team.subrefiAlliedDFThreat]..'; subrefiAlliedIndirectThreat='..M28Team.tTeamData[iTeam][M28Team.subrefiAlliedIndirectThreat]..'; is subreftEnemyFirebasesInRange empty='..tostring(M28Utilities.IsTableEmpty(tLZTargetTeamData[M28Map.subreftEnemyFirebasesInRange]))..'; refiEnemyMobileDFThreatNearOurSide='..M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refiEnemyMobileDFThreatNearOurSide]..'; refiAllyMobileDFThreatNearOurSide='..M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refiAllyMobileDFThreatNearOurSide]) end
                    if bMayHaveTooMuchIndirect or iFactoryTechLevel <= 2 then
                        ...
                        elseif iNearbyEnemyPDRange == 0 and tBaseLZTeamData[M28Map.subrefLZbCoreBase] and bDontGetCombat and aiBrain[M28Economy.refiOurHighestLandFactoryTech] >= 3 and (M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] > 0 or M28Conditions.GetTeamLifetimeBuildCount(iTeam, categories.TECH3 * categories.MOBILE * (categories.LAND + categories.AIR)) >= 10 * math.min(2.5, M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount])) and (M28Utilities.IsTableEmpty(tBaseLZTeamData[M28Map.subreftoActiveUpgrades]) or table.getn(tBaseLZTeamData[M28Map.subreftoActiveUpgrades]) <= 3) then
                        ...
                        elseif bMayHaveTooMuchIndirect and tBaseLZTeamData[M28Map.subrefLZbCoreBase] and M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refiEnemyMobileDFThreatNearOurSide] * 2 > M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refiAllyMobileDFThreatNearOurSide] and (M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refiEnemyMobileDFThreatNearOurSide] * 1.3 > M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refiAllyMobileDFThreatNearOurSide] or oFactory:GetAIBrain():GetCurrentUnits(M28UnitInfo.refCategoryIndirect * M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel)) >= 10 and (EntityCategoryContains(M28UnitInfo.refCategoryIndirect, oFactory[refsLastBlueprintBuilt] or M28Conditions.GetNumberOfUnitsMeetingCategoryUnderConstructionInLandOrWaterZone(tBaseLZTeamData, M28UnitInfo.refCategoryIndirect * M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel)) > 0))) then
                            if bDebugMessages == true then LOG(sFunctionRef..': We want more DF generally instead of indirectfire even if this zone specifically is requesting indirectfire') end
                        end
                        iBaseCategoryWanted = nil
                    end
                end
...
function GetBlueprintToBuildForLandFactory(aiBrain, oFactory)
        ...
        if iFactoryTechLevel == 1 or not(EntityCategoryContains(M28UnitInfo.refCategoryLandHQ, oFactory.UnitId)) or M28Utilities.bLoudModActive or M28Utilities.bQuietModActive or not(tLZTeamData[M28Map.subrefLZbCoreBase]) then
            if not(M28Conditions.CheckIfNeedMoreEngineersOrSnipeUnitsBeforeUpgrading(oFactory)) then
                ...
                if M28Utilities.IsTableEmpty(tLandFactoriesInLZ) == false then
                    ...
                    if iUpgradingLandFactories > 0 and iAvailableLandFactories <= 5 and (iUpgradingLandFactories > 4 or iUpgradingLandFactories + 2 > iAvailableLandFactories) then
                        if tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] and iFactoryTechLevel < aiBrain[M28Economy.refiOurHighestLandFactoryTech] and (not(tLZTeamData[M28Map.subrefLZbCoreBase]) or aiBrain[M28Economy.refiOurHighestLandFactoryTech] == 2 and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryLandCombat + M28UnitInfo.refCategoryMML - categories.TECH1) < 8) then
                ...
                if iFactoryTechLevel >= 2 and not(tLZTeamData[M28Map.subrefLZbCoreBase]) and not((tLZTeamData[M28Map.subrefLZFortify] and iFactoryTechLevel == 1)) and (aiBrain:GetEconomyStoredRatio('MASS') < 0.8 and aiBrain[M28Economy.refiGrossMassBaseIncome] < 80 and not((M28Map.bIsCampaignMap and not(bHaveLowMass)) or (tLZData[M28Map.subrefLZOrWZMexCount] == 0 and not(bHaveLowMass) and not(bHaveLowPower) and oFactory[refbPrimaryFactoryForIslandOrPond] and (oFactory[refiTotalBuildCount] or 0) >= 10) or (tLZData[M28Map.subrefLZOrWZMexCount] > 0 and tLZTeamData[M28Map.subrefMexCountByTech][3] >= math.min(2, tLZData[M28Map.subrefLZOrWZMexCount] * 0.5)))) then
                    bConsiderUpgrading = false
                    if bDebugMessages == true then LOG(sFunctionRef..': Dont want to go to T3 outside a core zone yet') end
                elseif not(tLZTeamData[M28Map.subrefLZbCoreBase]) and not((tLZTeamData[M28Map.subrefLZFortify] and iFactoryTechLevel == 1)) and iFactoryTechLevel >= aiBrain[M28Economy.refiOurHighestFactoryTechLevel] and aiBrain:GetEconomyStoredRatio('MASS') < 0.8 then
                    bConsiderUpgrading = false
                    ...
                    if not(oFactory[refbPrimaryFactoryForIslandOrPond]) and not((tLZTeamData[M28Map.subrefLZFortify] and iFactoryTechLevel == 1)) and (not(tLZTeamData[M28Map.subrefLZbCoreBase]) or bHaveLowMass or M28Team.DoesBrainHaveActiveHQUpgradesOfCategory(aiBrain, M28UnitInfo.refCategoryLandFactory)) and iFactoryTechLevel >= aiBrain[M28Economy.refiOurHighestLandFactoryTech] then
                        local iHQSearchCategory
                        if iFactoryTechLevel == 1 then iHQSearchCategory = M28UnitInfo.refCategoryLandFactory * categories.TECH1
                        elseif iFactoryTechLevel == 2 then iHQSearchCategory = M28UnitInfo.refCategoryLandHQ
                        end
    ...
    if M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyAirFactoryTech] <= 2 and M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech] >= 3 then
        if not(bDontConsiderBuildingMAA) and tLZTeamData[M28Map.subrefLZMAAThreatWanted] > tLZTeamData[M28Map.subrefLZThreatAllyMAA] + 0.5 * (tLZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] - tLZTeamData[M28Map.subrefLZThreatAllyMAA]) then
            if bDebugMessages == true then LOG(sFunctionRef..': Want more MAA, checking if more DF is more important, subrefbEnemiesInThisOrAdjacentLZ='..tostring(tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ])..'; subrefbLZWantsDFSupport='..tostring(tLZTeamData[M28Map.subrefbLZWantsDFSupport])..'; subrefLZDFThreatWanted='..tLZTeamData[M28Map.subrefLZDFThreatWanted]..'; subrefLZMAAThreatWanted='..tLZTeamData[M28Map.subrefLZMAAThreatWanted]..'; DF lifetime count='..M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryMobileDFLand * M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel))..'; MAA lifetime count='..M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryMAA * M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel))) end
            if not(tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ]) or not(tLZTeamData[M28Map.subrefbLZWantsDFSupport]) or (tLZTeamData[M28Map.subrefLZDFThreatWanted] < tLZTeamData[M28Map.subrefLZMAAThreatWanted] and (iFactoryTechLevel < aiBrain[M28Economy.refiOurHighestLandFactoryTech] or M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryMobileDFLand * M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel)) * 2 + 1 >= M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryMAA * M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel)))) then
    ...
    if aiBrain[M28Overseer.refbCloseToUnitCap] and (iFactoryTechLevel < math.min(3, M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyLandFactoryTech]) or (bHaveLowMass and not(tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ]))) then
        ...
        if not(bDontConsiderBuildingMAA) and not(tLZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ]) and iFactoryTechLevel >= 2 and not(M28Team.tAirSubteamData[iAirSubteam][M28Team.refbHaveAirControl]) and ((M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] or 0) > 0 or iNearbyMAAThreat < 2000) and (M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir] or iNearbyMAAThreat < math.max(iNearbyAirToGroundThreat, math.min(M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] * 0.5, 8000), math.max(0, 1000 - math.max(0, tLZTeamData[M28Map.subrefLZOrWZThreatAllyGroundAA] - iNearbyMAAThreat)) + 250 * iLandFactoriesInLZ) and M28Team.tTeamData[iTeam][M28Team.subrefiAlliedMAAThreat] <= math.max(4000, iNearbyAirToGroundThreat, math.min(8000, (M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] or 0) * 0.5))) then
            ...
            local oExpWantingMAA = M28Land.GetClosestExperimentalWantingMAAGuards(iPlateau, tLZData[M28Map.subrefLZIslandRef], iTeam, oFactory:GetPosition(), 200)
            if bDebugMessages == true then LOG(sFunctionRef..': Cur land experimentals='..aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryLandExperimental)..'; oExpWantingMAA='..(oExpWantingMAA.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oExpWantingMAA) or 'nil')) end
            if oExpWantingMAA or iNearbyMAAThreat < iNearbyAirToGroundThreat then
                ...
                if (iCurMAABeingBuilt == 0 or (iCurMAABeingBuilt <= 1 and (iLandFactoriesInLZ >= 4 or (oExpWantingMAA and oExpWantingMAA:GetFractionComplete() >= 0.8 and M28Utilities.GetDistanceBetweenPositions(oExpWantingMAA:GetPosition(), oFactory:GetPosition()) <= 80)))
                        and (iFactoryTechLevel >= 3 or not(tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ]) or iLandFactoriesInLZ >= 3 or iNearbyMAAThreat < iNearbyAirToGroundThreat * 0.75 or (oExpWantingMAA and M28Utilities.GetDistanceBetweenPositions(oExpWantingMAA:GetPosition(), oFactory:GetPosition()) <= 150)))
                then
                    ...
                    elseif oExpWantingMAA then
                        ...
                        --If have no MAA assigned to the exp then build T2 MAA first so at least have something
                        if M28Utilities.IsTableEmpty(oExpWantingMAA[M28Land.reftoAssignedMAAGuards]) then
                            if ConsiderBuildingCategory(M28UnitInfo.refCategoryMAA * categories.TECH2) then return sBPIDToBuild end
                        end
                        if ConsiderBuildingCategory(M28UnitInfo.refCategoryMAA - categories.TECH1) then return sBPIDToBuild end
                    elseif M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftoFriendlyLandExperimentals]) == false then
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': bExpOnIslandWantsMAA='..tostring(bExpOnIslandWantsMAA or false)..'; bExpOnPlateauWantsMAA='..tostring(bExpOnPlateauWantsMAA or false)) end
                        if bExpOnPlateauWantsMAA then
            ...
            if iCurT1T2BPWanted > 20 or (tLZTeamData[M28Map.subrefTbWantBP] and iLandFactoriesInLZ >= 4) then
                ...
                if iCurEngineersBeingBuilt < iMaxEngisAtOnce then
                    ...
                    if iCurEngineersInZone < 10 or (iCurEngineersInZone < 20 and iFactoryTechLevel == 2 and aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryLandCombat - categories.TECH1) > 30) then
                        --Exception if nearby enemies and havent built any combat of this tech level yet
                        local bIgnoreEngiBuilder = false
                        if iCurEngineersInZone >= 1 and tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] and tLZTeamData[M28Map.subrefbLZWantsDFSupport] and iFactoryTechLevel >= aiBrain[M28Economy.refiOurHighestLandFactoryTech] and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryMobileDFLand * M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel)) * 0.5 < iCurEngineersInZone then
                            if bDebugMessages == true then LOG(sFunctionRef..': Want more engis but we haven earby enemies and havent built any DF combat yet so will look to lower priority builders unless we havent built any mobile DF of this tech level in which case will try and build a skirmisher or failing that a mobile DF tank') end
                            local iCurLevelCategory = M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel)
                            local iCurMobileDFCount = M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryMobileDFLand * iCurLevelCategory)
                            if iCurMobileDFCount == 0 then
                                if not(aiBrain.M28Easy) and ConsiderBuildingCategory(M28UnitInfo.refCategorySkirmisher * iCurLevelCategory) then return sBPIDToBuild end
                                if ConsiderBuildingCategory(M28UnitInfo.refCategoryMobileDFLand * iCurLevelCategory) then return sBPIDToBuild end
                            end
        ...
        --Upgrade to T3 if no active upgrades and we have T3 HQ
        local bSaveMassForT3OrEnemyTurtling = false
        iCurrentConditionToTry = iCurrentConditionToTry + 1
        if iFactoryTechLevel < 3 and tLZTeamData[M28Map.subrefLZbCoreBase] and aiBrain[M28Economy.refiOurHighestLandFactoryTech] >= 3 then
            local iUpgradingT2LandFactoriesInZone = 0
            local iTotalT2Factories = M28Conditions.GetNumberOfConstructedUnitsMeetingCategoryInZone(tLZTeamData, M28UnitInfo.refCategoryLandFactory * categories.TECH2)
            if M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass] then bSaveMassForT3OrEnemyTurtling = true
            elseif bHaveLowMass and (iFactoryTechLevel == 1 or(not(tLZTeamData[M28Map.subrefbLZWantsDFSupport]) and not(tLZTeamData[M28Map.subrefbLZWantsIndirectSupport]) and (tLZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) == 0)) then  bSaveMassForT3OrEnemyTurtling = true
            end
            if M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subreftoActiveUpgrades]) == false then
                local tUpgradingT2Land = EntityCategoryFilterDown(M28UnitInfo.refCategoryLandFactory * categories.TECH2, tLZTeamData[M28Map.subreftoActiveUpgrades])
                if M28Utilities.IsTableEmpty(tUpgradingT2Land) == false then
                    iUpgradingT2LandFactoriesInZone = table.getn(tUpgradingT2Land)
                end
            end
            if bDebugMessages == true then LOG(sFunctionRef..': iUpgradingT2LandFactoriesInZone='..iUpgradingT2LandFactoriesInZone..'; iTotalT2Factories='..iTotalT2Factories..'; bHaveLowMass='..tostring(bHaveLowMass)..'; subrefbTeamIsStallingMass='..tostring(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass])..'; bSaveMassForT3OrEnemyTurtling='..tostring(bSaveMassForT3OrEnemyTurtling or false)) end
            if iUpgradingT2LandFactoriesInZone == 0 or (iUpgradingT2LandFactoriesInZone <= 1 and iTotalT2Factories >= 3) then
                if not(bHaveLowMass) or (iUpgradingT2LandFactoriesInZone == 0 and iTotalT2Factories >= 4) or (not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass]) and iTotalT2Factories >= 2 and (tLZTeamData[M28Map.subrefMexCountByTech][3] > iUpgradingT2LandFactoriesInZone)) then
                    if ConsiderUpgrading() then return sBPIDToBuild end
                end
            end
        end
        ...
        iCurrentConditionToTry = iCurrentConditionToTry + 1
        if not(bSaveMassForT3OrEnemyTurtling) and M28Utilities.bQuietModActive and iFactoryTechLevel >= 3 then
        ...
        if not(bSaveMassForT3OrEnemyTurtling) and iFactoryTechLevel == 2 and iFactoryTechLevel == M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyFactoryTech] and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryLandCombat - categories.TECH1) < math.min(3, aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryEngineer - categories.TECH1)) then
            ...
            if M28Team.tLandSubteamData[iLandSubteam][M28Team.refiEnemyMobileDFThreatNearOurSide] < 2000 and M28Team.tLandSubteamData[iLandSubteam][M28Team.refiAllyMobileDFThreatNearOurSide] > 4 * M28Team.tLandSubteamData[iLandSubteam][M28Team.refiEnemyMobileDFThreatNearOurSide] and (M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] < 1000 or M28Team.tAirSubteamData[iAirSubteam][M28Team.refbHaveAirControl]) then
                ...
                if tLZTeamData[M28Map.subrefMexCountByTech][3] < tLZData[M28Map.subrefLZOrWZMexCount] and M28Team.tTeamData[iTeam][M28Team.refiConstructedExperimentalCount] < 3 then
                    ...
                    elseif M28Conditions.ZoneWantsT1Spam(tLZTeamData, iTeam) and M28Conditions.GetEnemyTeamActualMassIncome(iTeam) < 1.2 * M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] then
                        ...
                        bSaveMassForT3OrEnemyTurtling = true
                    end
                end
            end
        end
        ...
        iCurrentConditionToTry = iCurrentConditionToTry + 1
        if iFactoryTechLevel >= 2 and bHaveHighestLZTech and not(bSaveMassForT3OrEnemyTurtling) and tLZTeamData[M28Map.subrefLZbCoreBase] and oFactory[refiTotalBuildCount] <= 16 and (iFactoryTechLevel == 2 or oFactory[refiTotalBuildCount] <= 12 or M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyGroundTech] < 3) then
        ...
        if iFactoryTechLevel < 3 then
            ...
            if iFactoryTechLevel >= 2 then
                ...
                if tLZTeamData[M28Map.subrefLZFortify] and M28Map.bIsCampaignMap and iUnitCountToUpgrade > 1 and M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subreftoActiveUpgrades]) then
                    if not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingEnergy]) and not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass]) then
                        if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingHQs]) then
                            iUnitCountToUpgrade = 1
                        elseif table.getn(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingHQs]) < 2 then
                            local iHighestHQProgress = 0
                            for iHQ, oHQ in M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingHQs] do
                                iHighestHQProgress = math.max(oHQ:GetWorkProgress(), iHighestHQProgress)
                            end
                            if iHighestHQProgress < 0.2 then
                                iUnitCountToUpgrade = 1
                            elseif iHighestHQProgress < 0.4 then
                                iUnitCountToUpgrade = 2
                            end
                        else
                            iUnitCountToUpgrade = 3
                        end
                    elseif M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingHQs]) then
                        iUnitCountToUpgrade = 2
                    else
                        iUnitCountToUpgrade = 3
                    end
                elseif iUnitCountToUpgrade >= 3 and not(bHaveLowMass) and tLZTeamData[M28Map.subrefLZbCoreBase] and iFactoryTechLevel < aiBrain[M28Economy.refiOurHighestLandFactoryTech]  and M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subrefTEnemyUnits]) and (M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageEnergyPercentStored] >= 0.99 or M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] >= 150) and not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingEnergy]) then
                    ...
                    if not(bAppliedLowerCount) and oFactory[refiTotalBuildCount] >= 3 then
                        --Still apply lower count if we have 1 or fewer land facs upgrading
                        local tLandFactoriesInLZ = EntityCategoryFilterDown(M28UnitInfo.refCategoryLandFactory, tLZTeamData[M28Map.subreftoLZOrWZAlliedUnits])
                        local iUpgradingLandFactories = 0
                        local iAvailableLandFactories = 0
                        if M28Utilities.IsTableEmpty(tLandFactoriesInLZ) == false then
                        ...
                        if iUpgradingLandFactories < 2 and iAvailableLandFactories > 1 and (iUpgradingLandFactories == 0 or (oFactory[refiTotalBuildCount] >= 5 and iAvailableLandFactories >= 3)) then
                            iUnitCountToUpgrade = 3
                            bAppliedLowerCount = true
                            if bDebugMessages == true then LOG(sFunctionRef..': Have multiple available land factories so will massively reduce unit count') end
                        end
                    end
                    if bDebugMessages == true then LOG(sFunctionRef..': bAppliedLowerCount='..tostring(bAppliedLowerCount)) end
                    ...
                    if tLZTeamData[M28Map.subrefLZbCoreBase] and not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass]) and ((iFactoryTechLevel == 1 and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryLandCombat + M28UnitInfo.refCategoryMML - categories.TECH1) >= 5) or (iFactoryTechLevel == 2 and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryLandCombat + M28UnitInfo.refCategoryMML - categories.TECH1 - categories.TECH2) >= 5)) then
                        iUnitCountToUpgrade = iUnitCountToUpgrade * 0.25
                        if bDebugMessages == true then LOG(sFunctionRef..': T1 fac in core base so will reduce unit count to being just 25%') end
                    elseif (iFactoryTechLevel == 1 or (tLZTeamData[M28Map.subrefLZbCoreBase] and iFactoryTechLevel < aiBrain[M28Economy.refiOurHighestLandFactoryTech])) and aiBrain:GetEconomyStored('MASS') >= 1000 and (aiBrain:GetEconomyStored('MASS') >= 2000 or  aiBrain[M28Economy.refiNetMassBaseIncome] > 0) and M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subrefTEnemyUnits]) and (M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageEnergyPercentStored] >= 0.99 or aiBrain[M28Economy.refiGrossEnergyBaseIncome] >= 150) and not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingEnergy]) and tLZTeamData[M28Map.subrefMexCountByTech][2] + tLZTeamData[M28Map.subrefMexCountByTech][3] >= 3 then
            ...
            --Non-FAF specific - upgrade fewer factories at once if this isnt highest tech since no support factories
            if not(M28Utilities.bFAFActive) and M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subreftoActiveUpgrades]) == false and iFactoryTechLevel < aiBrain[M28Economy.refiOurHighestLandFactoryTech] then
                --Do we still have mexes of this tech level to upgrade?
                if tLZTeamData[M28Map.subrefMexCountByTech][iFactoryTechLevel] > 0 then
                    local iActiveUpgradingFacsOfThisOrHigherTechLevel = 0
                    for iUnit, oUnit in tLZTeamData[M28Map.subreftoActiveUpgrades] do
                        if M28UnitInfo.GetUnitTechLevel(oUnit) >= iFactoryTechLevel then
                            iActiveUpgradingFacsOfThisOrHigherTechLevel = iActiveUpgradingFacsOfThisOrHigherTechLevel + 1
                        end
                    end
                    iUnitCountToUpgrade = iUnitCountToUpgrade + 25 * iActiveUpgradingFacsOfThisOrHigherTechLevel
                    if iActiveUpgradingFacsOfThisOrHigherTechLevel >= 2 and ((tLZTeamData[M28Map.subrefMexCountByTech][3] == 0 and iFactoryTechLevel == 1) or tLZTeamData[M28Map.subrefMexCountByTech][3] < iActiveUpgradingFacsOfThisOrHigherTechLevel) then
                        iActiveUpgradingFacsOfThisOrHigherTechLevel = iActiveUpgradingFacsOfThisOrHigherTechLevel * 5 + (tLZTeamData[M28Map.subrefMexCountByTech][1] + tLZTeamData[M28Map.subrefMexCountByTech][2]) * 10
                        if bDebugMessages == true then LOG(sFunctionRef..': Increased iUnitCountToUpgrade for iActiveUpgradingFacsOfThisOrHigherTechLevel='..iActiveUpgradingFacsOfThisOrHigherTechLevel) end
                    end
                end
            end
        ...
        if ((tLZTeamData[M28Map.reftRaiderSubtable][M28Map.subrefiCurRaidingZoneTarget] or 0) > 0 or (GetGameTimeSeconds() >= 360 and not(tLZTeamData[M28Map.reftRaiderSubtable]) and oFactory[refiTotalBuildCount] >= 8 and GetGameTimeSeconds() - (tLZTeamData[M28Map.subrefiTimeLastCheckedForZoneToTarget] or 0) >= 100 * iFactoryTechLevel)) and (tLZTeamData[M28Map.reftRaiderSubtable][M28Map.subrefiActiveRaidersCreatedByThisZone] or 0) < 2 and GetGameTimeSeconds() - (tLZTeamData[M28Map.reftRaiderSubtable][M28Map.subrefiTimeSinceLastConstructedRaiderAssignment] or 0) >= 60 then
            --Check we are on same island as enemy base, or there are enemies on this plateau
            local bHaveEnemiesToAttack = false
            if tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] or NavUtils.GetLabel(M28Map.refPathingTypeLand, tLZTeamData[M28Map.reftClosestEnemyBase]) == tLZData[M28Map.subrefLZIslandRef] then
                bHaveEnemiesToAttack = true
            elseif M28Utilities.IsTableEmpty(tLZData[M28Map.subrefLZAdjacentLandZones]) == false and M28Utilities.IsTableEmpty(M28Map.tAllPlateaus[iPlateau][M28Map.subrefPlateauIslandLandZones]) == false then
                for iIsland, tLandZonesInIsland in M28Map.tAllPlateaus[iPlateau][M28Map.subrefPlateauIslandLandZones] do
                    --Only consider islands with mexes (for performance reasons)
                    if iIsland == tLZData[M28Map.subrefLZIslandRef] then
                        --Get the land zone in this island that is closest to our current land zone
                        for iEntry, iIslandLZ in tLandZonesInIsland do
                            local tOtherIslandLZTeamData = M28Map.tAllPlateaus[iPlateau][M28Map.subrefPlateauLandZones][iIslandLZ][M28Map.subrefLZTeamData][iTeam]
                            if tOtherIslandLZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ] then
                                bHaveEnemiesToAttack = true
                                break
                            end
                        end
                        if bHaveEnemiesToAttack then break end
                    end
                end
            end
        ...
        if not(bSaveMassForT3OrEnemyTurtling) and iFactoryTechLevel <= 2 and GetGameTimeSeconds() <= 480 and not(tLZTeamData[M28Map.refbBaseInSafePosition]) and aiBrain[M28Map.refbCanPathToEnemyBaseWithLand] and
        ...
        iCurrentConditionToTry = iCurrentConditionToTry + 1
        if bDebugMessages == true then LOG(sFunctionRef .. ': iCurrentConditionToTry=' .. iCurrentConditionToTry .. '; Will consider if we want more engis due to only having a few in the land zone, bNeedCurTech=' .. tostring(bNeedCurTech) .. '; tLZTeamData[M28Map.subrefTThreatEnemyCombatTotal]=' .. tLZTeamData[M28Map.subrefTThreatEnemyCombatTotal]..'; tLZTeamData[M28Map.subrefLZbCoreBase]='..tostring(tLZTeamData[M28Map.subrefLZbCoreBase])..'; subrefLZFortify='..tostring(tLZTeamData[M28Map.subrefLZFortify])) end
        if bNeedCurTech and not (tLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] > 10) and (iFactoryTechLevel >= 3 or not(bHaveLowMass) or not(M28Map.bIsLowMexMap) or (tLZTeamData[M28Map.subrefLZFortify] and iFactoryTechLevel > 1 and M28Conditions.GetFactoryLifetimeCount(oFactory, M28UnitInfo.refCategoryEngineer) == 0)) then
            ...
            elseif bHaveLowMass and GetGameTimeSeconds() - (M28Team.tTeamData[iTeam][M28Team.refiTimeOfLastMassStall] or 0) <= 10 and tLZData[M28Map.subrefTotalSignificantMassReclaim] < 50 then
                if bDebugMessages == true then LOG(sFunctionRef..': Recently stalled so reducing engis wanted') end
                iEngisWanted = 2
            end
            if M28Utilities.bLoudModActive or M28Utilities.bQuietModActive then
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Fortify zone additional check - number of engis of this cat in zone='..M28Conditions.GetNumberOfConstructedUnitsMeetingCategoryInZone(tLZTeamData, M28UnitInfo.refCategoryEngineer * M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel))) end
            if tLZTeamData[M28Map.subrefLZFortify] and iFactoryTechLevel > 1 and M28Conditions.GetNumberOfConstructedUnitsMeetingCategoryInZone(tLZTeamData, M28UnitInfo.refCategoryEngineer * M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel)) == 0 then
                if ConsiderBuildingCategory(M28UnitInfo.refCategoryEngineer * M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel)) then return sBPIDToBuild end
            end
        end
        ...
        if not(NavUtils.GetLabel(M28Map.refPathingTypeLand, oFactory:GetPosition()) == NavUtils.GetLabel(M28Map.refPathingTypeLand, tLZTeamData[M28Map.reftClosestFriendlyBase])) or tLZTeamData[M28Map.subrefLZCoreExpansion] then
            ...
            if iNearbyDFThreat < iFriendlyDFThreatWanted or iNearbyIFThreat < iFriendlyIFThreatWanted or iNearbyGroundAAThreat < iFriendlyAAThreatWanted then
                ...
                if iNearbyDFThreat < iFriendlyDFThreatWanted or iNearbyIFThreat < iFriendlyIFThreatWanted or iNearbyGroundAAThreat < iFriendlyAAThreatWanted then
                    ...
                    if iNearbyDFThreat < math.min(iFriendlyDFThreatWanted, 100) then
                    ...
                    if iNearbyIFThreat < math.min(iFriendlyIFThreatWanted, 50) then
        ...
        if tLZTeamData[M28Map.subrefTbWantBP] and M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] > 0.01 and ((bHaveHighestLZTech and (not(M28Map.bIsLowMexMap) or not(bHaveLowMass))) or (M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] > 0.4 and tLZTeamData[M28Map.subrefLZbCoreBase])) then
            if not((M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and bHaveLowMass) then
                ...
                if tLZTeamData[M28Map.subrefLZbCoreBase] or oFactory[refiTotalBuildCount] < 5 or ((not(bHaveLowMass) or GetGameTimeSeconds() - (M28Team.tTeamData[iTeam][M28Team.refiTimeOfLastMassStall] or 0) > 10 or tLZData[M28Map.subrefTotalSignificantMassReclaim] >= 50)) and (GetEngiCountInZone() < 10 or tLZData[M28Map.subrefLZIslandRef] == NavUtils.GetTerrainLabel(M28Map.refPathingTypeLand, tLZTeamData[M28Map.reftClosestFriendlyBase]) or M28Map.tAllPlateaus[iPlateau][M28Map.subrefPlateauIslandMexCount] >= 7) then
                    if ConsiderBuildingCategory(M28UnitInfo.refCategoryEngineer) then
                        return sBPIDToBuild
                    end
                end
        ...
        iCurrentConditionToTry = iCurrentConditionToTry + 1
        if not (bHaveLowMass) and not(bSaveMassForT3OrEnemyTurtling) and (not (bSaveMassDueToEnemyFirebaseOrOurExperimental) or M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.45) then
            ...
            if bEnemiesRelativelyNear then
                ...
                elseif bHaveHighestLZTech or (iFactoryTechLevel == aiBrain[M28Economy.refiOurHighestLandFactoryTech] - 1 and ((iFactoryTechLevel == 1 and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryLandCombat - categories.TECH1) <= 6) or (iFactoryTechLevel == 2 and M28Conditions.GetLifetimeBuildCount(aiBrain, M28UnitInfo.refCategoryLandCombat * categories.TECH3) <= 6))) then
                    ...
                    --Upgrade from T1 to T2 even at a support factory if have built some units
                elseif (iFactoryTechLevel == 1 or (tLZTeamData[M28Map.subrefLZbCoreBase] and iFactoryTechLevel < aiBrain[M28Economy.refiOurHighestLandFactoryTech])) and tLZTeamData[M28Map.refiModDistancePercent] <= 0.55 and tLZTeamData[M28Map.subrefLZThreatEnemyMobileDFTotal] == 0 and (tLZTeamData[M28Map.subrefMexCountByTech][2] > 0 or tLZTeamData[M28Map.subrefMexCountByTech][3] > 0) and oFactory[refiTotalBuildCount] >= 8 and (oFactory[refiTotalBuildCount] >= 14 or tLZTeamData[M28Map.subrefMexCountByTech][3] > 0 or tLZTeamData[M28Map.subrefMexCountByTech][2] >= 3 or aiBrain[M28Economy.refiOurHighestLandFactoryTech] >= 3 or M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyGroundTech] >= 3 or M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech] >= 3) then
                    if bDebugMessages == true then LOG(sFunctionRef..': T1 fac in a T3 world so will upgrade') end
                    if ConsiderUpgrading() then return sBPIDToBuild end
                    --Core base - still consider upgrading if enemy not in this zone yet and we have at least 2 factories of a higher tech level
                elseif tLZTeamData[M28Map.subrefLZThreatEnemyMobileDFTotal] < 50 and M28Conditions.GetNumberOfConstructedUnitsMeetingCategoryInZone(tLZTeamData, M28UnitInfo.refCategoryLandFactory - M28UnitInfo.ConvertTechLevelToCategory(iFactoryTechLevel)) >= 2 and aiBrain[M28Economy.refiOurHighestLandFactoryTech] > iFactoryTechLevel and (M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subreftoActiveUpgrades]) or M28Utilities.IsTableEmpty(EntityCategoryFilterDown(M28UnitInfo.refCategoryLandFactory, tLZTeamData[M28Map.subreftoActiveUpgrades])) or table.getn(EntityCategoryFilterDown(M28UnitInfo.refCategoryLandFactory, tLZTeamData[M28Map.subreftoActiveUpgrades])) < 3) then
                    if bDebugMessages == true then LOG(sFunctionRef..': Core base so want to upgrade factory') end
                    if ConsiderUpgrading() then return sBPIDToBuild end
                else
                    if bCanPathToEnemyWithLand then
...
function IsFactoryReadyToBuild(oFactory)
    ...
    if oFactory:GetFractionComplete() == 1 and oFactory:GetWorkProgress() == 0 and not (oFactory:IsUnitState('Building')) and not (oFactory:IsUnitState('Upgrading')) and not (oFactory:IsUnitState('Busy')) and
        ...
        if GetGameTimeSeconds() > 10 or (M28Air.tbFullAirTeamCycleRun[iTeam] and M28Air.tbFullAirSubteamCycleRun[aiBrain.M28AirSubteam]) then
            ...
            elseif aiBrain.CheatEnabled and (aiBrain[M28Economy.refiBrainResourceMultiplier] or tonumber(ScenarioInfo.Options.CheatMult or 1.5)) >= 2 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.5 and M28Team.tTeamData[iTeam][M28Team.subrefiGrossEnergyWhenStalled] > (M28Team.tTeamData[iTeam][M28Team.subrefiGrossEnergyWhenStalled] or 0) * 1.25 and GetGameTimeSeconds() - (M28Team.tTeamData[iTeam][M28Team.refiTimeOfLastEnergyStall] or -100) >= 15 then
                M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                return true
            end
        end
...
function MovePotentialBlockingUnitsFromFactory(oFactory)
    ...
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerStart)
    local aiBrain = oFactory:GetAIBrain()
    if not(EntityCategoryContains(categories.MOBILE, oFactory.UnitId)) then oFactory[refiTimeOfLastFacBlockOrder] = math.floor(GetGameTimeSeconds()) end
...
function DecideAndBuildUnitForFactory(aiBrain, oFactory, bDontWait, bConsiderDestroyingForMass)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': Start of code for oFactory='..oFactory.UnitId..M28UnitInfo.GetUnitLifetimeCount(oFactory)..'; M28BuilderCheckActive='..tostring(oFactory['M28BuilderCheckActive'] or false)..'; time='..GetGameTimeSeconds()) end
                ...
                if iTicksWaited >= 200 and (iTicksWaited >= 400 or not(EntityCategoryContains(M28UnitInfo.refCategoryNavalFactory, oFactory.UnitId))) then
                    ...
                    if not(oFactory:GetAIBrain()[M28Overseer.refbCloseToUnitCap]) then
                        ...
                        if bDebugMessages == true then
                            ...
                            LOG(sFunctionRef..': will check for blocking units around oFactory='..oFactory.UnitId..M28UnitInfo.GetUnitLifetimeCount(oFactory)..' if havent recently, Time since refiTimeOfLastFacBlockOrder='..GetGameTimeSeconds() - (oFactory[refiTimeOfLastFacBlockOrder] or 0)..'; refiOurHighestAirFactoryTech='..oFacBrain[M28Economy.refiOurHighestAirFactoryTech]..'; Our highest land fac tech='..oFacBrain[M28Economy.refiOurHighestLandFactoryTech]..'; refiOurHighestNavalFactoryTech='..oFacBrain[M28Economy.refiOurHighestNavalFactoryTech])
                        end
                        if GetGameTimeSeconds() - (oFactory[refiTimeOfLastFacBlockOrder] or 0) >= 20 then
                            MovePotentialBlockingUnitsFromFactory(oFactory)
                        end
                    end
                    break
                        ...
                        if tLZOrWZTeamData then
                            ...
                            if bConsiderDestroyingForMass and M28Team.tTeamData[oFactory:GetAIBrain().M28Team][M28Team.subrefiHighestFriendlyFactoryTech] >= 2 and not(oFactory[refbPrimaryFactoryForIslandOrPond]) then
                                ...
                                if (not(M28Utilities.bFAFActive) or iFactoryTechLevel == 1 or not(EntityCategoryContains(M28UnitInfo.refCategoryLandHQ, oFactory.UnitId))) and M28Conditions.HaveLowMass(aiBrain) then
                                    if not(aiBrain[refiTimeLastDestroyedFactoryForMass]) or GetGameTimeSeconds() - aiBrain[refiTimeLastDestroyedFactoryForMass] >= 7 + 7 * iFactoryTechLevel then
                        ...
                        if not (bSelfDestructIfLowMass)then
                            ...
                            if sBPToBuild then
                                ...
                                if (not(oFactory[M28UnitInfo.refbCampaignTriggerAdded]) or not(M28Map.bIsCampaignMap)) and EntityCategoryContains(M28UnitInfo.refCategoryFactory - categories.EXPERIMENTAL, oFactory.UnitId)  then --entity category contains is a redundancy
                                    ...
                                    aiBrain[refiTimeLastDestroyedFactoryForMass] = GetGameTimeSeconds()
                                end
                            end
                        end
                    end
    ...
    if not(oFactory.Dead) and oFactory then
            ...
            for iFactory, oFactory in tOurFactories do
                if M28UnitInfo.IsUnitValid(oFactory) and oFactory:GetFractionComplete() == 1 then
                    ...
                    elseif oFactory:GetWorkProgress() == 0 and oFactory[refiFirstTimeOfLastOrder] and GetGameTimeSeconds() - oFactory[refiFirstTimeOfLastOrder] >= 60 and GetGameTimeSeconds() - (oFactory[refiTimeOfLastFacBlockOrder] or 0) >= 60 then
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': None of conditions for checks satisfied') end
                    end
                end
                M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                WaitTicks(1)
...
function GetBlueprintToBuildForAirFactory(aiBrain, oFactory)
    ...
    if bHaveLowPower or (bHaveLowMass and aiBrain[M28Overseer.refbPrioritiseLand] and ((M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech] == 3 and M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat] >= 1500) or (iFactoryTechLevel >= M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech] and not(M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir]) and M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat] >= 500))) then
        ...
        if iFactoryTechLevel == 1 and (M28Map.iMapSize > 256 or (M28UnitInfo.GetUnitLifetimeCount(oFactory) > 1 or aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryLandFactory) == 1)) and not(M28Team.tAirSubteamData[iAirSubteam][M28Team.refbDontBuildEngiHunterEngineers]) and (M28Team.tTeamData[iTeam][M28Team.refiEnemyAirAAThreat] or 0) == 0 and M28Conditions.GetAirSubteamLifetimeBuildCount(iAirSubteam, M28UnitInfo.refCategoryBomber) == 0 and aiBrain[M28Economy.refiGrossEnergyBaseIncome] >= 24 and (not(M28Utilities.bLoudModActive) or aiBrain[M28Economy.refiGrossEnergyBaseIncome] >= 40 or M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftTransportIslandDropShortlist]))  then
        ...
        iCurrentConditionToTry = iCurrentConditionToTry + 1
        if bDebugMessages == true then LOG(sFunctionRef..': low power medium priority transport builder, Is island shortlist empty='..tostring(M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftTransportIslandDropShortlist]))..'; refiGrossEnergyBaseIncome for brain='..aiBrain[M28Economy.refiGrossEnergyBaseIncome]..'; Time since last tried building transport='..GetGameTimeSeconds() - (M28Team.tAirSubteamData[aiBrain.M28AirSubteam][M28Team.refiTimeLastTriedBuildingTransport] or 0)..'; E stored%='..aiBrain:GetEconomyStoredRatio('ENERGY')) end
        if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftTransportIslandDropShortlist]) == false and aiBrain[M28Economy.refiGrossEnergyBaseIncome] >= 40 and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] >= 50 and (M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossEnergy] >= 150 or iFactoryTechLevel == 1 or M28Team.tTeamData[iTeam][M28Team.subrefiTeamNetEnergy] >= 15 * (0.5 + 0.5 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount]) * aiBrain[M28Economy.refiBrainBuildRateMultiplier]) and M28Conditions.GetNumberOfConstructedUnitsMeetingCategoryInZone(tLZTeamData, M28UnitInfo.refCategoryEngineer) >= 5 and aiBrain:GetEconomyStoredRatio('ENERGY') >= 0.15 then
...
function GetBlueprintToBuildForNavalFactory(aiBrain, oFactory)
    ...
    iCurrentConditionToTry = iCurrentConditionToTry + 1
    if tWZTeamData[M28Map.subrefWZThreatEnemySubmersible] > (tWZTeamData[M28Map.subrefWZThreatAlliedAntiNavy] or 0) * 0.25 then
        if bDebugMessages == true then
    ...
    if tWZTeamData[M28Map.subrefWZThreatEnemyVsSurface] > 0 then
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': Immediate threat - want frigate (or dstro if t3 naval fac and already have some combat) or sub if enemy lacks antinavy')
        end
        --get subs if enemy lacks antinavy
        if tWZTeamData[M28Map.subrefWZThreatEnemyAntiNavy] == 0 and not(tWZTeamData[M28Map.subrefbWZOnlyHoverEnemies]) and ConsiderBuildingCategory(M28UnitInfo.refCategorySubmarine - categories.TECH3) then return sBPIDToBuild
            --get battleship in campaign if lack any
        elseif iFactoryTechLevel >= 3 and tWZTeamData[M28Map.subrefWZTThreatAllyCombatTotal] >= 800 and oFactory[refiTotalBuildCount] >= 5 and aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryDestroyer) >= 3 and aiBrain:GetCurrentUnits(iCombatCategory) == 0 and ConsiderBuildingCategory(iCombatCategory) then return sBPIDToBuild
        elseif iFactoryTechLevel >= 3 and tWZTeamData[M28Map.subrefWZTThreatAllyCombatTotal] >= 800 and oFactory[refiTotalBuildCount] >= 5 and aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryFrigate) >= 3 then
    ...
    if not(bHaveLowMass) and not(bHaveLowPower) and aiBrain:GetEconomyStoredRatio('MASS') >= 0.5 and tWZTeamData[M28Map.subrefTbWantBP] then
        ...
        if iFactoriesInZone >= 2 or M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.9 or (M28Team.tTeamData[iTeam][M28Team.subrefiTeamNetMass] > 0 and (not(aiBrain[M28Map.refbCanPathToEnemyBaseWithLand]) or (not(bHaveLowMass) and not(bHaveLowPower)) or (M28Team.tTeamData[iTeam][M28Team.refiPriorityPondValues][iPond] or 0) >= 16)) and iEngineersUnderConstruction < iFactoriesInZone * 0.5 and (iFactoriesInZone > 1 or M28Conditions.GetFactoryLifetimeCount(oFactory, M28UnitInfo.refCategoryEngineer) < oFactory[refiTotalBuildCount] * 0.5) then
            if ConsiderBuildingCategory(M28UnitInfo.refCategoryEngineer) then return sBPIDToBuild end
        end
    end
    ...
    iCurrentConditionToTry = iCurrentConditionToTry + 1
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': Engi fi underwtaer start: tWZTeamData[M28Map.subrefWZbContainsUnderwaterStart]=' .. tostring(tWZTeamData[M28Map.subrefWZbContainsUnderwaterStart]) .. '; tWZTeamData[M28Map.subrefTbWantBP]=' .. tostring(tWZTeamData[M28Map.subrefTbWantBP]) .. '; bHaveLowMass=' .. tostring(bHaveLowMass) .. '; aiBrain[M28Economy.refiGrossMassBaseIncome]=' .. aiBrain[M28Economy.refiGrossMassBaseIncome]..'; subrefiTeamAverageMassPercentStored Mass%='..M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored])
    end
    if tWZTeamData[M28Map.subrefWZbContainsUnderwaterStart] or (not(bHaveLowMass) and M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.2 and (not(aiBrain[M28Map.refbCanPathToEnemyBaseWithLand]) or (not(bHaveLowPower) and M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.3) or (M28Team.tTeamData[iTeam][M28Team.refiPriorityPondValues][iPond] or 0) >= 16))  then
        if bDebugMessages == true then LOG(sFunctionRef..': Checking that last unit built wasnt an engineer so we dont build too many at once, oFactory[refsLastBlueprintBuilt]='..(oFactory[refsLastBlueprintBuilt] or 'nil')) end
        ...
        for iEntry, tSubtable in tWZData[M28Map.subrefWZOtherWaterZones] do
            if bDontCheckPlayableArea or M28Conditions.IsLocationInPlayableArea(tWZData[M28Map.subrefMidpoint]) then
                ...
                if not(bHaveWantedAA) and ((tOtherWZTeamData[M28Map.refiTimeLastRunFromEnemyAir] and GetGameTimeSeconds() - tOtherWZTeamData[M28Map.refiTimeLastRunFromEnemyAir] <= 60 and (GetGameTimeSeconds() - tOtherWZTeamData[M28Map.refiTimeLastRunFromEnemyAir] <= 20 or iOurCumulativeCombatThreat * iMinAARatioWanted * 2 > iOurCumulativeAAThreat)) or (iOurCumulativeCombatThreat >= 4000 and (tOtherWZTeamData[M28Map.subrefbWZWantsSupport] or iOurCumulativeCombatThreat >= 6000*iFactoryTechLevel) and iOurCumulativeCombatThreat * iMinAARatioWanted > iOurCumulativeAAThreat)) then
                    if bDebugMessages == true then LOG(sFunctionRef..': Want AA to maintain basic ratio to combat threat, iOurCumulativeCombatThreat='..iOurCumulativeCombatThreat..'; iOurCumulativeAAThreat='..iOurCumulativeAAThreat..'; refbHaveAirControl='..tostring(M28Team.tAirSubteamData[aiBrain.M28AirSubteam][M28Team.refbHaveAirControl])..'; refiEnemyAirToGroundThreat='..M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat]..'; tiMAAThresholdByTech[iFactoryTechLevel]='..tiMAAThresholdByTech[iFactoryTechLevel]) end
                    bHaveWantedAA = true
                    --Normally would wait to see if we want an antinavy unit, but if have run from enemy air then try getting AA now as a priority if our overall AA threat levels could be better
                    if iOurCumulativeAAThreat < iOurCumulativeCombatThreat and not(M28Team.tAirSubteamData[aiBrain.M28AirSubteam][M28Team.refbHaveAirControl]) and tOtherWZTeamData[M28Map.refiTimeLastRunFromEnemyAir] and GetGameTimeSeconds() - tOtherWZTeamData[M28Map.refiTimeLastRunFromEnemyAir] <= 60 and iOurCumulativeAAThreat < math.max(M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] * 2, tiMAAThresholdByTech[iFactoryTechLevel] * 3) then
                        if bDebugMessages == true then LOG(sFunctionRef..': Will try getting AA now') end
                        if ConsiderBuildingCategory(M28UnitInfo.refCategoryNavalAA) then return sBPIDToBuild end
                    end
                end
    ...
    if not (bHaveLowMass) and tWZTeamData[M28Map.subrefTbWantBP] and (not(bHaveLowPower) or iFactoryTechLevel > aiBrain[M28Economy.refiOurHighestLandFactoryTech]) and (M28Team.tTeamData[iTeam][M28Team.subrefiTeamAverageMassPercentStored] >= 0.25 or GetGameTimeSeconds() - (M28Team.tTeamData[iTeam][M28Team.refiTimeOfLastMassStall] or 0) >= 40) then
        if ConsiderBuildingCategory(M28UnitInfo.refCategoryEngineer) then
            return sBPIDToBuild
        end
    end

    ...
    if bContinue then
        ...
        if (oTarget[M28ACU.refiTimeLastWantedToRun] and GetGameTimeSeconds() - (oTarget[M28ACU.refiTimeLastWantedToRun] or -100) <= 5) or ((EntityCategoryContains(categories.COMMAND, oTarget.UnitId) and M28UnitInfo.GetUnitHealthPercent(oTarget) <= 0.6 and not(oTarget[M28ACU.refbUseACUAggressively]) and oTarget:GetAIBrain()[M28Economy.refiOurHighestFactoryTechLevel] >= 1)) then
            ...
            if tLZOrWZTeamData then
                if M28Conditions.BaseIsSafeToRetreatTo(tLZOrWZTeamData[M28Map.reftClosestFriendlyBase], oTarget:GetAIBrain().M28Team) or (M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies]) and not(tLZOrWZData[M28Map.subrefWZTeamData])) then
                    tCurDestination = {tLZOrWZTeamData[M28Map.reftClosestFriendlyBase][1], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase][2], tLZOrWZTeamData[M28Map.reftClosestFriendlyBase][3]}
                    if bDebugMessages == true then LOG(sFunctionRef..': setting ACU tCurDestination as the closest friendly base if it is in a valid zone, as it has recently tried running') end
                else
                    local iCurDistLessRange
                    local iClosestEnemyDistLessRange = 10000
                    local oClosestEnemy
                    function ConsiderNearestEnemy(tEnemies)
                        for iEnemy, oEnemy in tEnemies do
                            if not(oEnemy.Dead) then
                                iCurDistLessRange = M28Utilities.GetDistanceBetweenPositions(oEnemy:GetPosition(), oTarget:GetPosition()) - (oEnemy[M28UnitInfo.refiDFRange] or 0)
                                if iCurDistLessRange < iClosestEnemyDistLessRange then
                                    iClosestEnemyDistLessRange = iCurDistLessRange
                    ...
                    if M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies]) == false then
                        ConsiderNearestEnemy(tLZOrWZTeamData[M28Map.reftoNearestDFEnemies])
                        --If have water zone check for nearby land zones if we arent underwater
                    elseif tLZOrWZData[M28Map.subrefWZTeamData] and not(M28UnitInfo.IsUnitUnderwater(oTarget)) then
                        if M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.reftoNearestCombatEnemies]) == false then
                            ConsiderNearestEnemy(tLZOrWZTeamData[M28Map.reftoNearestCombatEnemies])
                        end
                        if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentLandZones]) == false then
                            local iTeam = oTarget:GetAIBrain().M28Team
                            for iEntry, tSubtable in tLZOrWZData[M28Map.subrefAdjacentLandZones] do
                                ...
                                if M28Utilities.IsTableEmpty(tAdjLZTeamData[M28Map.reftoNearestDFEnemies]) == false then
                                    ConsiderNearestEnemy(tAdjLZTeamData[M28Map.reftoNearestDFEnemies])
                                end
                            end
                        end
                    end
                    if bDebugMessages == true then LOG(sFunctionRef..': the base isnt safe and we have nearby DF enemies so will assume wanted to run from nearest DF enemy, oClosestEnemy='..oClosestEnemy.UnitId..M28UnitInfo.GetUnitLifetimeCount(oClosestEnemy)) end
                    if oClosestEnemy then
                        local iAngleFromNearestEnemy = M28Utilities.GetAngleFromAToB(oClosestEnemy:GetPosition(), oTarget:GetPosition())
                        local tPotentialMovePoint
                        local iDistToMove = 10
                        local iPlateauWanted = NavUtils.GetLabel(M28Map.refPathingTypeAmphibious, oTarget:GetPosition())
                        local bHaveMovePoint
                        for iAngleAdjust = 0, 45, 15 do
                            for iAngleFactor = -1, 1 do
                                if iAngleFactor == -1 or iAngleAdjust > 0 then
                                    tPotentialMovePoint = M28Utilities.MoveInDirection(oTarget:GetPosition(), iAngleFromNearestEnemy + iAngleAdjust * iAngleFactor, iDistToMove, false, false, false)
                                    if M28Utilities.IsTableEmpty(tPotentialMovePoint) == false and NavUtils.GetTerrainLabel(M28Map.refPathingTypeAmphibious,  tPotentialMovePoint) == iPlateauWanted then
                                        tCurDestination = {tPotentialMovePoint[1], tPotentialMovePoint[2], tPotentialMovePoint[3]}
                                        bHaveMovePoint = true
                                        if bDebugMessages == true then LOG(sFunctionRef..': Have a run location we can path to, iAngleFactor='..iAngleFactor..'; iAngleAdjust='..iAngleAdjust) end
                                        break
                                    end
                                end
                                if bHaveMovePoint then break end
                            end
                            if bHaveMovePoint then break end
                        end
                    end
...
function MoveAwayFromFactory(oUnit, oEngineerOrFactory)
    ...
    elseif EntityCategoryContains(M28UnitInfo.refCategoryMobileLand, oUnit.UnitId) and EntityCategoryContains(M28UnitInfo.refCategoryEngineer, oEngineerOrFactory.UnitId) then
...
function ConsiderAirAAHoverAttackTowardsTarget(oUnit, oWeapon)
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef..': Start of code, oUnit='..oUnit.UnitId..M28UnitInfo.GetUnitLifetimeCount(oUnit)..'; Is oTarget valid='..tostring(M28UnitInfo.IsUnitValid(oTarget))..'; oTarget='..(oTarget.UnitId or 'nil')..(M28UnitInfo.GetUnitLifetimeCount(oTarget) or 'nil')..'; Is special micro active='..tostring(oUnit[M28UnitInfo.refbSpecialMicroActive]))
    ...
    if M28UnitInfo.IsUnitValid(oTarget) and oTarget:GetFractionComplete() == 1 and not(oUnit[M28UnitInfo.refbSpecialMicroActive]) then
        ...
        if bProceedWithMicro then
            local iOurMaxSpeed = oUnit:GetBlueprint().Air.MaxAirspeed
            local iEnemyMaxSpeed = oTarget:GetBlueprint().Air.MaxAirspeed
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': iOurMaxSpeed='..(iOurMaxSpeed or 'nil')..'; iEnemyMaxSpeed='..(iEnemyMaxSpeed or 'nil')..'; Our facing angle='..M28UnitInfo.GetUnitFacingAngle(oUnit)..'; Enemy unit facing angle='..M28UnitInfo.GetUnitFacingAngle(oTarget)..'; Angle dif='..M28Utilities.GetAngleDifference(M28UnitInfo.GetUnitFacingAngle(oUnit), M28UnitInfo.GetUnitFacingAngle(oTarget))..'; Dist to enemy='..M28Utilities.GetDistanceBetweenPositions(oTarget:GetPosition(), oUnit:GetPosition())) end
            ...
            if iOurMaxSpeed and iEnemyMaxSpeed and iOurMaxSpeed >= iEnemyMaxSpeed and (iOurMaxSpeed >= iEnemyMaxSpeed * 1.2 or (M28UnitInfo.GetUnitLifetimeCount(oUnit) <= 100 and M28Utilities.GetAngleDifference(M28UnitInfo.GetUnitFacingAngle(oUnit), M28UnitInfo.GetUnitFacingAngle(oTarget)) <= 130)) then
                ...
                if iOurRange > 0 and EntityCategoryContains(M28UnitInfo.refCategoryAllAir, oTarget.UnitId) then
                    ...
                    if iEnemyMaxSpeed >= iOurMaxSpeed * 0.95 then bEnemyIsCloseToOurSpeed = true end
                    ...
                    else iMaxTimeBetweenShotsWanted = iMaxTimeBetweenShotsWanted + math.max(2, iMaxTimeBetweenShotsWanted * 0.25)
                        --v306 and earlier: else iMaxTimeBetweenShotsWanted = iMaxTimeBetweenShotsWanted + math.max(0.5, iMaxTimeBetweenShotsWanted * 0.15)
                    end
                    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                    WaitTicks(1)
                    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerStart)
                    ...
                    while M28UnitInfo.IsUnitValid(oTarget) and M28UnitInfo.IsUnitValid(oUnit) do
                        ...
                        if M28UnitInfo.IsUnitValid(oUnit) and ((bUseStopMicro and bLastOrderWasStopOrder and not(M28UnitInfo.IsUnitValid(oTarget)))
                            ...
                            if M28Utilities.IsTableEmpty( toNearbyEnemyAirAA) == false then
                                ...
                                for iEnemy, oEnemy in toNearbyEnemyAirAA do
                                    if not(oEnemy.Dead) then --redundancy
                                        ...
                                        if iCurDist < iClosestDist and not(IsTargetRunningAway(oEnemy, oUnit, iCurSpeed, iOurMaxSpeed, iTargetCurSpeed, iEnemyMaxSpeed, iOurRange, iCurFacingAngle)) then
                                            oAltTarget = oEnemy
                            ...
                            if M28UnitInfo.IsUnitValid(oAltTarget) then
                                ...
                                oTarget = oAltTarget
                                iEnemyMaxSpeed = oTarget:GetBlueprint().Air.MaxAirspeed
                            end
                        end
                    end
                    ...
                    if bEnemyIsCloseToOurSpeed and iCurDistToTarget > iOurRange and (iCurDistToTarget > iOurRange + 15 or iEnemyMaxSpeed >= iOurMaxSpeed or not(EntityCategoryContains(M28UnitInfo.refCategoryTransport, oTarget.UnitId)) or not(oTarget.GetCargo) or M28Utilities.IsTableEmpty(oTarget:GetCargo())) then
...
function IsTargetRunningAway(oPotentialTarget, oUnit, iCurSpeed, iOurMaxSpeed, iTargetCurSpeed, iEnemyMaxSpeed, iOurRange, iCurFacingAngle)
    --For some reason have to have this as a standalone function for the code to run
    local bDebugMessages = false if M28Profiler.bGlobalDebugOverride == true then   bDebugMessages = true end
    local sFunctionRef = 'IsTargetRunningAway'
    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerStart)
    ...
    if not(oUnit.Dead) then --wierd bug where despite exact same timestamp oUnit returns valid before function is called, and dead/invalid during the function
        ...
        if iCurSpeed >= iOurMaxSpeed * 0.7 then
            ...
            if iTargetCurSpeed >= iEnemyMaxSpeed * 0.7 and iTargetCurSpeed >= iCurSpeed - 0.25 then
                ...
                local iCurDistToTarget = M28Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oPotentialTarget:GetPosition())
                if iCurDistToTarget > iOurRange + 1 then
                    --is enemy infront of us?
                    local iCurAngleToTarget = M28Utilities.GetAngleFromAToB(oUnit:GetPosition(), oPotentialTarget:GetPosition())
